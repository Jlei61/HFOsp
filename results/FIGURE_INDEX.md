...
| [paper-ready-figure/fig_m4_dynamic_qi/figures/](paper-ready-figure/fig_m4_dynamic_qi/figures/) | **M4 除法共享抑制池 = runaway↔transient 之间的"第三态"（2026-07-06）**：E1146 twoend_equal 两轴向灶自发耗竭 q_I→runaway，全局除法池 `S_G` 能压成**窄窗口 bounded sustained attractor**（相图确认 2 格 `(k_q=0.10,aG16)+(0.25,aG16)`、`(0.10,16)` 3/4 seed）；但**空间宽（满宽横条、位置随 seed 漂=对称破缺、非 localized ictal core）、marginal、不可撤回**（reversibility：`qI_refill` 灌满 q_I 仍回有界吸引子、`inhibitory_pulse` 强抑制后反跳成 one_shot burst 317Hz）→ **非完整 seizure cycle**，池只 bound 不 terminate。图：4联代表图 / 2D热图 / alpha_G切片 / runaway·bounded·SEEG-readout 三 GIF。见本目录 `README.md`；机制对照(matched-subtractive)未做；分支 `topic4-m4-divisive-sg` 未 merge |
