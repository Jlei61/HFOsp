...
| 5 group-event state | [group_event_state/core_evidence/figures/](group_event_state/core_evidence/figures/) | 三张 candidate 核心证据图：H1 未来事件块、H2a 状态调制事件继续/规模/触点集合、H2b 发作迁移与 H3 反馈。v0.3.1 没有运行 `H+S` 相对 `H` 等 residual estimand，state learning 未决而非阴性；H2b/H3 也尚未运行，主 panel 均按真实状态保留 `not yet run`。 |
...
| [paper-ready-figure/fig2/figures/fig2-panelf.png](paper-ready-figure/fig2/figures/) | **Fig2-F 独立 cohort panel**：完整12名患者 signed `r` 分布（8/12<0，中位−0.353）及全触点 channel-shuffle cohort-median-shift null（Δmedian=−0.339，`P_perm=0.01840`）。配对 Wilcoxon P=0.088 与 within-shaft P=0.878 只作 sensitivity。 |
| [paper-ready-figure/fig3/figures/](paper-ready-figure/fig3/figures/) | **Figure 3 canonical package**：A–F 六个无左上角字母的独立 PNG/PDF，以及唯一带 A–F 字母的 `fig3-complete-layout`。A raw SEEG+TFR；B 四频带轨迹；C E1146 TA field↔early-ictal broadband；D clinical-onset gradient-field cohort（n=17/16/11，Gamma n.s.）；E 单病例 field-similarity 时程；F 17-subject signed A/B contrast heatmap。统一复现 `scripts/paper_figures/build_main_figure_3.py`。 |
