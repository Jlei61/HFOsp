...
| [topic5_ictal_recruitment/contact_similarity/figures/](topic5_ictal_recruitment/contact_similarity/figures/) | 触点相似性几何阶梯 (R1 无几何 / R2 同平面触点核 / R3 场)：判定间期↔发作相似性来自几何平滑还是网格 (sensitivity)；含 hfa；+ R2b native-3D 灵敏度 (r2b_sensitivity_{band}.png)：native-3D vs 2D-plane 等价通过(2D 平面足够)；+ paper 图集 (fig1 空间加权方法示意[1146] / fig2 rank 阶梯[1084 加权更像正例,示意] / fig2_sup 队列 null-比-null[R1=6→R2=5,诚实口径] / fig3 触点↔场同形+2D↔3D 距离等价[1146])，producer `scripts/plot_topic5_cs_paper.py` |
...
| [paper-ready-figure/fig_m4_dynamic_qi/figures/](paper-ready-figure/fig_m4_dynamic_qi/figures/) | **M4 pass1 除法共享抑制池 = bounded 第三态机制筛选通过，非完整 seizure cycle（2026-07-09 验收）**：E1146 twoend_equal 两轴向灶自发耗竭 q_I→runaway，活动依赖的除法池 `S_G` 能打开**窄窗口 bounded sustained attractor**；40s 多 seed 后稳健锚点是 `(k_q=0.10,aG16)` 3/4 seed，无 runaway，`(0.25,aG16)` 只能算 15s 过、40s 不稳健。限制同样明确：**空间宽**（满宽横条、位置随 seed 漂=对称破缺、非 localized ictal core）、**marginal**、**不可撤回/不自终止**（`qI_refill` 回到有界态，`inhibitory_pulse` 反跳失控）→ 池只 bound 不 terminate。机制对照已完成：dynamic divisive 有界，matched-subtractive 仍失控，clamped-SG 压死活动；连续性/经验本征模只支持 Hopf-like delayed-feedback 迹象，不能写成严格 Hopf 证明。图：4联代表图 / 2D热图 / alpha_G切片 / mechanism / reversibility / continuity-eigenmode / runaway·bounded·SEEG-readout GIF。验收文档：`docs/archive/topic4/sef_hfo/m4_pass1_divisive_shared_pool_acceptance_2026-07-09.md`；分支 `topic4-m4-divisive-sg` 未 merge |
| [topic4_m4_dynamic_p1_sweep/figures/](topic4_m4_dynamic_p1_sweep/figures/) | **M4-2 STD 终止器 P1 sweep = 3-seed clean no-go（2026-07-08）**：接 M4 pass-1——在同一有界工作点（`k_q=0.10, alpha_G=16`）上加 E→E 短时程疲劳（STD）当终止器，问"能否把有界持续态干净终止成可再触发间期"。扫削竭强度 `ee_std_u` × 恢复时标 `ee_std_tau_ms` + Arm0，seed 1/3/4 各一遍：**全网格 0 terminate_clean**——不加 STD=持续不灭，弱 STD=**碎裂**（疲劳快恢复→事件未终止又点火），强/慢 STD=**压死**，中间无干净终止；retrigger 全 not_run，无 runaway。分母：seed1/3 基线是 bounded persist（同质），**seed4 基线本身已 fragment**（此工作点仅 3/4-seed 干净有界）=作 robustness 非同质分母。scoped：此衬底/工作点/网格/3-seed 内 STD 单独 clean no-go，**非普适、非全工作点、非"必须换"**。图：(u,τ) 分类 map（`m4_2_p1_sweep_map.png`）+ persist/fragment/suppress 慢变量轨迹（`m4_2_p1_mechanism.png`，证判别在恢复时标）。下一杠杆（no-go 指示）：gK/慢离子终止器 或 `D_EE`/衬底异质。archive `docs/archive/topic4/m4_2_std_termination_p1_sweep_2026-07-08.md`；分支 `topic4-m4-divisive-sg` 未 merge |
