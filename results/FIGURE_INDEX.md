...
| [topic5_ictal_recruitment/field_reversal/figures/](topic5_ictal_recruitment/field_reversal/figures/) | A 线上游 gate：间期两模板 TA/TB 摊成空间场后是否反向对齐 (signed field corr vs within-shaft null) + 场平滑是否比离散触点更去噪 (LOO 头对头)；broad+narrow 双底物分开报告，narrow accountability 单列。**preliminary（缩减 n_perm，待全量复核 + 用户签核）**：反向逐病人二项 gate 两底物均有显著子集；逐触点 LOO 重建 sanity：field 不优于 contact self-mean（**非 axis robustness test**）；方向 / 轴鲁棒性由 axis-level supplement 另测（进行中）。精确数值见 archive `docs/archive/topic5/field_reversal_2026-07-06.md` |
