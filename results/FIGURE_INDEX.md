...
>
> 最近更新：2026-07-31。新增结论图目录时，请在对应 topic 表里补一行。
>
...
| [topic4_sef_hfo/zm_phase_c_tonic_identity/figures/](topic4_sef_hfo/zm_phase_c_tonic_identity/figures/) | **Z/M Phase C post-result futility stop（2026-07-31，incomplete but logically decisive）**：seed-1 primary 在用户授权停止前完成 59/60 runs；corrected-v2 把 250 Hz runaway gate 从 pathology-core 错误 scope 改回 all-sheet E-rate 后，59/59 均为 `tonic_non_AI`。图 a 显示 9 cells 6/6 + 1 cell 5/6 的覆盖；b 区分局部核心 436–443 Hz 与全场 140–160 Hz；c 显示 modulation 0.025–0.045，远低于 0.20 非-tonic 门；d 是代表性平坦 tonic continuation。10/10 cell 即使把唯一缺失 run 当阳性也达不到 5/6，故注册 primary maturation GO 不可达，停止 C1；这不是完整三-seed negative，也不建立 entry/offset/recovery/lifecycle。archive `docs/archive/topic4/sef_hfo/zm_phase_c_tonic_identity_futility_2026-07-31.md`；下一节点为 E→E 冻结的 fast inhibitory/membrane repair。 |
| [topic4_sef_hfo/zm_fast_lifecycle_development/figures/](topic4_sef_hfo/zm_fast_lifecycle_development/figures/) | **Z/M fast-lifecycle Stage A dynamic-threshold discovery（2026-08-01）**：四个真实 Phase-C tonic checkpoints × 6 个 E-only phi 参数点共 24/24 production cells 均变成 `burst_train`。动态阈值消除了旧 tonic refractory saturation，但产生 270–316 ms 间隔的同步 relaxation pulses；virtual-SEEG deep-energy gaps 占 79–82%，没有持续 ictal carrier，故不进入 reachability/exit vertical slice。主图左侧为完整 phenotype matrix，右侧为代表性 core/surround rate、连续能量、phi/S_G 和轴向 kymograph。archive `docs/archive/topic4/sef_hfo/zm_fast_lifecycle_stageA_2026-08-01.md`。 |
| [topic4_sef_hfo/zm_fast_carrier_repair/figures/](topic4_sef_hfo/zm_fast_carrier_repair/figures/) | **Z/M Phase D fast inhibitory/membrane repair（2026-07-31，baseline-gate NO-GO）**：新 conductance E 膜、local/weak-global GABA、sAHP 与动态阈值路径均为 off-by-default，且五个真实 Phase-C 状态迁移后 500 ms Arm-A 逐位一致。真正的动态 t0 Z/M reference 在 8.5 s 内复现 15 个返回式 IED；但注册的最大兴奋 dominance panel（`s_E=1.2`、全部三个 `s_I`）均严格 0 E spikes，`V_inf` 由约 11.97 mV 压至约 7.65 mV，因此在 baseline preservation 门即 `NO_GO_baseline_calibration_failed_zero_spike_dominance`。图 `phaseD_baseline_calibration_no_go.png` 是诊断图，不是 carrier/lifecycle 图；B/C/D carrier、空间、扰动返回、entry/offset/recovery 均未测试。archive `docs/archive/topic4/sef_hfo/zm_fast_carrier_repair_phaseD_2026-07-31.md`。 |
