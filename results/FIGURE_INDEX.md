...
| [topic4_sef_hfo/m4_snn_native_exit/figures/](topic4_sef_hfo/m4_snn_native_exit/figures/) | **M4 SNN-native containment→exit = 已测 actuator 下 BOUNDED-NEGATIVE（2026-07-21）**：给 M4 有界态加持续时间门控的局部恢复电流 `p(x,t)`；开环抬阈值（seed1、单 actuator、5 hold、`q_I` 到 0.87）+ 对称闭环电流（2 seed）都退不出（rebound / lower-rate-persistent / runaway）。**候选机制**（非 impossibility proof）：`q_I` 与 `S_G` 对活动响应**符号相反**、随周期负相关；但实测有 ~403ms 重叠、用的是空间均值 `q_I`（掩盖 core），**退出 basin 是否存在未测**（需 `q_core×S_G` frozen atlas）。⚠️`fig5_no_go_diagnostic.png` 第四列原"不对称"因 P0 bug 实为对称、已修+重跑中。archive `docs/archive/topic4/sef_hfo/m4_snn_native_exit_execution_2026-07-21.md`；分支 `codex/topic4-m4-snn-native-exit`（未合 main）|
