...
| [paper-ready-figure/fig2_shared_field_reversal/figures/](paper-ready-figure/fig2_shared_field_reversal/figures/) | **Fig2-F 最后一行候选**：仅纳入12名已有 shared axis 且二维几何有效的患者；左侧复用间期场统一 renderer 展示匿名投稿代号 E15/E14/E13/Y9 四个几何易读的负相关 TA/TB Viridis 场，保留毫米刻度并以单个竖直0–1 colorbar跨两行；右上完整显示12名患者的 signed `r` 分布（8/12<0，中位−0.353）及IQR，右下为全触点 channel-shuffle cohort-median-shift null（Δmedian=−0.339，`P_perm=0.01840`）。无 same/reversed/strict 分组；配对 Wilcoxon P=0.088 与 within-shaft P=0.878 只作 sensitivity。 |
