...
| [paper-ready-figure/fig_mz_early_bridge_v2_zm_tau500/figures/](paper-ready-figure/fig_mz_early_bridge_v2_zm_tau500/figures/) | **MZ early-field bridge V2（z+m，τ_adp=500）**：同 V1 图语的 seed1 主图（连续 z+m native virtual-SEEG 轨迹 + 蓝 TB event + 粉 pre-t120 早窗 + 红 t120；下排 event-order + early-energy 两场沿 E1146 长轴同向）+ 三 seed 配对诊断图（maxAB 与 t120 的 V1 z-only vs V2 z+m）。三个 seed native t120 逐位复现 onset（Δ0.0），contact maxAB 0.891/0.969/0.904 全过 within-shaft null，m 推后失控 +1.5～3.7 s。**审阅补充图 `fig_mz_v2_axis_temporal_supp`**：单凭固定长轴坐标就 3/3 显著预测早期能量、间期模板≈长轴（\|r\|>0.95）只多贡献 +0.10~0.13（控制长轴后残余仅 2/3 显著），且是早期 0–50 ms 现象（50–100 ms 减弱）→ 桥 **(A) 固定长轴被调用 supported / (B) 模板超越几何轴只部分**；m 不只改点火（也改招募/动态范围/跨尺度）。一块底物 × 3 seed，非队列/因果。详见 archive `mz_early_field_bridge_v2_zm_tau500_2026-07-20.md` §10。 |
