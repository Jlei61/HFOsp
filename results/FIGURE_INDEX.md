...
| [paper-ready-figure/fig_subject_snn_e1146_runaway/figures/](paper-ready-figure/fig_subject_snn_e1146_runaway/figures/) | **FCXR-LC3 全生命周期（2026-08-07）**：同一 E1146 摆位、**开慢变量**、无外部刺激跑 45 s，两个连线种子都自发从零星事件走到停不下来的放电（5 s / 4 s，`CUMULATIVE`）。中间两列按**起火位置**分（最早 10% 细胞重心；某源区近 2× 才算），不按沿轴相关系数符号——12 次事件里 9 次是围着单个核摊开 7–8 mm 的**局部斑块**（两核相距 13 mm），符号编码的是哪个核点火而非行进方向。种子 1 两源区各 5 次、种子 3 全 10 次在同一源区 → **自发双源依赖连线种子**。⚠️不可与上面 Fig4A 的电极级正反计数比数目（那个不开慢变量、电极层端点质心）。归档 `docs/archive/topic4/fcxr_lc3_subject_geometry_runaway_2026-08-07.md` |
