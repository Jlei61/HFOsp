...
| [paper-ready-figure/fig3_peri_onset_field_similarity/figures/](paper-ready-figure/fig3_peri_onset_field_similarity/figures/) | **Fig3-B peri-onset field similarity 素材图（全 subject）**：1-150 Hz signed robust-z，`[-120,+20]s`、10 s window / 2 s step；左=maxAB `max(|r_A|,|r_B|)` scaffold trajectory，右=signed A/B polarity sidecar；per-subject material pool（非 cohort/formal gate）。全 35 subject fail-closed 批处理：20 出图（18 Epilepsiae + 2 Yuquan）/ 15 因缺上游 T0 eligibility 缓存 drop；索引 `fig3_peri_onset_subject_index.{csv,json}`，批处理 `scripts/paper_figures/run_fig3_peri_onset_all_subjects.py` |
| [paper-ready-figure/fig3_peri_onset_field_similarity/spatial_null/figures/](paper-ready-figure/fig3_peri_onset_field_similarity/spatial_null/figures/) | **Fig3-B maxAB 空间置换 null（两档）+ 时间维校正**：每 subject 一张——观测 maxAB vs **all-contact**（弱）与 **within-shaft**（强，主；只在杆内打乱、保留植入几何）两个被试内空间置换 null，各配 pointwise/maxT/cluster 三档校正（R=1000）。粗 rust=观测、蓝带=within-shaft null 95%、灰点线=all-contact null 中位、rust 竖带=within-shaft cluster 显著区间、蓝三角=maxT。**换强 null 后大幅收紧**：13/20 有 within-shaft cluster、7/20 扛过 maxT、仅 2/20（E922/E1146）稳健；**塌缩** E548 all-contact 64/66→within-shaft cluster 0（相似几乎全是杆几何）。per-subject 素材非 cohort gate；within-shaft 分辨力依赖杆触点数（`n_singleton_shafts`）。复现 `scripts/run_topic5_fig3b_maxab_spatial_null.py --all-ok`，索引 `spatial_null/fig3b_maxab_spatial_null_index.{csv,json}` |
