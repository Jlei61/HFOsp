...
| [topic4_sef_hfo/m4_snn_native_exit/figures/](topic4_sef_hfo/m4_snn_native_exit/figures/) | **M4 SNN-native containment→exit = BOUNDED-NEGATIVE + `q_I`↔`S_G` 反相根因（2026-07-21）**：给 M4 有界态加持续时间门控的局部恢复电流 `p(x,t)`，穷举开环抬阈值（任意时长、`q_I` 灌到 0.87）+ 对称/不对称闭环电流**全退不出**（rebound / fragment-wander / runaway-train）。根因=`q_I` 只安静回灌、`S_G` 只活动起效，两者**时间反相**，没有连续轨迹让"油箱灌满"与"刹车就位"在释放时刻同时成立 → 统一解释 lineage 所有终止器失败 + R4 为何必须 fixed-bath+latch。`fig5_no_go_diagnostic.png` 四列结局对照 + `q_I`/`S_G` 反相。archive `docs/archive/topic4/sef_hfo/m4_snn_native_exit_execution_2026-07-21.md`；分支 `codex/topic4-m4-snn-native-exit`（未合 main）|
