...
| 5 patient-specific RNN bridge | [paper-ready-figure/fig6_patient_specific_rnn_bridge/figures/fig6_patient_specific_rnn_bridge.png](paper-ready-figure/fig6_patient_specific_rnn_bridge/figures/) | 患者内自监督 RNN：留出间期事件的 contact ordering 可恢复；冻结模型场与同患者 early-ictal 1–150 Hz 场在 all-contact null 下存在 target-free 跨状态对应 |
...
| [paper-ready-figure/fig6_patient_specific_rnn_bridge/figures/](paper-ready-figure/fig6_patient_specific_rnn_bridge/figures/) | **当前 patient-specific target-free RNN bridge 候选**：A/B 同一代表患者的 untouched interictal test events 与冻结 RNN 自由生成；C contact-pair precedence read-back；D 15 人真实 rank order vs within-event rank shuffle；E 同一真实 contact plane 上的 RNN-derived field 与 clinical-onset 后 0–10 s、1–150 Hz early-ictal field；F 15 人 all-contact channel-shuffle null。主张限于患者内间期结构恢复与 target-free 跨状态对应，不声称超过静态 scaffold 或排除全部 shaft geometry。 |
