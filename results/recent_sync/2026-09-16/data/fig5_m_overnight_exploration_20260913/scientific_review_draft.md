...
较晚补Z的原M网格对照继续保留其原定随访；未保存完成的阶段不据进度估计补画。

## 扫描与剩余计算

M网格完整随访 0/40；首次终点已确定 8/40；两种子齐备的F格 3/20。
固定M=.02/τM=2秒的Z扫描 127/147 完成。它和弱M示例是不同工作点，不能把这张Z图直接当作弱M的同条件F。
...
继续既定40个M条件、147个Z条件及已开始的有限对照，完成实际首次进入、原生恢复与再进入分类。高率计算明显更慢，GPU替换仅改变已验证逐位一致的加和执行，不缩短观察窗。
...
在固定参数下同时看到有限事件、自主高态结束以及再次事件之前，不把这条线当作已完成的自主发作周期模型。若全部预定M条件仍无法兼顾两端，应先修正高态反馈与原生节律机制，再讨论患者能量匹配和分岔命名。三维轨迹仅是观察投影，不能直接充当闭合向量场、nullcline或Hopf证据。
