# Topic 4 M3-v2.2 criticality Milestone 2 — 两段式点火/铺开判读（BOUNDED ACCEPTED 2026-07-12）

**Output framing:** `model_side_bounded_diagnostic` —— 本轮验收的是诊断机器、结果可复现性和下面这条有界结论，不是把 M2 升级为当前设计层。这是在同一段 v2.2 **仿真**轨迹（actual v2.2 SIMULATION trajectory，不是新病人数据、不是真实临床记录）上做的读数，从不声称“模型证明了 CSD 是否存在”；下面出现的“全场烧起来”也不是在说真实癫痫发作。当前机制设计已经进入 M4；M2 只作为 M4 上游的起燃定位证据保留。
...
- **接受**：M2 把“在哪里起燃”和“起燃后怎样铺开”拆成两个独立判读；在当前 v2.2 仿真工作点，线性起燃为 `core_localized`，双核对照仍表现为单核对称性破缺且走廊保持暗。
- **不接受为阳性**：非线性铺开。由于扰动强度/极性敏感，`onset/endgame/off_axis` 继续全部记为 `undetermined`；3/4 条轨迹的 axial 形态只作描述，不按多数票升级。
- **不覆盖 M4**：M2 不决定恢复变量、终止器或 M4 的 pass/fail；M4 的最新 spec、runner 和结果文档是这些设计问题的上位真值。
