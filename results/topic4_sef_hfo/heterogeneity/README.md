...
## Rate Step3b — 第一批 rate 可实现细胞参数的非空间敏感度筛查 (sensitivity_matrix.json)

⚠ 范围口径：这是**第一批 rate 可实现的细胞参数筛查**，不是完整生物物理异质性。只筛了 `V_th / tau_m / tau_ref / sigma` 四个、**各自独立**（无协方差结构）；spec §3 的 `E_L / g_AHP / tau_adapt` 等还没碰。

每个参数的相对离散度按同比例收窄 (CV 0.08→0.027)、mean-matched 到 nuE，看**静态有效输入-输出曲线**的斜率/曲率与闭环余量怎么变（方向算出来不预设）。杠杆排序：
- `V_th`(门槛)：斜率 **+11.9%**，闭环余量 Δ≈−0.0003；
- `sigma`(输入噪声)：斜率 **+5.8%**，余量 Δ≈−0.0001。⚠ sigma 是**输入噪声 / 状态轴**（spec §4 明确把背景噪声列为状态/外源输入，**不是干净的病理旋钮**，无文献锚）——保留为"输入噪声敏感度"，**不与 Rich 的细胞内在异质性并列为病理主轴**；
- `tau_m`(膜时间常数)：在**固定动力学时间常数的静态曲线积分里**斜率 +0.0%（≈无杠杆）。⚠ 仅限静态 transfer：本筛查把 tau_m 放进 lif_rate 静态曲线，但闭环 dispersion 仍用 nominal 膜时间常数（screen approx），**不能据此说 tau_m 对 rate dynamics 全局无用**；
- `tau_ref`(不应期)：斜率 −0.0%（≈无杠杆，工作点率极低、离 1/tau_ref 天花板太远，预料之中）；
- 对照（移工作点 +1mV，不 mean-match）：余量 Δ=+0.0124 → 闭环余量这个度量是活的，上面"余量不动"是真"没杠杆"不是测不出。

**筛查结论**：第一批四参数里没有一个的分布收窄能在 rate 层推动安全余量；只有 V_th / sigma 能改静态曲线斜率（~+12% / +6%），但传不到余量。下一步只测有斜率杠杆的组合 **V_th + sigma**；`V_th + tau_m + tau_ref` 因后两者静态无杠杆 ≈ 等于只收 V_th。

## Rate Step3b — 组合轴（共同驱动校准筛查, combo.json）

把多个参数一起按同比例收窄、用**共同输入偏移 δ** 把率拉回 nuE（这是合理的筛查定义，但 δ ≠ 0：例如 V_th+sigma 的 wide 层需要约 **−1.10 mV** 输入偏移才匹配回静息率——所以这是"共同驱动校准后的组合筛查"，**与单参数 own-mean mean-matched 不完全等价**）：
- `V_th + sigma`：斜率 **+16.4%**（≈ 两者相加，无超加协同），闭环余量 Δ=−0.0004；
- `V_th + tau_m + tau_ref`：斜率 +11.2%（≈ 只收 V_th，后两者静态无杠杆确认），余量 Δ=−0.0003。

**Step3b 总结论（rate 均场层，封存）**：在这个工作点上，**第一批 rate 可实现细胞参数（V_th/tau_m/tau_ref/sigma，单个或组合、无协方差）收窄都只让静态有效曲线变陡（+12~16%），传不到安全余量（Δ余量 ≈ 0，对照移工作点能推 +0.0124）。** 没有任何参数包改变余量 → 按"只有改变余量才进 patch"的纪律，**空间 patch 这一步正确跳过**。这把首轮 V_th-only 子结论扩到了第一批四参数（**不是**完整生物物理异质性，也未含协方差/动力学时间常数的闭环效应）。有限细胞的"突然集体同步爆发"均场看不见，移交 SNN（须设计成能区分"均场看不见同步"与"这个工作点上操作本就太小"）。
