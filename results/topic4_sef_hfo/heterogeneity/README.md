...
## Rate Step3b — 组合轴 (combo.json)

把多个细胞参数一起按同比例收窄、用共同输入偏移 δ 把率拉回 nuE：
- `V_th + sigma`：斜率 **+16.4%**（≈ 两者相加，无超加协同），闭环余量 Δ=−0.0004；
- `V_th + tau_m + tau_ref`：斜率 +11.2%（≈ 只收 V_th，后两者 0 杠杆确认），余量 Δ=−0.0003。

**Step3b 总结论（rate 均场层，封存）**：在这个工作点上，**整个细胞内在参数包（V_th/tau_m/tau_ref/sigma，单个或组合）收窄都只让有效曲线变陡（+12~16%），传不到安全余量（Δ余量 ≈ 0，对照移工作点能推 +0.0124）。** 没有任何参数包改变余量 → 按"只有改变余量才进 patch"的纪律，**空间 patch 这一步正确跳过**（不浪费时间）。这把首轮 V_th-only 子结论推广到了完整内在参数包。有限细胞的"突然集体同步爆发"均场看不见，移交 SNN（须设计成能区分"均场看不见同步"与"这个工作点上操作本就太小"）。
