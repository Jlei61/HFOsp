...
在同一块 E1146 模型底物上，把中间轨迹从"只有去抑制"换成"去抑制＋快速适应 m"之后，系统失控前 0–50 ms 的早期触点能量场**沿固定长轴组织、并在失控前被调用**——而且**单凭触点的固定长轴坐标就已能显著预测这个能量场（3/3，p<0.01）**；安静期时序模板几乎就是这条长轴（相关 −0.95～−0.98），只在长轴之上多贡献 maxAB +0.10～0.13（控制长轴后残余关联 2/3 显著）。所以安全结论分两层：**(A) 固定病理长轴在失控前被调用 = 3/3 成立；(B) 间期细粒度时序超越几何轴的额外信息 = 只 2/3 部分、未干净确立。** 且这是**早期（0–50 ms）现象**——到 50–100 ms 减弱。m 主要推后失控时刻（+1.5～3.7 s），但也改变招募/动态范围/跨尺度表达（不是"只改点火时间"）。这是观测层可行性（一块底物×3 seed），不是发作复现、因果或队列结论。
...
- **长轴几何对照（archive §10.1，最重要）**：单凭触点长轴坐标 maxAB = 0.789/0.839/0.779（3/3，p<0.01）；模板与长轴相关 −0.95～−0.98、增量只 +0.10～0.13；控制长轴后残余 partial 2/3 显著（seed1 p=0.032、seed3 p=0.001、seed4 p=0.078 边缘）。→ 主结论是 (A) 长轴被调用；(B) 模板超越几何轴只 2/3 部分。
...
不可以写：clinical seizure / clinical broadband power / complete seizure cycle / m 稳住发作或产生恢复态 / m "只改点火时间" / `z_i` 唯一生物机制 / 某端固定发作灶 / 间期事件因果触发失控 / 局部 z 图案因果 / contact 热点＝局部独占源 / 结果不依赖 core / **间期模板超越几何长轴提供预测信息**（只 2/3 部分、未干净确立）/ **双向轴都被稳定使用**（只 B→A）/ source 跨尺度方向一致。

## 完成层级

- engineering complete ✔ ｜ numerically eligible ✔ ｜ scientific observation ✔（**本轮上限**）：桥 **(A) 层 supported**（固定长轴在失控前被调用，3/3）；**(B) 层部分**（间期模板超越几何轴只 2/3、未干净确立）；早期 0–50 ms 现象
