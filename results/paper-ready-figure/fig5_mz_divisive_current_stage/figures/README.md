...
centered 250-ms envelope 给出的 13.8539 s 只作 retrospective 标记；trailing-only envelope 的跨越在 13.9788 s，晚 124.9 ms，并需再持续满 1 s 才被确认。

严格因果 trailing onset 之后，48/48 个轴向 bin 在 130 ms 内依次跨过因果 50-ms activity 门，轴向 Spearman rho=+0.20；这对应约数百 mm/s 的 fast event sweep，而不是秒级的 ictal tissue-recruitment front。原 capture 预计算的 centered-onset 后首个 1 s 中 47.4% 的 E 神经元发放，其中 18.3% 的全体 E 神经元超过 100 Hz。
