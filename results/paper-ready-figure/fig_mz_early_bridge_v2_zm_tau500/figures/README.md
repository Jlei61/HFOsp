...
一张**审阅补充图（非主图；2026-07-20）**，回答两个对照问题。**左**：把触点的**固定长轴坐标本身**当"模板"，做同样的 maxAB —— 灰柱=单纯长轴、红柱=间期模板；单凭长轴几何就已经 3/3 显著预测早期能量场（0.79/0.84/0.78，p<0.01），间期模板只多贡献 +0.10～0.13，且**控制长轴后残余关联只 2/3 显著**（partial p 标注在柱上）。**右**：把 0–25 / 25–50 / 50–100 ms 三个窗的 contact maxAB 连成折线（星号=杆内随机重排 p<0.05）—— 最初 50 ms 强，到 **50–100 ms 减弱**。

**关注点**：左图看间期模板相对"单纯长轴"到底多贡献多少、控制长轴后是否还显著（间期时序是否**超越几何轴**——本轮只 2/3 部分成立）；右图看这条读出是不是**只在最早 50 ms** 成立。
