...
- Interictal group-event internal propagation (event-inside-event topic; former Periodicity PR-3):
  ...
  - PR-6 supplementary rank displacement (2026-05-06): continuous-version footrule + Kendall τ from PR-2 cluster JSON × PR-6 anchoring JSON. Files: `src/rank_displacement.py` (`compute_signed_rank_displacement` + `aggregate_pair_metrics`), `scripts/run_rank_displacement.py` (worktree-aware: imports src from worktree, reads/writes results from canonical data root via `git rev-parse --git-common-dir`), `scripts/plot_rank_displacement.py` (cohort heatmap with rank_T_a_dense column sort to avoid circular sorting bias; 2-panel footrule+τ summary; per-subject strips), `tests/test_rank_displacement.py` (8 tests: identity, reversal, signs, partial valid_mask abort, shape ValueError, baseline-corrected SOZ split, excess>0 case, no signed_mean export). Outputs: `results/interictal_propagation/rank_displacement/` (per_subject JSON + cohort_summary.json + figures with Chinese README). Results: cohort n=23 (≤23 PR-6 cap), 6/6 fwd/rev-reproduced subjects τ<0 (median = −0.495, F_norm = 0.964), aligns with PR-6 swap geometry sign-test direction. Does **not** open new cohort claim; supplementary to PR-6 only.
