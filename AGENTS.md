...
- Interictal group-event internal propagation (event-inside-event topic; former Periodicity PR-3):
  ...
  - PR-4B Step 2–3 / PR-4C planned: L3 rate-coupling (lag span primary + Pearson r secondary) then seizure proximity. L3 metric hierarchy: **lag span = sole primary L3 metric** (works for all 29 eligible subjects), **Pearson r = secondary** (formal only on 8-subject high-confidence subset with dom_r>0.7). Lag span comparisons require n_participating matching. PR-4C = seizure proximity L1+L2+L3 (depends on PR-4B). See `docs/topic1_within_event_dynamics.md` §6–7 for framework details.
