...
- Interictal group-event internal propagation (event-inside-event topic; former Periodicity PR-3):
  ...
  - PR-6 supplementary rank displacement (2026-05-06): continuous-version footrule + Kendall τ from PR-2 cluster JSON × PR-6 anchoring JSON. **Cohort = PR-6 supplementary cohort = stable_k=2 ∩ PR-6 endpoint-defined = n=23/27** (PR-2 stable_k distribution {2:27, 4:2, 6:1}; 4 stable_k=2 excluded for missing PR-6: 1125/384/620/916). NOT a complete stable_k=2 cohort. Files: `src/rank_displacement.py`, `scripts/run_rank_displacement.py` (worktree-aware via `git rev-parse --git-common-dir`), `scripts/plot_rank_displacement.py`, `tests/test_rank_displacement.py` (8 tests). **Paper-level deliverable: single composite figure** `results/interictal_propagation/rank_displacement/figures/cohort_displacement_heatmap.{png,pdf}` — main heatmap (rows sorted by F_norm descending, columns by rank_T_a_dense, SOZ outlined; **x-axis ticks/xlabel on TOP** since bottom is occupied by horizontal colorbar) + 1 right-side shared-y F_norm summary track (with 2/3 dashed reference) + horizontal Δr colorbar **directly under the main heatmap** + SOZ channel mini-legend next to colorbar. NO PR-2.5 internal classification, NO group colors, NO Kendall τ track (collinear with F_norm, ρ ≈ -0.92, redundant), NO SOZ contribution_excess track (SOZ definition / channel coverage not yet stable for paper claim). τ + SOZ statistics retained in per-subject JSON, cohort_summary.json, archive doc §3.3 only. Per-subject strips kept for debug/supplement. Continuous statistics: Spearman ρ(F_norm, τ) = −0.92; Spearman ρ(F_norm, SOZ excess) = 0.19, p = 0.38, n=23 (both archive only). Channel selection caveat: lagPat uses high-HI / high-HFO-rate channels; metric reflects geometry on selected set, not "ground truth" reversal. Does **not** open new cohort claim; supplementary to PR-6 only. PR-2.5 / PR-6 binary cross-check is recorded in archive results doc §6 (methods background, dev log) — not on the paper figure.
