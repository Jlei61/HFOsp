...
**`v_th_per_neuron` in `run_fcxr_loop` (Topic 4 FCXR-LC3)** — the parameter defaults to `None`, and the loop then uses one uniform `p.V_th` for every cell. **The pathology in the subject substrate IS the per-neuron threshold field**: `PP.build_substrate` returns `S["vth"]` holding two 1.5 mm patches lowered to 17.5 against a base of 18.0, and those two patches are the only reason the sheet ignites at all. A run that omits the argument is a homogeneous sheet — it completes, writes plausible-looking records, and never enters. Observed 2026-08-08: a state-map reference trajectory reported disinhibition `D = 0.0000` and relay `1.000` at 12 s where the same registered working point reaches `D = 0.436` and relay `0.656`. **Every call site must pass `v_th_per_neuron=S["vth"]`**; verified callers are `run_topic4_fcxr_lc3_gba.py`, `run_topic4_fcxr_lc3_percell_adaptation.py` and `run_topic4_fcxr_lc3_phase_map.py` (the last routes all three of its runs through one `_loop` helper, with a test asserting there is exactly one `run_fcxr_loop(` call in the file).
