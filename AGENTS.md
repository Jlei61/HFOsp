...
- Interictal group-event internal propagation (event-inside-event topic; former Periodicity PR-3):
  ...
  - PR-6 supplementary rank displacement (2026-05-06): continuous-version footrule + Kendall τ from PR-2 cluster JSON × PR-6 anchoring JSON. **Cohort = PR-6 supplementary cohort = stable_k=2 ∩ PR-6 endpoint-defined = n=23/27** (PR-2 stable_k distribution {2:27, 4:2, 6:1}; 4 stable_k=2 excluded for missing PR-6: 1125/384/620/916). NOT a complete stable_k=2 cohort. Files: `src/rank_displacement.py` (`compute_signed_rank_displacement` + `aggregate_pair_metrics`), `scripts/run_rank_displacement.py` (worktree-aware: imports src from worktree, reads/writes results from canonical data root via `git rev-parse --git-common-dir`), `scripts/plot_rank_displacement.py` (cohort heatmap with rank_T_a_dense column sort to avoid circular sorting bias; 2-panel footrule+τ summary with Mann-Whitney U; per-subject strips), `tests/test_rank_displacement.py` (8 tests). Outputs: `results/interictal_propagation/rank_displacement/`. Cohort split is by PR-2.5 candidate threshold ρ_inter < −0.5: candidate cohort (TRUE+FALSE = n=7) vs non-candidate (None = n=16) — NOT the naive reproduced/not-reproduced binary, which lumps borderline ρ ≈ −0.4 subjects with truly random ones. Channel selection caveat: lagPat npz uses high-HI / high-HFO-rate channels; the metric reflects geometry on this selected set, not "ground truth" reversal. Does **not** open new cohort claim; supplementary to PR-6 only.
