"""Deterministic budget-matched binary dual-core fields."""
...
def distances_to_dual_core(
    ...
    """Return distance and nearest-center index for each position."""
...
def budget_matched_dual_core_h(
    ...
    *,
    ...
    """Select exactly ``target_count`` E nodes nearest to either core."""
    positions = np.asarray(positions, dtype=float)
    ...
    audit = {
        ...
        "strictly_inside_cutoff_count": int(np.sum(distance < cutoff)),
...
def dual_core_query_h(
    ...
    *,
    ...
    """Evaluate the same binary geometry on non-E query positions."""
