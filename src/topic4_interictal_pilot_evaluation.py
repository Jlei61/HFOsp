...
class PilotEvaluator:
    ...
    def metrics(self, t, seeds=None, *, detail=True):
        ...
        if not n:
            return {**result, 'modes': [], 'operational_mode_coverage': None, 'joint_distance': None}
        ...
        for m in range(self.k):
            ...
            row = {'mode': m, 'n_assigned': int(all_ids.sum()), 'n_supported': int(ids.sum()),
                   'supported_by_seed': unit_counts, 'operationally_observed': observed,
            ...
            if ids.any() and q.any():
                xq = event_kernel_features(probe[q], self.xy, self.groups, self.scale)['joint']
                    ...
                    for j in range(i+1, t.shape[1]):
                        ...
                        if len(d1) >= 5 and len(d2) >= 5:
                            pairs.append({'i':i,'j':j,'n_model':len(d1),
                                ...
                                'order_probability_error_at_2ms':float(abs(np.mean(d1>2)-np.mean(d2>2))),
                                'signed_lag_wasserstein_ms':float(wasserstein_distance(d1,d2))})
                row['pair_diagnostics'] = pairs
                row['order_mae_at_2ms'] = float(np.mean([v['order_probability_error_at_2ms'] for v in pairs])) if pairs else None
            ...
            modes.append(row)
        result['modes'] = modes
        result['operational_mode_coverage'] = float(np.mean([r['operationally_observed'] for r in modes])) if self.modes_stable else None
        return result

    def manifest(self):
        return {'role': 'REUSED_DEVELOPMENT_ONLY_NO_FINAL_MODEL_GATE',
