...
def lif_rate_gauss_legendre(mu_mv, sigma_mv, *, tau_mem_ms: float,
    ...
    # Nonlinear root solvers can temporarily probe extreme, nonphysical input
    # moments.  Their asymptotic rate is zero; avoid emitting overflow noise
    # before the separate physical-root gate rejects the iterate.
    with np.errstate(over="ignore", invalid="ignore"):
        denominator = (
            float(tau_ref_ms) + float(tau_mem_ms) * np.sqrt(np.pi) * integral)
        rate = np.divide(
