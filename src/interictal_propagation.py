...
def run_subject_interictal_propagation_pr1(
    ...
    # TA/TB canonical naming: cluster id 0 = larger cluster (more events). Pure
    # rename that keeps every per-index field in sync; done before within-cluster
    # bias below so it inherits the canonical labels. (topic5 bridge already
    # sorts clusters by fraction, so this aligns topic1 to that convention.)
    _relabel_clusters_by_size(cluster_result)
    _relabel_clusters_by_size(adaptive_cluster)
...
def _relabel_clusters_by_size(result: Dict[str, Any]) -> Optional[List[int]]:
    """Rename clusters so id 0 = largest (most events), preserving all structure.

    TA/TB is defined as "TA = the cluster with more events". KMeans emits labels
    in arbitrary init order, so this renames in place, keeping every per-index
    field in sync: ``labels``, ``clusters`` (+ ``cluster_id``),
    ``inter_cluster_corr_matrix`` and ``candidate_forward_reverse_pairs``. It is a
    pure rename -- per-cluster stats, the (symmetric) correlations and the
    forward/reverse relationship are unchanged; only the id that names each
    cluster moves.
    """
    clusters = result.get("clusters")
    if not clusters or len(clusters) < 2:
        return None
    sizes = [int(c["n_events"]) for c in clusters]
    order = sorted(range(len(clusters)), key=lambda i: (-sizes[i], i))
    if order == list(range(len(clusters))):
        return None
    inv = {old: new for new, old in enumerate(order)}
    if result.get("labels") is not None:
        result["labels"] = [inv[int(x)] for x in result["labels"]]
    new_clusters = []
    ...
    matrix = result.get("inter_cluster_corr_matrix")
    if matrix:
        k = len(order)
        result["inter_cluster_corr_matrix"] = [
            [matrix[order[i]][order[j]] for j in range(k)] for i in range(k)
        ]
    for pair in result.get("candidate_forward_reverse_pairs") or []:
        if "cluster_a" in pair:
            pair["cluster_a"] = inv[int(pair["cluster_a"])]
        if "cluster_b" in pair:
            pair["cluster_b"] = inv[int(pair["cluster_b"])]
    return order


def _relabel_subject_clusters(subject: Dict[str, Any]) -> None:
    """Apply size-based relabeling to a full per-subject result dict in place.

    Relabels the ``cluster`` and ``adaptive_cluster`` blocks; when the adaptive
    block is reordered, remaps ``within_cluster_centered.per_cluster`` (keyed by
    adaptive cluster id) to match. PR-4 diagnostic per-cluster fields
    (rate_state_coupling / absolute_lag_validation / seizure_proximity_coupling)
    are regenerated by rerunning their augment stages against the relabeled
    adaptive labels, not remapped here.
    """
    cluster = subject.get("cluster")
    if isinstance(cluster, dict):
        _relabel_clusters_by_size(cluster)
    adaptive = subject.get("adaptive_cluster")
    ada_order = _relabel_clusters_by_size(adaptive) if isinstance(adaptive, dict) else None
    if ada_order is not None:
        wcc = subject.get("within_cluster_centered")
        if isinstance(wcc, dict) and isinstance(wcc.get("per_cluster"), dict):
            inv = {old: new for new, old in enumerate(ada_order)}
            wcc["per_cluster"] = {str(inv[int(k)]): v for k, v in wcc["per_cluster"].items()}
