...
def stationary_covariance(A,sigma):
    ...
    # Keep forward and backward on one deterministic CUDA backend.  Overnight
    # workers enforce one training process per GPU: the observed failures all
    # occurred when independent processes concurrently entered this solve on
    # the same RTX 3090, while isolated replay was finite and reproducible.
    A64=A.double()
    sigma64=sigma.double()
    eye=torch.eye(d,dtype=A64.dtype,device=A64.device)
    M=torch.kron(A64,eye)+torch.kron(eye,A64)
    rhs=-torch.diag(sigma64*sigma64).reshape(-1)
...
class Dynamics(nn.Module):
    ...
    def __init__(self,coupled=False,nonlinear=False,tau_init=(5./60.,1.,8.),generator=None):
        ...
        # Store the fixed upper-triangle basis once. This removes the only
        # dynamic CUDA index_put in the latent core while keeping the checkpoint
        # schema unchanged (the derived basis is non-persistent).
        oi=torch.nn.functional.one_hot(idx[0],LATENT).to(torch.float32)
        oj=torch.nn.functional.one_hot(idx[1],LATENT).to(torch.float32)
        self.register_buffer('omega_basis',oi.unsqueeze(2)*oj.unsqueeze(1),persistent=False)
    ...
    def tau_hours(self):
        return torch.exp(torch.clamp(self.log_tau,math.log(TAU_MIN_HOURS),math.log(TAU_MAX_HOURS)))

    def A(self):
        w=self.omega if self.coupled else self.omega*self.omega_within
        M=torch.einsum('k,kij->ij',w,self.omega_basis.to(w))
        M=M-M.transpose(0,1)
...
def propagate_moments(dyn,m,P,dt_hours,max_step=MAX_STEP_HOURS,cache=None):
    ...
    # For linear dynamics an arbitrary gap has an exact, differentiable OU map.
    if not dyn.nonlinear:
        key=('linear_mom',round(float(dt_hours),12))
        if cache is not None and key in cache:F,Q=cache[key]
        else:
            A=dyn.A().to(m.dtype);sigma=dyn.sigma().to(m.dtype)
            F,Q=ou_transition(A,sigma,dt_hours,_p_inf(dyn,A,sigma,cache))
            F,Q=F[0],Q[0]
            if cache is not None:cache[key]=(F,Q)
        return m@F.T,F@P@F.T+Q
