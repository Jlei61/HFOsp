...
def test_m2_integrator_frame_hook_is_additive_and_uniform(monkeypatch):
    grid = SimpleNamespace(n=1, size=1)
    op = SimpleNamespace(rE=np.zeros((1, 1)))
    monkeypatch.setattr(m2.spm, "op_state_vector", lambda op, kernels, grid: np.zeros(6))
    monkeypatch.setattr(m2.spm, "field_rhs", lambda z, *args, **kwargs: np.zeros_like(z))
    monkeypatch.setattr(m2.spm, "unpack_state", lambda z, grid: {"rE": np.asarray(z[:1]).reshape(1, 1)})
    monkeypatch.setattr(m2, "_footprint_metrics",
                        lambda d, grid, core, theta: {"active_frac": 0.0})
    v = np.zeros(6); v[0] = 1.0
    out = m2.integrate_footprint(
        grid, kernels=None, op=op, core=None, theta=0.0, v=v,
        eps=0.1, dt=0.5, t_max=2.0, sample_ms=[1.0, 2.0],
        return_rate_frames=True, frame_dt_ms=1.0,
    )
    frames = out["rate_frames"]
    np.testing.assert_allclose(frames["times_ms"], [0.0, 1.0, 2.0])
    assert frames["rE_kick"].shape == (3, 1, 1)
    assert frames["rE_control"].shape == (3, 1, 1)
    # Existing footprint output remains present; the hook adds frames rather than replacing it.
