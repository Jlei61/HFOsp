...
# ---------------------------------------------------------------- B6 real 4-dim (not placeholder)
def test_observed_sep_count_v2_counts_four_dims():
    n = 200
    lg = np.zeros((n, 3))
    lg[10:14, 0] = 10.0                                 # brief pre-onset event on the best contact
    lg[40:, 0] = 12.0                                   # long high macro
    obs = dict(best_contact_idx=0, lg_db=lg, frame_dt_ms=25.0, pre_frames=20, n_sustained_contacts=2,
               best_macro=dict(onset_ms=1000.0, duration_ms=3000.0, occupancy=0.95, peak=12.0))
    assert observed_sep_count_v2(obs) >= 3
    weak = dict(obs, n_sustained_contacts=1,
                best_macro=dict(onset_ms=1000.0, duration_ms=100.0, occupancy=0.30, peak=12.0))
    assert observed_sep_count_v2(weak) < 3
