...
50% collapse rate + 高 unr 比例说明：**baseline noise > ictal effect size** 是绝大多数 subject 的 ground truth。raise λ 改不了这点。
...
**当前推荐：D**。理由：
1. 用户目标是"产出一份合理可用的 channel-level seizure-onset label metric"，v2.2 在 stable+moderate (γ_a primary, 5 cells) 已有 working signal；γ_a sensitivity tier (unstable+concordant) 还能扩 cohort
...
### 5.1 916 gamma 是 λ-cap "假阳性 stable" 的清晰案例
...
**对 atlas 的修正**：916 在 atlas Phase 1 描述里被标记为 "stable + discordant"（指 anatomy 与 clinical i 标注不一致，gamma rank 偏向深端 PB / TS / AM 高号触点）。Phase 2 sensitivity 数据进一步表明：**916 的 stable tag 在 λ_max 选择上脆弱**，不应作为 cohort 唯一 stable cell 的 anchor。
...
4. **Plan archive §6.3.1 (γ_a Layer B 策略) 加 sensitivity 注释**：
   - primary tier 锁定 baseline λ100 (5 cells: 583 双 + 1084 broad + 583 gamma + 139 broad)
   - sensitivity tier 拓展时考虑 1096 双 + 922 gamma + 958 broad（Phase 2 改善证据）
