...
#### 3.8.1 v2.2.3 schema 修订（2026-05-07）

v2.1 schema 上方未包含 v2.2 引入的 `producer_health` / `clinical_concordance` 元数据，也未包含 v2.2.2 的 `boundary_tie_inclusive` 与 v2.2.3 的 `lambda_fragile`。Layer B 输出文件名从 v2.2 起改为 4 份（per `(ER × tier)`，见 §6.3.1）：

- `data_driven_soz_core_channels_gamma_ER_primary.json`
- `data_driven_soz_core_channels_gamma_ER_sensitivity.json`
- `data_driven_soz_core_channels_broad_ER_primary.json`
- `data_driven_soz_core_channels_broad_ER_sensitivity.json`

每个 subject entry schema（v2.2.3 锁定）：

```json
{
  "channels": [str, ...],                 // top-K data-driven SOZ channels
  "rsz_full_rank": {"<ch>": float, ...},  // full r_sz per channel (v2.1 protected)
  "rsz_valid_count": {"<ch>": int, ...},
  "n_ok": int,
  "s_sz": float,
  "topk_total": int,                       // ≥ 10; > 10 if boundary tie inclusive
  "boundary_tie_inclusive": bool,          // v2.2.2
  "producer_health": "stable|moderate|unstable|insufficient",
  "clinical_concordance": "concordant|partial|discordant|not_assessable",
  "lambda_fragile": bool                   // v2.2.3, see §6.3.2
}
```

**`lambda_fragile` 计算合同（v2.2.3）**：
- `lambda_fragile = true` ⟺ baseline λ100 producer_health ∈ {stable, moderate} **AND** Phase 2 λ500 sweep producer_health == insufficient（即同 cell 在 raise λ_max 后崩溃）
- 输入要求：B.1 builder 必须同时读取 `per_subject/` (baseline λ100) **和** `per_subject_lambda500/` (Phase 2 sweep)；缺任一则 raise（不许 default false 静默回归）
- 当前 cohort 唯一 `lambda_fragile=true` cell：**916 gamma_ER**

**TDD 增项（v2.2.3，进 §10）**：
- B-LF1: 916 gamma 必须 `lambda_fragile=true`
- B-LF2: 583 broad（baseline stable+conc，λ500 仍 stable+conc）必须 `lambda_fragile=false`
- B-LF3: builder 缺 `per_subject_lambda500/` 输入时必须 raise（不能给 default false）
- B-LF4: file-level provenance 必须记录 phase2 sweep 路径 + 用过的 λ_max=500

**file-level provenance（v2.2.3 扩展）**：

```json
"provenance": {
  "layer_a_per_subject_dir": ".../per_subject",                 // baseline λ100
  "layer_a_phase2_sweep_dir": ".../per_subject_lambda500",      // v2.2.3 NEW
  "layer_a_phase2_lambda_max": 500.0,                           // v2.2.3 NEW
  "layer_a_sanity_passed": bool,
  ...其余 v2.1 字段不变
}
```
...
**对 916 gamma 主标签的修订（重要）**：916 gamma 在 baseline λ100 是 cohort 唯一 stable+gamma cell（s_sz=0.83），按 §6.3.1 γ_a 进 primary。Phase 2 显示 916 gamma 在 λ500 下完全崩溃为 insufficient，机制是 916 baseline 噪声极大（pooled 195min），λ100 cap 让 baseline 警报阈值压低（与 λ500 真实校准的 λ 比）→ stable tag 对 λ_max 选择敏感。

**口径（2026-05-07 review）**：先前措辞写 "λ-cap 假阳性 stable" 太重，**不**等同于 "916 ictal 期没有真实 gamma 升高"——只说明 v2.2 stable tag 对 λ_max 阈值合同敏感。正确说法：**916 gamma 是 λ100 cap-sensitive / λ-fragile stable**。生理信号真假是独立问题，本 sweep 不能裁定。

**Layer B schema 改动（v2.2.3）**：Layer B 每条 entry 必须额外携带 `lambda_fragile: bool` provenance flag。计算合同：
- `lambda_fragile = true` ⟺ 该 (subject, ER) cell 的 baseline λ100 producer_health ∈ {stable, moderate} **AND** 同 cell 在 λ500 sweep 下 producer_health == insufficient
- `lambda_fragile = false` 否则
- 判定输入：`results/data_driven_soz/layer_a_ictal_er_rank/per_subject/` (baseline) + `per_subject_lambda500/` (Phase 2 sweep)
- B.1 实现时 Phase 2 sweep 输出是**必需**输入（不存在则 builder 必须 raise，不能给默认 false）

当前 cohort 唯一 lambda_fragile=true 的 cell：**916 gamma**。详细 schema + TDD 见 §3.8 (v2.2.3 修订)。
...
| B13 | `test_v2_2_entry_schema_lock` | **v2.2.3 (2026-05-07)**：每个 subject entry 必须含 `producer_health` / `clinical_concordance` / `boundary_tie_inclusive` / `lambda_fragile` 四个 v2.2 metadata 字段；缺任一 raise；v2.1 schema 字段（`rsz_full_rank` 等）继续保留 |
| B14 | `test_layer_b_files_split_by_tier` | 4 份输出文件 `<er>_<tier>.json` (gamma_primary / gamma_sensitivity / broad_primary / broad_sensitivity) 按 §6.3.1 γ_a 锁定切分；no subject 同时出现在同 ER 两个 tier |
| B-LF1 | `test_lambda_fragile_true_for_known_collapse_cell` | **v2.2.3**：fixture 模拟 916 gamma (baseline stable, λ500 insufficient) → entry `lambda_fragile==True` |
| B-LF2 | `test_lambda_fragile_false_for_held_cell` | **v2.2.3**：fixture 模拟 583 broad (baseline stable, λ500 stable) → entry `lambda_fragile==False`；同样 583 gamma (baseline moderate, λ500 moderate) → False |
| B-LF3 | `test_lambda_fragile_requires_phase2_input_or_raises` | **v2.2.3**：builder 调用时若 `per_subject_lambda500/` 不存在或缺该 subject JSON，必须 raise `FileNotFoundError`，**不**给 default `False`（防止静默回归到 buggy path） |
| B-LF4 | `test_lambda_fragile_provenance_records_phase2_path` | **v2.2.3**：file-level `provenance` 必须含 `layer_a_phase2_sweep_dir` 路径 + `layer_a_phase2_lambda_max` 数值（默认 500.0）；用 grep 验证 |
