...
### `rnn_learns_connectivity_wall_report_2026-08-07.md` — **合作者报告：让循环网络学出连接图撞的墙**（exploratory）
- 合成可辨识性检验：连接身份在 36/72/288 组织块与 2/3/4/6 毫米读出下**全部贴随机线**；整体行进方向同样认不出。
- 三个技术原因各以只动一个变量的直接实验排除：读出范围、未知数数量、稀疏限制与训练预算。
- 文献侧：从活动反推连接是已知病态反问题（大量不同连接产生近乎相同的动力学），故此结果为该路线的预期形态。
- 读出宽度**确实**影响"影响推多远的排序"这一层（6/6 配对改善，p=0.016）；每档自身 p=0.125 是 3 种子的功效下限而非阴性。
- 唯一正结果：只凭位置预测未训练过的触点，好过一无所知（+0.0036，17/21，p=0.026）。
- 先验审计：连线成本无长度尺度、**完全没有传导延迟**；附四条已撤回表述与八项会静默产出错数的缺陷。
