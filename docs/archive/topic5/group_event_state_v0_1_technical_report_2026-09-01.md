...
## 8. 结果（**中期，8/27 患者**，2026-09-01 15:30）

**分母**：135 个 run，10 位患者有产出，**8 位在 5 个臂 × 3 seeds 上全部完成**
（`epilepsiae_1073 / 1077 / 1096 / 1125 / 1146 / 253 / 384 / 548`）。
队列仍在跑（132/405 核心 + 162 消融待跑）。**本节所有数字都是中期数字。**

### 8.0 训练质量（判读的前置条件）

| | |
|---|---|
| 停止原因 | 130/135 early_stopping，5 time_budget |
| 选中最后一个 epoch 的 run | **0/49**（16-epoch 口径下曾是 13/21） |
| 非有限训练步 | **0** |
| CUDA OOM 重试 | **0** |
| 逐模块参数更新幅度（中位） | encoder 0.039 / state 0.120 / heads 0.208 |
| 45 个（患者×臂）组里 payload 重复 | **0**（三个 seed 确实不同） |
| 单 run 峰值显存 | 1.27 GB |
| 单 run 训练时间（中位） | 791 s |

各臂自然收敛长度不同（中位 epoch）：基线 28 / rank 25 / delay 25 / 全模态 33 / 背景 13。

### 8.1 H1 — 是否存在跨事件持续的状态

**证据 A：历史截断阶梯**（同一个训练好的 `a4`，评测时每 K 个事件重置状态；越低越好）

| 端点 | K=1 | K=20 | K=100 | 整段 |
|---|---|---|---|---|
| timing | 2.7865 | 2.5008 | 2.4321 | **2.4080** |
| participation | 0.5915 | 0.5581 | 0.5551 | **0.5438** |
| group_size | 0.9839 | 0.9213 | 0.8985 | **0.8813** |
| delay | −2.2467 | −2.3161 | **−2.3210** | −2.3160 |

| 对比 | timing | participation | group_size | delay |
|---|---|---|---|---|
| 整段 vs K=1 | −0.328 **8/8** p=0.0078 | −0.040 **8/8** p=0.0078 | −0.097 **8/8** p=0.0078 | −0.041 **8/8** p=0.0078 |
| 整段 vs K=20 | −0.074 7/8 p=0.070 | −0.003 7/8 p=0.070 | −0.010 7/8 p=0.070 | −0.015 6/8 p=0.29 |
| 整段 vs K=100 | −0.019 6/8 p=0.29 | −0.002 6/8 p=0.29 | −0.003 5/8 p=0.73 | −0.006 5/8 p=0.73 |

**读法**：状态确实跨事件持续——每个事件清零一次，四个端点在 **8/8 患者**上都变差。
但增益在 **K≈100 次事件处饱和**：从 100 次历史扩到整段（数千到数万次事件），
四个端点都只有 5–6/8 患者变好、`p≥0.29`。
**这一版没有证据支持"数千次事件尺度的长状态"，证据只支持"约一百次事件尺度"。**

**证据 B：正确时刻的状态 vs 时刻错配的状态**（把 test 段状态轨迹整体置换，
边缘分布不变、只破坏"状态对得上时刻"）

| 端点 | 中位(正确−错配) | 患者数 | sign p |
|---|---|---|---|
| timing | **−0.623** | 8/8 | 0.0078 |
| band_energy | −0.156 | 8/8 | 0.0078 |
| cross_band_lag | −0.055 | 8/8 | 0.0078 |
| group_size | −0.036 | 8/8 | 0.0078 |
| band_peak | −0.031 | 8/8 | 0.0078 |
| participation | −0.029 | 8/8 | 0.0078 |
| delay | −0.026 | 8/8 | 0.0078 |

**七个端点、8/8 患者全部一致。** 状态起作用靠的是它对得上时刻，不是它的取值分布。

**证据 C：`a4` vs 无潜状态的近期历史基线 `a1`**

| 端点 | 中位 Δ | 患者数 | Δ/种子噪声 | sign p |
|---|---|---|---|---|
| timing | **−0.064** | 7/8 | 0.65 | 0.070 |
| delay | −0.005 | 6/8 | 0.18 | 0.29 |
| group_size | −0.004 | 6/8 | 0.12 | 0.29 |
| participation | −0.001 | 5/8 | 0.07 | 0.73 |

逐患者（timing，负=状态更好）：−0.135 / −0.123 / −0.101 / −0.083 / −0.045 / −0.030 / −0.018 / **+0.226**。
**只有 3/8 患者的差超过它自己的种子噪声**，且 `epilepsiae_548` 方向相反。

**H1 综合读法**：在同一个网络内部，"保留状态"明显强于"每个事件清零"，
且状态的作用依赖时刻对齐——这两条在 8/8 患者上一致。
但一个**不带任何潜状态、只吃最近 1/5/20 次事件手工摘要**的基线，已经把这部分优势
基本拿满了；潜状态相对它唯一方向一致的增益在"下一次事件什么时候来"，
而且个体层面多数不超过种子噪声。

### 8.2 H2a — 状态改变了群体事件表达的哪些方面

**精确毫秒延迟 + 并列招募组 vs 旧的整数名次（`a3` vs `a2`，n=9）——全线为零**

| 端点 | 中位 Δ | 患者数 | sign p |
|---|---|---|---|
| timing | −0.007 | 5/9 | 1.00 |
| participation | +0.001 | 4/9 | 1.00 |
| group_size | +0.001 | 2/9 | 0.18 |
| delay | −0.001 | 5/9 | 1.00 |
| participation AUC | +0.001 | 6/9 | 0.51 |
| recruitment order | **−0.026** | 1/9 | **0.039**（方向不利） |
| prefix continuation | −0.011 | 2/9 | 0.18 |

**这是本轮最明确的负结果。** §3 已经证明旧的整数名次在中位事件上把 6 个触点
强行排成 6 个位次、而 producer 的 10 ms 分辨率只支撑得起 2 档先后；
但把这个"被编造出来的顺序"换成精确的毫秒延迟 + 并列组，**在任何一个端点上都测不出改善**，
招募顺序这一项甚至有 8/9 患者变差。

**波形 + 多频带加在精确延迟之上（`a4` vs `a3`，n=8）——方向一致但都在 p≈0.07**

| 端点 | 中位 Δ | 患者数 | sign p |
|---|---|---|---|
| timing | −0.050 | 7/8 | 0.070 |
| participation | −0.005 | 7/8 | 0.070 |
| recruitment order | +0.033 | 7/8 | 0.070 |
| prefix continuation | +0.020 | 7/8 | 0.070 |
| participation AUC | +0.008 | 6/8 | 0.29 |
| group_size | −0.010 | 6/8 | 0.29 |

7/8 是 n=8 下 sign test 能达到的最好非平凡结果（p=0.070）；**8/8 才到 p=0.0078**。
方向一致，但在这个分母下**够不到常规判据**。

**背景 SEEG 修正（`a5` vs `a4`，n=8）——一致地更差**

| 端点 | 中位 Δ | 患者数（a5 更好） | sign p |
|---|---|---|---|
| timing | **+0.150** | **0/8** | 0.0078 |
| delay | +0.048 | **0/8** | 0.0078 |
| participation | +0.018 | **0/8** | 0.0078 |
| participation AUC | −0.024 | **0/8** | 0.0078 |
| prefix continuation | −0.060 | **0/8** | 0.0078 |
| prefix next-contact | −0.010 | **0/8** | 0.0078 |
| group_size | +0.021 | 1/8 | 0.070 |

**这是整个实验里最一致的效应，而且是负的**：加入连续背景 SEEG 观测修正，
在 8 位患者上**无一例外**地让事件预测变差。
`a5` 也是收敛最快的臂（中位 13 个 epoch，其余 25–33）。

必须并报的分母限制（§1）：背景锚点规则要求 2 秒窗与任何 packed event core 不重叠，
`huangwanling` 只有 **6.2%** 的事件能找到此前完成的干净背景窗、`zhaojinrui` 86%。
但**当前这 8 位患者的背景锚点覆盖率都在 98% 以上**，所以这个负结果
**不能**用"背景数据不够"解释。

### 8.3 判读边界（本节允许写和不允许写的话）

允许：
- "在同一个网络内，保留跨事件状态比每个事件清零更好，8/8 患者、四个端点一致。"
- "状态的作用依赖它对得上时刻：打乱时刻对齐后，七个端点在 8/8 患者上全部变差。"
- "有用的历史长度在约一百次事件处饱和。"
- "把旧的整数名次换成精确毫秒延迟，在这一版的任何端点上都没测到改善。"
- "加入背景 SEEG 修正在 8/8 患者上使事件预测变差。"

**不允许**：
- 不得写"发现了跨任务的病理网络状态"——H2b 尚未在真实 checkpoint 上跑。
- 不得写"学到了数小时尺度的慢状态"——K=100 与整段无差别，证据只到约一百次事件。
- 不得写"潜状态胜过无状态基线"——只有 timing 方向一致，且 3/8 超种子噪声。
- 不得写"精确延迟比名次更有信息量"——数据说的正相反：测不出差别。
- 不得把 8/27 患者的中期数字当作队列结论。

### 8.4 尚未完成

| 项目 | 状态 |
|---|---|
| 核心队列 | 132/405（27 患者 × 5 臂 × 3 seeds） |
| 消融队列（`b4_memoryless` / `b1_no_real_dt`） | 0/162，核心跑完自动启动 |
| H2b 跨任务迁移（冻结 checkpoint） | 仪器已验证，未在真实 checkpoint 上跑 |
| H3 暴露量探针 | 仪器已在合成数据上定标，未在真实 run 上跑 |
| 5 seeds 承重复核 | 未做（当前 3 seeds） |
