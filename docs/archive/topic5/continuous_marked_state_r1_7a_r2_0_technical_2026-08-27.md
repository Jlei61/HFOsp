...
## 6d. 尺度可行性普查：长尺度不是"没做够"，是数据撑不起

退役长尺度路线的理由之一是"逐事件滑窗大量重叠、行数远大于独立信息量"。
本轮把它从判断变成可计算的事实：对 17 位 development 患者，统计**完整、
互不重叠、且不跨记录缺口**的 N 事件块数（`audit_t2_scale_block_support.py`）。

格式：总块数 / 达到 ≥5 块（队列级门槛）的患者数

| 数据层 | N=100 | N=200 | N=500 | N=1000 | N=2000 | N=5000 | N=10000 |
|---|---|---|---|---|---|---|---|
| `D_mechanism`（合同 T2 层） | 271 / 7 | 121 / 5 | 31 / 2 | 8 / 1 | **0 / 0** | **0 / 0** | **0 / 0** |
| 全 validation（复用 H1 数据，不可作 T2 证据） | 787 / 15 | 354 / 10 | 108 / 5 | 27 / 2 | 2 / 0 | **0 / 0** | **0 / 0** |
| TRAIN（模型拟合处，永远不是留出证据） | 2313 / 17 | 1023 / 17 | 303 / 9 | 90 / 2 | 20 / 1 | **0 / 0** | **0 / 0** |

三条结论：

1. **N≥5000 在任何数据层上都凑不出一个独立块。** 退役的 N=3000 / N=10000 实验
   **在结构上就不可能完成它们声称的检验**；其行数全部来自重叠滑窗。
   这把退役决定从"结果不可信"升级为"设计上不可行"。
2. **N=2000 在合同 T2 层是 0 块**；即使违规使用全 validation 也只有 2 块、
   且无任何患者达到 ≥5。作为留出证据不存在。
3. **合同层内还站得住的最大尺度是 N=200**（5 位患者 ≥5 块）；N=500 只剩 2 位。
   因此"把 N 往上推"这条路在本数据集上已到顶，**下一轮要扩的是患者与记录时长，
   不是名义尺度**。

产物：`r1_7b_cohort_extension/reports/t2_scale_block_support.json`。
...
4. **尺度只能到 N=200**（见 §6d）。不要再投入 N≥1000 的长尺度实验：
   合同 T2 层在 N=2000 上是 0 个独立块，N≥5000 在任何层上都是 0。
   要往长尺度走，唯一出路是**更长记录 / 更多患者**，不是更大的名义 N。
