...
在预测未来事件数时，`q(t)` 相对静态水平：5 min 为 5/8 为正，中位 +0.1516，sign p=0.219；30 min 为 5/8 为正，中位 +0.1283，sign p=0.219；120 min 为 3/7 为正，中位 +0.0000，sign p=1。这不是整段一个常数：它只用 anchor 之前的事件与有效观测时间，在 2 min、10 min、30 min、2 h、8 h 五个固定时间尺度上更新。
...
2. 在 `q(t)` 上再加学习的非线性残差只有 2/8 为正，中位 -0.0004，sign p=0.688，当前可解释的多尺度因果滤波已经吸收了大部分可见负荷变化。
3. 固定时钟背景 SEEG 在 `q(t)` 之外为 0/8 为正，中位 +0.0000，sign p=1。在当前低容量背景接口下没有额外信息；这不等于原始背景波形没有信息。
...
真正学到的 `m(t)` 在 next-event contact grammar 上只有 2/5 为正，中位 -0.0007，sign p=1；到 next-20 仍是 2/5 为正，中位 -0.0034，sign p=1。患者结果如下：
...
另一面，若不只看 contact grammar，而把连续 lag、参与场、波形和多频带形态作为一个功能读出，next-20-event 的结果为 4/5 为正，中位 +0.0043，sign p=0.375；正确时刻为 0/0 为正，中位 NA，sign p=NA；相对 FIT 期平均状态为 4/5 为正，中位 +0.0049，sign p=0.375。逐端点看，最一致且量级最大的增量来自波形形态和多频带能量；连续 lag 也有 4/5 位同向但较小，峰时和跨频带 lag 不稳定。它支持“短到中程的 event-content predictive memory”，但还没有证明稳定的传播 grammar 状态。

换成固定物理时间的未来 30 min，`m(t)` 在 `q(t)` 之外为 3/5 为正，中位 +0.0001，sign p=1，正确时刻为 0/0 为正，中位 NA，sign p=NA，相对 FIT 均值为 2/5 为正，中位 -0.0005，sign p=1。三个条件没有在同一队列层面同时成立，120 min 的独立块更少。因此**小时尺度网络表达状态仍未建立**。

## 5. H2b：从间期状态跨到发作

风险 readout 使用单一离散 survival 合同；发作标签只训练冻结状态之上的小型读出，不回流到间期模型。只有同时有训练期发作和 selection 阳性窗的患者—时间尺度才进入风险分母。

30 min 风险层中，动态负荷相对临床/历史基线为 1/4 为正，中位 -0.0021，sign p=0.625；完整事件状态在 `q(t)` 之上为 3/4 为正，中位 +0.0015，sign p=0.625；正确时刻为 2/4 为正，中位 -0.0005，sign p=1；相对 FIT 均值为 3/4 为正，中位 +0.0013，sign p=0.625。
