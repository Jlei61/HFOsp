...
- `scripts/plot_topic5_axis_direction_rose.py`（重写）+ 纯函数 `src/topic5_axis_direction.py`（9 tests）
  — **方向玫瑰图**：发作早期激活方向归一化 0°（黑线），两模板逐事件方向画成两个整圆空心直方图；
  ECoG/SEEG 由 Epilepsiae SQL `electrode_array.type` 判定（SEEG 标投影 caveat）；含队列汇总 rose。
...
3. **C 线（子型修饰）**：planned，未执行。子型标签不稳 + 每人发作少 → 只能病人内 modifier，
   不做队列主张。
