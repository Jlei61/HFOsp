...
### 2.1 符号自由轴对齐 cohort 检验（2026-06-27 追加）

**测什么**：每被试取发作主方向（占多数类均向）到**较近**的那条间期模板方向的角距 δ（min over A/B；因两模板≈反向，这是
符号自由的"发作主方向是否落在间期轴上"，**不判极性/哪一端**）。**怎么测**：cohort 统计量 = 各 δ 中位数；null = 每被试主
方向换均匀随机方向（模板固定）重算，B=20000，单侧。**结果**：

| band | 中位 δ | null 中位 | rotation-null p | 备注 |
|---|---|---|---|---|
| broadband | **12.2°** | ~45° | **0.0052（显著）** | 6 个里 5 个 δ≈9–18°（紧贴间期轴）；1084 离群 δ=151°（其两模板 Δ_AB=6° 不成轴 = 退化） |
| hfa | 32.2° | ~45° | 0.150（不显著） | δ 更散、更噪 |

**口径（重要）**：这把 A 线粗轴结论（场级、符号自由、已队列显著）在**方向角层面重述**——**符号自由、非方向重放、非独立新主张**。
**caveat**：旋转 null 不控"发作梯度与间期梯度同处一张 2D 触点平面"的几何偏置；严格控几何的是 A 线 channel-shuffle null（已 coarse 显著），
故此处显著 = 与 A 线一致、非假阳。**极性（哪一端）仍阴性**：逐被试 p_align（两类↔两模板，sign-aware）全队列 ≥0.05（最低 384=0.053）。
**一句话**：发作主方向**落在间期轴上**（符号自由、broadband 队列显著），但**朝哪一端不随间期模板**（极性阴性）。
图：`figures/cohort_axis_alignment_test.png`。
