...
每人 8 次更新（即上一轮冻结的更新分辨率）在第 1 轮为 **1.852**。

**承重对比：上一轮冻结训练预算 vs 延长训练预算**

| 量 | 值 |
|---|---|
| 上一轮冻结（1 遍 × 每人 8 次更新），患者中位 | 1.852 |
| 延长训练（7 遍 × 每人 32 次更新），患者中位 | 1.707 |
...
连续两遍落入判据带**。因此本轮只能写"比上一轮冻结配置接近 plateau 得多"，不能写"已收敛"。
...
| lr 1e-3, AdamW, wd 1e-4（**上一轮冻结所用**） | 1.7071 | 0.0197 ± 0.0036 |
...
- **学习率有真实影响**：3e-4 比上一轮冻结所用的 1e-3 好 0.0197 nats/decision（配对，SE 0.0036）。
...
- **选中的学习率位于预注册网格的下边缘**。按 spec 不做网格外扩展，这是本轮的明确限制。
  方向上它是**保守的**：真实最优学习率若更小，只会让"上一轮冻结实现训练不足"的差距更大。
...
|---|---:|
| 上一轮冻结预算：1 遍 × 每人 8 次更新，offset 4 轮 | 1.86783 |
| 上一轮冻结预算，offset 8 轮 | 1.84499 |
...
|---|---:|---:|---:|---:|
| 选中 vs **上一轮冻结预算** | **+0.13404** | [+0.0862, +0.1991] | **34/34** | 1.16 × 10⁻¹⁰ |
...
1. 非-LOSO 筛选给出的冻结-vs-延长训练差距是 0.124，LOSO 结构下是 **0.134**——两段式
...
**参照条件就是上一轮冻结的那个模型本身**：直接加载归档 checkpoint，不重训数学等价的复制品。
...
| 上一轮冻结 | 复合 | 0.635 | 0.0943 | 0.201 | 0.0850 | 0.0167 | 0.0300 |
| 延长训练 | 复合 | 0.815 | 0.1084 | **0.014** | 0.1255 | 0.0167 | 0.0300 |
| rollout-aware | 复合 | 0.802 | 0.1061 | 0.067 | 0.1283 | 0.0167 | 0.0300 |
| 上一轮冻结 | 模型自身 | 0.621 | 0.0874 | 0.459 | 0.0991 | 0.0874 | 0.0819 |
| **延长训练** | **模型自身** | **0.867** | **0.0497** | **0.804** | **0.0681** | 0.0477 | 0.0714 |
| rollout-aware | 模型自身 | 0.876 | 0.0575 | 0.807 | 0.0772 | 0.0533 | 0.0797 |

**复合发生器下：模型训练得越好，整场事件反而越差。** 成对先后相关从 0.184（完全不用
历史）→ 0.201（上一轮冻结）→ **0.014**（延长训练）；参与误差从 0.0834 → 0.0850 → **0.1255**。
延长训练模型经复合发生器读出后，整场事件的成对顺序与真实数据**几乎不相关**。

**模型自身发生器下：全部单调改善。**

### 5.2 延长训练 vs 上一轮冻结（模型自身发生器，逐患者配对，n=34）
...
|---|---|---:|---:|---|
| 上一轮冻结 | 复合 | **9/34** ← 精确复现上一轮 | 17 | 否 |
| 延长训练 | 复合 | 7/34 | 17 | 否 |
| rollout-aware | 复合 | 7/34 | 17 | 否 |
| （任一） | 只用静态骨架 | 10/34 | 17 | 否 |
| 上一轮冻结 | 模型自身 | 10/34 | 17 | 否 |
| **延长训练** | **模型自身** | **13/34** | 17 | **否** |
...
1. 上一轮的 9/34 **低于完全不用历史的静态对照 10/34**——那个数字里有一部分是读出方式的
   人为损失，不是模型的性质。
2. 用正确的读出方式加延长训练，从 9–10/34 提高到 13–14/34，是真实进步，但**仍低于 17/34**。
...
> 上一版"完整事件生成阴性"里，有两个可分离的成分：一部分来自**训练不足**（上一轮冻结的
> 训练预算比延长训练预算差 0.134 nats/decision，34/34 患者），另一部分来自**读出方式**（用于评价的
...
> 自身前后半一样接近"的预注册门槛（13–14/34，门槛 17/34）**。**测试的三种 rollout-aware
> 目标不支持曝光偏差作为主要解释**——它们同时损害局部预测与整场生成，且呈单调剂量反应；
> 这不等于所有曝光偏差修正都无效。

### 训练充分性是否关闭

**关闭。** 一步预测方向上：上一轮冻结配置差 0.134 nats/decision（34/34 患者，
...
因此允许写"比上一轮冻结配置接近 plateau 得多"，**不允许写"已收敛"**。
...
- 理由：那些句子背后的模型每个 fold 只有 264 次共享参数更新、只完整过一遍训练事件，
  在留出一步预测上比延长训练预算差 0.124 nats/decision（34 人中 33 人，p=3.5×10⁻¹⁰），
...
- 事实：在延长训练预算上，`3e-4` 优于上一轮冻结所用的 `1e-3`；选中的学习率位于预注册网格的边缘。
- 修订形式：报告这一点，并说明它的方向是**保守的**——它只会让"上一轮冻结实现训练不足"
...
- 事实：其中一部分来自训练不足（见 R1），另一部分来自**评价用的复合发生器**——它把
  模型对事件起点的偏好当作参照减掉，再加回训练集的静态参与先验；模型越好，这个组合
  注入的系统性偏差越大。外层留出数据上，延长训练模型经复合发生器读出后成对先后相关塌到
...
- 事实：上一轮 9/34（门槛 17/34）。本轮精确复现 9/34；用正确读出方式加延长训练是
  13–14/34，仍低于 17/34；完全不用历史的静态对照是 10/34。
- 修订形式：保留"未达到"的结论，但把 9/34 改写为"9/34（该读出方式）／13–14/34（模型
  自身读出方式，延长训练）"，并点明静态对照是 10/34——否则读者会把 9/34 误读成
  "历史信息毫无价值"。
...
- **上一轮冻结模型复现**：102/102 个参照单元从归档 checkpoint 复算的留出 NLL 与归档值最大
  偏差 3.0×10⁻⁷、中位 6.2×10⁻⁸。
