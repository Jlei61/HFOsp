...
**留一被试稳健性（关键 caveat，2026-07-04 更正）**：注意区分"原始 Wilcoxon p"与"官方判据 Holm 校正 p"。narrow 原始 Wilcoxon p 去任一被试后 ∈ [0.016, 0.031]（看似稳）——**但 `cohort_h3b_pass` 用的是 Holm 校正 p**：full-cohort narrow Holm=**0.031**（刚过 0.05），**去掉 7 个中任一被试（唯一例外是 442）Holm 都翻到 0.0625 失败 → 6/7 单被试 drop 下主队列判据不通过**。即 **narrow（主队列）的显著是刀刃上的、对单被试不稳健**；只有 broad Holm 去一稳（∈ [0.008, 0.016]）。→ 这把本已脆弱的阳性进一步削弱：tier-4 实际依赖 narrow 恰好 7 个被试的配置。
...
5. **主队列显著是刀刃上的、对单被试不稳健**（见 §4.1）：narrow Holm=0.031 刚过 0.05，去掉 7 个被试中任一（除 442）都翻到 0.0625 失败（6/7 drop 不通过）；只有 broad 去一稳。tier-4 依赖 narrow 恰好这 7 个被试的配置。
