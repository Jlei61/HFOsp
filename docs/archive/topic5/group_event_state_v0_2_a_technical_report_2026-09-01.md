...
| 量 | 实测 |
|---|---|
| 单 job 峰值显存（reserved），前 80 个 job | 中位 **2.11** GB / p90 **5.03** GB / 最大 **6.77** GB（`yuquan_chengshuai` `P_slow`） |
| 3 个 slot × 最坏情形 | 20.3 GB，对 24 GB 卡的"留 4 GB 余量"预算已经**贴边** |
...
**为什么停在每卡 3 个**：起初按前几个 job 的 2.9 GB 峰值估算，显存预算允许每卡约 5 个；
**这个估计后来被推翻**——跑到 80 个 job 时最坏峰值是 6.77 GB，3 个 slot 已经用到 20.3 GB，
正好贴住"24 GB 减 4 GB 余量"的边。也就是说 3 个/卡不只是吞吐上的选择，事后看也是显存上的上限。
（截至该时点 0 次显存降级；真发生时代码会依次减半 chunk 与 batch 最多重试 3 次，
仍失败则记 `resource_failed` 并**明确标注这不是科学阴性**。）
