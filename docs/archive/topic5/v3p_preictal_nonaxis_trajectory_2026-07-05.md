...
date 2026-07-05 · 前身 V3a（P3→I1 onset 对比，tier 2 脆弱阳性）· 姊妹 V2a（restricted-axial，偏阴性）
分支 `topic5-v3p-preictal-trajectory`（off V3a HEAD `ac042f3`，只加新文件、read-only 继承 V3a，未 merge）
...
**没看到。** 全 `n_perm=1000` 校正后：narrow 主队列 7 人、broad 复制队列 13 人（含策展核心 9），两个承重量都没有队列方向，没有一个被试过完整机制硬门。broad 里有**少数**被试在单条 label-null 上碰巧显著（`253/620/139` 的流、`1084/916` 的模态），但它们散在不同被试/不同腿、方向不一致，且**全被更严格的 rate / lag0 / phase / block / 双 span 门筛掉**——正是这些硬门要滤掉的"激活多 / 同步共激活 / 频谱-平滑假象"。所以这是一次**完整硬门阴性**（不是"发作前完全没有非轴向变化"），加固 Topic 5 主线：间期 HFO 轴是患者内共享的**粗骨架 trait**，不是发作前被逐渐重放/搬迁的路径。

---

## 1. 为什么在 V3a 之后做 V3p

V3a（`docs/archive/topic5/v3a_mode_transition_2026-07-04.md`）用"发作前 P3（−30~−10s）对比发作后 I1（+10~+30s）"两时刻差，只有个别被试像、凑不成队列（tier 2 脆弱阳性）。命门是 **I1 落在发作已点着之后**——信噪比塌 / 饱和 / 全场淹没，终点脏。V3p 把终点从"发作后"挪走，只看纯 preictal 段的斜率，是同一科学问题（轴向→非轴向搬迁）的更干净一枪。
...
- **实现**：17 个实现 commit（`84ba936`..`45c6233`，11 files，+3382）+ 2 个验收硬化 commit（`dbe72b4` admission/degenerate、`1caa656` per-cohort README）。read-only 复用 V3a 全部机制，**只加 V3p 新文件、未碰任何 V3a/V2**。测试：11 纯 + integration，`dbe72b4` 后 28 测。
...
三层已较完整：V2a（preictal criticality 是否压 HFO 轴，偏阴）、V3a（P3→I1 轴向→非轴向搬迁，tier 2 队列不支持）、V3p（preictal-only 非轴向爬升，**tier 0 干净阴性**）。三者一致把"非轴向搬迁/重放"从主叙事降级。主线口径（**待用户签字后入主文档**）：

> **trait geometry 有；state relocation 没有。** 间期 HFO 传播轴是稳定 trait skeleton（发作表达可沿它展开）；但发作前 state dynamics 没有以轴向减弱 / 非轴向搬迁的形式出现（V2a/V3a/V3p 无队列级支持）。
...
- **不建议**继续在"非轴向搬迁"上加窗/换阈值/换指标的变体（易成 fishing）——V3a + V3p 已两枪不支持。
- **建议**：(A) 完成 V3p 归档验收（本文档）；(B) 少量"阴性稳健性"敏感性（z_threshold 1.5/2.0/2.5、non-axis participation 阈、full vs safe span、global/axial positive-control slope）——只为让阴性更可信，不救主假设；(C) 把 V2a/V3a/V3p 合成一个 negative synthesis（trait/expression skeleton ≠ preictal state relocation）；V3b 定位为**模型 observability / falsification**（M3B Jacobian 在哪些参数预测非轴向 transient amplification、投影到 SEEG 覆盖后是否可见），而非"救 V3p"。

---

**产物**：`results/topic5_ictal_recruitment/v3p_preictal_trajectory/{admission.json, narrow/, broad/}`（trajectory CSV + `v3p_cohort_tier.json` + `v3p_summary_subject.csv` + `figures/`）；driver `.superpowers/sdd/big_run.sh`（80 核 / 20 job / 7h35m）。分支 `topic5-v3p-preictal-trajectory`@`1caa656`，**未 merge，待用户 review + 合并决定**。
