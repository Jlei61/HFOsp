...
**揭示了什么**：两件事，一真一假，必须分开说清楚。**第一，反向确实存在**：逐病人对着自己的零假设检验，`broad` 7/26、`narrow` 9/26 病人的反向强度显著超过随机水平（二项检验 p 分别约 2.2e-4 和 2.8e-6）——这是本次分析里最站得住的正面结论：TA、TB 这两个间期传播模板在空间场层面确实是同一根轴的两个相反方向，不是巧合。**第二，"摊成场能去噪"这个原始动机被证伪**：用留一法比较，平滑场预测未见触点真实值的准确度，在全部 26 个 `broad` 病人和全部 26 个 `narrow` 病人里都不如直接用离散触点自己的原始均值（配对检验 p≈3.0e-8，两个触点覆盖范围结果一致）——每个触点的间期均值本身已经是很多次事件平均出来的稳定量，摊成空间场的平滑步骤没有换来更稳，反而更糊。还有一个细节值得说：如果换一种"整个队列平均起来是不是普遍更负"的问法（配对 Wilcoxon 比较每个病人的观测值和他自己零假设的中位数），`broad` 这个问法下不显著（p≈0.11），只有 `narrow` 显著（p≈0.0001）——这不是矛盾，是两种问法本来就可能给不同答案：前者看"有没有个别病人打穿自己的门槛"，后者看"整个队列平均有没有系统性偏移"。**本次是缩减 `n_perm=300` 的初步结果，正式 `n_perm=1000` 全量复核和用户签核尚未完成——preliminary, pending review。**
...
### 2.4 H_supplement：去噪 LOO 可复现性 —— REFUTED（关键阴性，免疫幅值膨胀）
...
**在全部 26 个 `broad` 病人和全部 26 个 `narrow` 病人里，直接用离散触点原始均值都比平滑场预测更准**（两个底物的配对 Wilcoxon p 都是统计上能拿到的最小值，26/26 同号）。摊成平滑空间场**没有**让方向估计的留出精度变得更稳，反而更差——这是本次分析里最干净的阴性结果：把离散电极序列铺成加权空间场，不是对传播方向估计的"去噪"，至少在留一预测精度这个标准下不是。原始动机（"平滑 = 去噪，因而更鲁棒"）不被支持。
