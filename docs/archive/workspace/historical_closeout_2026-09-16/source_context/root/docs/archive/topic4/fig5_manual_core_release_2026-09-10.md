# 手放双核 Figure 5：连续读出、补回后释放 Z 与参数—转变时间
...
## 连续轨迹
首次达到操作性高活动判据为10.68秒；补回11.18–12.18秒；随后原生演化至26秒。全部高态触发时刻(ms)：[10680.0, 22220.0]。

| 窗口 | E均率(Hz) | I均率(Hz) | 安静比例 | 平均Z |
|---|---:|---:|---:|---:|
| baseline [0.5, 1] s | 17.70 | 44.08 | 0.330 | 0.971 |
| pre_refill_high [10.68, 11.18] s | 300.83 | 389.97 | 0.000 | 0.696 |
| after_release [12.68, 13.68] s | 14.13 | 35.40 | 0.500 | 0.963 |
| late [25.0, 26.0] s | 474.44 | 598.12 | 0.000 | 0.380 |
...
|---|---|---|---|
| 75.0000 | 1.00 / 4.70s | 1.00 / 8.63s | 1.00 / 16.42s |
| 95.1985 | 1.00 / 5.33s | 1.00 / 10.86s | 1.00 / 18.68s |
| 120.0000 | 1.00 / 6.40s | 1.00 / 12.77s | 0.67 / 23.27s |
...
## 文件
结果目录：`/home/honglab/leijiaxin/HFOsp/results/topic4_sef_hfo/fig5_manual_core_release_v1`。图见`figures/README.md`；参数逐运行表为`transition_times.csv`；相图箭头原数组为`phase_drift_data.npz`；科学读出见`analysis.json`与`paired_control_summary.json`；数值QA见`artifact_qa.json`。
