...
> **2026-09-10 当前患者传播线修订**：下文含历史全场 OU 和混合阈值参数化，不能直接作为新版执行默认。当前 `core_driven_input_v1` 将 E 阈值正偏移截为零，并以 core 的 E 成员掩码加载共享 OU，空间 OU 默认关闭，固定均值 Poisson 背景保留；没有强制指定事件起点。后续局部连接干预允许改变实际总量，不限于总输入守恒重分配。完整定义、执行身份与关键图见[共享设计](topic4_patient_geometry_prior_snn.md)及[当前报告](snn_model_report_current.md)。状态/分岔分支仍按各自原模型解释。

> **共享设计入口（2026-09-09）**：[患者传播几何先验下的网络拟合](topic4_patient_geometry_prior_snn.md)。core 初值由患者端点几何提供，范围、中心及有效连接参数在分版本合同下拟合；当前先验通过初始化和有界变化表达，未自动新增概率先验、损失项或修改下列物理方程。患者传播拟合与 Figure 5 状态/分岔证据分开。
