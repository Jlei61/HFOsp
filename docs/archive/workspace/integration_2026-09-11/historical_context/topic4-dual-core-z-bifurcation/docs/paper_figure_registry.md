...
| `data_driven_zm_runaway_transition` | Fig5 historical A–D candidate | `HISTORICAL` | `results/paper-ready-figure/fig5/figures/` | 旧 `joint_04_control seed1801` 单轨迹全版，保留视觉与机制谱系；它不是当前 `dualcore_s39 + Joint=1.25` 底物，不能把其中 A/B 与新的 dual-core C/D 拼成同一实验。原 strict morphology gate 亦未通过，不得写成临床发作复现或多 seed 确认。 |
| `dual_core_transition_susceptibility_v3` | Fig5-A–F current candidate | `CANDIDATE_PENDING_AUTHOR_REVIEW` | `results/paper-ready-figure/fig5_transition_susceptibility_v3/figures/` | 同轨迹三阶段 A/B/C，真正 pre-onset 的 16-site probe/sham，原生步长上侧 Z×M 两初值 80 条扫描。修复旧图 x/y 混用及 M gain 多乘一倍。支持有条件的 runaway 进入边界及空间响应变化；不支持普遍 pre-onset 放大或各向异性因果。基底仍为 Joint=1.25，非待闭合的 rev22 新候选。详见 `docs/archive/topic4/sef_hfo/fig5_model_lineage_and_susceptibility_audit_2026-09-05.md`。 |
| `dual_core_transition_story` | Fig5-A–F layout prototype | `REJECTED` | `results/paper-ready-figure/fig5_dual_core_transition_story/figures/` | 作者已拒绝的旧 v2；保留历史资产。其空间轴与 M-current 显示问题已在 producer 修正，新主图由 `dual_core_transition_susceptibility_v3` 接替。作者当前只要求进入 tonic runaway，不再把持续深调制振荡或发作恢复作为这一图的门槛。 |
| `dual_core_spatial_z_bifurcation_panels` | Fig5-E source + mechanism supplement | `SOURCE` | `results/paper-ready-figure/fig5_dual_core_spatial_z/figures/` | 同一冻结 `dualcore_s39 + Joint=1.25` 底物的 spatial-Z multi-branch atlas、同位置 low/early-ictal perturbation response 与机制补图。分支图已被新 Fig5-E 消费；perturbation response、零模空间图和 `Z_A×Z_B` finite root catalog 保留为 supplement。线型不编码 delay-aware 稳定性。 |
| `dual_core_zm_native_regime_audit` | Fig5-E replacement evidence | `DIAGNOSTIC_ONLY` | `/data/hfosp_topic4_fig45_artifacts/fig5/data_driven_dual_core_spatial_z/zm_regime_map/figures/` | 原生 0.1-ms realized-delay coarse system，frozen dual-core spatial Z × dynamic M，77 个参数格并显式比较 low/pre-fold 与 recruited 初态。结果为 low side 58 low + 19 localized，high side 16 localized + 59 tonic + 2 oscillatory；强 M 只把局部 recruited boundary 从约 `s=0.27` 推到 0.28。fold 后 5-s 轨迹仍为 localized，故该图可替代旧 E 的科学语义，但在作者接受和多 topology/resolution 前仍是诊断 source，不是热力学相图。 |
| `core_model_s3_brakeoff` | Fig5 historical candidate | `HISTORICAL` | `results/paper-ready-figure/archive/2026-08-13_non_main_figure_packages/fig5_core_model_s3_brakeoff/figures/` | 已被 canonical `fig5/` 的 data-driven Z/M 候选替代，仅保留机制谱系 |
