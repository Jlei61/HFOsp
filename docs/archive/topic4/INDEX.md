...
- 2026-07-20 MZ inhibitory-reserve recovery-timescale R2：`docs/archive/topic4/sef_hfo/mz_inhibitory_reserve_recovery_corridor_2026-07-20.md`。常数 `tau_r` continuation 数值完整，但只有孤立的 `80 s` 节点在全部三个 `q_hold` 上同时通过 entry、periodic hold、schedule 与 hybrid handoff；未满足至少 3 个连续节点，故 clean no-go，禁止运行 coupled R2。下一步仅解锁不改 E→E 的 M-gated state-dependent recovery cheap-first。
- 2026-07-20 MZ inhibitory-reserve R0/R1：`docs/archive/topic4/sef_hfo/mz_inhibitory_reserve_corridor_2026-07-20.md` 与 `docs/archive/topic4/sef_hfo/mz_inhibitory_reserve_mapping_2026-07-20.md`。R0b 证明 `.835-.845` 存在固定-q additive-exit safe strip；R1 证明线性 reserve 可稳定维持 bounded CCO，但 `tau_r=20 s` 的原始 event map 在第 5 事件提前 crossing。threshold/Hill eligibility 完整扫描未形成稳健 entry corridor。
- 2026-07-20 MZ autonomous regional latch：`docs/archive/topic4/sef_hfo/mz_spatial_autonomous_latch_2026-07-20.md`。固定背景事件可使 regional Z 自主过真实 fold；在固定 bath-resource mask 下 bath 未被招募。但 fast-M 只在 3 returns 后 finite exit，保留 4 returns 的 arms 均先离开 transfer support。base/half-dt 一致的 registered-margin clean no-go 关闭继续扫 additive-latch 参数，下一节点为 inhibitory-reserve frozen corridor。
- 2026-07-20 MZ regional frozen entry/exit：`docs/archive/topic4/sef_hfo/mz_spatial_regional_entry_exit_2026-07-20.md`。localized real fold `z_R=.8558315843`、bounded CCO post-fold、event-locked regional A exit 与恢复后同一 LLL basin 全通过；它证明 fast geometry，不等于 autonomous lifecycle。
- 2026-07-20 MZ spatial P=3 front handoff：`docs/archive/topic4/sef_hfo/mz_spatial_p3_front_handoff_2026-07-20.md`。预置 core+annulus 存在 bounded localized CCO，但 CLL/LCL fast hand-off 全 negative；问题从“无 target state”收缩为“缺 entry path”。
...
- 2026-06-16 → 06-28 **M0→M3 机制建模线（异质核 + 慢变量 + 谱相图，机制 screen 未 PASS）**：主文档 `docs/topic4_m3_stage.md`（一个主文档 + A/B 分文档）。
  ...
  - **MZ-FCXR full-conductance + Stage D（2026-07-20→22，bounded-negative 收口）**：`sef_hfo/mz_full_conductance_spatial_relay_execution_2026-07-20.md`（Stage 0A engineering ACCEPT + Stage 0B 加性力匹配 workpoint NO-GO）→ RC1 base（recurrent 电导 + 平滑饱和 `g_sat=21.6`，恢复间期工作点）→ **Stage D `sef_hfo/mz_fcxr_stage_d_branch_map_2026-07-22.md`**：把抑制刹车冻在不同磨损档、只跑快系统找"稳定高活动分支"——**没有稳健独立高分支**。同一种核区放电随磨损**连续加密**（占比 0.125→0.15 平滑 0.15→0.71，斜坡非悬崖）、最密处（D≈0.145–0.15）是**自发熄灭、种子/起点依赖的亚稳密事件串**；饱和挡住 runaway 但不造稳定高态。safe claim=*no robust independent finite-high branch; bounded elevated event trains and seed/IC-dependent metastable dense-event regimes near maximal frozen depletion*。D=0.145 标签对指标分辨率敏感、0.5 是操作阈值非动力学断点。D2 模态分析本轮不做。状态账本 `results/topic4_sef_hfo/mz_full_conductance_spatial_relay/{STATUS.md,run_manifest.json}`。下一轮=围绕亚稳密事件区构造非线性慢反馈闭环（不再扫冻结磨损轴）。分支 `topic4-mz-fcxr-stage-d`。
...
- 2026-09-01 **空间 Z/M + 持续平稳随机背景输入（fresh audit，边界已量化的阴性）**：`docs/archive/topic4/data_driven_zm_ictal_transition/spatial_zm_persistent_ou_2026-09-01.md`。把「一直有细碎随机推力」当成这块组织所处的环境（不再要求零输入自持），重跑同一冻结底物。**先补上一轮漏掉的一整段**：抑制强度网格原本从 1.0 直接跳到 0.75，补扫后发现分岔在 0.80–0.825 之间、而「所有触点读到 40–52 Hz、全片被招募」的窄带落在 0.775（正好夹在旧网格两点之间），按旧的只看触点频谱的判据能到 **14/15**（旧最好 9/15，门槛 12/15）。**但那不是振荡态**：改去量「群体放电在一个周期里被推起来又压下去多少」之后，这块组织自己的间期基线 22 Hz 节律能把放电压掉 **54%**，而所有 30–80 Hz 的高态只动了 **2–5%**、且每个兴奋细胞在每个 20 ms 窗里都在放电。据此本轮补上判据 10 的落地定义（群体率周期峰谷差 ÷ 均值 ≥ 0.20；阈值在看到定位扫描之后、任何动态轨迹被打分之前声明，结论对 (0.083, 0.543] 区间内任何阈值都不变）。29 条动态轨迹**九条 LFP 子句 0 通过、完整门 0 通过**；沿抑制强度轴是干净的取舍——深调制只在 20–26 Hz 出现，进入 30–80 Hz 就退化成浅涟漪，全轮深度最高的一条（0.313）主频是 20 Hz。⚠️ 已量化排除「深但空间不同相」（1–2 mm 局部、扣掉计数噪声后高态只有 0.06–0.07，比同尺度间期低约 18 倍）。⚠️ 本轮更正了自己两处说法：整段周期图算带功率被慢瞬变泄漏污染（已改为与门同一批 250 ms 窗）；「节律按距离从核区向远端扩散」不成立（失败/通过触点到核距离区间重叠）。**不重新标记 blocked** —— 下一步明确（改快重置环节：可消融的 E→E 短时突触抑制/恢复，或重新标定 E–I 突触时标/延迟），只是超出「纯两慢变量」的授权范围。
- 2026-09-02 **同一空间 Z/M + 持续 OU 的 tonic/global-high endpoint（prospective 3/3 确认，正式 Fig5A 已生成）**：`docs/archive/topic4/data_driven_zm_ictal_transition/spatial_zm_persistent_ou_tonic_runaway_2026-09-02.md`。Fig5A 当前只要求持续、全局招募的近饱和高态，不再把 30–80 Hz 深调制当作该子图硬门。全新 `tonic_b0_v2` seeds 1841/1842/1843 **3/3 全部门通过**；代表 seed 1842 按 onset 中位数自动选择并生成同状态 PNG/PDF/SVG。前一条“旧深调制 30–80 Hz 门阴性”不撤回；本结果也不提供 recovery/termination，因此不能封闭完整 Fig5 生命周期合同。
