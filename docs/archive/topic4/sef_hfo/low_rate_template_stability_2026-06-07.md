...
## LR-7 学习曲线：事件少，还是状态变了？

**日期**：2026-06-07
**代码**：`scripts/run_denovo_learning_curve.py` + `scripts/plot_denovo_learning_curve.py`。
**结果**：`results/topic4_sef_hfo/low_rate_template_stability/{cohort_learning_curve.json, per_subject_learning_curve/*.json, figures/denovo_learning_curve.png}`。

### 这一步在回答什么

前面三种恢复（方向 / 轴线 / 端点）都显示低事件窗从零重建不可靠。但"不可靠"有两个可能原因，必须分开：
- **(A) 事件太少**：哪怕是理想采样，M 个事件就是不够估计这套结构。
- **(B) 安静时段本身状态变了**：连续安静时段的传播结构和全程不同，不只是采样问题。

区分方法：**从全程随机抽 M 个事件**（时间打散，纯采样效应）做同样重建，画"恢复度 vs 事件数"学习曲线；再把**真实连续安静窗**（同样 M 个事件，但来自一个连续安静时段）放到同一条曲线上。
- 真实窗落在曲线**上**（同 M 同恢复）→ 失败纯粹因为事件少（原因 A）。
- 真实窗落在曲线**下**（同 M 更差）→ 安静时段本身状态变了（原因 B）。

主参照始终是**全程传播结构**（ground truth），不是发放计数。

### 朴素话结论

**测了什么**：从一段记录里随机抓 5、10、20…800 个 HFO 事件，看用这些事件能不能重建出全程的"谁先谁后传播结构"；再把真实的安静一小时（事件也少）放到同一张"恢复度 vs 事件数"的图上对比。

**怎么测的**：随机抓 M 个事件做重建、和全程比，重复多次取中位 = 学习曲线（这条曲线只反映"M 个事件够不够"）。真实安静窗如果和"随机抓同样多事件"恢复得一样好，就是单纯事件少；如果明显更差，就是安静时段本身不一样。

**揭示了什么（两个原因都有，但状态变化是主因）**：

- **事件少确实是门槛**：随机抽的学习曲线随事件数上升——方向恢复要约 100 个事件才接近饱和（M=5→0.39，M=100→0.90，M=200→0.94）。所以即使理想采样，少于约 100 个事件也重建不全方向。
- **但安静时段本身也变了**：真实安静窗系统性地落在随机曲线**下方**。同样事件数下，方向恢复差 −0.29（低事件档），轴线差 −0.13~−0.27，随事件增多差距缩小到 −0.07。这说明连续安静时段的传播结构和全程不一样——不只是采样不足，是状态漂移了。
- **关键反向确认**：发放计数也一样往下掉（同 M 差 −0.29 → −0.11）。所以"传播 vs 计数"的相对差距没怎么变——**这正是之前"扣对照后 ≈ 0"所掩盖的东西**：安静时段是一个不同的状态，传播结构和发放计数**一起**偏离全程，相减就互相抵消了。学习曲线把这个抵消打开了。
- **端点是例外**：端点集合的天花板很低（随机抽 M=800 也只 0.57），且真实 ≈ 随机（几乎无差距）——离散 4 通道指标太粗，对"状态变化"不敏感，分辨不出。

### 关键数字（ALL，全程结构为 ground truth）

| 事件数 M | 随机抽 方向 | 真实窗 方向 | 差(状态变化) | 随机抽 端点 | 真实窗 端点 |
|---|---|---|---|---|---|
| 5 | 0.39 | 0.10 | **−0.29** | 0.33 | 0.33 |
| 20 | 0.67 | 0.38 | **−0.29** | 0.38 | 0.33 |
| 50 | 0.83 | 0.52 | **−0.31** | 0.43 | 0.38 |
| 100 | 0.90 | 0.74 | −0.16 | 0.50 | 0.43 |
| 200 | 0.94 | 0.83 | −0.11 | 0.57 | 0.50 |
| 800 | 0.97 | 0.90 | −0.07 | 0.57 | 0.60 |

发放计数（操作性参照）同样下掉：随机 vs 真实 差 −0.29(M=5) → −0.11(M=200)。方向饱和点 ≈ M=100-200；端点天花板 ≈ 0.57（粗指标）。

---

## LR-7 统一结论（read-back + 三种 de novo + 学习曲线）

**主参照是全程模板（ground truth），问的是"恢复全程结构了吗"，不是"赢发放计数了吗"。** 发放计数只作操作性参照（"如果只有短窗，拿计数 top-k 当指纹实不实用"）。

**三种恢复分开报**：
- **轴线**（同一条传播梯度，不管哪头是源）：低事件窗 |·| ≈ 0.48-0.64，随机抽同 M 约 0.53-0.79，真实窗低 0.13-0.27。
- **方向**（哪头是源哪头是汇，最难）：低事件窗 signed ≈ 0.36(epi)，随机抽同 M 约 0.39-0.67，真实窗低 0.29。方向是最难恢复的。
- **端点集合**（最早/最晚通道是不是同一批，离散粗指标）：低事件窗 Jaccard ≈ 0.38，天花板低（即使多事件也 ~0.57），真实 ≈ 随机（粗指标分辨不出状态变化）。

**统一主句（写回）**：

> 全程传播模板一旦学到，低事件窗能稳定读回它，而且比发放计数更抗采样不足（read-back +0.131）；但低事件窗自己从零学习这套传播结构不可靠。失败不是单纯方向认反——轴线、端点集合也恢复不好。而且失败不只是事件少：学习曲线显示真实安静时段比"随机抽同样多事件"恢复得更差（方向差 −0.29），说明安静时段本身是一个不同的状态，传播结构和发放计数一起偏离全程。**结论是：传播模板的正确使用方式是"多事件学习、短窗读回"，不是"短窗从零发现"。**

这不降级 read-back 主结果——恰恰相反，它从机制上解释了为什么 read-back 必要且有价值：模板是一个需要用多事件/全程数据先学出来的稳定结构，学到之后才能在短窗里稳定读回；指望短窗从零发现它（尤其在状态已漂移的安静时段）做不到。

（内部归档代号：LR-7 learning curve, axis_signed/axis_abs/endpoint recovery, random-draw vs real-window, state-change gap, M_GRID, window_endpoint_stability_denovo, denovo_window_axis）
