...
**引擎迁移(并行 stream,2026-06-15)**:执行中途引擎目录由 `results/topic4_sef_hfo/lif_snn/engine/` **整体迁到 `src/snn_engine/`**(另一条工作流的重构,intentional)。迁移**保留了**本轮的 E/I 病灶 edit(`connectivity_rot.py` 12 处 `local_scale_EI/w_EE_gain_core/core_mask_E`)与 re-bless sha(`fe8527c6e440`),`engine_versions.json` 路径同步更新到 `src/snn_engine/`。后续 sim(relay screen)在新路径下正常穿过守护运行——edit + 守护一致,无需再动引擎。
...
## §6.7 "成核 vs 传播接力" 2×2 screen 执行结果(`relay_screen/relay_screen.json`)— NO survivor

| 条件 | clean directional | n_part(clean)中位 | bare-sheet 安静 | Go gate |
|---|---|---|---|---|
| ① `V_th↓` r=1.5(positive ref) | **9** | 8 | ✓ | **GO**(验证 gate:阳性对照过) |
| ② `E/I` r=1.5(NULL ref) | 1 | 7 | ✓ | 不过 |
| ③a `E/I` larger core r=2.5 | **0** | — | **✗ 破** | 不过 |
| ③b `E/I` larger core r=3.0 | **0** | — | **✗ 破** | 不过 |
| ④ `E/I` + mild `V_th` seed 17.5 | **0** | — | **✗ 破** | 不过 |

**结果:NO survivor。** 两种"外向接力条件"(加大 core / 加 mild V_th seed)**都不是把局部成核救成可读模板,而是把 bare-sheet-quiet 弄破**(背景活动爆表、仍 0 个 clean 方向模板)。阳性对照 `V_th↓` 正常 GO,说明 gate 不是过严。

**揭示了什么(把 A3 NULL 收得更紧)**:问题不只是"E/I 太弱传不出去"——把 E/I **往强里推**(更大核 / 给阈值 seed),它**先破坏"只从核点火"的安静底线,也没传成 clean pulse**。即局部 E/I 缺的是 `V_th↓` 那种**"在安静底线兼容的前提下形成一个向外接力 pulse"**的能力;在测试点上,"既安静又出模板"的窗口**没找到**。按用户协议:**无 survivor → STOP,不继续扫 `tau_m/tau_ref/sigma/tau_I`**。
