...
(一句话:"病灶内部机制不同 → 传播指纹不同"这个 A 轴主假设,**两条都是 NULL**——阈值离散度只改点火频率不改指纹;局部 E/I 病灶在测试磁量内不复现 V_th↓ 的可读方向模板。净效果是**收窄而非支持**:V_th↓ 这个唯象代理在该读出层面**不被这两类局部机制等价替代**,read-out 的方向模板签名看起来 specific to 降阈值。按 spec,V_th↓ 保留唯象标签,E/I 轴写 future direction。)
...
3. **机制解读(受 A1 约束、coherent):** A1 已证**连窄档 V_th↓(几乎无离散度)都干净传播**,所以传播不是离散度 seed 驱动的,而是**降阈值**驱动的——V_th↓ 把核周围的细胞也降到 17.0、波能招募出去;局部 E/I 只动核内平衡、核外细胞仍在 18.0 满阈值满抑制 → 波在核内停住。**read-out 的"传播方向模板"签名在该读出层面看起来 specific to 降阈值,不被局部 E/I 复现。**
...
**screen 级 caveat(不是 verdict)**:A3-0a 单 seed、T=2000、coarse 磁量格、单一 core(r=1.5/位置/单灶)、flat threshold。未测的 future directions:E/I 核**加配 V_th 离散度 seed**、**更大 core**、**调 drive/core_r**、多 seed 复现。本轮**不跑 A3 formal**(无可行工作点窗口),按"阻塞降级继续"以 screen 收口。
