...
(2) **诊断结果——"这张真网络有没有一个线性空间算子"是个测量能不能辨识的问题；目前只在两个孤立条件内
稳定辨识，不能说有一个跨状态/跨种子共同算子**：第一版审计（只 seed1、每格强度只有踢的 1/12、单条未来噪声）量出"到处
都非线性（0.4–2.6）"——那是测量假象。改用**集成平均 + 每格 RMS 匹配到踢的量级 + 平衡低波数 + 把 16 条独
立未来拆成两组各 8 条互相验证 + 检查有没有 fork 饱和**重审后，差异全面降到 0.06–0.36（确认第一版是假
象）；**但严格判据（全 16、两组各 8 的振幅一致性、两半算子一致性都过线，且没有饱和）下，9 个"种子×时刻"
里只有 2 个稳健辨识**（seed3 baseline、seed4 midpoint）。这两个模式在各自条件的独立 halves 内高度稳定，
但来自不同 seed 和 state，彼此结构/朝向不同，不能用来检验同状态跨种子复现。好几个点的"全 16 条平均"能压到 15% 线以下，但
拆成两组独立 8 条就各自越线——说明它们不稳健。所以：**"整网到处没有线性算子"不成立（是测量假象），但也
远没到"有一个可复现的、跨种子一致的轴向算子模式"**；现有证据只支持"在两个孤立条件内稳定辨识出的模式各自
有沿轴/横轴取向"，尚不能建立跨条件规律。真正跨种子复现的是固定"踢"的状态条件化空间响应。
...
### 3.2 算子可辨识性：第一版审计（假象）→ 修正后的严格复现审计
...
每状态 **N=16 条独立未来** + 把 16 拆成两组各 8 条互验 + 检查 fork 饱和；**严格判据 = 全 16 过线、两半各
8 的振幅一致性都过线、两半估计出的算子彼此也过线、且无饱和**，只有过判据才 SVD）。full = 全 16 条差异，
repA/B = 两组独立 8 条各自的振幅一致性差异；cross-half = 两半算子之间的差异。
...
σ̂₁=8.57 **u1_axis=+0.52（沿轴）** corridor_frac=0.00（沿轴但不落走廊）。两个模式在各自条件内的 8+8
half-to-half U₁ overlap 分别≈**0.996/0.999**，说明各自可稳定重现；但它们来自**不同 seed 且不同 state**，
所以彼此 overlap≈0.05 不能拆分为种子效应或状态效应，也不能充当同状态跨种子复现检验。
...
时刻，不足以定义连续"窗口"。(3) 两个孤立条件下的模式各自在条件内稳定、但结构不同；由于 seed 与 state 同时
变化，而且没有同一状态的两个稳健模式，**当前无法评估同状态跨种子复现，也没有建立共同轴向模式**。只能说个别
条件下可辨识的模式各自有取向倾向（orientation tendency）。`corrected_audit_summary.json` + 每
...
| pre_onset | 3 | 2.90 | 0.227 | 0.221 | >1（distal=0.0308, off=0） | **合格** |
...
- pre_onset：轴向走廊响应三种子一致≈0.2，**且远端走廊/离轴对照 seed1/seed4 ≈2.3**；seed3 的 matched
  off-axis 区域包含 8 格但响应为 0（distal=0.0308），因此只登记为方向性 lower bound、不报有限比值。这说明
  响应具有往汇方向铺开的倾向，而不是四面八方等量扩散；arrival-vs-distance 三种子合格（seed1 R²≈0.92，
...
- **唯一可复现的轴向证据是 fixed-kick，不是算子模式**：冻结-q 速率场是线性化，天然有干净算子/本征模，其
  ...
  算子的 U₁。
- **不能用算子模式去印证轴向**：修正审计里稳健辨识出的 2 个模式分别来自 seed3 baseline 与 seed4 midpoint；
  它们在各自条件内可重复，但结构/朝向不同（重叠≈0.05；seed4 沿轴但不落走廊，seed3 横轴）。由于 seed 与
  state 同时改变，当前既不能归因状态重组，也不能检验同状态跨种子复现，更不能当作第二条共同轴向证据。
- **本质差别**：速率场"全状态都有干净算子"在真 spiking 网络不成立——不是"SNN 没有算子"，而是**在这组测量
  设置下算子只在两个孤立条件内稳定辨识，尚不能评价跨状态/跨种子规律**（差异来源 spike/reset/delay/noise/
  非线性，spec §9）。**不
...
- **算子可辨识性（N=16 严格判据）**：第一版审计只 seed1、thin-input、单轨迹 → "到处非线性 0.4–2.6" 是假象；
  修正审计（集成 + RMS 匹配 + 平衡低波数）把差异全面降到 0.06–0.36（确认假象）。**但严格判据 = 全 16、两组
  独立 8 的振幅一致性、两半算子一致性都过线，且无 fork 饱和 → 9 个种子×时刻里只 2 个稳健辨识**（seed3 baseline、seed4 midpoint）。
...
- 种子稳健：**fixed-kick 主发现（pre_onset 走廊≈0.2、arrival 合格）跨三种子复现**；远端 distal/off≈2.3
  在 seed1/4 复现，seed3 只提供方向性 lower bound。算子仅 2/9 孤立条件稳健辨识，缺少同状态的第二个稳健模式，
  因而**跨种子复现不可评估**。

## 6. 结论口径（允许/禁止）

**允许**：同一 MZ spiking 骨架不同慢状态下有限时空间易感性不同；快到失控前同一局部踢的响应从局部转为**往汇
方向定向铺开**（远端走廊/离轴 seed1/4≈2.3，seed3 为方向性 lower bound；三种子走廊≈0.2 一致），到达时间随距离线性（compatible with 轴向
招募）；修正审计**推翻了"整网到处非线性"（是测量假象）**；在两个孤立的种子×时刻条件内稳定辨识出的模式各自有
沿轴/横轴的**取向倾向**（orientation tendency）。

**禁止**：operational runoff = 临床发作起始；复现完整间期—发作—恢复；把稀疏辨识的低波数 V₁/U₁ 说成**精确
full-SNN 本征模**、或说成"跨种子一致的轴向传播模式/复用了同一条 kick 传播路径"（没有同状态的两个稳健模式）；
说"整网到处没线性算子"（假象，已推翻）；说"midpoint 是可辨识窗口"（只有 3 个离散时刻、且
...
- **最大局限**：低波数经验算子只在 2/9 孤立种子×时刻条件稳健辨识；各自条件内稳定，但没有同状态的第二个稳健
  模式，无法评价跨种子复现，也没有建立一致轴向模式；仍不是精确 full-SNN 本征模。**fixed-kick 承重可复现结论**（状态条件化、往汇
  方向定向）；轴向招募是"像"非证明。
...
- 下一步候选（**待用户定，不自主**）：(a) 先把统计/复现坐实（已做 N=4/8/16 + 两组独立 8 + cross-half + 饱和
