...
**仍缺/待**：rate 四对照（用户在跑 runner v2）；cm 真实 4mm 尺 SNN 读出（需 density≥4000 重 build,
后调）。**LFP 前向模型 = 用户问的, 已答 = 有且验证过（2mm, current-LFP err 6.4°=firing, 16/16）**;
真实 cm 尺读出受阻于 SNN 点火（与前向模型是否成立无关——前向模型已验证）。
