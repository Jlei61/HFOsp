...
- **电极**：用干净早 baseline，30-80Hz 峰值最高的活跃触点是 **ICL6/7/5/2/3/4**（默认 300ms baseline；峰值
  ...
  **且** peak≥6dB）**0 个触点通过**——所以正确说法是"**0 个触点通过完整持续门**"，**不是**"0 个触点达 0.80"。对
  ...
  ~0.6s 的短簇**，始终连不成 ≥2s 的持续 macroepisode——所以缺口不只是"提高占空比"。**下一步的机制假设**是让不同
  局部区域**错相接力**（phase-staggered inter-region relay）把多个亚秒能量簇连成持续 macroepisode——**注意：这个
  relay 尚未在任何模型里观测到，它是待验证的候选机制，不是已观察到的现象。**（v1 说的"最优触点 occ 1.0"是安静平
  触点如 SCL6 峰值 2.2dB 的假象。）
