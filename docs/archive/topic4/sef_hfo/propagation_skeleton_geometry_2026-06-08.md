...
**揭示了什么。** 在这个尺度上看，传播骨架是厘米级的——传播主轴长度的中位数大约 18–22 毫米；起点和终点这两小撮通道在 Yuquan 里挤得相当紧凑（半径中位数约 3 毫米），在 Epilepsiae 里更松（约 7–9 毫米）、且有 4 个 Epilepsiae 被试起止 core 沿轴交错、轴长不可信（见正文 §2）；绝大多数病人的传播不是沿一根电极杆的一维直线，而是跨多根杆、铺成一片的分布式采样；沿轴的刻板顺序整条轴上都为正（但这一条**部分是模板定义出来的**，不是独立证据）。这些都是给模型用的尺度数，**不是**结论性的临床主张。
