...
- **真正还没拿到的是"行波"那一档**：本次扫到的（沿轴 `k_perp=0` 切片上的）有限 k 失稳全是**静态 Turing 花样（ω*=0）**，没有一个是**有限 k 行波（finite-k Hopf, ω*>0）**——而 SEF-HFO 要的是会**移动**的自限瞬态（对应 finite-k Hopf）。两点限定要讲清楚：(i) `leading_mode` 只扫 `k_perp=0`，所以"0 个 finite-k Hopf"严格说是**沿轴**结论（对一条传播轴够用，但不是全 2-D 判定）；(ii) 三个 regime **恢复变量全程关着（`b_a=0`）**——而慢负反馈正是教科书级的 Hopf/振荡来源，是**唯一没试过的关键旋钮**；相反**延迟是开着的（`erlang_n=2, delay_d=1.0`，已进色散矩阵）却只给出静态 Turing**。所以"会不会动起来"的**头号嫌疑是恢复变量**（和/或更大的延迟），**不是"延迟"本身**——这一点本次探针已经部分排除了"延迟一般性"。

**这意味着什么（诚实表述，不修饰）**：(1) 机器搭好、测齐、跑通，且**经体检证明有能力**产出框架要求的三区相结构与有限 k 空间模；(2) **scaffold 默认参数坐在深度稳定、k=0 的角落**，所以本次 0a/0b 全退化是参数位置问题，不是机器算不出结构；(3) **唯一真正悬而未决的科学问题**是"会移动的有限 k 行波（finite-k Hopf, 沿轴）可不可达"——头号嫌疑是**打开恢复变量 `b_a`**（本次全程关着，慢负反馈是经典 Hopf 来源），其次是更大的延迟（当前 `delay_d=1.0` 的延迟已开却只给静态 Turing）；要和数据锚定一起回答，不是只换数值。
...
**定向下一步（Abbott/Liou 2020 焦点发作传播模型）**：用户两次带出 `docs/paper/abbott_model.md`（Liou, Smith, …, Abbott 2020, *A model for focal seizure onset, propagation, evolution, and progression*）。这正对我们这唯一的悬而未决点——他们的模型**产出的就是会移动的发作传播波**，靠的是 E-I/抑制性 restraint 动力学（恰是能把静态 Turing 变成行波的那类成分）。**正式跑（Step 1 unlock）之前，建议先读 Abbott 2020 的传播机制章节**，对照我们缺的 finite-k-Hopf 成分（恢复变量 / restraint 崩溃 / 延迟），作为给 `b_a` 与延迟结构定形的参考。本次不展开分析，只登记为定向下一步。
