...
**有稳健离散带 σ∈[1.8, 2.2]**（≥60% seed，σ=2.0 在三个 τ 全是 5/5），跨 τ 50–200ms（4×）稳定 —— 不是单点、不是幸运 seed。高于带（σ≥2.5）→ sustained；低于带（σ≤1.5）→ extinction；**σ=0 确定性对照 → extinction（无噪不自发持续，full-5s 网格已含此 cell）**。

**事件率（修正口径，2026-06-03 review — 之前过报）**：rate 只在带的**低边 σ=1.8 与数据同量级**（discrete seed 的 rate ~0.3–0.5/s，落 [0.01,1]/s、贴 target ~0.3/s）。**σ=2.0/2.2 支持"可离散"但 rate 偏高**（mean rate σ=2.0：τ50 2.64/s、τ100 1.52/s、τ200 1.2/s，**超出 [0.01,1]/s 带**）。所以准确说法：**存在"既离散又率同量级"的点（σ=1.8）**；σ=2.0/2.2 是"可离散但已过驱动、率偏高"。不报"整带都率同量级"。
...
**判定（修正）**：window A primary —— **无噪不持续（σ=0/σ≤1.5 extinction）+ 噪声→宽稳健离散带（σ∈[1.8,2.2]）+ 自终止** 三项 PASS；**率同量级**只在带**低边 σ=1.8** 成立（σ=2.0/2.2 偏高，见上）—— 记为 **"存在既离散又率同量级的点（σ=1.8）"**，不是"整带率同量级"。**仍待（承重，全部 OPEN）**：
...
**当前判定（分开写，不可合并）**：
- window A **离散性四项 PASS（§9.3）独立成立**，不依赖方向测度。
- **方向承重判别项 = OPEN / UNRESOLVED**（测度待按上面 1–3 重做）。**因此现在不能写"window A 通过 Step 1"**——方向是 SEF-HFO vs 几何 artifact 的承重判据，未决。方向阻断的是 window A 的**完整**主张，不阻断离散性子结论。
