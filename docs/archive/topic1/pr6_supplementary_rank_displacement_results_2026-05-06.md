...
## 3. 主结果（paper-level 叙事）

本节按 paper-level supplementary figure 的口径写：只讲科学对象（template-pair rank reversal、SOZ contribution），**不**写 PR-2.5 内部分类系统（reproduced / candidate-fail / non-candidate / candidate gate）。PR-2.5 与本 supplementary 的方法学比对、cross-check 与方法学背景放在 §6。

### 3.1 Cohort rank-displacement heatmap

`figures/cohort_displacement_heatmap.png`：23 个 two-template subject × 通道的逐通道 Δr 热图。行按 Kendall τ 从最负到最正排序；每行内通道按 rank_T_a_dense 排序（T_a source → sink）。SOZ 通道用黑色边框标注。右侧 Kendall τ 条状图为连续单色，bar 长度本身编码 τ 值。

观察：
- 最上几行（Kendall τ < −0.5）呈"红→蓝单调梯度"，对应明显反向几何
- 中段 τ ≈ 0 的几行颜色散乱、无单调梯度 —— 反 sorting bias 校验通过（列轴严格按 rank_T_a_dense 排序，所以单调梯度只出现在真反向 subject 中）
- 最下几行（τ > 0.3）多为 white/pale 色，反映两个 template 的 rank 几乎一致
...
### 3.2 Reversal spectrum

`figures/footrule_kendall_summary.png` Panel B：23 subject 按 F_norm 从高到低排列的 bar 图。F_norm 范围 [0.393, 1.000]，**呈连续谱，不是离散二分**。

| 位置 | F_norm | 含义 |
|---|---|---|
| 高端（顶部 5 subject）| 0.92 – 1.00 | 接近完全反向（ranks 几乎全部翻转）|
| 中段（≈10 subject）| 0.67 – 0.92 | 高于 D-G 渐近随机参考的中间区间 |
| 低端（底部 8 subject）| 0.39 – 0.67 | 在或低于 D-G 渐近随机参考，反向几何弱或不存在 |

**2/3 是 Diaconis-Graham 渐近随机置换期望**（n→∞），不是显著性阈值。它在图上作为参考刻度出现，**不**作为分类器。"F_norm 高"的正确读法是"比均匀随机置换的渐近期望更大的归一化 footrule"，**不是**"反向显著存在"。

### 3.3 SOZ contribution does not predict reversal strength

`figures/footrule_kendall_summary.png` Panel C：x = F_norm，y = SOZ contribution_excess（baseline-corrected，已减去通道占比 baseline）。Marker 形状仅区分 dataset（Epilepsiae 圆 / Yuquan 方）。

cohort 上 23 个 subject 的散布范围：
- F_norm 范围：[0.39, 1.00]
- soz_contribution_excess 范围：[−0.36, +0.19]，median ≈ 0
- soz_channel_fraction median ≈ 0.55

**Spearman ρ(F_norm, soz_contribution_excess) = 0.193, p = 0.376, n=23** — 几乎不相关。

**诚实 negative finding**：rank reversal 强（F_norm 高）的 subject 并没有系统性的 SOZ enrichment。即使在 F_norm 高端 15 subject 中，soz_contribution_excess 散布在 [−0.07, +0.19]，median ≈ +0.07，effect size 小且方向不齐。在 lagPat 选定通道集内（§5.1 caveat），rank reversal 程度不能由 SOZ-vs-nonSOZ 通道占比解释。

通道选择本身已把"全脑"裁剪过了，所以这个 negative finding **不能**推到"SOZ 在全脑层面不参与反向"的强声明。详见 §5.1。

## 4. 图

- `results/interictal_propagation/rank_displacement/figures/cohort_displacement_heatmap.{png,pdf}` — Panel A：two-template subjects (n=23) × 通道 Δr 热图（行按 Kendall τ 排序，列按 rank_T_a_dense 排序，连续 τ bar）
- `results/interictal_propagation/rank_displacement/figures/footrule_kendall_summary.{png,pdf}` — Panel B：ranked F_norm spectrum；Panel C：F_norm vs SOZ contribution_excess scatter
- `results/interictal_propagation/rank_displacement/figures/per_subject/<subject>_displacement.png` — 每 subject 详图（共 23 张，supplement / debug 用）
...
本 supplementary 的 Panel B/C 经过四轮 user review 才收敛到 paper-level 设计：
...
5. **v5（当前）**：ranked F_norm spectrum + F_norm vs SOZ excess scatter，**完全去 PR-2.5 内部分类**，paper-level 只讲连续谱

每一轮的具体批评见 worktree git log；当前归档文档与 figure 已收敛到 v5。
