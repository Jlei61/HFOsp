...
- `src/interictal_propagation.py`
  - `load_subject_propagation_events()` — `start_t` 排序 + 绝对时间重建 + block/event 合同 + `lagPatRaw` loading
  - `load_subject_propagation_patterns()` — backward-compatible wrapper
  - `validate_absolute_lag_clustering()` — PR-4B Step 0: relative lag reconstruction + order validation + stratified Pearson r
...
- `scripts/run_interictal_propagation.py`
  ...
  - `--pr4b-step0`：PR-4B Step 0 absolute lag validation batch
...
### 7.8 模板调控读数框架与 PR-4B/4C/4D 合同

#### 框架定义

PR-4 系列回答的核心问题是：**固定传播模板受到什么慢调控？**

读数分三个独立类别（详见 `docs/topic1_within_event_dynamics.md` §6）：


| 读数类别            | 因变量                                 | 指标                            | 数据来源                        | 状态                  |
| --------------- | ----------------------------------- | ----------------------------- | --------------------------- | ------------------- |
| **模板混合 / 模式选择** | Occupancy fraction；template-decomposed absolute event rate | Cluster fraction per time bin；per-template rate envelope + stacked count histogram | `lagPatRank` cluster labels | PR-4A done；PR-4D accepted |
| **模板内顺序一致性**    | Within-cluster τ                    | Pairwise Kendall τ            | `lagPatRank`                | PR-4B done          |
| **模板内相对时延结构**   | Within-cluster lag span / Pearson r | Absolute lag statistics       | `lagPatRaw`                 | PR-4B done          |
...
- 模板内顺序一致性（tau）是纯 ordinal — 通道顺序一致性，但对 lag compression 盲。86% identity-bias 使其对 H2 的检测灵敏度有限，所以它是辅助检测层
- 模板内相对时延结构需要 `lagPatRaw` → 相对 lag，捕获 Kuramoto coupling modulation (H2) 最自然的预测：lag compression。这是 H2 的主检测层
- MI 是 `np.sign()` 比较，对 absolute lag 与 rank 给出完全相同结果，不适用于模板内相对时延结构读数
...
**科学问题**：慢 rate state 改变的是模板混合、模板内顺序一致性，还是模板内相对时延结构？
对应 `docs/archive/topic4/layered_model_framework.md` 的 H1（纯触发门：后两类读数不变）vs H2（触发 + 耦合调制：后两类读数随 rate 升高）。

**Step 0 — Absolute lag validation（DONE 2026-04-13）**：

核心发现：

1. Per-event min-subtraction 正确：**30/30 subjects exact order match = 1.0**，pairwise concordance = 1.0。`lagPatRank` 和 `relative_lag` 完全保持 participating channel 内的序。
2. Dominant cluster Pearson r cohort median = **0.601**（mean = 0.543, range 0.213–0.925）
3. Pooled eligible median Pearson r = **0.453**（mean = 0.466）— 修正了初始版本 pooled median 导致的 validation_pass 假阴性（小簇弱 stereotypy 拉低均值）
4. **8/30 subjects** dominant cluster median_r > 0.7 → pass
5. **1/30** (huangwanling) eligible_frac = 0 → 模板内相对时延结构读数完全 ineligible（仅 n_part=3–4 事件）
6. Dominant τ 与 dominant Pearson r 的 **Spearman ρ = 0.910, p < 0.0001 (n=29)**

关键洞察：

- 低 Pearson r 不是 min-subtraction 的 bug，而是与弱 ordinal stereotypy 高度共变 — 这些 subject 的 rank clustering 本身就不紧
- Rank-based clustering 是 absolute-lag clustering 的"有噪声代理"：ordinal 好 → cardinal 也好，但反过来不保证
- 后续相对时延结构的 rate-coupling 分析应做**全 cohort (n=29) + high-confidence subset (dom_r > 0.7, n=8)** 两层报告
- validation_pass 已修正为基于 dominant cluster median_r（不再是 pooled eligible median_r），避免小簇弱 stereotypy 污染判断

代码入口：

- `validate_absolute_lag_clustering()` in `src/interictal_propagation.py`
- `--pr4b-step0` flag in `scripts/run_interictal_propagation.py`
- 结果：`results/interictal_propagation/pr4b_lag_validation.json`
...
- **模板内顺序一致性**：within-cluster pairwise τ (raw + centered)（辅助层，86% identity-bias 限制灵敏度）
- **模板内相对时延结构**：within-cluster mean lag span（需 n_participating 匹配）；within-cluster Pearson r on relative lag vectors（**仅 n_part ≥ 5 事件**）
- **模板混合**：occupancy fraction per cluster
...
- 报告 n_part ≥ 5 eligibility rate per subject（若某 subject 80% 事件 n_part < 5，该 subject 的相对时延结构结论需降级为 exploratory）

**统计单元**：subject。不做 pooled event-level p-value。报告为 exploratory H1/H2 判别，不是 mechanistic proof。

**实现**：`src/interictal_propagation.py` 新增 `compute_rate_state_coupling()` — 同时输出模板混合、模板内顺序一致性、模板内相对时延结构
...
#### PR-4C 合同：Seizure proximity（双轨口径）

**科学问题**：发作邻近改变的是模板混合，还是固定模板内部的结构？

**前置依赖**：PR-4B 完成后再做。PR-4B 提供模板内顺序一致性与模板内相对时延结构的基础指标实现和 absolute lag 验证层。

**分析内容**：

1. 借用 Topic 2 PR-2.7 已有的 seizure-triggered rate framework 定义 pre-ictal [-6h,-1h] / baseline [-12h,-6h] / post-ictal [+1h,+6h] 窗口
2. **次级描述层**：seizure-triggered **rate×type** trajectory —— 发作前后固定模板分解后的绝对事件率曲线与离散计数，先回答“哪个模板贡献了 rate 变化”
3. **辅助层**：within-cluster τ trajectory — 发作邻近的模板内顺序一致性变化（灵敏度受 identity-bias 限制）
4. **主检测层**：within-cluster lag span trajectory + Pearson r — 发作邻近的模板内相对时延结构变化（仅 n_part ≥ 5 事件，**lag span 比较需 n_participating 匹配**）
...
- `results/interictal_propagation/figures/pr4c_seizure_triggered_template_mix.png`
- `results/interictal_propagation/figures/pr4c_seizure_triggered_lag.png`

#### PR-4D 合同：Template-rate decomposition（PR-4A 描述层补强，2026-04-16 accepted）

**科学问题**：当总 event rate 随时间变化时，变化来自哪个固定模板？  
不是“把 occupancy 画得更平滑”，而是把总 rate 分解到 template 上。

**最终接受版读数**：

对每个模板 `c`，在时间点 `t` 上定义 Gaussian KDE 绝对事件率：

`λ_c(t) = (3600 / (h * sqrt(2π))) * Σ_i exp(-(t - t_i)^2 / (2h^2)) * 1[label_i = c]`

- `h = smoothing_hours * 3600`
- 单位：`events / hour`
- `t_i` 只来自**真实观测窗口内**的事件
- 暴露时间 denominator 只按 `coverage_ranges` 求和，**不允许**把 recording gap 算进 rate

同时输出离散辅助层：

- `n_c(b)`：每个固定模板在离散 bin `b` 内的事件数
- 这个柱图是辅助，不是主读数；它的作用是让人看清楚 smoothed envelope 没有凭空发明结构

**画图合同（已验收版）**：

1. 每个 subject **只保留一张 PR-4D 图**
2. 上面板：每个固定模板的 absolute rate envelope（同色折线）
3. 下面板：相同模板颜色的离散计数堆叠柱
4. `xlim` 从 0 开始，`ylim` 从 0 开始
5. recording gap 必须留白：
  - 不得在 gap 内制造零计数 bin
  - 不得用连续折线跨 gap 连线
6. 不额外生成“为了看起来完整”的 cohort 图；只有当 cohort 图回答了**per-subject 图和文字都不能直接回答**的问题时，才允许存在

**为什么拒绝最初版本**：

最初的 PR-4D 方案把 occupancy 连续化、再加一个所谓 `rate-aware` 版本。这个设计被否掉，原因很直接：

1. 它没有回答新问题，只是把离散比例曲线画圆滑了
2. 它会把 gap 错画成低活动或连续轨迹
3. 它把真正该看的 absolute rate 信息归一化掉了

**当前结果（descriptive only）**：

- dominant template rate fraction cohort median = **0.584**（range `0.262-0.866`）
- `25/30` subject 至少有一次主导模板交叉
- 若只看总 rate 高于各自 peak 的 `25%` 的区间，仍有 `17/30` subject 出现交叉
- `6/30` subject 出现反复交叉（`>=3` 次）：`590`, `1125`, `916`, `958`, `253`, `922`
- `5/30` subject 在当前口径下没有任何主导模板交叉：`chenziyang`, `hanyuxuan`, `litengsheng`, `sunyuanxin`, `xuxinyi`

**解释边界**：

- 这些交叉是**描述层现象**，不是正式统计结论
- 很多交叉仍发生在 rate 下降段，所以不能直接写成“状态切换机制”
- 但 `17/30` 在中高 rate 区间仍出现交叉，说明“主导模式切换并不只是尾部噪声”

**当前 batch 的投射质量**：

- 当前 30-subject batch 中，`n_events_used == adaptive_cluster_labels` for `30/30`
- 也就是说，这一版 PR-4D 读数没有因为 fixed-template projection event dropout 被系统性污染

**输出**：

- `results/interictal_propagation/pr4a_followup_template_mix_dynamics.json`
- `results/interictal_propagation/figures/per_subject/*_pr4d_template_rate.png`
...
- ~~**PR-4B Step 0**：absolute lag validation~~ ✅ 已完成（2026-04-13）
  - 30/30 order match = 1.0；dominant cluster Pearson r median = 0.601；Spearman(τ, r) = 0.910
  - 8/30 pass (dom_r > 0.7)；1/30 ineligible (huangwanling)
- **PR-4B Step 1–3**（P0）：rate × 模板内顺序一致性 + 模板内相对时延结构 — H1/H2 判别核心实验
- **PR-4C**（P1）：seizure proximity 双轨分析（次级描述层直接复用 PR-4D 的 rate×type 图，主检测层仍是模板内相对时延结构）
- ~~**PR-4D**（P1）：模板混合读数补强（continuous / rate-aware / switch-dwell）~~ ✅ **已验收为 template-rate decomposition 描述层**
...
- PR-4A 已经回答了"占比是否昼夜漂移"（答案：很弱）；PR-4D 则把这层描述补成了 gap-aware 的 `rate×type` 读数。下一步应转向 PR-4C，直接问发作邻近时是谁在贡献总 rate 变化，以及固定模板内部的相对时延结构是否改变
- PR-4B 的 H1/H2 判别结果将决定论文叙事：如果模板内顺序一致性/相对时延结构在高率状态更紧 → H2 更有支持；如果无差别 → H1 更有支持，Layer A 偏结构性
...
- **PR-4B Step 0 完成**（2026-04-13）：absolute lag validation 30/30, dominant cluster Pearson r median = 0.601, Spearman(τ,r) = 0.910
