...
### Step 4b — Node-level template-pair anatomy（preliminary，2026-04-26）

**交付**：
- `src/template_anatomical_anchoring.py`：新增 `classify_template_pair_nodes`（5 类节点划分：`swap_node` / `same_side_node` / `template_specific_endpoint` / `shared_endpoint_unassigned` / `non_endpoint`）+ `soz_breakdown_by_node_class`（每类节点 SOZ + focus_rel-i/l/e tally）
- `scripts/run_pr6_template_anchoring.py`：所有 endpoint-defined subject 算 node anatomy；per-subject JSON 加 `node_anatomy` field（含 classification + soz_breakdown + swap_minus_same_side_count）
- Cohort summary 加 `node_anatomy` block，含 4 分层（all_endpoint_defined / h1_eligible / forward_reverse_reproduced / non_forward_reverse_h1_eligible）的 median counts + **subject-level paired tests**（swap−same Wilcoxon + sign test、SOZ frac swap−tspec Wilcoxon + sign test、SOZ frac swap−same_side Wilcoxon + sign test）+ pooled SOZ fraction（标记为 NOT valid cohort claim，仅作 transparency）
- 5 项新 TDD（full swap / identical / template-specific only / partial mixed pattern / SOZ breakdown bipolar）；测试 `tests/test_pr6_template_anchoring.py` + `tests/test_interictal_propagation.py` 一起跑 **78/78 全绿**（其中 PR-6 子集 29 项，interictal_propagation 子集 49 项）

**Cohort 数字 — subject-level paired tests 才是 cohort claim**（pooled 数字保留但仅作 transparency）：

`swap − same_side` 数（per subject）：

| Stratum | n | median | n_pos / n_neg / n_zero | Wilcoxon-greater p | sign-test p |
|---|---|---|---|---|---|
| h1_eligible | 21 | +2.0 | 14 / 6 / 1 | **0.012 ✓** | 0.115 ✗ |
| forward/reverse reproduced | 6 | +5.0 | 6 / 0 / 0 | **0.016 ✓** | **0.031 ✓** |
| non-fwdrev h1_eligible | 16 | +1.0 | 9 / 6 / 1 | 0.180 ✗ | 0.607 ✗ |

`SOZ frac swap − SOZ frac template_specific`（per subject，仅在两类都非空时计入）：

| Stratum | n | median Δ | n_pos / n_neg / n_zero | Wilcoxon-greater p |
|---|---|---|---|---|
| h1_eligible | 19 | 0.000 | 9 / 4 / 6 | 0.191 ✗ |
| forward/reverse reproduced | 4 | +0.167 | 2 / 0 / 2 | 0.090 ✗ |
| non-fwdrev h1_eligible | 15 | 0.000 | 7 / 4 / 4 | 0.328 ✗ |

`SOZ frac swap − SOZ frac same_side` 在 non-fwdrev (n=12) 也是 NULL（median 0, p > 0.4）。

**节点 count median (per subject)**：

| Stratum | n | med swap | med same | med tspec |
|---|---|---|---|---|
| h1_eligible | 21 | 3.0 | 1.0 | 4.0 |
| forward/reverse reproduced | 6 | 5.0 | 0.0 | 2.0 |
| non-fwdrev h1_eligible | 16 | 2.5 | 1.5 | 4.0 |

**Pooled SOZ fraction**（**NOT** cohort claim — channel-count-weighted artifact，保留作 transparency）：
- h1_eligible: swap 0.702 (40/57), same 0.640 (16/25), tspec 0.568 (50/88)
- fwd/rev: swap 0.621 (18/29), tspec 0.429 (6/14)
- non-fwdrev: swap 0.735 (25/34), same 0.640 (16/25), tspec 0.595 (44/74)

**关键发现 + caveat**：

1. **Cohort-wide swap-leaning geometry — 部分支持**：
   - h1_eligible cohort `swap − same_side` Wilcoxon-greater **p=0.012**（α=0.05 通过），但 sign-test p=0.115（不通过）
   - 14/21 = 67% 正方向，**Wilcoxon 检测到 magnitudes 系统偏正，但 sign-test 仅看符号没达到 0.05**
   - 这是个混合结果 — Wilcoxon 与 sign-test 不一致，老实记录两个数字

2. **fwd/rev 子集 swap-leaning 强（但部分 tautological）**：
   - 6/6 全部 positive，sign-test **p=0.031**，Wilcoxon **p=0.016**
   - 但 fwd/rev 子集是 PR-2.5 按 rank 反向选的，swap > same 是部分预期；这里的真正贡献是把 cluster-rank 反向**翻译成节点级 swap 计数**

3. **non-fwdrev 子集 swap-leaning 不显著**：
   - 9p/6n/1z，median +1，Wilcoxon p=0.18，sign-test p=0.61
   - **non-fwdrev 单独看不构成 cohort-level swap-leaning 证据**
   - h1_eligible cohort 的 Wilcoxon 显著主要是 fwd/rev subset 拉的

4. **Regime B (non-fwdrev) 局部 swap 检查 — heterogeneous（per-subject 表，不依赖 cohort claim）**：
   - 590: 0 swap, 1 same, 10 tspec → 几乎全 template-specific（接近 "独立网络" 端 — 仅 1 subject）
   - 1084, 1096, 253: same_side > swap
   - 1146, 442, 583, 922, chengshuai: swap 与 same_side 共存
   - 其余 non-fwdrev 多 swap > 0 但小：weak partial local swap
   - **是 spectrum，不是离散 regime**

5. **Regime A (fwd/rev) 非特异性节点检查**：
   - 5/6 fwd/rev subjects same_side = 0；chenziyang 也是 0 same_side 但有 4 template-specific
   - fwd/rev subjects 倾向于"全 swap 无 same"，但**仍可有 template-specific endpoints (n=2-4)**

6. **Subject-level SOZ 富集 swap vs template-specific 是 NULL**：
   - h1_eligible (n=19)：median Δ=0，9p/4n/6z，Wilcoxon p=0.19，sign-test n_pos/n_neg=9/4 不显著
   - 所有 strata 都不显著
   - **结论：swap nodes 与 template-specific endpoints 之间没有 cohort 级 SOZ 富集差异**
   - Pooled 数字（swap 0.702 vs tspec 0.568）是 channel-count-weighted artifact，不是 cohort claim

**论文级口径（已收紧）**：

✅ 可以说：
- "Swap-leaning template-pair geometry is detectable cohort-wide on Wilcoxon (h1-eligible n=21, p=0.012) but does not reach significance on the more conservative sign test (p=0.12); the cohort-wide effect is largely driven by the forward/reverse-reproduced subset"
- "Forward/reverse subset shows strong node-level swap (6/6 positive, sign-test p=0.031), operationalizing PR-2.5 cluster-rank anticorrelation; this is partially expected from the way that subset was selected"
- "Non-fwdrev subset alone does not show cohort-significant swap-leaning (Wilcoxon p=0.18, sign-test p=0.61); per-subject swap counts span from 0 (subject 590) to moderate, NOT a single regime"
- "**The swap-vs-same geometry is not explained by subject-level SOZ enrichment** (h1-eligible n=19, median Δ=0, Wilcoxon-greater p=0.19, sign-test 9p/4n/6z)"

❌ 不能说：
- ~~"Swap-vs-same geometry is independent of clinical SOZ annotation"~~ — 我们只检验了 swap vs template_specific 的 SOZ 富集差异 NULL；没检验整套几何结构是否独立于 SOZ
- ~~"Cohort-wide swap-leaning is robust"~~ — sign-test 不通过，主要由 fwd/rev 拉
- ~~"Two discrete regimes"~~ — fwd/rev vs non-fwdrev 是 quantitative spectrum
- ~~"Non-fwdrev subjects are independent networks"~~ — 仅 590 接近，其他 partial swap
- ~~"Swap nodes are SOZ-rich nodes"~~ — pooled artifact，subject-level null

**Step 4b 验收（preliminary）**：
- ✅ `tests/test_pr6_template_anchoring.py` + `tests/test_interictal_propagation.py` 一起跑 78/78 通过
- ✅ Per-subject JSON 含完整 node anatomy + SOZ breakdown
- ✅ Cohort summary 4 分层含 **subject-level paired Wilcoxon + sign test**（swap−same、SOZ swap−tspec、SOZ swap−same）— P0/P1 review fix 已落
- ✅ Pooled SOZ fractions 仍报告但显式标 "NOT cohort claim, channel-count-weighted artifact"
- ✅ pooled vs subject-level 区分明确

**Step 4 整体（4 + 4b）综合科学叙事**：

> Stable interictal HFO group-event template pairs show **swap-leaning node geometry that is detectable cohort-wide on Wilcoxon (h1-eligible n=21, swap_node count − same_side_node count: median +2, p=0.012) but fails the more conservative sign-test (p=0.12)**. The cohort-wide effect is driven primarily by the forward/reverse-reproduced subset (n=6, all 6 positive, sign-test p=0.031), which translates the PR-2.5 cluster-rank anticorrelation into node-level swap counts. The non-forward/reverse subset alone (n=16) does not reach cohort significance (Wilcoxon p=0.18) and spans a heterogeneous spectrum from no-swap (subject 590, all template-specific endpoints) to partial local swap. **The swap-vs-same geometry is not explained by subject-level SOZ enrichment** (h1-eligible n=19, SOZ frac in swap-nodes minus SOZ frac in template-specific endpoints: median Δ=0, Wilcoxon-greater p=0.19, sign 9p/4n/6z); pooled SOZ fractions (swap 0.70, tspec 0.57) are channel-count-weighted aggregates and not valid cohort claims. Endpoint geometry is time-stable within a fixed metric (Step 5b), but H1 SOZ-anchoring direction is sensitive to the choice of endpoint metric (Step 5a, 7/20 direction-discordant). **Single-metric H1 cannot determine whether stable templates anchor clinical SOZ; the more robust observation is the structured node-level pair geometry, which is itself not explained by clinical SOZ annotation**.

节点级 anatomy 是真正区分 fwd/rev 与 non-fwdrev 的精细镜片，**且**它揭示了 cohort 级别的 paired SOZ 检验在 swap vs template-specific 上是 null — 这两件事一起给出比单纯 H1 更稳健的 paper-level 故事。

**Step 6 / Step 3 启动条件（再更新）**：Step 4 + 4b 都已落定，可以进 Step 3 + Step 6。Step 3 主图建议三联：
1. Per-subject node anatomy bar chart（swap / same / tspec count，按 fwd/rev 状态着色）
2. Subject-level SOZ frac scatter (swap_frac vs tspec_frac, diagonal, n=19)
3. Split-half robustness scatter (split_a_endpoint vs split_b_endpoint Jaccard)

---

### Step 3 — Publication figures（preliminary，2026-04-27）

**交付**：
- `scripts/plot_pr6_template_anchoring.py`（新建，~570 行）：复用 `src/plot_style.py` Morandi palette，所有图导出到 `results/interictal_propagation/template_anchoring/figures/`
- 1 张主图（6-panel）：Panel A 单 subject 节点示意（epilepsiae:635 fwd/rev），Panel B split-half × odd-even endpoint Jaccard 散点（fwd/rev 高亮），Panel C n_swap_node vs n_same_side_node 散点 + subject-level Wilcoxon/sign-test 标注，Panel D per-subject 节点解剖 stacked bar（按 swap−same 降序，fwd/rev 顶端 F/R 标记），Panel E fwd/rev vs non-fwdrev 在 swap_score / same_side_score / Spearman ρ(T0,T1) 三指标 violin，Panel F SOZ frac 在 template-specific vs swap-node paired 散点 + 主结论 stats
- 3 张 supplements：coreness sensitivity paired scatter（直接展示 7 direction-discordant + 1 one-is-zero）；full-data T0∩T1 endpoint Jaccard per-subject bar；fwd/rev 6 subject T0 src ↔ T1 snk 角色互换 small multiples
- `figures/README.md`（中文，AGENTS.md 规范）：每张图 2–4 句解读 + "**关注点**：" 行；明确不给读者写成"independent of clinical SOZ annotation"，收紧成"swap-vs-same geometry is not explained by subject-level SOZ enrichment"

**Step 3 验收（preliminary）**：
- ✅ 6-panel 主图 + 3 张 supp 全部生成（4 个 PNG，1 个 README.md）
- ✅ 主图中心是 template-pair geometry（Step 4 / 4b）而非 H1 SOZ null —— H1 null + Coreness 不 robust 已经在 Step 2 / Step 5a 各自归档过，主图不再以此为视觉中心
- ✅ Panel C / F 文字标注里把 cohort-wide swap-leaning 的 partial 性质（Wilcoxon p=0.012 vs sign-test p=0.115）直接显示给读者，不让画面假装强阳性
- ✅ Panel D 排序使 fwd/rev 集中在左侧、non-fwdrev 异质性右侧，让读者直接看出"swap-leaning 主要由 fwd/rev 拉动"
- ✅ Panel A 把节点 5 类（swap / same-side / template-specific / shared / non-endpoint）映射到一个真实 subject 上，定义直观可溯源

**已知不足**（用户验收时不满意，登记为 Step 3 v1，后续 v2 时优化）：
- Panel A 的 schematic 在多 channel 下密度偏高，T0/T1 列对应"role"和"node class"两套 legend 共存，对单图而言信息密度还偏大；下一版本可考虑：(a) 分到独立的图、(b) 缩到 6 channel 案例、或 (c) 只保留 connector 颜色 + 一套 legend
- Panel D 的 stacked bar 信息丰富但对 cohort 的快速理解仍不够"一眼看懂"，建议后续单独做一张"按 fwd/rev 分组的双 violin 图"作为视觉摘要
- Panel E 现有 violin 在 n=5 子集上很稀，需后续补 individual jitter dots 或者 boxplot+strip 替代

**v2 主图修订建议**（Step 6 之前再做一次）：
- 主图改 4 panel：A bar=fwd/rev vs non-fwdrev 三指标对比 violin（Panel E 提升至 A）；B = swap−same paired sign-flip dot plot；C = SOZ frac swap-vs-tspec paired scatter；D = split-half 时间稳定性散点
- Panel-A 节点示意拆出去做成独立 schematic figure（不进 main figure）
- 减少 5 类堆叠 bar 的视觉重量，把它放到 supplement

**Step 3 不影响接下来的科学结论**——结论本身在 Step 2 / 4 / 4b / 5a / 5b 都已经落档；Step 3 的不满意主要是表达侧而非证据侧。

---

### Step 6 — 科学性总结 + 下一步验证（preliminary，2026-04-27）

**核心问题（论文级）**：间期事件刻板时序能否成为一个**重要的癫痫病理网络指示器**？

**PR-6 给出的诚实答案**：
- ✅ **能描述**间期 group event 的传播在 cohort 内是 stereotyped 的（PR-2 30/30 stable adaptive cluster；PR-2.5 87% identity-bias；PR-3 MI 30/30 显著），且 stereotypy 内部存在**结构化的 pair-level 几何**（Step 4：fwd/rev 子集为干净 swap，non-fwdrev 子集为弱混合 spectrum；Step 4b：节点 5 类分类与计数稳定）
- ❌ **不能直接将这套 stereotypy 标定为 SOZ 锚定**：H1 endpoint vs middle SOZ enrichment cohort-level **NULL**（Step 2 pooled Wilcoxon p=0.32），且 endpoint 主定义本身在 7/20 subject 上与 coreness 定义方向不一致（Step 5a），定义敏感
- ❌ **swap geometry 也不能直接归因于 SOZ 富集**：subject-level paired test (swap-node vs template-specific endpoint) median Δ=0、Wilcoxon p=0.19、sign-test p=0.27（Step 4b）

**当前归宿**：PR-6 为 Topic 1 给出的不是"stereotypy = SOZ 锚定"capstone，而是"stereotypy 内部存在结构化几何但与 clinical SOZ 标注不简单同构"的去 framing 结论。这本身仍是科学上有信息量的结果（**stereotypy 不依赖于 clinical SOZ 标签即存在**），但与"间期 stereotypy = 病理网络指示器"的强论文论点之间还有差距。

**最值得做的下一步验证（按收益排序）**：

1. **PR-6 endpoint vs seizure-onset channel 单 subject overlap**（高收益，最便宜）：H1 用了 SOZ JSON（粗粒度），但 Topic 1 archive 已经 reproduce了 Yuquan/Epilepsiae 的 seizure detection 与 onset estimation。把 endpoint（不是 middle）和 **per-seizure onset channel set** 做 per-subject Jaccard / hit-rate（不是 SOZ binary），cohort 用 subject-level Wilcoxon。这检验"endpoint 是否锚定真实癫痫起病通道"，比 SOZ JSON 二值化更严，且不需要重做 PR-2/3/4。预算 0.5 d。
2. **PR-6 endpoint vs PR-4B/4C rate-state 高放电期 dominant cluster 复现**（中高收益，跨 PR）：PR-4B Step 2–3 在高放电期 8 个 high-confidence subject (`dom_r > 0.7`) 上 7/8 显示 lag span 更大、dominant cluster Pearson r 更高（p=0.016）。把这 8 个 subject 的"高放电期 dominant cluster 的 endpoint" 与"低放电期 dominant cluster 的 endpoint"做 Jaccard，看 endpoint 在状态切换时是否稳定 / 是否更靠 SOZ。这把 PR-6 与 Topic 1 内部最强的 H2-supported 子结论联起来。预算 1 d。
3. **PR-6 forward/reverse subject 与 ictal propagation 方向独立比对**（中收益，需要 ictal 轨迹）：fwd/rev 6 subject 上的 T0 source / T1 sink 是否和**真实 ictal LFP propagation onset → end** 通道有一致性（reuse Topic 3 PR-2.5 epilepsiae onset estimation），即"间期 stereotype geometry 是否预测 ictal 传播路径"。这条是论文最强的可能 capstone，但需要 ictal LFP 轨迹分析，预算 1.5 d 起步。
4. **PR-6 in held-out time（不重选 subject）**：PR-2.5 已做了 split-half；下一步是用前 50% 时间训练 template，后 50% 做 endpoint anchoring SOZ test，避免 PR-2 + PR-6 在同一份数据上选 subject 又选指标的隐性 double-dipping。预算 0.5 d。
5. **PR-6 negative control —— interictal control time vs pre-ictal time 的 endpoint anatomy 对比**（中收益）：reuse PR-2.7 seizure-triggered window，把 endpoint 在 pre-ictal vs baseline 上分别建 template，看 endpoint 通道集合是否在 pre-ictal 偏移。这是把 stereotypy 直接挂在 ictal 时间轴上的最干净检验。

**优先级建议**：
- 短线（1 周内）：1 + 4，把"endpoint 锚定真实 onset"的故事确认或证伪 + 解决 double-dipping 担忧
- 中线（2–3 周）：2 + 5，在已知 PR-4B 强信号子集上加一层 PR-6 endpoint geometry 验证，并把 stereotypy 挂上 ictal 时间轴
- 长线（论文 capstone）：3，在 fwd/rev subject 上做 ictal vs interictal propagation 路径直接对应

**显式不做（防止漂移）**：
- 不在当前数据上重新调 PR-6 endpoint 定义（top-3、coreness、median 之外）—— Step 5a 已证明定义本身敏感，再调反复会陷入 garden of forking paths
- 不重做 PR-2/3/4/4B 的核心 cluster pipeline
- 不引入 ictal anchor / ER / CUSUM（PR-6 pivot 时已显式排除）

**Step 6 验收标准（推迟到下一步验证执行后再写正式归档 doc）**：当 §1 或 §4 的 held-out / dual-time 验证有结果后，再写 `docs/archive/topic1/pr6_template_anchoring_results_2026-05-xx.md` 与 topic1 §7 / topic3 §7 一句话回写。当前 PR-6 主线（Step 1–5b + 4 + 4b + 3）所有结果归档到本文档 §15。
