...
5. **1/30** (huangwanling) eligible_frac = 0 → L3 完全 ineligible（仅 n_part=3–4 事件）
...
- 后续 L3 rate-coupling 分析应做**全 cohort (n=29) + high-confidence subset (dom_r > 0.7, n=8)** 两层报告
...
- **PR-4B Step 1–3**（P0）：rate × L2 + L3 coupling — H1/H2 判别的核心实验
