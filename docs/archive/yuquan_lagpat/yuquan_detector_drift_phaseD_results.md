...
后台执行：`scripts/archive/yuquan_lagpat/_phaseD_run_references.sh`，单 GPU 串行：
...
Phase D 的 detection 全部用了 `--gpu`（参见 `scripts/archive/yuquan_lagpat/_phaseD_run_references.sh`），detector envelope 那一段 (`_compute_envelope_gpu`) 走 cusignal `firwin(201) + fftconvolve + hilbert + abs`，和 legacy `p16_cuda_24h_bipolar.py:band_filt_cu` **逐行相同的 GPU 实现**；`_find_high_enveTimes_gpu` 也是 cuPy 复刻。所以 detector 主路径**没有** GPU↔CPU 差。
...
- Pre-D 旧 detection 已搬到 `results/hfo_detection/<subject>.pre_phaseD_backup/`
- 编排: `scripts/archive/yuquan_lagpat/_phaseF1_run_yuquan_main.sh`
- 接力: `scripts/archive/yuquan_lagpat/_phaseE_to_F1_chain.sh` (pid 688904) 在 Phase E runner 退出且 `_runner.log` 含 "Phase E ALL DONE" 时自动 launch
...
新脚本:
  ...
  scripts/archive/yuquan_lagpat/_phaseF1_run_yuquan_main.sh     # Phase F-1 detection runner
  scripts/archive/yuquan_lagpat/_phaseE_to_F1_chain.sh          # watchdog (auto-launch F-1)
后台进程:
...
scripts/archive/yuquan_lagpat/_phaseD_run_references.sh
