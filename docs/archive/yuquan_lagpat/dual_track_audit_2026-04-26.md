...
- **Track A** 跑完：21 个 subject 里 13 个支持 `threshold_sensitive_FP_drift`，1 个 (zhangjiaqi) 因为 legacy 是 monopolar reference 而新代码统一用 bipolar 被显式排除在 detector-equivalence 主张外，1 个 (pengzihang) 是 degenerate evidence，6 个无 legacy gpu_npz。Cohort 标签 `coarse_logic_divergence` 完全是 zhangjiaqi 一项拉低的，不是普遍失败。
...
→ 这 13 个 subject 的 detector 行为可以 **支持** "detector 找到的是同一批 HFO，仅有 phys-FP threshold-edge 进出"。
...
`coarse_logic_divergence` —— 这个 Cohort verdict 标签是**因为 zhangjiaqi 一项**触发的（Layer 1 要求 alias-collapsed legacy_containment 中位 ≥ 0.95 + max |global_shift| ≤ 1.25 ms，zhangjiaqi 的 7.5 ms 把 max 拉爆了）。

如果只看 13 个 detector-comparable subject，cohort verdict 是 `physical_FP_only_threshold_sensitive`。

文档行文必须显式区分这一点，不能写成"14/14 detector cohort 全过"或"detector 阶段普遍失败"——两种说法都不准确。
