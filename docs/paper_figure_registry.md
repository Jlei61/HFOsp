...
| `dual_core_spatial_z_bifurcation_panels` | Fig5-C/D candidate | `CANDIDATE` | `results/paper-ready-figure/fig5_dual_core_spatial_z/figures/` | 同一冻结 `dualcore_s39 + Joint=1.25` 底物：C 是异质空间快子系统中 core-A per-neuron mean E rate 的区域群体分岔读数，不冒充 literal single-neuron bifurcation；D 是同一 rev21 轨迹在 runaway 前低态与 early-ictal 状态、同一 16 个随机位置、同一 16-cell 弱脉冲下的 exact-resume paired probe–sham 平均响应。零模空间图与 `Z_A×Z_B` finite root catalog 已降为机制补图，不占用 D。只可与同一 dual-core 底物重画的 A/B 合并，不能直接和旧 `joint_04_control seed1801` A/B 拼成同一实验。 |
