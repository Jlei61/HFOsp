...
> **Figure 4/SNN 设计口径（2026-09-09）**：新一轮采用[患者传播几何先验下的网络拟合](topic4_patient_geometry_prior_snn.md)，展示几何初始化、分布拟合及参数 × 观测作用。此为设计更新；下表现有 Figure 4/5 的资产身份和验收状态保持各自记录，运行中的 pilot 尚未替换正式图。
