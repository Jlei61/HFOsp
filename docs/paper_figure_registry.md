...
| `dual_core_spatial_z_bifurcation_panels` | Fig5-C/D candidate | `CANDIDATE` | `results/paper-ready-figure/fig5_dual_core_spatial_z/figures/` | 同一冻结 `dualcore_s39 + Joint=1.25` 底物的尺度递进机制图：C 是 core-A 内 per-neuron mean E rate 的局部分岔图，不冒充 literal single-neuron bifurcation；D 左是 runaway-entry 零模的 20 mm 物理空间图，D 右是固定 `Z_surround=0.80` 的 `Z_A×Z_B` 11×11 finite multi-start root catalog。只可与同一 dual-core 底物重画的 A/B 合并，不能直接和旧 `joint_04_control seed1801` A/B 拼成同一实验。 |
