...
| `data_driven_zm_runaway_transition` | Fig5-A–D | `CANDIDATE` | `results/paper-ready-figure/fig5/figures/` | 当前消费 `fig5-data-driven-zm-etoi005-main`；data-driven Joint 底物、Z/M 全开、作者目视选定 E→I 5% 单轨迹。A 为连续 readout 与全局招募，B 为 Z/M 投影，C 为事件顺序与早期高态活动，D 为单网络配对 probe canary。原 strict morphology gate 未过，不能写成临床发作复现或多 seed 确认 |
