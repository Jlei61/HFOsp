...
| `data_driven_zm_runaway_transition` | Fig5-A–D | `CANDIDATE` | `results/paper-ready-figure/fig5/figures/` | 当前消费 `fig5-data-driven-zm-etoi005-main-v4`；data-driven Joint 底物、Z/M 全开、作者目视选定 E→I 5% 单轨迹。A 为连续 readout 与全局招募，红线是原形态检测后 300 ms 的可见全局高态工作点；B 为同一时刻 Z/M 投影；C 用 Fig4 原始 0–20 mm sheet x/y 展示一次真实间期事件的逐 E 神经元 first-spike order，以及红线后 0–100 ms 的蓝色全场 rate-squared 能量图；D 只画 1.0 s 低活动态和红线时刻的 exact-resume probe-minus-sham 响应，不叠慢场。高态 E1 ignition 不可评价；原 strict morphology gate 未过，不能写成临床发作复现或多 seed 确认 |
...
| `spatial_zm_qigk_global_oscillation` | Fig5-A proposed replacement | `BLOCKED_NO_ELIGIBLE_ARTIFACT` | `results/topic4_sef_hfo/data_driven_zm_ictal_transition/spatial_zqim_hybrid/` | 完整 learned EE/EI 上的空间 Z/qI–M/gK screen；23 个 frozen-q + 17 个 gK 状态均未过冻结全局门。最佳 40–46 Hz 只有 9/15 触点持续节律；全片 gK 降为 20–26 Hz。formal producer 要求同一参数至少 3 个新 seed 全过，当前不生成图片；见 2026-08-31 archive 报告 |
