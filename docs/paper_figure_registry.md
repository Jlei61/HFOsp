...
| `data_driven_zm_runaway_transition` | Fig5-A–D | `CANDIDATE` | `results/paper-ready-figure/fig5/figures/` | 当前消费 `fig5-data-driven-zm-etoi005-main-v3`；data-driven Joint 底物、Z/M 全开、作者目视选定 E→I 5% 单轨迹。A 为连续 readout 与全局招募，红线是原形态检测后 300 ms 的可见全局高态工作点；B 为同一时刻 Z/M 投影；C 用 Fig4 原始 0–20 mm sheet x/y 分别展示间期事件范围/方向与红线后 0–100 ms 全局招募；D 只画 1.0 s 低活动态和红线时刻的 exact-resume probe-minus-sham 响应，不叠慢场。高态 E1 ignition 不可评价；原 strict morphology gate 未过，不能写成临床发作复现或多 seed 确认 |
