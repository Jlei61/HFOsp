...
| `working_subject_snn_fig4` | Fig4-A–G | `CANDIDATE` | `results/paper-ready-figure/fig4/figures/` | 当前参数化 E1146 工作版：A/B setup，C 为空白 data-driven reserved slot，D/E 双向读出，F/G validation；后续替换为 data-driven core-field 版本，当前不是最终锁图 |
| `data_driven_snn_dual_mode_validation` | Fig4-C source | `SOURCE` | `results/topic4_sef_hfo/data_driven_core_field_rev10_d/spatial_ou_accessibility_d5_2_confirmation/figures/` | data-driven SNN 唯一双图规范源：连续场 + MTA/MTB same-network readout，以及复用 Figure 1E painter 的 KMeans 核验图；图型已确认，但当前 natural KMeans 未复现患者 TA/TB（direction purity `0.674` < patient-matched q05 `0.884`，pooled MTB↔TB `−0.60`），不能升级为 `LOCKED`。2026-08-14 重绘：裁定与限定符已上画布 |
