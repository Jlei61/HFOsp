...
| `spatial_zm_ou_tonic_saddle_organizer` | Fig5-A–D | `CANDIDATE` | `results/paper-ready-figure/fig5_spatial_zm_ou_tonic/figures/` | 当前消费 `fig5-spatial-zm-ou-tonic-main-v2`，严格使用作者指定的静态 A–D 布局。A=seed 1842 同一连续轨迹的全局招募与 15 触点 tonic readout；B=`q_core/q_mean/M/rE` 对 patient-matched 1 mm critical manifold 的投影；C=同一 seed 的低态 first-spike order 与 onset 后固定 100 ms firing energy；D=同一 weak probe 在低态和 early-runaway 的 exact-resume probe-minus-sham 响应。代表 seed 来自 frozen prospective 1841–1843 全过家族。B 的 saddle-node 只属于 reduced frozen-q fast subsystem，高支为 delay-unstable skeleton；三网格 fold rate 未收敛。D 的高态 susceptibility 因 sham 已 tonic 而不可评价，只作 saturation-response 描述。禁止写临床发作复现、稳定高 fixed point、精确有限 SNN 阈值或 phase transition |
| `data_driven_zm_runaway_transition` | Fig5-A–D historical candidate | `DIAGNOSTIC_ONLY` | `results/paper-ready-figure/fig5/figures/` | 旧 `fig5-data-driven-zm-etoi005-main-v4` 为作者目视选定 E→I 5% 单轨迹，已被 prospective 3-seed full-edge OU/ZM 候选替代；保留作工作流历史，不再作为当前 Fig. 5 source |
| `pure_zm_high_recruitment` | Fig5-A diagnostic candidate | `DIAGNOSTIC_ONLY` | `results/paper-ready-figure/fig5_pure_zm_dynamics/figures/` | 只消费完整 learned EE/EI、`primary_zm_only=true` 的 full-edge 候选；按模型内部 1 秒 broad-recruitment duty 预先选中 `ith080_ta0062`。该轨迹有 120 Hz/100 ms 操作性 runaway 和持续广泛高率招募，但触点频率 16.0→13.9 Hz，未通过完整 Fig5 高频形态资格；不替代当前 Fig5 A–D，也不支持恢复、多 seed 或临床发作复现 |
| `spatial_zm_qigk_global_oscillation` | Fig5-A deep-oscillation alternative | `BLOCKED_NO_ELIGIBLE_ARTIFACT` | `results/topic4_sef_hfo/data_driven_zm_ictal_transition/spatial_zqim_hybrid/` | 完整 learned EE/EI 上的空间 Z/qI–M/gK deep-oscillation screen；23 个 frozen-q + 17 个 gK 状态均未过冻结全局门。最佳 40–46 Hz 只有 9/15 触点持续节律；全片 gK 降为 20–26 Hz。formal deep-oscillation producer 仍不生成图片；该阴性不再阻断作者已接受的 tonic/global-high Fig. 5 endpoint。见 2026-08-31 archive 报告 |
| `core_model_s3_brakeoff` | Fig5 historical candidate | `HISTORICAL` | `results/paper-ready-figure/archive/2026-08-13_non_main_figure_packages/fig5_core_model_s3_brakeoff/figures/` | 已被 `spatial_zm_ou_tonic_saddle_organizer` 候选替代，仅保留机制谱系 |
