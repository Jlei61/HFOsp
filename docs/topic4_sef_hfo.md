...
> `layered_model_framework`）、SEF-ITP 队列方向分析整条（`src/sef_itp_phase1/2/3`、`sef_itp_direction_axis`
> 及其 scripts/tests/archive）。**保留**：SNN engine、`sef_hfo_*` rate field/LIF、phase4_v2 LIF 理论、
> stage2/3/4、M-stage（见 §13 + `docs/topic4_m3_stage.md`）。本文里 §3 H1–H6 数据合同 / §6 Phase 1-3 段
> 中指向已删 itp 目录的链接为悬挂引用，待下一轮内容精炼时清理（结构已收口，引用清理是 follow-up）。
...
> **M4 spatial P=2 frozen-fast（2026-07-20）**：canonical M3B single-core/full-complement Galerkin reduction 已锁真实面积 `f_core=.0490` 与 `f^T K=f^T`。LL、同步/相对相位 CC 均保存且 base/half-dt 一致；但四个 `CL` phase 在 fixed K 下全部回 `LL`，在 `K=I`（shared pool 保留）下全部越出 transfer support。当前 p=1 shared pool 对小核心太弱，而完整低态 bath 对 core 的 coupling 又过度压制，因此没有可维持 focal C sheet。反向 `LC→CC` 由 95.1% active surround 主导，不能当 local front。P=2 full-complement 会稀释 boundary wake，故不作 wavefront no-go；下一步只做 P=3 core–equal-area-annulus–bath，不进入 slow latch 扫参。入口：`docs/archive/topic4/sef_hfo/mz_spatial_p2_frozen_sheets_2026-07-20.md`。
>
> **M4 spatial P=3 frozen-fast（2026-07-20）**：质量守恒 core–equal-area-annulus–bath 约化中，预置 core+annulus 的四相位均形成 bounded localized `CCO`：约 29–30 returns、period `.6705–.6708 s`、period CV `<2.5e-7`、bath 0 returns，base/half-dt 与 Poincaré closure 均通过；约 136-Hz 瞬时峰不是 sustained ceiling。与此同时四个 `CLL` 与四个 `LCL` phase 全回 `LLL`，所以 target local cycle 存在，但 fast core→annulus hand-off 不存在。入口：`docs/archive/topic4/sef_hfo/mz_spatial_p3_front_handoff_2026-07-20.md`。
>
> **M4 regional frozen entry/exit geometry（2026-07-20）**：regional `z_R` continuation 找到 localized real fold `z_R=.8558315843`（23D fast leading eigenvalue 约 `-2.10e-8+0i/ms`，bath critical-mode fraction `2.08%`）；同一 low history 在 `.8560` 保持 LLL、`.8555` 进入 bounded CCO。cycle 已完成至少 4 returns 后，regional `A=(A,A,0)` 在 `z=.855/.850` 的首个确认 exit 为 `.02/.12 mV`，且参数恢复后 `16/16` 回同一 LLL basin。8 个 frozen gate 与 base/half-dt 全通过，但这仍只是慢变量可穿越的几何。入口：`docs/archive/topic4/sef_hfo/mz_spatial_regional_entry_exit_2026-07-20.md`。
>
> **M4 autonomous regional Z–p–M latch（2026-07-20）**：固定-seed ordinary focal-event challenge 下，slow-off 保留 6 次 returning events；共享 regional Z 在两套 dt 均于 `11.0 s` 自主跨 fold。在预注册 fixed bath-resource mask 下，bath `z=.90`、0 returns；这不是 emergent containment。完整 slow-loop 得到 registered-margin clean no-go：`tau_m_up=200 ms` 可 finite exit，但只有 3 个 pulse-free returns；`.115/225 ms` 与 delayed `.120/100 ms` 均保留 4 returns，却分别在约 `13.91/13.61 s` 先离开 transfer support。225-ms arm 的第4次 annulus return 后仅约 `18.9 ms` 即失败，因此不能排除无稳健余量的极窄中间值，但它违反 `<0.25 return` stop rule。六个 outcome 的 base/half-dt 标签完全一致，没有合格的 `>=4 returns + finite exit` arm。失败来自同一 Z-use law 在 recruited CCO 中继续把 exit boundary 向上推，不是 A 的符号错误；关闭继续扫 `tau_m/p_on/Amax`，下一 cheap node 改为 **depletable + nondepletable inhibitory reserve 的二维 `(q_I,A)` frozen corridor**，dynamic threshold 只作条件第二臂。入口：`docs/archive/topic4/sef_hfo/mz_spatial_autonomous_latch_2026-07-20.md`。
>
> **M4 inhibitory-reserve R0/R1（2026-07-20）**：R0b 在 fixed effective inhibitory capacity `q=.835-.845` 上确认了连续 additive-exit safe strip：smooth occupancy-gated M ramp 可在不越 support 的情况下退出，并在参数恢复后回到同一 LLL basin。R1 随后从完整 CCO sensor 反解 `q_res/tau_D`，24/24 phase×source-dt periodic orbits 均有界、contractive 且 exact closure；但原 `tau_r=20 s` 的不规则六事件 map 在第 5 事件提前跨 entry fold。threshold/Hill eligibility 的完整 864-row root scan 仍没有对阈值扰动稳健的 safe cell，因此关闭“再加一个单调 eligibility gain”的路线。入口：`docs/archive/topic4/sef_hfo/mz_inhibitory_reserve_corridor_2026-07-20.md`、`docs/archive/topic4/sef_hfo/mz_inhibitory_reserve_mapping_2026-07-20.md`。
>
> **M4 inhibitory-reserve recovery R2（2026-07-20）**：将常数 `tau_r` 正式 continuation 到 `20-160 s` 后，30/30 参数映射与 4,800-row root scan 均数值闭合，完整 periodic oracle 全通过；延长恢复确实把 entry ordering 推迟到第 6 事件。但 entry 需要慢恢复保存跨事件耗竭，而 postictal latch reset 需要快恢复到 `q=.885`，两者只在孤立的 `80 s` 节点对全部三个 `q_hold` 同时成立。预注册的至少 3 连续节点门失败，canonical status 为 clean no-go，coupled R2 不运行。`72/88 s` fixed-parameter sensitivity 只证明局部轨迹不脆弱，不能替代全 q-hold corridor。下一步只做 **M-gated state-dependent recovery** 的 scalar/path necessary-condition test；它不改 E→E/conductance，也不等于 Hopf、autonomous lifecycle 或 spatial containment。入口：`docs/archive/topic4/sef_hfo/mz_inhibitory_reserve_recovery_corridor_2026-07-20.md`。
>
...
- `docs/topic4_sef_hfo.md`（本文）= Topic 4 模型层 formal entry
