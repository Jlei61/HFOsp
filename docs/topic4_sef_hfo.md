...
> 标签 `10S_BOUNDED_OSCILLATORY_CONTINUATION_EXPOSED_BY_D_CLAMP`（2026-08-16 复审收窄，原名读起来像已证明稳定 attractor），两个快照一致。
> **允许说**：在这两个自然访问到的状态上，D 的持续加深是后续冲顶的**必要驱动**，H 的持续增长**不是必要条件**。
> **不得说**：D 单独产生发作；H 不重要；已证明稳定 ictal attractor；已证明 bistability。
...
> ② **六个场都没有检测到宏观的初始化分裂**（四个独立读数：逐细胞率向量相关 ≥ 0.9953、粗空间图
> 相关 ≥ 0.99584、相位对齐后群体率相关 ≥ 0.9325）。⚠️**零延迟群体率相关全部为负（−0.42 ~ −0.47）**：
> 两条是同一节律的不同相位，零延迟判读会把相位差报成结局差异。⚠️三条轨迹接受同一串输入，
> 因此**分不开**"唯一吸引子"与"多个吸引子被共同噪声同步"，正式判决只写
> `NO_MACROSCOPIC_INITIALISATION_SPLIT_DETECTED; MONOSTABLE_BOUNDED_BURST_REGIME_CANDIDATE`；
> ⚠️**不得由此推出"没有载体"**——单稳极限环同样可以承载发作，只是不需要迟滞，
> 分辨它的是**扰动回归实验**（未做）；⚠️入口处 D 与 H **同时**变化，本轮判定不了是谁打开了它；③ 随 `D` 单调长大的是这个爆发串**有多强多大**（25→65 Hz、90→311 mm²），
> **不是它是不是爆发串**（爆发频率恒在 10.0–10.5 次/秒）。
> **⚠️ 流程缺陷已修**：round 2 的裁决原本由画图脚本产生（注册规则标三个候选，画图里的事后容差
> 把它们改读成同一结局）。结局距离已独立成 `src/topic4_fcxr_lc6b_outcome.py`（11 测试），唯一裁决入口
> `scripts/finalize_topic4_fcxr_lc6b_outcome.py` → `atlas_outcome_adjudication.json`，画图只读不判。
> 注册分类器标签与结局向量**分开报告**。
