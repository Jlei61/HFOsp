...
- **dual-core spatial-Z 更新（2026-09-04，development）**：同一 `dualcore_s39 + Joint=1.25` realized graph 的 10×10 deterministic reduction 已确认多条 spatial fixed-point families 与 saddle-node fold chain。Fig.5C 现画 21,332 点/14 folds 的 global-recruited family 和 8,002 点/6 folds 的 core-A-localized family；两族不手工补接，线型不再外推稳定性。OU-on 中位截面有 6 个空间根；保留全部 realized delays 并包含 mean/instantaneous diffusion-variance gain 的 matched coarse assay 中仅低根稳定，而高率 fixed points 虽线性失稳，300 ms OU 下仍可驻留于邻近高活动 regime。故低支 fold 控制低根的存在边界，但不独自决定随机转变时刻或高态 attractor。Fig.5D 保持同位置 paired probe–sham 的 low/early-ictal 响应语义。该结果不是患者 Z 场、临床发作阈值、热力学相变或完整 SNN 稳定性定理。入口与边界见 `docs/archive/topic4/sef_hfo/dual_core_spatial_z_bifurcation_result_2026-09-04.md`。
- **Fig.5 A–F transition-story 候选（2026-09-04，development）**：A/B 绑定同一条 40,000-cell dual-core、continuous-OU、Z/M-on SNN 轨迹并严格共用时间轴；C 的 interictal / pre-onset / early-recruited maps 是该时间轴上的三个精确窗口；D 使用冻结临床 strict-broadband cohort statistic；E 复用 verified spatial-Z multi-branch atlas；F 使用既有 `tau_z×tau_m` operational-runaway latency 网格。真实 tonic endpoint 的群体率与全片招募上升、但 10–250 Hz virtual-contact band power 下降，因此 B 以 population rate 表示 runaway magnitude。C 尚缺多 seed 空间轴确认，D 不是 individualized two-core prediction，F 尚缺 `depletion strength×tau_z` 专扫。producer、图和待补分析见 `docs/archive/topic4/sef_hfo/fig5_dual_core_transition_story_v2_2026-09-04.md`。
- **Fig.5 振荡与 Z×M regime 审计（2026-09-05，development）**：原 `dt=0.2 ms` 的 `s=0.28,tau_GABA=8 ms` 通过点在原生 0.1 ms 下变为 tonic，确认是步长伪影；完整原生 25 格另找到 `s=0.27,tau_GABA=9 ms` 窄窗（30 Hz/depth 0.207），5 s 持续且与 low basin 共存，但主网/zoom 仅 1/25 与 2/15 通过。新增含 4×/8× M 扩展的 77 格 native spatial-Z × dynamic-M 图显示 low/pre-fold 初态为 58 low + 19 localized，recruited 初态为 16 localized + 59 tonic + 2 oscillatory；强 M 仅把 `s≈0.27` 的 recruited boundary 推到约 0.28。fold 后的 low/pre-fold 轨迹在 `s=0.34/0.36` 延长 5 s 仍为 localized，故 fold 控制低根消失而不保证 global runaway。完整 40,000-cell `tau_GABA=8/4/2 ms` 与 `si=0.8,tau_GABA=9 ms` 虽达到全区域高率和 52/59/66/54-Hz 群体主峰，但 250-ms + 100-ms 联合 persistence 均为 SCL 0/4；2-ms 新 recorder 还显示 `Z_A/Z_B/Z_surround=0.547/0.503/0.690`，证实动态 Z 是异质的。尚无可用于 A 的持续全空间 runaway oscillation。当前 E/F 原图仍分别只是 branch atlas 与 latency response surface；新增 Z×M 图是 E 的 development regime-map 证据，F 仍缺 no-transition/right-censor。完整结果见 `docs/archive/topic4/sef_hfo/dual_core_spatial_z_bifurcation_result_2026-09-04.md` §7。
- **Fig.5 stable / tonic-runaway 分界（2026-09-05，development）**：按作者接受的 near-saturated tonic plateau 语义，另冻结末 1 s population `>=300 Hz`、三分区各 `>=250 Hz`、半窗漂移 `<=5 Hz` 的 runaway gate。带动态 M 的 localized equilibrium fold 精确为 `s=0.355450824`，但其两侧 fixed points 在原生 realized delays 下均被约 37.26-Hz 复模线性失稳，因此不能把它叫作 delay-system stable/runaway bifurcation。同 low/pre-fold 初值、原生 `dt=0.1 ms` 的 10-s 精扫把 operational boundary 夹在 `0.428 < s_c <= 0.429`；两侧是稳定的 `221.8 Hz` partial state 与 `316.2 Hz` tonic runaway。原生 carried-state 检验先在 `s=0.428` 驻留，再把 rates/synapses/全部 delay histories/M 原样带到 `0.429`，仍于 2.889 s 后越过 300 Hz，确认该 bracket 夹住 bounded attractor 的上侧 escape boundary，而不只是初值分类线。同一 `s=0.40` 的 low/high 初值仍分别到约 `105/398 Hz`；连续状态 0.5-ms discovery 上/下扫又出现强滞后（上扫 `0.465→0.470` 完成 runaway 驻留，下扫至 `s=0.30` 仍不退出）。后者数值尚需 native-dt 复核，但已排除无历史依赖的唯一阈值。当前可写 nonlinear attractor bifurcation/escape bracket；在 periodic-orbit continuation/Floquet 分析前，不在 limit-cycle fold 与 boundary crisis 之间定型。证据与诊断图见同一 archive §7.4。
