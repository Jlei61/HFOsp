...
- **Z/M carrier→lifecycle 线收口（2026-07-24 → 2026-08-01）**：M4 之后主线转成 per-neuron 慢变量（\(z\) 抑制效力 + \(m\) 适应）的 SNN-native 生命周期尝试。**核心目标（自发间期 → 有界发作 → 自终止 → 恢复）尚未建立**：entry→runaway 成立；加除法共享抑制池后 runaway 被整形成有界但低占空 burst train，不过持续载体门；frozen visited-state 上存在持续 tonic branch，但不是已证明的 ictal 振荡；Phase C 在用户授权下以 post-result futility stop 收（59/59 `tonic_non_AI`，调制深度 0.025–0.045 vs 门 0.20，**不是**三-seed 完整 bounded negative）；Phase D 的全程 conductance replacement 在 **baseline preservation 门**零放电 NO-GO，carrier arms 未运行。2026-08-01 的量级复核保留四项 **design-risk diagnostics**：当前 \(m\) 电流尺度小、\(z\) 不能独自承担 offset、\(S_G\) 缺少长 post-activity memory、Phase-C tonic 工作点靠近不应期上限；它们帮助约束下一版设计，但**不是 lifecycle 不可能性的算术证明**。当前安全口径：Phase C/D 已完成各自阶段判决；下一节点必须先在冻结 slow state 下建立 observation-consistent fast carrier，再依次打开 entry、offset、recovery、control。索引见 `docs/topic4_sef_hfo.md` 状态块 + §13；诊断复核 `docs/archive/topic4/sef_hfo/zm_lifecycle_feasibility_screen_2026-08-01.md`。
...
**当前进展（2026-08-01，Z/M 生命周期线收口）**：M4 之后我们换到“每个细胞各自带两个慢变量”的做法，想让同一网络自己走完“平时间歇事件 → 有界发作 → 自终止 → 返回平时”。现有仿真只建立了病理 scaffold、返回式间期事件和 entry→高活动的链条；进入后的状态要么 runaway、要么低占空 burst train、要么局部约 439 Hz 的低调制 tonic branch。Phase D 的 conductance replacement 又在 baseline gate 把间期事件压成零放电，因此没有真正测试 carrier。量级复核显示当前 \(m\) 较弱、\(z\) 是 entry latch、\(S_G\) 的 post-activity memory 短、Phase-C 工作点高率；这些是新设计必须处理的风险，**不是四条数学必要条件，也不能推出 lifecycle 在该衬底上不可能**。下一步 rev2 不再完全串行 D1–D6：先立即运行旧 checkpoint 上的 \(\phi\) phenotype discovery，再把 top candidate 放回 \(\phi/z/m/S_G\) 从间期起全动态的 reachable vertical slice，同时看 native/controlled exit 与 recovery；只有出现 lifecycle-compatible candidate 才补多 seed、真实数据和正式确认。（`docs/topic4_sef_hfo.md` 状态块 + §13；诊断复核 `docs/archive/topic4/sef_hfo/zm_lifecycle_feasibility_screen_2026-08-01.md`）
