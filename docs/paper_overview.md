...
- **Spatial Z/M 相图与 deterministic fold（2026-09-03）**：持续 OU 打开的有限 SNN 已确认 tonic global runaway，但单-seed 双初值 pilot 仍是 low→broad intermediate→tonic-high，没有严格 `L/H` 同点共存。使用实际患者邻接、阈值场与 self-consistent M 的 1 mm reduction 经 pseudo-arclength + Jacobian 实零模确认 saddle-node（`eta_m=0.02: q_fold=0.890825886`）；它与 OU-on SNN 经验边缘 `q≈0.80–0.825` 不重合。故允许写“reduced frozen-q fast subsystem has a saddle-node”，禁止写有限 SNN phase transition；仍缺网格收敛、colored-OU closure、delay-aware Hopf 与 3-seed empirical denominator。详见 `docs/archive/topic4/data_driven_zm_ictal_transition/spatial_zm_phase_bifurcation_analysis_2026-09-03.md`。
