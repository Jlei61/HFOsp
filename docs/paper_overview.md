...
- **Figure 5 candidate（2026-07-19）**：当前候选改为 E1146 同一条连续 SNN 轨迹上的 state-dependent readout：runaway 前一个明确的单次 TB 群体事件在完整 15-contact montage 上给出 contact rank，operational runaway onset 后、下一 pulse 前的能量场在相同 registered plane 上与其 earliness 呈正相关（单轨迹 Spearman=0.814）。该图支持 `same scaffold, different state` 的模型侧可行性 bridge，不是临床 seizure、cohort 统计或终止/恢复证明。四个 SCL contact 均有 readout peak，但 local E-neuron gate 为 0/4，因此不能写局部 SCL 组织已直接招募。主图合同：`docs/fig5_snn_state_readout_spec.md`。
- **M4 MZ inhibitory slow-path update（2026-07-21，独立 worktree）**：R3 先证明 M-gated q recovery 在 frozen-sensor/path 层可把常数恢复的孤立节点扩成 `80/90/100 s` corridor；闭合 q→fast feedback 后，旧 event-6-first calibration 因 event 5 提前过 fold 而 formal no-go，但按真实 entry 对齐出现 4 次 core/annulus autonomous paired bursts。R4 不重标 q，从该真实轨迹继续，完成一次真实 latch reset、M 自然释放、同一 LLL basin，以及相同六事件 challenge 的 protected no-cycle / recovered recurrent-cycle 分离；共同 20-s checkpoint 后的 recovery 与 recovered challenge 在 `.125/.0625 ms` 一致，原 Segment A 仍为 `.125 ms` source。当前只支持 **fixed-bath three-patch center-point hybrid lifecycle existence proof**，不支持 electrical silence、零输入 spontaneous onset、continuous spatial wavefront、smooth bifurcation 或 full SNN；三条下游 workflow 仍冻结，下一门是 fully coupled local robustness + causal ablation。证据见 `docs/archive/topic4/sef_hfo/mz_actual_entry_lifecycle_closure_2026-07-21.md`。
...
**Figure 5 当前候选（2026-07-19）**：在 E1146 同一条 `q_I build-up → operational runaway` 连续轨迹中，runaway 前一个单次 TB event 的 15-contact recruitment order 与 onset-locked early-runaway energy field 同向（单轨迹 earliness–energy Spearman=0.814）。这使模型叙事从“只有间期双向 readout”推进到 `same scaffold, different state`，但证据层仍是单模型 observation-layer bridge；不是完整 seizure cycle，也不能把 4/4 SCL contact peaks 写成 SCL 下方局部组织招募（local gate 0/4）。
