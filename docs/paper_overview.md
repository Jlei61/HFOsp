...
**当前进展（2026-08-01，Z/M 生命周期线收口）**：M4 之后我们换到“每个细胞各自带两个慢变量”的做法，想让同一网络自己走完“平时间歇事件 → 有界发作 → 自终止 → 返回平时”。现有仿真只建立了病理 scaffold、返回式间期事件和 entry→高活动的链条；进入后的状态要么 runaway、要么低占空 burst train、要么局部约 439 Hz 的低调制 tonic branch。Phase D 的 conductance replacement 又在 baseline gate 把间期事件压成零放电，因此没有真正测试 carrier。量级复核显示当前 \(m\) 较弱、\(z\) 是 entry latch、\(S_G\) 的 post-activity memory 短、Phase-C 工作点高率；这些是新 spec 必须处理的风险，**不是四条数学必要条件，也不能推出 lifecycle 在该衬底上不可能**。下一步收缩为 frozen-slow D1 fast-carrier existence，先测试已经实现但尚未运行的 per-neuron dynamic-threshold feedback；只有 carrier 通过 observation、空间和扰动返回门，才依次开放 entry、offset、recovery、control 与 shared-scaffold causality。（`docs/topic4_sef_hfo.md` 状态块 + §13；诊断复核 `docs/archive/topic4/sef_hfo/zm_lifecycle_feasibility_screen_2026-08-01.md`）
