...
- **dual-core spatial-Z 更新（2026-09-04，development）**：同一 `dualcore_s39 + Joint=1.25` realized graph 的 10×10 deterministic reduction 已确认 spatial saddle-node fold chain。低支 fold 是 deterministic runaway boundary；rev21 的 100 个 OU-on operational transition 中位发生在低/高根共存区，说明 OU 可早于 fold 触发 basin crossing。Fig.5C 只能称异质空间系统的 core-A local population bifurcation readout；Fig.5D 已恢复为同位置 paired probe–sham 的 low/early-ictal 平均响应，并显示单轨迹 hotspot 重排，而非 high-state 响应普遍下降。该结果不是患者 Z 场、临床发作阈值或热力学相变。入口与边界见 `docs/archive/topic4/sef_hfo/dual_core_spatial_z_bifurcation_result_2026-09-04.md`。
