...
- **Spatial Z/M saddle organizer + paper-facing Fig. 5（2026-09-04）**：持续 OU 的有限 SNN 已由 prospective seeds 1841–1843 以 3/3 确认 tonic global runaway。1 mm patient-matched frozen-q reduction 存在经双根、simple real zero mode 和 generic-fold 条件确认的 saddle-node（`q_fold=0.890825926`）；三颗 SNN 的 `q_core/q_mean` 均在操作性 onset 前穿过该 fold，后期率与 high skeleton 相差 6.8–7.8%。但真实 recurrent delay 使 high equilibrium 全段线性不稳定（远离 fold 主复模约 27 Hz），所以它只是 high-rate skeleton，不是稳定高 fixed point。2/1.33/1 mm 三种保守粗化都保留 generic fold 与同一 high branch，最细两网格 `q_fold` 差 0.00282；fold rate 却差 32.9%，未收敛。允许写 reduced saddle-node 在被测 coarse reductions 中是 robust organizer；禁止写精确 finite-SNN threshold、稳定高平衡或 thermodynamic phase transition。静态 A–D Fig. 5 candidate 已生成；详见 `docs/archive/topic4/data_driven_zm_ictal_transition/spatial_zm_saddle_organizer_delay_grid_audit_2026-09-04.md`。
