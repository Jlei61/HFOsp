...
- **dual-core spatial-Z 更新（2026-09-04，development）**：同一 `dualcore_s39 + Joint=1.25` realized graph 的 10×10 deterministic reduction 已确认 spatial saddle-node fold chain。低支 fold 是 deterministic runaway boundary；rev21 的 100 个 OU-on operational transition 中位发生在低/高根共存区，说明 OU 可早于 fold 触发 basin crossing。该结果只承重“冻结双核 fast subsystem 存在 fold 并组织 susceptibility”，不是患者 Z 场、临床发作阈值或热力学相变。入口与边界见 `docs/archive/topic4/sef_hfo/dual_core_spatial_z_bifurcation_result_2026-09-04.md`。
