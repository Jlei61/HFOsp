...
### 5.9 PR-2.7 — Rate-Trace 谱特征 + 发作邻近调制（实验 7D，2026-04-09 完成）
...
结果（30 subjects）：
- **cohort 中位 β = 0.67**（均值 0.62，范围 0.06–1.62）
- 拟合质量中位 r² = 0.698
- Yuquan：n=10，中位 β=0.74（范围 0.06–1.62）
- Epilepsiae：n=20，中位 β=0.61（范围 0.12–1.09）

**解读**：β 介于 0 (white noise) 和 1 (pink noise) 之间，说明率过程确实具有超越白噪声的长程依赖，但不是严格的 1/f。这与 PR-2.5 的 Δ_frac 近似平坦（宽频段，无单一主导时间尺度）结论一致。β 的 subject 间变异较大（0.06–1.62），说明慢调制结构存在个体差异。
...
- 中位 coherence = 0.576（16/26 > 0.5）
- 去除 coh ≈ 1.0 的伪影 subject（短段导致的虚高）后：n=19，中位 = 0.471

**解读**：约半数 subject 的 coherence > 0.5，支持全局状态变量假说在部分频段成立。但去除短段伪影后中位 coherence 略低于 0.5，说明率和参与度的共调制不是完美耦合——参与度可能存在率以外的独立局部兴奋性成分。这与 PR-2.5 的 event-index 互相关 r = 0.742 **方向一致但强度更弱**，反映连续时间相干是更保守的度量。

**注意**：7/26 subject 的 coherence 为 1.0，这是连续有效段太短（<= nperseg bins）时的数学伪影，不应作为科学结论。排除后的中位 0.471 更可信。

#### 5.9.3 Seizure-triggered rate average (Exp 7G)

对每次发作提取 ±12h 窗口的 z-scored rate trace 并平均。21/30 subjects 有 ≥ 1 个可用发作窗口（总计 341 seizures）。

结果：
- **Pre-ictal [-6h, -1h] vs baseline [-12h, -6h]：Wilcoxon p = 0.019，16/21 pre > baseline**
- Pre-ictal 中位 z-rate = 0.097，baseline 中位 z-rate = -0.025
- 效应方向：**发作前 1–6h 内 HFO 群体事件率系统性高于 6–12h 前的基线**

**解读**：这是 PR-2 至 PR-2.7 系列中第一个 **population-level significant** 的发现（p < 0.05），也是唯一一个将率调制与发作事件联系起来的结果。注意 PR4–PR6 的同步性分析（phase synchrony）在群体水平未发现 pre vs post 差异（p = 0.279），但 rate 在发作前系统性升高。这意味着：

1. Rate 比 synchrony 是更敏感的 pre-ictal 标志物（至少在当前度量框架下）
2. 慢调制不仅是纯生理背景（sleep/circadian），还与 seizure proximity 有关联
3. 由于 Yuquan 只有 1 个 subject 有可用发作窗口，该发现主要由 Epilepsiae 驱动

**谨慎之处**：(i) 窗口 ±12h 对于只有 24h 记录的 Yuquan 来说几乎无法满足；(ii) 多次发作的 subject 贡献更多权重；(iii) z-score 使用全局均值/标准差，发作密集的时段本身可能拉高均值。需在后续分析中控制发作密度。
...
17. Rate-trace PSD 直接测量 1/f 斜率 **β 中位数 = 0.67**（30 subjects），r² 中位 0.698 → 确认率过程具有超越白噪声的长程依赖，但不是严格 1/f (β=1)，与 PR-2.5 宽频段结论一致
18. Rate × n_participating 连续时间相干：中位 coherence = 0.576 (16/26 > 0.5)，去除短段伪影后 0.471 → 全局状态变量假说在部分频段成立，但参与度存在独立成分
19. **Seizure-triggered rate average：Pre-ictal [-6h,-1h] vs baseline [-12h,-6h] Wilcoxon p = 0.019，16/21 pre > baseline** → 发作前 HFO 率系统性升高；rate 比 synchrony 更敏感的 pre-ictal 标志物
...
- PR-2.7 的 seizure-triggered rate average 是整个调制分析线中**唯一 population-level significant** 的发现（p=0.019）：HFO 群体事件率在发作前 1–6h 系统性升高。Rate 比 phase synchrony 更敏感的 pre-ictal marker。后续可考虑构建预测框架
- Rate-trace 谱指数 β 中位 0.67 确认宽频段结构但不是严格 1/f；coherence 中位 ~0.5 说明全局状态变量假说在频域部分成立
