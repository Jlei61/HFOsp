...
### 5.9 PR-2.7 — Rate-Trace 谱特征 + 发作邻近调制（实验 7D，PLANNED）

**目标**：PR-2 至 PR-2.6 建立了慢调制的存在、宽频段特征、多小时连续延伸以及 IEI / n_participating 同源性。PR-2.7 收尾三个方向：

1. **Rate-trace PSD + 1/f 斜率 (Exp 7E)**：对 PR-2.6 的 5 分钟 bin rate trace 做 Welch PSD，拟合 log-log 斜率 β。PR-2.5 的 Δ_frac 平坦是间接推断；PR-2.7 直接测量谱指数。β ≈ 1 → pink noise；β ≈ 2 → Brownian。拟合范围 0.02–0.5 mHz（≈30 min 至 8h 周期）。仅在连续有效段上做 Welch，不做 gap-fill 插值。

2. **Rate × n_participating 连续时间相干 (Exp 7F)**：对 rate trace 与 mean(n_participating) trace 做 cross-spectral coherence。PR-2.5 的 event-index 互相关 r = 0.742 是全频段标量；相干函数分频展示全局状态变量假说的频域结构。

3. **Seizure-triggered rate average (Exp 7G)**：提取每次发作 ±12h 窗口的 z-scored rate trace 并平均。测试 pre-ictal [-6h, -1h] vs baseline [-12h, -6h]。已知 PR4–PR6 的同步性在群体水平无变化；若 rate 在发作前系统性升高 → rate 比 synchrony 更敏感的前驱标志物。

**已知风险**：exp 7G 受限于发作数量（Yuquan 每 subject 1–3 次/24h；Epilepsiae 因 recording 覆盖而变异大）。预期可用 subject 可能 < 10；如此则结论为探索性。

详细计划见 `.cursor/rules/event-periodicity-pr-plan.mdc`。
...
PR-2.7 证据链（rate-trace 谱特征 + 发作邻近，PLANNED）：
17. Rate-trace PSD 直接测量 1/f 斜率 β → 升级 PR-2.5 Δ_frac 间接推断
18. Rate × n_participating 连续时间相干 → 频域验证全局状态变量假说
19. Seizure-triggered rate average → 建立 rate 与发作的因果联系（或确认独立性）
...
- `src/event_periodicity.py` — 新增：
  ...
  - `compute_rate_trace_psd()` — rate trace Welch PSD + 1/f 斜率（PR-2.7 Exp 7E，PLANNED）
  - `compute_rate_npart_coherence()` — rate × n_participating 相干（PR-2.7 Exp 7F，PLANNED）
  - `compute_seizure_triggered_rate()` — seizure-triggered rate average（PR-2.7 Exp 7G，PLANNED）
- `scripts/run_periodicity_phase2.py` — Phase 2 批量驱动（exp 1-7c + exp7d PLANNED）
- `scripts/plot_periodicity_phase2.py` — Phase 2 可视化（exp 1-7c + exp7d PLANNED）
...
- `results/event_periodicity/phase2/exp7d_rate_spectral.json`（PR-2.7，PLANNED）
