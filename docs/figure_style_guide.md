...
- **2026-09-15 用户确认：核内 E→E 权重倍率统一记为 `J_{\mathrm{EE,core}}`**，`EE,core` 整体为下标。它仍是相对于参考权重的无量纲倍率；当同时缩放两个核内的 E→E 连接时须在说明中写清。分岔图的临界点用简短 `Fold` 或分岔类型及数值标注，不再另引 `g_c` 等参数名。诊断图按本次要求可用近方形布局，并以同一编号对应原生 SNN 的放电率和 raster；原生有限时窗状态不得冒充降阶模型的平衡点或周期轨道。
- **2026-09-11 用户确认：患者—模型接触读出的 Y 轴固定按杆分组。** E1146 从上到下统一为 `SCL9, SCL8, SCL7, SCL6, ICL11, ICL10, ICL9, ICL8, ICL7, ICL6, ICL5, ICL4, ICL3, ICL2, ICL1`。患者/模型、TA/TB、候选、网络/噪声、静态图/GIF及前后版本共用这15个通道槽位、刻度、方向和范围；不得按投影x、rank、参与情况或事件时序重新排Y轴。未参与或无有效质心的触点保留原位并标明，不能删除后压缩其他行；无该类事件时仍保留相同Y轴。杆间加分隔线，质心只在同杆有效相邻行之间连接，缺值处断开，禁止跨杆连线。患者STFT仍保留真实频率cell和频率质心在对应通道槽位内的位置；同杆顺序不因模型而更改。二维field保持原物理坐标及电极布局。旧交错排序图只作为带说明的历史诊断，当前报告改用固定行序重绘版。实现入口：`src/snn_contact_display.py`；这条新约定优先于下方历史“只画active contacts”的说明。
...
- **布局（单行 4 列）**：
  ...
  - **electrode readout**：右侧宽 panel，画同一 montage 的多事件虚拟 SEEG train。Y轴按杆分组并固定通道槽位，未参与触点不得删行；x 轴为 time。用不同颜色 shading 区分 forward / reverse clean propagation events，黑点/线标每个事件的 peak order，杆间不连线。
