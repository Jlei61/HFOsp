...
**⚠️⚠️ 全节的机制解释在 2026-08-08 被实测推翻，见下（用户裁定）。**

先前写的是"一条贯通细丝和两端两个核给出的先后顺序高度相似，顺序这个量看不见中间空不空"。
**这是错的。** 逐事件把"名次 vs 触点在轴上的位置"拟合一条直线，实测：

| 场 | 每事件拟合优度 R² > 0.8 的比例 | R² 中位 |
|---|---|---|
| 单块放**中段** | **0%** | 0.056（V 形：从中间往两头） |
| 单块放**"源"那头** | 94% | 0.998 |
| 单块放**"汇"那头** | 58% | 0.869 |
| 手放两个核（21 网络，分开驱动） | 95% | 0.923 |
| 学出来的贯通细丝（120 网络） | 71% | 0.876 |
| 病人 `t_a` / `t_b`（模板层） | — | 0.936 / 0.268 |

⇒ **单块放中段被逐事件的剖面形状一票否决（0% vs 95%）。信息就在顺序里。**

**真正的原因是我们的目标函数把它扔掉了**：现行打分把每个事件压成一个**方向正负号**
（`k_dir` 端点估计），再在**同号事件内求平均**得到两条模板。V 形顶点的位置在这一步被平均没了；
方向符号本身也是极粗的摘要。**信息在数据里，是估计量丢的。**

能承重的仍然只有那条经验观察：在本次搜索实际访问过的候选集合上，匹配分与双峰程度不相关
（`rho = −0.10`, n=167，自适应搜索样本不独立，不报 p 值）。**但其解释是"目标函数看不见"，
不是"数据里没有"。**
...
## 9. Stage 3 — 逐事件剖面拟合（rev5 重写，2026-08-08 用户裁定）

### 9.0 这一版换掉的是**目标函数**，不是参数化

rev4 把场从轴的坐标系里解放出来（自由中心混合），那一步是对的，保留。
**但目标函数没动**——仍然是"把事件压成方向符号、同号求平均、比两条模板"。
§8.3.3 的实测证明**这一步扔掉了区分模型所需的全部信息**。

rev5 只改一件事：**观测量从"两条平均模板"换成"逐事件的剖面分布"。**

### 9.0a 读出到底看得见什么（实测，决定观测量的选择）

| 量 | 对源位置敏感吗 | 证据 |
|---|---|---|
| **剖面形状**（名次 vs 轴上位置的拟合优度 + 斜率符号） | **敏感** | 源放中段 R²>0.8 占 **0%**；放一头 94%；放另一头 58%；两端斜率符号相反 |
| **"最早响的那个触点在轴上的位置"** | **不敏感，禁用** | Leg A 真值标定：读出 vs 真实位置**斜率仅 0.25**，相关 0.51；**五个相隔 18 mm 的源位置读出来全是 +0.4 mm**。病人侧同样：**前 2 个触点就占 31%、前 4 个占 50%** —— 这个量主要反映"哪个触点容易先响" |
| 滞后剖面二次拟合的顶点 | **未定** | 病人侧可算（给出与上一行**相反**的单峰图像），但**只能用 40% 的事件**（要求内部极小），**结构上对"源在两端"是盲的**；模型侧当前**没有存滞后**，要用必须重跑 |

**⚠️ 更正（2026-08-08 同日）：上面这张表把两件事混成了一道门，必须拆开。**
先前写的"最早响触点不过门"是**错的** —— 它在区分门上得 4.38，跟 `r2` 的 4.34 一个量级，**是过的**。
真正不过的是**另一道门**。两道门的定义与实测：

| 门 | 问的是 | 谁需要它 | 实测 |
|---|---|---|---|
| **区分门** | 不同源位置上，这个量的取值**有没有差别** | **进目标函数**的前提 | 斜率 5.94 / 拟合优度 4.34 / 曲率 1.38 / **最早触点 4.38** —— **全过** |
| **反演门** | 这个量**跟不跟得上**真实位置 | 说出"源在 x 毫米"**之前**的前提 | **最早触点：回归斜率 0.25、相关 0.51，五个相隔 18 mm 的源读出来全是 +0.4 mm —— 不过** |

**⇒ 预注册规则（两条，分别管两件事）**：

1. **进目标函数**：任何统计量必须先过**区分门**（§9.3a）
2. **说成位置**：任何统计量被写成"源在 x 毫米"之前，必须先过**反演门**。
   **本轮已确证不过反演门的：最早响触点的轴向位置** —— 它**可以**留在报告里，
   但**不得**被读成一个位置

**⇒ 默认观测量**：**逐事件的剖面形状统计量**（斜率 + 拟合优度）。
理由：形状统计量只要求"对源位置敏感"，**不要求可逆** —— 弱得多、也已被证实可达。

**⇒ 最早响触点为什么仍然不进默认目标函数（这是判断，不是门的结论，写在这里供反驳）**：
病人侧**前 2 个触点就占 31%、前 4 个占 50%**，这个量主要由"哪个触点容易被招募"决定。
若模型与病人的触点可招募性因**与病灶无关**的原因不同（阻抗、局部密度、包络定义），
把它放进目标函数会让**场去吸收一个仪器失配**。

### 9.1 冻结 vs 释放

| | Stage 2 | Stage 3（rev5） |
|---|---|---|
| `E→E` 各向异性 `theta_deg` | 冻结 −22.8° | **仍冻结**（"只给一个方向的限制"） |
| LIF / 突触 / 慢变量、虚拟触点与读出 | 冻结 | 仍冻结 |
| 冻结的 15 触点支撑集 | 冻结 | 仍冻结 |
| 预算 `Σ h = 1129` | 冻结 | 仍冻结 |
| 场的位置与范围 | 钉在轴上 | **自由**（§9.2） |
| **观测量** | **两条方向平均模板** | **逐事件剖面分布**（§9.3） |
| **方向标签 / `k_dir`** | 目标函数的最高位 | **整个从目标函数里移除**（§9.3b） |
| **病人事件用量** | 2 条模板（15 数 × 2） | **46,682 个逐事件剖面** |

### 9.2 参数化：单核 / 双核 / landscape 不枚举，是学出来的

保留 rev4 的自由中心各向异性高斯混合（§9.2 参数化细节不变，`SIGMA_MIN/MAX`、圆盘约束、
17 自由维、禁止 warm start、≥3 次重启全部照旧）。

**关键设计决定（用户 2026-08-08 裁定）**：**不把"单核 vs 双核"做成两个待选假设。**

- 想要一块 ⇒ 两个权重自己趋近 0
- 想要两块 ⇒ 一个权重趋近 0
- 想要贯通的 landscape ⇒ 三个铺开重叠成一条

**模态是学出来的性质，不是人为选项。** 并且：

**`K` 做成嵌套模型比较**：分别拟合 `K = 1, 2, 3`，在**留出的病人记录段**上比同一个距离。
三个结果**全部报告**，不得只报赢的那个（§11 红线 14 同款）。
⇒ "单核还是双核"是白送的正规比较，不是猜。

### 9.2a 为什么**不**用"单块基底做混合拟合"（否决一条更省事的路线）

一个诱人的省事方案是：拿 Leg A 的 49 个单块位置当基底，
`P(事件) = Σ_x w(x)·P(事件 | 源在 x)`，用病人事件解 `w(x)`，**一次仿真都不用跑**。

**这条路被否决**，理由是物理的：**两个核同时存在 ≠ "有时只有 A、有时只有 B" 的混合**。

1. **预算被分掉**：`Σ h` 固定，两个核各自比单核弱 ⇒ 点火率与波幅都不同
2. **预激**：A 点火的波传到 B 时，B 已经是低阈值区 ⇒ B 附近的招募顺序被压缩，剖面形状改变
3. **竞争**：`scripts/paper_figures/pool_driven_runs.py` 的注释已经写明"自发时两个核会竞争，
   总是一个占上风" —— 混合模型把这条完全假设掉了

**⇒ 交互必须交给仿真器**：直接仿真候选场本身，交互是精确的。
**扩基底也不是出路**：两两配对 1176 种 × 4 网络 ≈ 80 小时，三核更糟 —— 是结构不对，不是算力不够。

### 9.3 观测量与目标函数

**每个事件压成一组形状统计量**，模型侧与病人侧**用完全相同的代码**计算：

```
只取冻结支撑集内、该事件实际参与的触点（病人侧必须先过 phantom 掩码 bools）
要求参与触点 >= 6，否则该事件不进统计
x = 触点在共享轴上的位置，  y = 该触点在本事件里的名次
slope, r2   = 一次多项式拟合的斜率与拟合优度        # 单调性与方向
curvature   = 二次项系数（>0 = V 形，从中间往两头）  # 中段源的指纹
n_part      = 参与触点数
```

**目标函数** = 模型事件的形状分布 与 病人事件的形状分布 之间的距离
（预注册为 `(slope, r2)` 二维上的能量距离 energy distance；`curvature` 与 `n_part` 作并列报告量）。

---

#### ⚠️⚠️ 实现与本合同不符（2026-08-08 审阅发现；两层问题，都要记）

**第一层：流程。** 第一轮实际用的是 **`sign(slope) × r2` 一维分箱总变差**，
不是这里冻结的二维能量距离。改动发生在第一轮开跑前，理由是实测的尺度失配
（模型每事件平均招募 7 个触点、病人 12 个 ⇒ 名次跨度不同 ⇒ **斜率大小被参与数污染**，
模型/病人斜率标准差之比 0.73；归一化后过度矫正到 2.34）。
**这个理由本身成立，但改动没有回写本合同 —— 冻结的目标被换掉而文档没动。**

**第二层：实质。这个替换事后被证明是错的。**
一维边缘分布**丢掉了二维联合结构**，而联合结构是承重的：拟合出来的场把边缘分布对上了
（正负号配比 14:4，病人 73:27），**但它的两个方向是正相关的（组织层 +0.700、触点层 +0.651）**,
而手放双核是 −0.748 / −0.685、病人自身两套模板是 −0.464。
⇒ **一个坐在阵列中段的单一生成器，靠噪声决定哪个触点先响，就能满足这个一维边缘分布。**
被丢掉的那一维正是能排除它的那一维。

**⇒ 下一轮的合同修订（必须在开跑前冻结）**：观测量必须能表达
**"事件聚成两个彼此相反的簇"** 这个联合结构，而不只是边缘分布。
`(slope, r2)` 二维能量距离是一个候选，但**它是否足够也必须先证**（用已有的场做区分性检验，
照 §9.3a 的做法）。**不得**把 §10.3 的验收门直接塞进目标函数。

**必须满足的三条**：

1. **同码同规则**：两侧用同一个函数、同一个参与门槛、同一个支撑集。任何一侧单独的预处理都禁止
2. **模型侧按候选池化**：一次 8 秒跑约 22 个事件，太少；同一候选的多个网络种子事件**合并**后再算分布
3. **病人侧留出按记录段切，不按事件切**：同一记录段内的事件不独立（§9.5）

### 9.3a 敏感性门（任何统计量进目标函数之前必须过）

用 Leg A 的 **196 次真值跑**（源位置已知）标定。对每个候选统计量，要求：

> **同一位置不同网络之间的散布，明显小于不同位置之间的散布。**
> 预注册判据：以该统计量预测"源在 49 个位置中的哪一个"，
> 分类准确率必须显著高于随机（1/49），且**至少能把"中段 vs 两端"分开**。

**不过区分门的统计量不得进入目标函数**，只能作描述性报告。
**本轮实测：剖面形状与最早响触点都过了区分门**（4.34–5.94）。
最早响触点被排除在默认目标之外的理由是 §9.0a 末尾那条**判断**（触点可招募性混杂），
不是它没过这道门 —— 两者不可混为一谈。

**另设反演门**（只在要把某个量说成位置时才需要）：该量对真实源位置的回归斜率 ≥ 0.5。
实测最早响触点 0.25，**不过** ⇒ 它不得被读成一个位置。

### 9.3b 方向标签与 `k_dir` 从目标函数中移除

新目标函数**不需要**方向符号，因此：

- `k_dir` 那个阈值、`2·k_dir+1` 参与门槛、`n_dir` 档位、交换不变匹配分**全部退出目标函数**
- 方向变成**结果**而非输入：模型是否产生两个方向，由 `slope` 符号的分布**读出来**，不再由估计量指派
- §8.3.5 缺口 2 的修法（触点招募档进候选键）**保留**，但降为**次级排序位**，
  因为新目标函数已经直接看每个事件招募了哪些触点

**⚠️ §10.3 的四条充分性门保持不变**，仍作为**验收门**在最后评估 —— 它不进目标函数
（对着验收标准优化会使通过失去证据力，§9.3 原则不变）。

### 9.3c rev6 冻结：完整事件剖面的联合距离（2026-08-09，零仿真标定通过）

`(slope, r2)` 二维候选仍把一条完整触点剖面压成两个数，而且 `slope` 大小受参与触点数影响。
rev6 改用**不带方向标签的完整归一化剖面**，模型侧与患者侧运行同一个函数：

1. 只取实际参与且属于冻结 15 触点支撑集的触点，要求 `n_part >= 6`；
2. 每个事件内把 rank 做 z-score，消除 7 vs 12 个参与触点造成的名次尺度差；
3. 沿冻结轴的 `[min(contact_s), max(contact_s)]` 插值到 **31 个固定位置**，观测跨度外作常数延拓；
4. 每条曲线去均值并归一到单位 L2 范数；
5. 只在患者**训练记录段**上拟合 PCA，固定前 **8** 维（当前解释 92.9% 方差）；
6. 固定 seed `20260809`，从患者训练段抽 4096 条 reference curves；以 8 个 PCA 轴 + 56 个
   固定随机方向组成 64 个投影；
7. 优化侧固定取 **20 条 usable events**，用覆盖整个事件序列的确定性等距索引选取；目标距离
   `D_curve` = 64 个一维 Wasserstein 距离的平均值（sliced Wasserstein）。候选产生超过 20 条事件时
   不得因样本数更多而得到距离优势；少于 20 条时只进入 feasibility key，不计算主目标。

这套目标**不使用**方向标签、`k_dir`、患者 TA/TB 标签、两模板相关或 §10.3 的任何门。
两簇原型相关只在拟合后作 held-out falsification diagnostic。

零新仿真的冻结标定（producer：
`scripts/calibrate_topic4_core_field_stage3_joint_observable.py`）：

四臂横向比较统一配平到现有最小臂的 **18 条事件**；表内为 500 次固定事件数 bootstrap 的
中位数与 5--95% 区间。全事件经验 Wasserstein 距离受样本数影响，只保留在 JSON 中作臂内描述，
**不得横向比较**。

| 已有 artifact | usable events | 配平 `D_curve`（n=18） | 后验 K=2 原型相关 |
|---|---:|---:|---:|
| 患者留出记录段 | 16,633 | **0.300 [0.251, 0.375]** | −0.884 |
| 手放双核 | 220 | 0.675 [0.597, 0.790] | −0.640 |
| Stage 2 细丝 | 507 | **0.636 [0.562, 0.715]** | −0.565 |
| Stage 3 第一轮灵活场 | 18 | **0.740 [0.740, 0.740]** | **+0.473** |

与优化目标同构的患者自身地板固定为 20 条患者留出事件对患者训练 reference：
`0.287 [0.236, 0.358]`（500 次 bootstrap）。它是解释下一轮 `D_curve` 的参照区间，不是一个可优化的
惩罚项，也不把患者留出事件暴露给拟合。

已知源位置的 Leg A 留一网络标定：左/中/右三分类 `82/104 = 78.8%`（chance 33.3%）；
49 格精确分类 `63/92 = 68.5%`（chance 2.0%）。因此它同时满足：患者留出最接近、自然惩罚
Stage 3 单中段生成器、已知位置敏感，而且没有把最终验收门写进目标。

**裁定**：`D_curve` 进入下一轮**有界优化验证**；这只许可测试优化器能否找到更好的场，不许可写机制、
...
三档判据 `AXIS_REDISCOVERED` / `AXIS_NOT_REQUIRED` / `POSITION_UNIDENTIFIABLE` 原样保留，
结局分类仍**返回全量分量**。

**⚠️ 循环性的表述边界原样保留**：Stage 3 只释放了场的位置；连接的方向性仍固定在
由患者秩次拟合出的轴上，评分目标也来自同一批秩次。**禁止**写"独立地重新发现了传播轴"。

### 9.5 留出与统计单位（rev5 新增，因为现在拟合到病人事件）

Stage 2 的留出只在模型侧（网络种子）。rev5 **多了一层**：

| 层 | 留出单位 | 防的是什么 |
|---|---|---|
| 模型侧 | 网络种子 | 过拟合仿真器的随机实现 |
| **病人侧（新）** | **记录段（recording block）** | 过拟合某几段记录的特性 |

**⚠️ 病人侧留出必须按记录段切，不得按事件切。** 同一段记录内的事件**不独立**
（同一夜、同一状态、同一电极接触阻抗），按事件随机切会严重高估。
E1146 有 80 个记录段、46,683 个事件，切分在开跑前冻结进 config 并计入校验和。

### 9.6 保留的次级检验

横向平移检验（§9.6 rev4）原样保留为次级证据。

### 9.7 开启条件

§9.3a 的敏感性门已通过（剖面形状），§9.2a 的否决已记录，§9.5 的切分已冻结 ⇒ 可开。
**不需要** Stage 2 产出正结果（理由见 rev4 §9.7，不变）。
