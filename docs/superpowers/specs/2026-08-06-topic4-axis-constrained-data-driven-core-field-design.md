...
- 横向宽度：**0.38 mm**（手放 1.5 mm），且**从第 0 代就贴在被允许的最窄档**
  （全部候选 `ρ` 中位数 = 4.00 = `LOG_RHO_LIMIT` 上限），20 代没再移动
...
> **`S_rank` 与场的双峰程度相关 `rho = −0.10, p = 0.19`（n = 167）。**
> 搜索**确实走到过**双峰程度 0.52 / 0.83 / 1.00 的场，它们的分数并不更高，全部被丢弃。

第一性原理：秩次模板就是"这次事件里各触点谁先谁后"。**一条贯通细丝和两端两个核给出同一个先后顺序** ——
顺序这个量里不装"中间是空的还是满的"。**只用秩次做目标，双峰性在信息上就不可辨识。**
...
**⚠️ held-out 池被扩大过，且是在看到结果之后决定的，必须如实记录**：预注册的 held-out 是种子 9–12，
其反向事件合计只有 **4 个**，低于第 1 条预注册的最低 10 个 —— 该条**根本无法评估**。
因此改用 120 个全新种子（21–140，与训练种子及原 held-out 均不相交）。
**判据本身没有改，改的只是支撑它的样本量。**

**⚠️ 第 2 条不过的归因是具体且可定位的**：横向那根电极杆的 4 个触点，模型只在 **0.7–1.4%** 的事件里
招募到。它们在正反两套模板里都被同一条缺失规则填成同一个值，于是把相关从 **−0.509**（只算沿轴那根杆的
11 触点）拉到 **−0.130**。**这不是"两个方向长得一样"，是"模型只在一根杆上有内容"。**
...
- "无监督把 562 个事件分成两簇，能以 98.8% 的准确度还原方向标签；两簇分别对上病人的两套模板，非对角为负"
...
**Stage 3 释放第 2 条，保留第 1 条。** 场可以出现在片子上的任何地方；唯一还给它的先验是传播的方向
（连接各向异性）。**预注册的主问题：优化器在完全自由的二维空间里，会不会自己把病理区域放回轴上。**
