...
## 18. Round 3（2026-08-16 复审授权）：边界处的 D×H 因果交叉

### 18.1 为什么需要它

round 2 定位到有界爆发区的入口在 onset 与 onset+1 s 之间，但在那一段里 `D` 与 H 门是**同时**变化的
（`D` 0.0656 → 0.0870，门 0 → 0.2696）。所以 round 2 **判定不了是谁打开了这个区**。
继续加密自然路径不会解决这个问题 —— 沿路径两个变量永远一起动。唯一能拆开的做法是把它们**交叉**。

### 18.2 设计

四个合成慢场，由两个自然场的分量交叉组成：

| 格 | `z`（即 `D`）取自 | `h_lc2_E` 取自 |
|---|---|---|
| `D11_H11` | onset 场（11 s） | onset 场（11 s） |
| `D12_H11` | onset+1 s 场（12 s） | onset 场（11 s） |
| `D11_H12` | onset 场（11 s） | onset+1 s 场（12 s） |
| `D12_H12` | onset+1 s 场（12 s） | onset+1 s 场（12 s） |

每格从两种**共用**起始快态出发（间期态、420 Hz 高态），共 **8 条**，各 10 秒，
全部共享 round 2 的同一条标准输入流。

### 18.3 判读

- 只有抬 `D` 才进入爆发（`D12_H11` 爆发、`D11_H12` 不爆发）→ **`D` 打开这个区**
- 只有抬 `H` 才进入（反过来）→ **`H` 打开**
- 两个都抬才进入（只有 `D12_H12` 爆发）→ **协同**
- 单独抬任一个都能进入 → **冗余入口**

结局判定用与 round 2 **同一个**版本化裁决器（`src.topic4_fcxr_lc6b_outcome`），
"是否进入爆发区"以该格自身与 `D11_H11` 格的结局距离来判，不新造判据。

### 18.4 两个对角格是免费的正确性校验

`D11_H11` 与 `D12_H12` 在构造上分别等同于 round 2 的 `field_t11s` 与 `field_t12s` 点
（同一场分量、同一起始态、同一输入流、同一双钳）。因此它们的放电序列**必须与 round 2 逐位相同**。
不同即说明交叉合成路径与 atlas 路径不等价，**finalize 拒绝发布**。

### 18.5 边界

单 seed、单图、单条输入流。本节**不**授权：扰动回归（Step 3 另行注册）、`U`、`M`、
lifecycle、termination、更大的 `D`/`H` 网格。
