...
> 状态：v0.1 draft 2026-05-27（brainstorming 阶段已锁，待 user review；review pass 后用 writing-plans 出实施计划）
...
| **C-only**: 连接驱动 | aniso D (D_x/D_y=3) + isotropic θ (blob) | H-C reentry / anatomical fiber 方向性 |
...
- **proxy 不一致**：P1 (x) 通过但 P2 (dx/dt) / P3 (envelope) NULL → observation-mapping dependent，不进 framework mechanism 栏
...
- 数值方案：RK4, Δt = 0.05 HR time unit
- 参数扫描：`I ∈ [-2.0, 0.0]` step 0.1, `r ∈ [0.002, 0.015]` step 0.001, `σ_OU ∈ [0, 0.5]` step 0.1, 5 seeds per cell
- 可视化：时间序列 x/y/z trace + phase portrait (x-y plane, z 等值面叠) + nullclines (`y = ax³ − bx² + z − I`, `y = c − dx²`) + burst detection overlay

**Burst detection**：`x > x_burst_thresh = 1.0` 持续 ≥ 5ms（hysteresis 防抖动）

**Regime classifier**：silent / single-burst / repetitive-burst / unstable，四类自动分类
...
| aniso (D_x/D_y = 3) | 各向异性扩散 | H-C reentry / anatomical fiber 方向 |

**2×2 factorial 切分**（替代原"3 θ × 12 shaft"线性矩阵）：

|  | isotropic D | aniso D |
|---|---|---|
| **isotropic θ** | **Cell A** negative control | **Cell B** C-only test (H-C) |
...
| L3 rank-displacement | swap_sweep T_obs / decision_k / p_fw | strict swap class > 30% events |
| L4 principal-curve audit | 1D manifold fit BIC vs k=2 GMM BIC | k=2 GMM 显著优于 manifold (不是 H-F artifact) |

**3 proxy verdict 处理（修订）**：

stage 5 同 cell × shaft × seed 在 **P1 / P2 / P3 三 proxy** 上各自跑 L1-L4，**并列报告**：

- framework v1.0.8 mechanism 栏**只引用 P1 verdict**（与 PR-2 lag pattern 对齐）
- P2 / P3 verdict 在 archive 作 sensitivity report
- 若 P1 PASS 但 P2/P3 NULL → archive note "primary observation mapping dependent"
- 若三 proxy 全 PASS → 强结果，archive 单独 highlight robust
...
- swap_class strict 比例 < 10% events
- principal-curve audit 返回 unimodal/manifold
...
- Cell A 全 fail L2/L3/L4 (negative control gate pass)
- Cell C 或 Cell D **至少一个** P1 proxy 通过 L2/L3/L4
- Cell B 表现作为 H-C reentry 假说的独立证据（可以通过或 NULL，都报告）
- Cross-shaft：aligned 强 / orthogonal 弱 / random 中等（shaft sensitivity 合理）
- 3 proxy verdict 三套并列报告（不一致则 archive caveat）
