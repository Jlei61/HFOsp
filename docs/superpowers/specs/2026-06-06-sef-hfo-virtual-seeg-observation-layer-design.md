...
- **增量 1 门**（主门 = rank-vs-真方向投影 Spearman + endpoint 轴角误差，§3.5；2D 对称居中 montage）：wave 主判据 Spearman ρ ≥ τ_pass(0.9) **且** endpoint 轴角误差 < 25° **且** C1（居中径向源）`direction_readability` < τ_fail(0.3) **且** C2（严格无到达梯度源 `a=b(x)h(t)`）`direction_readability` < τ_fail 或 NaN。任一破 → 停。
