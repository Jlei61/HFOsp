...
- **C1 居中径向源（无优势方向）**：合成一个**居中**的径向扩散鼓包（无优势轴），用 2D montage 读出。**必须**：主估计子返回「无轴」（早 / 晚 rank 质心退化到同一点、各向异性比 < τ_fail、拟合 ill-conditioned），方向一致性 **< τ_fail**（提议 0.3）。**注意**：源**必须居中**且用 **2D montage**——偏离中心的单杆会把径向扩散读成单调到达斜坡 = 假方向，会让 C1 因错误原因 fail must-fail；居中 + 2D + endpoint 质心退化才是正确的 no-axis。
- **C2 无到达时间梯度源（严格版，电极排列自造时序陷阱）**：`a(x,t) = b(x)·h(t)`——所有位置**同起同落**（共用一个 `h(t)`），只有空间幅度 `b(x)` 不同、**无任何到达时间梯度**。电极杆**对齐**到某轴放置。测的是「电极排列 + 越阈定义本身会不会从纯幅度差造出假时序」。**必须**：读出 Spearman / 各向异性比 **< τ_fail**。这正是 §4.1 timing 阈的试金石——全局绝对阈 → 高幅触点先越阈 = 假序（fail）；per-contact 相对峰值阈 → 同时越阈无假序（pass）。
...
- **增量 1 门**（主门 = rank-vs-真方向投影 Spearman，§3.5；2D montage）：主判据 Spearman ρ ≥ τ_pass(0.9) **且** C1（居中径向源 → no-axis）/C2（严格无到达梯度源 `a=b(x)h(t)`）< τ_fail(0.3)。任一破 → 停。
...
- [x] 增量 1 两条 must-fail：C1 **居中**径向源 + 2D montage（避免偏心单杆假斜坡）+ C2 **严格无到达时间梯度源** `a(x,t)=b(x)h(t)`（测电极排列自造时序）
