...
**纪律**：本合同是 **rate substrate 专属**；SNN 侧仍用触点 aggregate + 0.5（§4.1 原口径）。两基底**估计子 + 阈值相同**（parity 所在），**检测前端按基底特性分锁**（因为踢是 artifact、两基底踢盘/触点尺度比不同）。不允许把 rate 的 0.10/ext 口径反向套回 SNN，也不允许事后再调这两个值——已锁、已 sensitivity。
