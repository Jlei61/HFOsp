...
> - **(a) 解析规则（首选）**：`Q_scale := Q_on`。无自由度，只依赖已锁的 `Q_on`。
>   **举例数字订正**：在 `q_v = 2·Q_on` 处 deadband 先减掉约 `ε_q`，
>   `u = (Q_on)²/(1.1·Q_on) = 0.9091·Q_on`，执行器达 **`tanh(0.9091) = 0.7207·I_R,max`**，
>   不是 `tanh(1) = 0.762`。**规则本身未变。**
...
>
> **仍然成立的部分，而且范围要扩大**：`Q_on` = calibration 最大值 × 1.1 ⟹
> **第 1 条**在 calibration 半段上按构造必然通过；
> **第 2 条同样按构造通过**——`q_peak·exp(−GAP/τ_R) ≤ 0.01·Q_on` ⟺ `exp(−GAP/τ_R) ≤ 0.011`，
> **正是 `τ_R` 的选择规则本身**，只差 1.10 的 margin。它只能在比 `GAP_05` 更短的 5% 空白上失败。
>
> **所以 B0 真正的判据只有第 3 条（`q_v` 棘轮）与第 4 条（事件统计）。** 报告时必须按此收紧。
