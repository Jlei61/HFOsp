...
（**rev5**）§9 全节 + §8.3 收口记录。**每个 Task 开工前重读对应 §。**

> **⚠️ 本 plan 落后于 spec（2026-08-08 审阅发现，已在此说明，正文尚未逐条重写）。**
> spec 已到 rev5，与本 plan 正文的三处不一致：
>
> 1. **目标函数**：spec §9 rev5 已把观测量从"两条方向平均模板"换成**逐事件剖面分布**，
>    `k_dir` 与方向标签退出目标函数。本 plan 的 Task 2/5/6 仍按旧目标写。
>    ⚠️ 而且**实际跑的又与 rev5 不同**：spec §9.3 冻的是 `(slope, r2)` 二维能量距离，
>    第一轮跑的是 `sign(slope)×r2` 一维分箱总变差，**事后证明一维不足以排除单一中段生成器**
>    （详见 spec §9.3 的偏离记录）。
> 2. **留出**：spec §9.5 rev5 新增**病人侧按记录段留出**这一层；本 plan 只写了模型侧种子。
>    实际分析已经用了病人侧记录段留出。
> 3. **确认扫描的地位**：本 plan Task 4 写"两者之差就是 winner's curse 的直接估计"——
>    **这句已撤回**。拟合值与确认值之间事件数也变了，而直方图总变差在小样本下偏高，
>    选择偏差与样本量一起动，该比较分不开这两者。地板也必须**与评分器同构**
>    （模型样本 vs **全量**病人训练集），不是"同样大小的两份互比"。
...
3. **最高区域必须用一批独立种子确认**（与建图的 4 个种子不相交），
   确认结果与建图值并列报告。
   ⚠️ **2026-08-08 更正**：先前写"两者的差就是 winner's curse 的直接估计"——**撤回**。
   两次评估的事件数通常也不同，而直方图之间的总变差在小样本下**系统性偏高**，
   选择偏差与样本量一起动，单靠这一个比较分不开。两个数并列报告，**不作差后归因**
