...
（零自由度）。**举例数字订正**：在 `q_v = 2·Q_on` 处，deadband 先减掉约 `ε_q`，`u = (Q_on)²/(1.1·Q_on) = 0.9091·Q_on`，所以执行器达 **`tanh(0.9091) = 0.7207 × I_R,max`**，不是 `tanh(1) = 0.7616`。**规则本身未变。**
...
### 4.5 已测的一条 deadband 覆盖事实（2026-07-31，在 B0 / A0 之前记录）

两条 calibration run 实测：**约一半的占据体素 `b_v = 0`**（seed1 **47.3%**、seed3 **52.8%**）——
它们在 0.5 ms 块里放电的比例不到 1%，所以自己的 99 分位背景就是零。

后果必须写明：**在那一半组织里 deadband 不起任何作用，`Q_on` 是唯一的门** ——
两级门（先 deadband 再 `Q_on`）在那里**塌成一级**。

同时 `b_v` 是**量化**的：一个体素-块里的一个 spike 对应 `n_cells/dt_R ≈ 51.3 Hz`，
所以 `b_v` 只能取它的整数倍。这就是为什么 **`b_v` 的中位数是个坏摘要**：
两 seed 的零占比只差 5.5 个百分点（47.3% vs 52.8%），中位数却在 51.3 与 0 之间翻转。
**报告一律用零占比，不用中位数。**

**不因此调 `Q_BG`**（§4 已锁）。这条只是把设计的实际覆盖范围如实记下来，
以免 B0 / A0 的结果被误读成"deadband 起了作用"。
...
> **⚠️ 写测试时发现的结构性事实（2026-07-31，在任何仿真之前）——第 2 条也基本按构造通过。**
> 因为 `Q_on = 1.10 × (间期事件峰值)`，第 2 条写开是
>
> `q_peak · exp(−GAP/τ_R) ≤ 0.01 · Q_on`  ⟺  `exp(−GAP/τ_R) ≤ 0.011`
>
> —— **这就是 `τ_R` 的选择规则本身**（`≤ 0.01`），只差那个 1.10 的 margin。
> 所以对**任何不超过 calibration 最大值的事件**，第 2 条按构造成立；它**只能在比 `GAP_05` 更短的
> 那 5% 空白上失败**（测试已锁：`GAP = 40 ms` 时它确实失败）。
>
> **结论：B0 的第 1、2 条都不是独立证据。真正的判据只剩第 3 条（`q_v` 棘轮）与第 4 条（事件统计）。**
> 这把先前"第 1 条按构造通过"的范围扩大了一条。报告时相应收紧。

> 子句 2 用**下一事件前的小窗**而不是整个 gap：事件刚结束时 `q_v` 本来就高，
> 把它算进"残留"会让这条**按定义必败**，而且它测的是衰减起点不是残留。
> 子句 3 用 **`Q_on` 归一化的差值**而不是比值：首段接近零时比值数值不稳。
>
> **`Q_on` = calibration 最大值 × 1.1 ⟹ 子句 1 与子句 2 都按构造通过**（见上）。
...
## 附录 A — 已回填（2026-07-31，`calibration_lock.json` + `z_response_axis.json`）

| 项 | 锁定值 |
|---|---|
| `T_event,guard` | **22.0 ms**（LC1 24 s 冻结 bar，两 seed 事件时长最大值；canonical run 独立复现） |
| `GAP_05` 逐 seed | seed1 **225.4 ms**、seed3 **195.25 ms** |
| **`GAP_05` 锁值**（§3.6 跨 seed 最小） | **195.25 ms** |
| **`τ_R`（全局）** | **30.5411 ms**，区间 `[22.0, 42.40]`，余量 **20.40 ms** |
| 尾部残留 `exp(−GAP/τ_R)` | `GAP_05` **0.00167**、`GAP_01` **0.00205**、最短 GAP（189.0 ms）**0.00205** |
| `b_v` seed1 | 零占比 **47.3%**、量子 0.5 Hz、均值 176.0、最大 620.7 Hz |
| `b_v` seed3 | 零占比 **52.8%**、量子 33.9 Hz、均值 187.8、最大 634.1 Hz |
| `eps_s` | seed1 **34.5491**、seed3 **46.5116** Hz |
| **`Q_on` = `Q_scale`** | seed1 **112.5047**、seed3 **173.4290** |
| `eps_q` | seed1 **11.2505**、seed3 **17.3429** |
| `I_R,max` | **4.134151260609386**（逐位复现） |
| `S_Z` 端点（`T_cal = 24 s`、`C = 0.793`） | q75 **0.1717**、q50 **0.4079**（比值 2.37） |
| **三个 `I_th_EI`** | S25 **37.487**、S50 **13.606**、S75 **3.655** |
| Gate B0 ELR-off 臂 | seed1 / seed3 均 `INTERICTAL_BASELINE`，零 clip |
| 输入 artifact sha256 | 见 `calibration_lock.json::inputs_sha256`（9 项，preflight 已核） |

### A.1 §6.4 要求的并列对照：新的三点**是**内点，HYB1 的三点**都不是**

把 HYB1 那三档放到**同一条** `S_Z` 坐标上（q75→q50 跨度的百分位）：

| | `I_th_EI` | `S_Z` | 占 q75→q50 跨度 |
|---|---:|---:|---:|
| HYB1 H_LO | 96.30 | 0.1709 | **−0.3%**（**就是 q75**） |
| HYB1 H_MID | 72.35 | 0.1909 | **8.1%** |
| HYB1 H_HI | 46.80 | 0.2186 | **19.8%** |
| **HYB2 S25** | **37.49** | 0.2315 | **25%**（按构造） |
| **HYB2 S50** | **13.61** | 0.2882 | **50%** |
| **HYB2 S75** | **3.66** | 0.3597 | **75%** |

**不只是 H_LO 贴着 q75——HYB1 的三档全部挤在弱端 20% 以内。** HYB2 的 S25 阈值比 q75 低 2.5 倍。
这条是**实测**，不是论证：换轴确有必要。
（`S_Z` 仍只是参数坐标，**不预测**这三点在闭环里会不会分叉——那由 12 格短屏回答。）

---

## 修订 rev2（2026-08-01）—— Gate A0 的 Z 输入档位，**在重跑之前预注册**

### 起因

按原 §5.2，A0 的唯一授权输入是 LC1 q50（`I_th_EI` = 1.6652801609959704）。实跑判
`A0_CEILING_CONFOUNDED`：off 臂在 `[t_gate, t_gate+1000]` ms 窗内已招募 **930/1024 占据体素
（90.8%）**，越过 0.9 天花板判据，`≥10%` 招募范围改进没有余量可显示。

回看 §6 锁定的 `S_Z` 响应轴（`z_response_axis.json`，`monotone: true`）：

| 点 | `S_Z` | `I_th_EI` |
|---|---|---|
| q75（轴的弱端） | 0.1718 | 95.199 |
| **S25**（筛选档 1） | 0.2315 | 37.4865 |
| **S50**（筛选档 2） | 0.2882 | 13.6057 |
| **S75**（筛选档 3） | 0.3597 | 3.6553 |
| **q50 = A0 当前输入（轴的强端）** | **0.4079** | **1.6653** |

**A0 跑在了整条轴的最强端点上。** 三个已锁定的筛选档位全部弱于它。

### 修订内容

A0 改在**已锁定的三个筛选档位**上重跑（S25 / S50 / S75），每档 off / on 两臂，共 6 条 9 s。
**没有引入任何新的自由参数**——这三个点是 §6 在任何 A0 结果出现之前就锁定的，
`I_th_EI` 数值原样取自 `z_response_axis.json`。q50 那一轮保留为 `_q50` 后缀的归档。

### 主判据的选取规则（**在跑之前锁定**）

> **A0 的主判据取自「三个档位中最弱的 *eligible* 档」**，其中 `eligible` 指原 §7.2 三分判决
> 里既非 `A0_INPUT_INSUFFICIENT` 也非 `A0_CEILING_CONFOUNDED`。三档一律报告。
> 若三档都不 eligible，**A0 收口为不可判**，不再降档、不再改窗、不改天花板判据。

这条规则不可被结果反向操纵，理由是：`eligible` 只由 **off 臂**的性质决定
（天花板是 off 臂的招募范围；输入充分性是"是否越过 `Q_on`"与"窗口剩余长度"），
**完全不涉及 on-vs-off 的比较**。`≥10%` 的效力判据只在规则选出的那一档上评。

### 与原文的关系

原 §5.2「q50 是唯一授权输入」被本条取代。原 §7.2 的三分判决、`A0_CEILING_FRAC = 0.9`、
`A0_WINDOW_MS = 1000`、`A0_RUNAWAY_HZ = 300`、`≥10%` 效力判据**全部不变**。
`Q_scale` 与 `I_R,max` 仍然不扫。仍然禁止为过门调 `Q_on` / `τ_R` / `I_R,max` / drive 幅度 /
连接（"降档"指在**已锁定的 `S_Z` 轴**上换预注册的点，不是调一个连续旋钮）。
