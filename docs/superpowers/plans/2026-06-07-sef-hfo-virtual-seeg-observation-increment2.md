...
> **2026-06-07 PIVOT — Option B adopted (user-ratified estimator/kick fork). The center-kick + `onset_front_axis` body below is SUPERSEDED (retained for audit; §-1 CLAIM unchanged).** The SNN smoke FAILED: `onset_front_axis` (principal axis of the early-contact SET) needs DENSE 2D sampling — 16 contacts on 2 shafts pin the axis to a shaft. **Oracle-parity diagnostic (decisive):** same AR=2/θ_EE=45 center-kick event → dense-neuron oracle `anisotropy_front.principal_axis` = **44.9° (ratio 3.65, err 0.1°)** (model+kick FINE, event genuinely elongated along θ_EE); sparse 16-contact `onset_front_axis` = **100° (err 55°)** (read fails). → `onset_front_axis` demoted to **dense-oracle parity reference only, NOT a sparse-electrode gate**.
> **Option B (sparse-friendly, = Increment-1 estimator):** **single-end kick (unidirectional front) → `endpoint_centroid_axis`** (source→sink; compared UNDIRECTED mod-180° vs θ_EE). The **kick-track control becomes LOAD-BEARING** — single-end reintroduces the seed-position confound, so θ_EE FIXED + move the kick end → undirected axis must STAY at θ_EE; the two end-kicks = H2 forward/reverse. C-track = kick at the θ_EE end, rotate θ_EE.
> **B validation (diagnostic, sparse 16c, L=3):** θ_EE=45 → endpoint-axis err **6.4°**, readability 0.76, n_part 16 (PASS, vs onset_front_axis 55° FAIL); θ_EE=90 → **28.4°** (marginal, n_part 9); kick@90-end & AR=1 iso → n_part 2–3 (INSUFFICIENT; iso fizzle independently supports "anisotropy necessary").
> **STATUS: B core read VALIDATED; full four-control run NOT run — needs an observation-GEOMETRY tuning pass (montage extent/placement, kick r_end, sheet size/density) so θ_EE∈{0,45,90} + kick-track all reach ≥7 contacts with err<25°. Verdict thresholds (25°/k_dir=3/n_seed_min) UNCHANGED, never tuned.** Throwaway diagnostics: `scripts/_smoke_inc2_obs.py`, `_diag_oracle_parity.py`, `_diag_singleend.py` (uncommitted).
> **Execution continues in the CLEAN Option-B plan: `docs/superpowers/plans/2026-06-07-sef-hfo-obs-increment2-optionB.md` (it REPLACES this plan's Task 5/6; this file is audit-only for the superseded center-kick + onset_front_axis approach). No more patching this plan.**
