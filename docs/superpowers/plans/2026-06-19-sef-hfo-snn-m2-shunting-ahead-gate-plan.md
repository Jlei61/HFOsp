...
**Goal:** **First (Task 0, data-side, NO model change): audit whether real interictal HFO group events actually axially self-limit (cover only a segment of the pathological axis) or are merely laterally narrow while propagating far along it — this gates whether the model work below is even the right target.** Then, if axial self-limit is a real event-level phenomenon, faithfully test the M2 front-inhibition gate by fixing the two implementation gaps the 2026-06-19 audit found — the engine's inhibition is current-subtraction (not conductance shunting) and the gate's inhibition is recruited AT the front (not ahead) — with direct ahead-of-front + axial-clamp diagnostics, then decide whether the gate axially contains a readable event or whether the mechanism genuinely must change.
...
- **Claim discipline.** The faithful-test verdict is "with conductance shunting + ahead-of-front recruitment, the gate {does / does not} axially contain a readable self-limited event at workpoint X (reach_axis bounded, L-invariant, not via killing ignition)". NOT a seizure claim. The prior "change mechanism" verdict stays WITHDRAWN until this test runs. Only if shunting+ahead STILL gives span↔suppress does change-mechanism trigger.
...
- **Data-audit gate (P0-science).** Task 0 (a data-side audit, NO model change) runs FIRST and gates the whole effort: it decides whether "axial containment" is even a real phenomenon to model, or whether real interictal events propagate far along the pathological axis and are merely laterally narrow (in which case the SNN's radially-narrow events may already match the data and the axial-containment target is wrong).

---

### Task 0: DATA-SIDE AUDIT — is real-event "spatial self-limit" lateral confinement or axial containment? (no model change; gates everything)

**Why first:** the M2 verdict hinges on what real interictal HFO group events actually do. If they propagate far along the pathological axis but are laterally narrow, the SNN's radially-narrow events may ALREADY match the data → axial containment is the wrong target. If they cover only a SEGMENT of the propagable axis, axial self-limit is a core phenomenon M2 must reproduce. Existing skeleton geometry (`results/topic4_sef_hfo/skeleton_geometry/`) says most subjects have a reproducible axis (axis_length median ~24mm; perp_spread.rms median ~12mm) — but that perp_spread is participating-channel transverse spread (confounded by implant sampling + participation rate), and endpoint/source/sink are template/subject-level, NOT event-level footprint. So existing data supports "propagation has an axis" but does NOT answer "does each event fill the axis." This audit answers it at the EVENT level.
...
- [ ] **Step 9: Interpret — the gate decision** (PRE-REGISTERED numeric thresholds; write the chosen branch AND the numbers into `cohort_summary.json`). Define, over the eligibility-spine subjects: `AF` = cohort median of (per-subject median `axial_fraction`); `LR` = cohort median of (per-subject median `lateral_ratio`); and for each metric the observed−null gap `Δ` = paired subject-level `median(per-subject observed − shaft_matched-null median)`, with a bootstrap 95% CI over subjects AND a Wilcoxon signed-rank test as a second check. "Observed below null" ⇔ that metric's Δ CI excludes 0 in the confining direction (and Wilcoxon p < 0.05). Branches:
  - **AXIAL-EXTENDED + LATERALLY-NARROW (reframe target):** `AF ≥ 0.75` AND `LR ≤ 0.5` AND the lateral_ratio Δ is below null → real events fill the axis but are narrow sideways → the SNN's radially-narrow events plausibly already match → "axial containment" is the WRONG target → model work below is OPTIONAL / reframed to lateral self-limit (Task 7 option 2).
  - **AXIAL SEGMENT (axial self-limit is real → model it):** `AF ≤ 0.5` AND the axial_fraction Δ is below the shaft_matched null → events cover only a segment of the propagable axis, below random same-n_part sampling → axial self-limit IS a real event-level phenomenon → proceed with Tasks 1–7.
  - **SAMPLING ARTIFACT:** the axial_fraction Δ vs the shaft_matched null has a CI that INCLUDES 0 (observed ≈ null) → the apparent confinement is implant coverage, not a neural self-limit → reframe; do not over-model.
  - **INCONCLUSIVE (no hard branch — DEFAULT when none of the above cleanly holds):** `0.5 < AF < 0.75`, OR the deciding Δ CI straddles 0 while point estimates point opposite ways, OR fewer than 10 eligible subjects → record `INCONCLUSIVE`; do NOT hard-reframe AND do NOT start the model — report and bring the numbers back to the user for a direction call.

  This verdict决定 whether Tasks 1–7 (model) are needed at all, or whether M2 is already data-consistent. Commit Task 0 before starting any model task.
...
- [ ] **Step 3: 4-cell gate-on sanity (P1 — the gates ALSO change the balance).** At the Task-5 shunting workpoint `(g*, drive*)`, run four cells and tabulate, so Task 6 can tell "axial containment" apart from "killed ignition / went tonic":
...
- [ ] **Step 3: Acceptance metrics** (reuse `event_field_geometry` fields in the fullfield JSONs): per readable event (`n_part≥7`, or ≥5 at finer readout) measure **`reach_axis_mm`** (axial extent — the load-bearing containment metric, NOT just `r95`), `edge_margin_mm`, `r95_mm`, clean fwd/rev. **Containment = `reach_axis` bounded well below the focus-to-focus distance (~14mm at L20) AND not growing L20→L32, with readable + bidirectional events still present, NOT achieved by collapsing event count** (cross-check against the Task-5 Step-3 4-cell sanity: a count drop with rising inter-event floor = tonic, not containment).
...
- [ ] **Step 0 gate (from Task 0):** if the data audit found real events are axially-extended + laterally-narrow (axial self-limit NOT a real event-level phenomenon), the whole "axial containment" target is wrong → the verdict is "reframe to lateral self-limit", and the model result below is secondary. Only if Task 0 found real events cover an axial SEGMENT (axial self-limit IS real) is "does the gate axially contain" the load-bearing question.
- [ ] **Step 1: Classify** the combo vs the Task-5 4-cell sanity + the Task-6 Step-4 diagnostics:
  - **GATE WORKS (faithful)** — combo has readable + bidirectional events, `reach_axis` bounded + L-invariant + spontaneous + NOT via killing ignition/tonic (4-cell sanity) **AND** the diagnostics support the mechanism (`I_lead_ms > 0` ahead-of-front AND `frac_axial_gated_by_shunt > 0` / axial-leading group suppressed under shunting) → the M2 ahead-of-front shunting brake DOES axially contain. Proceed to a full sweep for all-5.
  - **CONTAINS BUT MECHANISM UNCLEAR** — `reach_axis` shrinks but `I_lead_ms ≤ 0` or axial group NOT clamped → it's global/local suppression, not front-pinning; do NOT claim the gate; investigate (timing / shunt strength) before any verdict.
  - **STILL FAILS** — combo still span↔suppress even with shunting + ahead recruit + the diagnostics confirm the mechanism WAS instantiated (I ahead + axial cells clamped) → the prior "change mechanism" verdict is now EARNED (the faithful mechanism was tested and is insufficient). Pivot (structural barrier / radial-self-limit reframe per the 2026-06-19 recap).
- [ ] **Step 2: Write the result** with the §8 plain-language abstract + the口径 lock (what was tested: shunting + ahead-recruit + I→E gate (+recovery); at which workpoint; result). Update `m2_stage_recap_2026-06-19.md` (collaborator recap) + the M0/M2 memory.

---

## Self-Review

1. **Spec coverage:** DATA audit lateral-vs-axial (Task 0, gates everything; pinned broad-pool source + canonical loaders + accepted-axis projection, 3-mode matched null incl. shaft-matched, pre-registered AF/LR thresholds with an INCONCLUSIVE default) ✓; Q2 shunting (Tasks 1–3) ✓; Q1/GapA ahead-recruitment (Task 4) ✓; ahead-of-front + axial-clamp DIRECT diagnostics (Task 4.5 + Task 6 Step 4 — P1) ✓; operating-point caveat for shunting AND gate-on (Task 5 incl. 4-cell sanity — P1) ✓; Q3 axial-vs-lateral-vs-far-ahead cell groups (Task 6 Step 4 — P1, no longer assumed) ✓; Q4 (EE shortcut — ruled out in audit) ✓; coexistence guard shunt+recovery+gate (Task 3 Step 7 — P0) ✓; faithful re-test + verdict (Tasks 6–7) ✓.
...
7. **Verdict integrity:** Task 7 cannot declare "gate works" on `reach_axis` alone — it requires the ahead-of-front lead + axial-clamp diagnostics to confirm the mechanism was actually instantiated; and the whole model verdict is gated by Task 0 (if axial self-limit isn't a real data phenomenon, the target itself changes).
