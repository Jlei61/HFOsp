...
- [x] **Step 6 (DECISION GATE):** inspect `feasibility.csv`; **pilot-lock** `nonaxis_hfo_participation_max`, `beta_axis_reliability_min`, `lowrank`, `single_contact_energy_frac_max`; confirm `finite_horizon_k=3`; record locked values in config + this plan. **Auto-select one `geometry_sufficient AND i1_eligible` subject per cohort as the downstream integration subject.** If <4 narrow subjects qualify → STOP + report (narrow may be non-viable).
  - **PILOT-LOCK RESULT (2026-07-02): GATE PASSED — narrow 7/7 qualify (≥4 required), broad 9/9.** All-clean pools 30–124 contacts (matches spec's "60–124"). Locked: `nonaxis_hfo_participation_max=0.10` (clean split: non-axis participation=0.0 vs axis ≥0.34), `beta_axis_reliability_min=0.20` (H3a supportive-only), `lowrank=6`, `single_contact_energy_frac_max=0.50`, `finite_horizon_k=3` confirmed. **Downstream integration subject = `epilepsiae_253`** for both cohorts (smallest all-clean pool: narrow 30/8-axis/22-nonaxis, broad 30/20-axis/10-nonaxis; 6/6 i1-eligible seizures → fast integration tests). Pool construction is now the shared `scripts/_topic5_v3_io.py::classify_subject_contacts` (Tasks 6/8/9 import it; real per-channel `channel_is_valid` QC replaced the vacuous `meta["drops"]` filter).
