# Topic 4 rev10-D5 execution plan

1. Add the optional spatial OU hook with an exact off-path hash test and an exact local/permuted value-multiset test.
2. Freeze nine candidates: off plus four local and four matched-permuted arms. All edge coefficients remain exact zero.
3. Run a measured-RSS sentinel, then up to nine one-thread workers under 20/24 GiB cgroup limits and a 180 second wait.
4. Aggregate with equal network weights and returned-only events. Patient-training labels remain a development readout.
