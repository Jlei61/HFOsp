# [SUPERSEDED] Topic 5 Fig3-B peri-onset field similarity — old own-plane handoff

> **2026-07-18 状态**：本计划锁定的是旧 observation-readout / own-plane 合同和旧 E1146 数值，已被二维 frozen shared-gradient 合同取代，**不得继续执行或作为验收标准**。当前 source of truth 为 `docs/figure_style_guide.md` §5a、`docs/main_figure_plan.md` Fig3-B、`fig3_peri_onset_subject_index.json` 和 `fig3_peri_onset_run_manifest.json`。当前正式分母为 12 个二维 shared candidates，7 人出图；E139 为 1D sensitivity；旧 spatial null 已归档。
...
## 历史验收标准（已废弃，不得用于当前 shared-gradient 版本）
