...
**CLI:** required `--subjects ds_sid...` XOR `--cohort` (explicit; no implicit run). `--substrate {broad,narrow,both}` default `both`. `--input-results-root` (labels+geometry live in the main tree, gitignored — default `/home/honglab/leijiaxin/HFOsp/results`). **NO `--root` flag** — the narrow canonical lagpat root is chosen BY DATASET inside `_narrow_lagpat_dir` (Task 0, `YUQUAN_ROOT`/`EPILEPSIAE_ROOT`), matching the broad path's no-override convention. Do NOT thread a single root into the loader (a single `--root` defaulting to the Yuquan root reintroduces the epi-root bug). `--n-perm` (1000), `--n-split` (200), `--loo-split` (50), `--min-eff` (6), `--out`. `--cohort` discovers subjects per substrate by globbing `GEOM[substrate]/*_t_a.json`.
...
2. **Cohort reversal Data-vs-Null** (`field_reversal_cohort_stat.png`, ONE figure, 2 groups **broad | narrow** side by side — never pooled): paired violin+box+jittered-points, `Data` = per-subject signed corr(TA_field,TB_field) | `Null` = per-subject within-shaft null **median**, light-gray lines connecting each subject's Data/Null pair, NO subject names. Signature test = paired **Wilcoxon one-sided `alternative="less"`** (cohort Data more negative than Null); significance bracket + p-stars per group; per-group footnote = `n (D degenerate excluded)`, Wilcoxon p, `n(data<null)`, AND `cohort_binomial` K/N pass (the formal per-subject gate). y-axis = `"TA–TB field reversal (signed r)"` with an **r=0 reference line** (reversal = points below 0 AND below Null). **Mirror the visual idiom of `scripts/paper_figures/plot_fig3_field_concordance_cohort_stat.py`** — reuse its `_add_violin_box_points` / `_add_sig_bracket` / `_p_stars` / `_fmt_p` pattern — but with three mandatory departures from fig3: (a) **signed r, not |r|** (y-axis spans negative; the sign IS the reversal claim); (b) **less-tail Wilcoxon, not greater**; (c) **grouped by substrate, not by band**. Write a metadata JSON + Chinese README with the `interpretation_boundary`: "visualizes cohort-level reversal shift below the within-shaft null; formal pass still requires the per-subject binomial gate." Diagnostic supplement (optional, NOT a paper panel per §7 — same question as this panel): per-subject null-forest `{substrate}/field_reversal_null_forest.png` (observed vs own null p05, sorted, black=passed).
