...
2. **B — 更强刺激下还是 null 吗**：固定核，把戳的强度调成 1×/2×/3×。**结论 = matched（纯参差）的诱发同步在锁定强度（2×）和更强（3×）下都贴零 → null 对更强刺激稳健**（这是 roadmap 明确要答的问题，答 YES）。**意外观察（已大声标记、没抹平）**：在**弱戳（1×）**下，窄核在固定窗 [150,350] 内的同步比宽核**明显更低**（−0.41，10/12 网络）。**⚠ 但机制未定（advisor 抓的核查缺口，正是 §1 固定窗踩过的坑）**：是 (a) 窄核招募更少（尾巴帮招募），还是 (b) 窄核事件延后/变慢、被固定窗截在上升段——本轮没记 `event_peak_t`、无法分辨。**只能写"弱戳下窄核窗内同步更低、机制 (a)/(b) 未定"**，要钉死需补记事件峰时刻。**不动 headline**（2×/3× null 稳健）。
...
**机制收口（一句话，措辞收紧）**：自点火需要"够多细胞集体点着、达到临界量"。**平均门槛下移设定整体驱动**（窄核无尾，只要够大够低也能点——C: r0.6 窄在 16–16.5 点）；**尾巴宽度（A/B）和核面积（C）是 modulator，调节"多容易凑齐临界量"**，不是"低门槛细胞绝对数量唯一决定"。临界量存在（C: r0.3 小核全程不点）。这是放电版独有、率均值场看不见的有限尺寸现象。**锐利检验（cross-prediction，留下一步）**：同样低门槛细胞数下，大窄核 vs 小宽核 应点火相近。
...
**建议下一步（给 user 选，未做）**：① 正式裁决需"连接/阈值场/噪声"三类种子分离 + 更多网络；② forward-LFP/HFO 读出层（才敢对应真实 SEEG）；③ B 的弱刺激 regime 深挖（刺激强度 × 参差二维）；④ C 的 r=0.3 边界没在 spec 范围内钉死（要钉需 mean<16 = 动 gap-limit，框架级决定）；⑤ 率/F-I 配平控制去 Jensen 残留；**⑥（advisor 直接点的、最便宜）给 `kick_amp` 补记 per-cell `event_peak_t`，确认 1× 窄核事件峰落在 [150,350] 内——分辨 B 弱刺激效应是"招募更少"(a) 还是"事件延后"(b)，再决定能不能归因到尾巴**；⑦ synthesis 锐利检验：大窄核 vs 小宽核（低门槛细胞数配平）应点火相近。
