...
> **状态**：v1.0.4 framework lock，2026-05-22；**Phase 0 解锁 2026-05-21（lagPatRank phantom-rank broad re-derivation Step 5a–5h 全部完成 + Checkpoint A/B advisor consult 通过）**
> **版本历史**：v1 lock 2026-05-20 (initial) → v1.0.1 advisor 5-fix 同日 → v1.0.2 user review 6-fix 同日（H2 工程不可行修复 + H1 概念冲突拆分 + H3 verdict naming 修订 + H4 normalized instability + H5 统计合同收紧 + Topic 1/3 cross-doc 实际落字）→ **Phase 0 完成 2026-05-21**（Topic 0 §3.1 phantom-rank 修复跑完所有下游 PR）→ **v1.0.3 lock 2026-05-22**：H2 prose 改 PR-6 swap_check ingest（CLAUDE.md §5+§6 错误三段修复：cohort 用错 candidate 字段、重新发明 PR-6 已有 helper、升级 mechanism-sanity tier 为 cohort claim 都修正）+ H1 v1.0.7 null pool 扩到全 SEEG implantation（v1.0.6 lagPat 池循环论证 bug 修正）→ **v1.0.4 lock 2026-05-22**：H2 input order 收紧，PR-2 只作模板发现 / provenance；Topic 4 建模前 channel-label 以 masked rank-displacement `swap_sweep` variable-k (`decision_k`) 为准，PR-6 fixed top-3 endpoint 作为摘要层而非最稳 H2 输入
...
>
> **判读 tier lock**: PR-6 plan §3.3 + §15 → H2 是 **directional mechanism sanity, NOT cohort claim**（pre-registered tier，不允许 results 阶段升级）。每 subject 报 label 层 (`swap_class` / `decision_k` / `T_obs` / `p_fw` / `swap_endpoint_channels`) + spatial 层 (`source_side` / `sink_side` / `combined_endpoint` compactness verdict)。cohort 报 `swap_class_distribution` 与 spatial verdict distributions；**不给 cohort-level PASS/NULL/FAIL 整合裁决**，也不再报旧的 `exceeds_null_95th` sign-test。
>
...
**H2 报告规则**（v1.0.4 lock，旧 v1.0.2 "PASS/NULL/FAIL 整合裁决"已弃用）：

H2 不产生 cohort 整合 verdict。它是 **directional mechanism sanity（pre-registered tier）**，输出两层 per-subject + 一个 cohort descriptive bar：

- **label 层** (per subject)：`swap_class` ∈ {`strict`, `candidate`, `none`} + `decision_k` + `T_obs` + `p_fw`。来源是 masked rank-displacement `swap_sweep`。
- **spatial 层** (per subject)：source-side `k` 个 swap 节点、sink-side `k` 个 swap 节点 各自做 `PASS / NULL / FAIL_DIFFUSE` compactness 裁决（null = `all mapped SEEG minus swap endpoint` 无 shaft 约束随机抽 `k`）。`combined_endpoint (2k)` 只作辅助描述。
- **cohort 层**：报 `swap_class_distribution`、`spatial_compactness_verdict_distribution{source_side, sink_side, combined_endpoint}`、median swap_score / p_fw。**不报 cohort PASS/NULL/FAIL**，不做 cohort Wilcoxon / sign-test 升级裁决。

**允许的判读语言**：
- ✅ "rank-displacement 派生的 swap-k source-side 与 sink-side endpoint，相对其他 mapped SEEG contacts 呈现非随机空间聚合（多数可测 subject）"
- ✅ "H2 spatial sanity supported / partially supported / not supported (per-subject only)"
- ✅ 当 `swap_class == strict ∩ source/sink 双 PASS` 时，可报"该 subject 上 swap-k 与 spatial 双层证据一致"
- ❌ "H2 证明 source/sink reversal 是 cohort-level 主效应"
- ❌ "所有 source/sink 节点都有空间聚合"
- ❌ "H2 cohort PASS / partial PASS"
- ❌ 报 `combined_endpoint` 当作唯一空间裁决（source / sink 可能本身就在传播轨迹两端）

**Phase 1 失败模式表**（H6 × H1 × H2 三轴；H2 列填的是 spatial 层 source/sink 一致性 + label 层 swap_class 多数态）：

| H6 (participation 分隔) | H1 (endpoint 锚点) | H2 (swap-k spatial sanity) | Framework 解读 | 后续动作 |
|---|---|---|---|---|
| PASS | PASS | source ∩ sink 多数 PASS + swap_class strict/candidate 多 | 完整证据链：空间场 + 结构化锚点 + swap 节点空间聚合一致 | 进 Phase 2 |
| PASS | PASS / partial | 主要 NULL / FAIL_DIFFUSE | 空间场 + 锚点成立，但 swap-k 节点不空间聚合 | swap 节点可能不是同一空间结构；H2 改写为"label only, no spatial coherence"；framework 仍活 |
| PASS | NULL | source ∩ sink 多数 PASS | 空间场成立 + swap 节点空间聚合，但 H1 endpoint 不构成锚点——内部矛盾 | 回 Phase 0 查 endpoint 定义 + phantom rank 是否漏修 |
...
**重要**：H2 列"多数态"只是描述阅读辅助，不是 cohort verdict。任何 cell 落到 NULL / FAIL 都必须按表执行后续动作。**禁止**事后调整 H1 / H2 / H6 数字判据来"挽救" verdict。
