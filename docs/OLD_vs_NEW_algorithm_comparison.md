...
- `resample_sfreq=auto` 对 legacy 复现不安全；EDF 起跑验证里它曾实际跑成 `1000Hz`，而不是老代码固定的 `800Hz`
- 新代码虽然已有 `drop_shaft_edges`，但当前 EDF 起跑日志显示 `130 -> 110`，而老 `*_gpu.npz` 是 `120` 通道；说明**通道保留合同**仍未对齐
- 虽然 `packing_channels` / `subject-level refine recount` 已经补进新代码，但 `_refineGpu.npz` 的历史 artifact 仍未做到 1:1 重建
- 若不用现成 `packedTimes.npy` 而从 EDF/`*_gpu.npz` 重建，群体事件定义已接近但仍未完全重合
- 滤波实现仍是老代码 FIR / `cusignal` vs 新代码 IIR / `scipy`
...
### 7.2.1 2026-03-31 CUDA EDF 起跑验证

为了把“环境兼容问题”和“算法差异”拆开，这轮不是只对比旧资产，而是让**新老代码都从 EDF 起跑**，先跑 `chengshuai / FC10477Q / 500s` 的 smoke/regression。

先说环境：

- `cuda_env` 原先缺 `cusignal`，现已通过 `conda install -n cuda_env -c rapidsai -c conda-forge cusignal` 补齐
- 老代码在当前软件栈上还需要两层兼容：
  - EDF 读取要显式 `encoding='latin1'`
  - 新版 `numpy` 下，ragged `whole_dets` 直接 `np.savez()` 会报错
...
- 老代码从 EDF 重跑（500s）：
  - `packedTimes = 191`
  - channels = `E11, K3, K5, K6, K7, K8, K9, K10`
- 新代码从 EDF 重跑（500s）：
  - `packedTimes = 190`
  - channels = `E11, K3, K5, K6, K7, K8, K9, K10`
- 两边 channel set 已完全对齐
- `178 / 190` 个新窗口可以在 `5ms` 容差内匹配到老窗口
- 已匹配窗口的平均边界误差约 `0.86ms`

这说明：

- 新代码的 EDF 起跑链路已经不是“完全跑偏”
- 但窗口定义还在局部漂移，`lagPatRaw / lagPatRank` 也因此还没法硬对齐

这轮验证里抓到的更大问题，是上游通道合同：

- 老 `*_gpu.npz` = `120` 个 bipolar 通道
- 新代码这次预处理日志 = `140 -> 130 bipolar`，再 `drop_shaft_edges -> 110`

所以当前最值得追的根因不是 `lagPat` 小数点，而是：

- 哪 `10` 个通道在新链里被额外删掉了
- 当前 `drop_shaft_edges` 是否真的等价于老 `return_valid_chan_index + subs_drop_info`
- 如果这个问题不解决，后面的 `packedTimes`/`lagPat*` 不可能精确复现
