...
> **2026-09-10 当前审阅重点**：新版传播线采用非正阈值 core 和优先加载 core 的随机 OU 输入，历史图须标明原实际模型。患者 Fig2C 黑底完整时序、核内外全部活动的多事件 2D 图、配对参数×观测是当前承重材料；KMeans 标签与平均 rank 不能替代它们。局部连接权重/数量允许直接增减，不预设总输入守恒。具体定义及稳定报告入口见[共享设计](topic4_patient_geometry_prior_snn.md)与[协作者报告](snn_model_report_current.md)；不自动替换已登记正式图。

> **2026-09-09 共享科学口径更新**：当前 Figure 4 的设计链为患者几何先验、事件分布拟合、参数作用分析和独立验证，见[统一规范](topic4_patient_geometry_prior_snn.md)。图中须分清端点/轴作为输入、实际优化参数以及未被直接规定的传播表现；保留历史几何对照和参数 × 观测响应。本文既有图型约定继续适用，旧 producer/候选不因口径更新成为新实验；正式图件身份与验收不自动改变。
