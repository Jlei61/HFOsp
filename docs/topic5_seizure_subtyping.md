...
**⚠️工程教训**：并行 numpy nulls worker 必须 `OMP_NUM_THREADS=1`（否则 26 worker×80 核 BLAS 超订 → 单被试 crawl，7h/40min 之差）；broad=16（1146=within_shaft_strong 离群、单被试 n_perm=1000 单线程 ~2h、OMP 不助（Python-loop permute）→ 排除；保守、已在 narrow、后台补跑）。
