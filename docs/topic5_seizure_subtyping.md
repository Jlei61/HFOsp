...
### 3.12 让循环网络学出组织间连接图（2026-08-07，exploratory，**连接身份不可辨识 · 三个技术原因已排除 · 仅预测未观测触点这一层有正结果**）

把网络状态从「每个触点一个单元」搬到「患者传播平面上的一小块组织」，触点只作观测口，
输出强制经过一个按真实触点坐标造的固定空间读出层，网络里没有任何触点到触点的直连通路。
两版：v0.1 让组织块之间的连接**自己学**（带省线成本与边数预算），v0.2 把连接**写死成最近邻**、
只学八个有物理含义的数。

**在答案已知的合成数据上，"哪些组织块连着哪些"认不出**：排序真边的能力在所有条件下都贴随机线
（判据 ≥0.60，随机 0.50）——组织块数 36/72/288 全部 0.485–0.527，读出半径 2/3/4/6 毫米全部
0.492–0.511。"活动整体往哪走"同样认不出。唯一认得出的是**各组织块影响能推多远的相对排序**。

**三个技术原因用直接实验排除**（每次只动一个变量，其余逐位固定）：不是读出范围太大（缩到四分之一
纹丝不动）；不是未知数太多（问题缩小 65 倍纹丝不动）；不是稀疏限制太松或训练不够（去掉稀疏预算
预测不变好、打乱几何图变短但预测不变、所有单元早停自收敛零截断）。文献侧：从活动反推连接是
**已知的病态反问题**——大量不同连接矩阵产生几乎相同的动力学，故自由图路线下"认不出边"是预期结果。

**读出宽度确实影响另一层**：把一个触点听的范围从 6 毫米缩到 2 毫米，"影响推多远的排序"在
6 个（患者×种子）配对上**全部改善**（中位 +0.185，符号检验 p=0.016，两位患者各自完全分离）。
⚠️ 每档自身的检验都是 p=0.125，那是 3 个种子下符号检验的**功效下限**，不是阴性。

**唯一的正结果**：扣掉每位患者四分之一触点（不进训练、测试不喂、自身偏置换成中性值，身上只剩
「它在哪」），预测这些触点是否被点亮**好过对它一无所知**——无输运的场 +0.0036（17/21，p=0.026）、
完整算子 +0.0032（16/21，p=0.009）。两臂打平说明起作用的是固定观测核带来的空间结构，**不是**
可学的空间输运。这与文献里连接组约束路线的成果形态一致（结构本身定不住动力学，
"结构 + 记录 → 预测未记录部分"才是可达的）。

**禁止的说法**（本版无依据）：「学出了这位患者的连接图」「患者间连接图差异」「定向删连接会怎样」
「间期传播不是空间的」（无输运臂本身即经观测核的距离依赖空间模型，未加成的只是**可学**的那部分）。

**先验审计**：现有连线成本把距离除以中位距离而**无长度尺度**；模型**完全没有传导延迟**，因而
一条长边与一串短边接力在模型内等价——这本身即简并空间的一个具体来源。方法学原则：生物物理先验
的增删只能依据独立生物学依据，**不得按拟合结果调整**；可辨识性检验的输出是"哪些结论被允许"。

完整方法、逐格数字、已撤回表述与建设期缺陷清单：
`docs/archive/topic5/rnn_learns_connectivity_wall_report_2026-08-07.md`；
分版归档：`spatial_latent_propagation_rnn_v0_1_2026-08-06.md`、
`spatial_propagation_operator_rnn_v0_2_2026-08-07.md`（后者在
`codex/topic5-spatial-propagation-operator-v0-2` 分支）。
...
- `docs/archive/topic5/rnn_learns_connectivity_wall_report_2026-08-07.md` — **给合作者的完整报告：让循环网络学出组织间连接图撞在哪**（exploratory）。连接身份在所有组织块数与读出宽度下均不可辨识；读出范围太大 / 未知数太多 / 限制太松 三个技术原因各以直接实验排除；文献侧确认这是已知病态反问题。唯一正结果为「只凭位置预测未训练过的触点」。含已撤回表述与八项会静默出错的缺陷清单。
