...
> **Patient-specific target-free RNN bridge（2026-08-03，v0.1）**：不再做跨患者 readout；每位患者只用自己的 masked interictal contact-rank events 独立自监督训练。16 人 × 3 模型 × 3 seeds 共 144 units 全完成；15 人 primary 中真实顺序 GRU 相对 rank-shuffle 的 heldout NLL 在 14/15 人改善（median 0.0586 nats/event，exact P=0.000122），自由 rollout precedence 相关中位 0.775（rank-shuffle 0.052），支持患者内 RNN 学到间期传播结构。checkpoint 与 rollout 冻结后才读取同一患者 clinical-onset `[0,10] s`、`1–150 Hz` target；GRU 场 median max|rho|=0.584，相对 all-contact null 的 margin +0.167，13/15 为正、P=0.0256。该跨状态对应不是有序 GRU 独有：完整 static fit60 也阳性，GRU−static 增量 +0.025、P=0.305；within-shaft sensitivity P=0.149。安全结论是**RNN 恢复的患者间期空间 scaffold 与发作早期场有关**，不是“RNN 顺序动力学额外预测发作”或“自动恢复唯一 A/B 轴”。详见 `docs/archive/topic5/patient_specific_target_free_rnn_bridge_v0_1_result_2026-08-03.md`。
> **Static-anchored HistoryRNN refinement（2026-08-03，v0.4 正式）**：按已有论文结果冻结患者特异 A/B 静态场，并用完全相同的 sign-free maxAB 训练受限 history residual。15 位 development-excluded strict clinical-onset 患者、31 次发作完成 15 LOSO × 3 seeds。静态 M0 保持阳性（median maxAB 0.5545；相对 matched all-contact null 的 margin +0.1500，P=0.01245），但 M3−M0 中位 0（1 正/7 负/7 并列，P=0.0391），也不优于冻结 state M1 或非递归 M2；真实顺序和正确 seizure-history 配对均无优势。最终结论是**静态跨状态 scaffold 保留，逐发作 causal-history refinement 未建立**，进入 Supplementary bounded result。详见 `docs/archive/topic5/history_conditioned_field_refinement_v0_4_result_2026-08-03.md`。
> **HistoryRNN next-event proxy（2026-08-02，provisional bounded-negative）**：用真实 IEI 衰减、跨事件传递且事件内编码器逐事件重置的双级模型，检验 ordered inter-event history 是否超越 static + matched unordered history。3 seeds × 34 LOSO 和 102 个同-prefix严格顺序对照全部完成；31 人 primary 的 chronological increment 中位 −0.000186、15/31 为正、单侧 p=0.101，且 seed/数据集方向不一致，因此主对比未过。同 prefix 打乱较早事件顺序会改变模型输出，但不能证明顺序提供了无序模型拿不到的信息。Synthetic recoverability 已通过，state branch 非零；c10→c30 训练审计仍敏感，因此 G1 只约束当前 next-event objective，不作为模型类正式阴性，也不否决 direct early-ictal transfer。详见 `docs/archive/topic5/history_rnn_early_ictal_field_v0_1_closeout_2026-08-02.md`。
> **HistoryRNN direct early-ictal transfer（2026-08-02，v0.2 最终；同日 code review 后重跑）**：15 名 development-excluded strict clinical-onset 患者、31 次发作完成 target-patient LOSO；每次发作用 onset−10 min 前 causal history，患者内以 seizure median 折叠，并使用每患者 5000 次 all-contact channel shuffle。**每患者的场只覆盖 6–16 个骨架触点（中位 9），不是完整蒙太奇**，因此阳性与阴性都是粗分辨率。按合同实现的全前缀顺序打乱下，**真实事件顺序在 c10 与 c30 两个预算下都没有优势**（c30 P=0.207、c10 P=0.368；早期报告的 c10 顺序阳性来自只打乱最近 64 个事件的对照，已撤回）。c10 仍有 RNN 相对 M1 与非零状态两项阳性，c30 均未复现（R2−M1 median Δρ=0.0571，8 正/7 负，P=0.138；zero-state P=0.138）。c30 的绝对 RNN 场为 median ρ=−0.025，channel-null margin P=0.598，只有 2/15 超过各自 null p95；正确 history–seizure pairing 亦阴性（P=0.527）。整个模型阶梯（含吃进冻结 TA/TB 骨架的 M0，中位 ρ=−0.214）绝对场都不超过零，因此阴性只覆盖“这一族有符号、跨患者共享的 readout”。最终定位为 `ACCEPTED_SUPPLEMENTARY_TRAINING_SENSITIVE_BOUNDARY`：不支持训练预算稳定的 latent-state→early-ictal field，也不推翻既有 sign-free static morphology。详见 `docs/archive/topic5/history_rnn_direct_early_ictal_transfer_v0_2_result_2026-08-02.md`（§5.4 修正、§10 已知边界）。
