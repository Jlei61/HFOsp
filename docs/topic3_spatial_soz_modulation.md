...
**Yuquan：21/21 完成；Epilepsiae：19/20 完成（1077 CPU 模式 189/189）**。所有结果在 `results/hfo_detection/`。GPU OOM 修复 (`_flush_gpu_memory()` 释放 CuPy 池) 解决 1073；1077 121 ch >24GB 走 CPU 模式。Mixed-fs subjects (139, 384, 583) 部分 256 Hz block 被 `min_sfreq=500 Hz` 门禁跳过。

完整 per-subject 状态表（20 行 subject × blocks × ch_count × 备注）见 [`docs/archive/topic3/pr1_spatial_modulation/hfo_detection_rebuild_2026-04-15.md`](archive/topic3/pr1_spatial_modulation/hfo_detection_rebuild_2026-04-15.md) §4.3。
