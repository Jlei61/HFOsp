...
- **PR4–PR6（Epilepsiae 侧）已完成**：16 subjects / ~1,280,824 event rows / 232 intervals 上的 event-level 同步性全链分析已跑通。**结论：队列水平 null**（固定窗口三指标 p>0.35；within-interval trajectory legacy p=0.290, phase p=0.933, span p=0.053 但方向与假设相反）。个体异质性远大于队列效应。详见 `docs/archive/topic1/synchrony/plans/interictal_synchrony_analysis_v4.plan.md` 的"当前科学结论"章节。
