...
**Step 6 cohort 结果（2026-05-10）**：n=35 stable_k=2 subject（PR-6 main 23 ∪ rank_displacement v14 sensitivity n=35 去重）跑完。Tier 分布：strong 20 / moderate 13 / weak 2 / fail 0；cohort 中位数 `template_spearman` = 0.922、`endpoint_position_recall` = 0.833、`assignment_coverage` = 1.000、swap_class concordant = 24/35 = 68.6%。**Supportive null caveat（advisor 2026-05-10 post-hoc，epilepsiae_548 单 subject per-event 二半 rank-shuffle 50 trials，仅作 caveat，不算正式校准）**揭示 pipeline 有结构性 selection-bias 底：`template_spearman` null median = **0.747**，`endpoint_position_recall` null median = **0.667**，`swap_class_concordant` null = **36%**（18/50）。在该 single-subject null 下 cohort 信号近似为 spearman gain +0.17、recall gain +0.17、**swap_class concordance gain +33 pp**——按这个 single-subject 标尺看，`swap_class_concordant` 是三量里最 discriminative 的（与 raw count 视角相反）。**正式 paper-level tier rebalancing 需要 per-subject null 或至少 stratified null（按 n_valid / n_events 分层）**，本批次未做。**结论收紧措辞**：Step 6 仅证明 **endpoint instability 不足以解释 H1 NULL**；PR-6 主线 H1 cohort NULL 与 §8 dual-tier swap_class 在 within-recording held-out endpoint geometry 下保持稳健。**不**证明 SOZ 标签 / clinical annotation / lagPat 通道选择没问题——这些维度本 Step 不覆盖。Plan §7.1 strong-tier 阈值 (spearman > 0.7、recall > 0.6) 跨在 single-subject null 分布之上；tier 计数保留作 plan-of-record 一致性，paper-level 表述应基于 supportive caveat 而非作 calibrated 校准。strong tier 中 12/20 (60%) 是 §8 swap_class = none —— hold-out 稳健性不依赖 swap geometry，与 H1 cohort NULL 同向。weak 2 subject (`yuquan_litengsheng`、`yuquan_zhaochenxi`) 走 case-series；`zhaochenxi` 的 §8 strict 标签在 hold-out 下推翻为 none，提示 single-fold pattern。**Scope caveats**：(a) 本 Step 只覆盖论文记录窗口内（Yuquan 24h / Epilepsiae 数日）within-recording stationarity，**不是** H_drift 全盘否定；(b) supportive null caveat 仅一个 subject，不是全 cohort 校准；(c) `assignment_coverage` 是 assignable fraction（plan §7.1 的 nearest-vs-2nd-nearest gap purity 未实现，descriptive only）。详见 `docs/archive/topic1/pr6_template_anchoring/pr6_step6_held_out_template_results_2026-05-10.md` §4.6。

**PR-6-sup1 plan-of-record 落盘（2026-05-10，Topic 4 mechanism preflight）**：First-rank entropy / symmetry-breaking diagnostic 独立成档为 `docs/archive/topic1/pr6_template_anchoring/pr6_supplementary_rank_entropy_plan_2026-05-10.md`（v3）。问 "iHGE 事件 rank vector 在 endpoint position 是否比 middle position 更不稳定"（Liou-Abbott 风 noise-driven confluence-point sharp prediction）。**与 Step 6 数据流分开**：sup1 不切时间、不依赖 endpoint 集合 / SOZ；§8 swap_class 仅作 stratifier。Tier = Topic 4 preflight，descriptive only，不进 paper α，不进 PR-6 主线。

**PR-6-sup1 cohort 结果（2026-05-10）**：n=35 stable_k=2 cohort；19 subject 走完 subject-level Option B（all-valid-participating events filter）；16 subject 因 partial-participation 导致 cluster `kept_events < 50` 走 `excluded_one_or_both_clusters`；1 subject (epi_1096) 因 PR-2 labels 长度对不上 `_valid_event_indices(min_participating=3)` 走 `pr2_labels_len_mismatch`。**`Δ_obs_subject` 19/19 全 cohort 一致 < 0**（median = −0.123，sign test on Δ<0 p = 1.9e-6），三组 §8 swap_class（strict 6 / candidate 3 / none 10）的 Δ 分布无统计差异（Kruskal-Wallis H=0.87, **p=0.65**）。`subject_combo_percentile` median = 0.0023（端点对组合在所有位置对的 Δ 排序里接近最小）；0/19 `is_subject_combo_max` = True。即：**iHGE 事件 rank vector 是 roof-shape**——中段 H_p_norm ≈ 1.0 接近满熵，端点 H_p_norm 跌到 0.6-0.9 显著较确定。Plan v3 §6.3 列出 "endpoint deterministic / middle 随机 → Δ < 0" 是三种预期形态之一；本批数据**实际命中该档**，**否决** Liou-Abbott 风 noise-driven confluence-point sharp prediction。**重大 caveat**：drop_rate cohort median = 0.98（Option B 严格过滤后只看到事件总数 ~2% 的 stereotyped 子集）；43/68 cluster `high_drop_rate_warning`（plan §12.4 的 30% follow-up 触发线远被超过）；本批结论严格只对全员参与子集成立，selection bias vs stable-pathway 真信号无法从本批数据区分，X=80% partial-participation sensitivity 列入独立 follow-up。**反过来对 PR-6 主线的方法论意义**：sup1 给出独立证据 rank vector 两端是 event-wise 最稳定位置——PR-6 endpoint = top-3 ∪ bottom-3 这个约定在 method 层面被支持，与 PR-6 H1 / §8 / §9 cohort NULL 一致：**endpoint 几何信号是真的，但它不锚定 clinical SOZ**。详见 `docs/archive/topic1/pr6_template_anchoring/pr6_supplementary_rank_entropy_results_2026-05-10.md` + 5 张图（主图 `H_p_norm_cohort_overlay` normalized x ∈ [0,1] 叠加 + cohort median + IQR 带）。
