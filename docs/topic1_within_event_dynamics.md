...
**Continuous-version supplementary（2026-05-06）**：把 PR-6 Step 4b 离散 swap_node count 升级到逐通道 signed rank displacement（Δr = rank_T_b − rank_T_a），加 Spearman footrule + Diaconis-Graham 归一化 + Kendall τ + baseline-corrected SOZ contribution split。**不**立独立 PR、**不**开 cohort gate。**Cohort = PR-6 supplementary cohort = stable_k=2 ∩ PR-6 endpoint-defined = n=23 / 27**（PR-2 stable_k 分布 `{2:27, 4:2, 6:1}`，4 个 stable_k=2 无 PR-6 anchoring 被排除：`1125/384/620/916`）—— **不是**完整 27-subject stable_k=2 cohort，是 PR-6 supplementary。**Paper-level deliverable 是单张 composite supplementary figure**：cohort heatmap 主体（行按 F_norm 降序，列按 rank_T_a_dense，SOZ 黑框）+ 三条共享 y 轴 summary track（F_norm 带 2/3 ref / Kendall τ 带 0 ref / SOZ contribution_excess 带 0 ref）。每一行 = 一个 subject，所有信息在同一坐标系。结论：(a) cohort 在 F_norm 范围 [0.39, 1.00] 上呈连续谱，不是离散二分；(b) Spearman ρ(F_norm, Kendall τ) = −0.92 (n=23) 验证两个指标基本测量同一件事；(c) Spearman ρ(F_norm, soz_contribution_excess) = 0.19, p = 0.38, n=23 —— **rank reversal 强不强与 SOZ 通道是否更参与 displacement 几乎无关**（诚实 negative finding，受通道选择 caveat 限制）；(d) 与 PR-6 离散 swap_node 同向（fwd/rev-reproduced 6 subject 的 Kendall τ 全部 < 0，PR-6 sign-test n=6 p=0.031）。详见 `docs/archive/topic1/pr6_supplementary_rank_displacement_plan_2026-05-06.md` 与 `pr6_supplementary_rank_displacement_results_2026-05-06.md`。
