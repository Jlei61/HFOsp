...
**Step 6 cohort 结果（2026-05-10）**：n=35 stable_k=2 subject（PR-6 main 23 ∪ rank_displacement v14 sensitivity n=35 去重）跑完。**整体稳健**：strong 20 / moderate 13 / weak 2 / fail 0；cohort 中位数 `template_spearman` = 0.922、`endpoint_position_recall` = 0.833（baseline 0.30）、`cluster_assignment_purity` = 1.000、swap_class concordant 比例 = 24/35 = 68.6%。三个 acceptance 阈值（spearman > 0.5、recall > 0.5、concordant > 50%）全过 → **PR-6 主线 H1 cohort NULL 与 §8 dual-tier swap_class 不是 PR-2 + PR-6 same-events double-dipping artifact**，在论文记录窗口内稳健。strong tier 中 12/20 (60%) 是 §8 swap_class = none，说明 hold-out 稳健性**不依赖**于 swap geometry —— 与 H1 cohort NULL 同向。weak 2 subject (`yuquan_litengsheng`、`yuquan_zhaochenxi`) 走 case-series；其中 `zhaochenxi` 的 §8 strict 标签在 hold-out 下被推翻为 none，提示其 swap geometry 是 single-fold pattern。swap_class concordance 是四量中最脆的（strong tier 100%、moderate tier 31%、weak tier 0%），反映 variable-k max-null 在 N/2 events 功率边界 —— 不冲击 §8 既有"mechanism sanity tier"判读。**Scope caveat**：本 Step 只覆盖论文记录窗口内（Yuquan 24h / Epilepsiae 数日）的 within-recording stationarity，**不是**对 H_drift 的全盘否定，week-month timescale drift 仍在视野外。详见 `docs/archive/topic1/pr6_template_anchoring/pr6_step6_held_out_template_results_2026-05-10.md`。
