...
4. **高 k 复核**（P1）：不阻塞，可并行
5. **§7.6 / §7.7 可选方向**（P2/P3）：PR-4C P0 已 CLOSED 后，可作为 Topic 1 后续的接续探索（§7.6 提升为 rate-by-template 信号的正式 Topic 1 × Topic 2 桥接；§7.7 仍是 exploratory 子集）
...
> 性质：可选 / 后续；PR-4C P0 已 CLOSED（2026-04-19），本方向已具备启动条件。
...
2. 把每个窗口里的事件按 PR-4D 的固定模板分解，只看 per-template event rate 在 baseline / pre_ictal / post_ictal 之间的差异
...
**判定边界**：这条线**不**回答"模板内部几何是否变形"——那由 PR-4C 主分析回答（PR-4C P0 已封板：geometry stable across baseline/pre/post）。这条线只回答"哪些模板更常被招募"。两条线的结论已并存（geometry stable + recruitment shifted），不应混写。
