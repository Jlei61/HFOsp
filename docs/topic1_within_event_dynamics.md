...
- **空间信息增益（2026-08-27）**：在双向记录块交叉验证中，真实三维事件方向只进入训练折的模板发现，留出事件仍仅按招募秩次分配模板。25 名可评估患者中，方向得分中位数由仅时序的 0.506 提高到时序--空间联合的 0.568，21/25 名患者提高；中位配对增益为 0.028（95% CI 0.001–0.063；单侧配对 Wilcoxon p=9.08e-4）。该结果支持三维电极空间信息提高患者内跨记录块的方向一致性，不是未见患者泛化或传播机制证据。Artifact：`results/interictal_propagation_masked/spatial_information_gain/spatial_information_gain_summary.json`。
...
- **PR-7 Template Antagonistic Temporal Pairing（已验收 2026-04-30，phantom-rank 修过版 2026-05-22 验证）**：检验 forward/reverse template 的时间耦合签名。**核心结论 = 几何上相关，已测试时间尺度上未见 mark dependence**：H1 主检验三条 metric 全部 NULL——event-level Δt ∈ [10s, 30s] opposite-template excess（orig N2 主 null Wilcoxon p=0.844；**mask h1_primary n=8 Wilcoxon p=1.000 / sign p=1.000 / median(10s)=−0.037 / median(30s)=−0.020 → 仍 NULL stays NULL**）；N3 robustness 一致 NULL；N2 window sweep {10/30/60 min} 三个尺度全部 NULL。Step 3.5 burst diagnostic 在无 ISI 阈值 same-label run 定义下未见 persistence（orig cohort run_length_lift median=0.977；mask 1.006，都 ≈ 1）。**P3 cohort-level TOST equivalence framework-locked gate**：on orig P3 cohort (n=6) 用 masked features 重跑，**verdict = INCONCLUSIVE 完全保持 (4/4 flag 与 orig 一致)；framework-level revision 未触发**，`docs/paper1_framework_sba.md` v1.1.2 不修订；broader masked cohort (n=8 或 n=30) verdict 形式上滑到 NULL，但这是 cohort × power × 5b fwd/rev 翻动互相作用而非 phantom-rank 在 P3 statistic 上的直接效应。**精确 framing**：在已测试的 event-level fixed-window + lag-1 + run-based 三类 metric 上数据 **compatible with mark-independent sampling**（最简洁描述，**不等于证明独立**）。PR-6 已建立的 fwd/rev 几何相关性**保留**；bouncing-back / 短时接力版本 Ping-Pong 撤回。**未测**：alternative burst definitions、rate-state / seizure-proximity switching、form (4) latent-state coupling、history-dependent regression。详见 `docs/archive/topic1/pr7_template_pairing/pr7_template_pairing_results_2026-04-29.md` §17 + `docs/archive/topic0/lagpat_phantom_rank/step5g_pr7_results_2026-05-21.md`。
...
> **逐事件「入口」补注（2026-06-08 真实数据探索，归档级，主结论不动）**：上面的 identity bias 是**跨事件平均**层面的话——平均模板的排序大半由固定通道身份决定。换到**逐事件**层面看「每次事件最先亮起来的那个通道」，它并**不**钉死在一个点上：95 条稳定模板里 82 条的最早通道铺在不止一个通道上（最常出现的入口通道中位只占 37% 的事件）。所以「两个固定端点反复点火」这个图景应松绑成「**事件反复进入一条有抖动的中观刻板传播通路**」——平均模板及其两端仍是这条通路的稳定锚点，但**不是逐事件的固定开关**。这与本节 identity bias **不矛盾**：平均排序被通道身份主导 ≠ 每次事件都从同一通道起步；模板本就是统计性的（簇内一致性 τ~0.25、远不到 1），入口恰是变异最大的位置。早端是一小撮通道轮流领先（前 3 名入口通道累计覆盖中位 74% 的事件），且 k=2 两条模板从大致相反的小入口群进入（两模板入口频率向量相关中位 −0.49）。比这更强的「这一小撮入口比单一带抖动通路本身更分散、构成一个独立入口结构」**未被证实**（取决于零假设选择、且与聚类松散度/电极通道数混淆）。详见 `docs/archive/topic4/sef_hfo/entry_dispersion_vs_single_template_2026-06-08.md`。此处「端点」= 逐事件最早到达通道，**与 §7.10 PR-6 从平均 `template_rank` 取的 source/sink 端点锚定是不同对象，本探索不触及 PR-6 结果**。
...
- 真实三维方向信息提高留出记录块的模板方向一致性（n=25；21/25 提高；median Δ=0.028，95% CI 0.001–0.063）
...
**Step 6 + supplementary 系列 verdict 表（全部细节归档；本主 doc 只摘要）**：

| 子项 | Verdict | 一句话结果 | 详细归档 |
|---|---|---|---|
| **Step 6 — Held-out time validation (2026-05-10)** | Robustness/sensitivity tier，不开新 cohort claim | n=35 stable_k=2 走完。Tier 分布 strong 20 / moderate 13 / weak 2 / fail 0；cohort 中位 `template_spearman`=0.922、`endpoint_position_recall`=0.833、swap_class concordant 24/35=68.6%（masked-features 重跑 2026-05-21 升至 0.82）。Single-subject (548) post-hoc null caveat 指出 spearman 增益 +0.17、concordance 增益 +33 pp，三量里 concordance 最 discriminative。**仅证明 endpoint instability 不解释 H1 NULL**，不证明 SOZ 标签 / lagPat 通道选择没问题。 | `pr6_step6_held_out_template_{plan,results}_2026-05-10.md` |
| **Supplementary 1 — First-rank entropy (2026-05-10)** | Topic 4 preflight，descriptive only，不进 PR-6 主线 | n=19/35（partial-participation 16 个 + 1 个 PR-2 labels 长度不匹配走 excluded）。`Δ_obs_subject` 19/19 一致 < 0（median −0.123，sign test p=1.9e-6），三组 swap_class 无差异（Kruskal-Wallis p=0.65）。**iHGE rank vector 是 roof-shape**（中段 H_p_norm≈1 满熵，端点 0.6–0.9 较确定），**否决** Liou-Abbott noise-driven confluence-point。重大 caveat：drop_rate cohort median 0.98。方法学意义：endpoint=top-3∪bottom-3 在 event-wise 稳定性上被独立支持。 | `pr6_supplementary_rank_entropy_{plan,results}_2026-05-10.md` |
| **Continuous-version rank displacement (2026-05-06, v14)** | Supplementary，不开 cohort gate | Cohort=35 stable_k=2（PR-6 valid_mask 30 + PR-2 sentinel fallback 5）。F_norm median 0.800，Kendall τ median −0.203，Spearman ρ(F_norm,τ)=−0.916 p=1.2e-14（两量共线，τ-track 冗余）。23/35 上 2/3 dashed reference；reproduced cohort 6→11（含 `epi_620, epi_1125, yuq_zhangjiaqi/wangyiyang/zhaochenxi`）。Paper deliverable=单张 cohort_displacement_heatmap（无 SOZ outlines / 无 τ track / 无 SOZ contribution_excess track）。 | `pr6_supplementary_rank_displacement_{plan,results}_2026-05-06.md` |
| **Variable-k swap classifier §8 (2026-05-07, v3 dual-tier)** | Mechanism-sanity，不开新 cohort claim | Family-wise max-null `T_obs = max_k swap_score(k)`，1000 perm。Decision rule：strict (T≥0.5 AND p_fw<0.05) n=10/35；candidate (T≥0.5 AND 0.05≤p<0.20) n=8/35；none n=17/35。strict 10 中 8 个落 PR-2.5 fwd/rev-reproduced。User motivation "2 节点 swap" 在 strict tier 无显著支持，在 candidate tier 有 2 例 exploratory (`epi_1125, epi_384` dk=2)。**PR-2.5 fwd/rev 不是 ground truth**。 | archive `pr6_supplementary_rank_displacement_results_2026-05-06.md` §8 + `cohort_displacement_heatmap.{png,pdf}` markers |
| **Swap × Clinical SOZ §9 set-relationship (2026-05-08)** | Topic 1 论文第一部分空间收束 | Primary strict∩informative n=5: sign test p=0.500, median enrichment +0.042, bootstrap CI [−0.071, +0.098] **NULL**（与 PR-6 H1 hard-coded k=3 Wilcoxon p=0.19 NULL 一致）。Sensitivity candidate∩informative n=5: 4/5 positive, median +0.127。Typology: `E⊊S`=0；`S⊊E`=4；多数 `partial`。方向上 swap 倾向"覆盖且包含"clinical SOZ。**Channel-selection circular caveat**: lagPat 已对 SOZ 富集，量化的是 high-HI 区内 swap 是否进一步富集 SOZ，不是全脑关系。 | `pr6_supplementary_swap_clinical_soz_plan_2026-05-08.md` + rank_displacement_results §9 |
