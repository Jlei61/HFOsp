...
### 7.8 PR-5：Template Recruitment Around Seizures（当前 P0，已立项 / 待执行）
...
1. **PR-5-A（Falsification gate）**：peri-ictal 事件能否被全局稳定模板库以与 baseline 同质的 best-template Spearman r、reconstruction error 与 assignment gap 解释？
   ...
   - 直接复用 PR-4C P0 修复后的 `compute_seizure_proximity_coupling` 输出 + 既有 `T_global = build_cluster_templates(...)`
...
- 测试：`tests/test_interictal_propagation.py` 增加 8 项（archive §5.3 列出，含 dual-dominant 行为测试与 same-event-pool 测试），36→44 项全绿
