#!/usr/bin/env python3
"""Wait coarsely for Stage-AL, then aggregate and analyze once."""
...
def main() -> None:
    ...
    while True:
        if controller.exists():
            ...
            if payload.get("status") != "REV12ND_NODE_WORKER_QUEUE_COMPLETE":
                raise RuntimeError("Stage-AL controller ended without completion")
            if payload.get("expected_git_commit") != expected:
                raise RuntimeError("Stage-AL controller commit changed")
            break
        time.sleep(interval)
    subprocess.run([
    ...
    subprocess.run([
        str(PYTHON), str(ROOT / "scripts/analyze_topic4_rev12_dual_node_local_bridge.py"),
        "--config", str(config_path), "--artifact-root", str(artifact_root),
    ], cwd=ROOT, check=True)
