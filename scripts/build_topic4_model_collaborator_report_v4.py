...
OUT=ROOT/'results/topic4_sef_hfo/model_collaborator_report_v4_2026-09-08'
...
PDF=OUT/'model_collaborator_report_v4.pdf'
...
def start(title,wide=False,subtitle=None):
    ...
    para('双核空间 SNN｜合作者讨论稿 · v4',size=8,color=GRAY,space=0)
...
para('合作者讨论稿 · 第 4 版 · 2026-09-08',10,color=GRAY,space=18)
...
table([['阅读顺序','本节要回答什么'],['模型总览、定义与优化（第 2–5 页）','双核在哪里进入方程？哪些参数可调？训练的样本究竟是什么？'],['患者对照与节律（第 6–11 页）','分布哪些部分相似、哪些不相似？为什么进一步审计波形和核内活动？'],['SNN 干预与 Z（第 12–16 页）','四条件对照，尤其 C 的高态进入／退出，及自主 Z 方案。'],['空间 rate 与分岔（第 17–21 页）','近似保留什么？什么数学对象失稳？状态如何切换？'],['讨论与来源（第 22–23 页）','哪些结论可用，关键缺口在哪里，图由哪些已有实验产生？']],widths=[160,W-244],size=9.3)
para('本版在模型说明开头新增 Figure 4A 结构图，衔接局部 E/I、Z/M 和空间电极读出；保留前版的原始观测量对照。已有图保留数据及坐标；单参数图的错误 GABA 面积图注依据代码与脉冲核验更正。本报告不新增网络搜索，不替换正式 Fig. 5。',9,color=GRAY)

start('模型总览｜从局部 E/I 回路到患者电极读出')
...
para('局部回路怎样产生和限制活动｜E→E 递归兴奋放大局部放电，E→I 招募抑制细胞，I→E 反馈约束兴奋性活动。红色椭圆表示 EE 连接的方向偏好，蓝色范围表示局部抑制作用示意；它们不是两个 core 的轮廓。实际网络也包含图中未展开的 I→I 通路，后面的 C 对照正说明其作用不能忽略。',10.2)
para('局部活动怎样被电极观察｜右侧以 −10 至 10 mm 展示二维网络和电极几何。触点周围的淡绿色区域表示高斯空间采样范围（σ＝0.25 mm，示意 95% 权重半径约 0.61 mm），模型神经元活动经该读出形成接触点放电密度包络。虚线连接的是代表性局部回路与空间邻域，不表示这里已经识别出病理起源。',10.2)
para('Z 与 M 怎样对应方程｜蓝色 z 表示有效抑制资源／效能，紫色 m 表示由 E spikes 积累的适应变量。在现有实现中，Z 乘 E-target GABA 电流，M 提供 E 细胞的适应负反馈。Data-driven 结果未开启自主 Z/M；四条件 raster 中 C 使用人工 z(t)，自主 Z/M 仍是下一步实验。',10.2)
para('双核在哪里｜该结构图沿用已接受的 Figure 4A 底图，没有叠加当前候选的双核阈值区域。双核由 E 细胞阈值场定义；本报告的实际中心、范围规则和连接参数见后面的“模型定义 2”。下一页先给出单细胞及突触的实际更新方程。',10.2)
...
para('图编号为本报告内部编号。原图均来自已有计算输出，按题意重新选择与排列，未调整点位、坐标或结果。公式及其余图保留矢量；图 3 的源 PDF 中文嵌字渲染失败，改用同次绘图的原始高分辨率 PNG。图 4 仅撤去错误的末行积分剂量图注，具体更正与单脉冲复算另存。新增第 14–16 页收录完整四条件图、C 的细时间尺度判读和自主 Z/M 的下一里程碑。',10)
