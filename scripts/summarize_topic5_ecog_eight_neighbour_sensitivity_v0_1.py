#!/usr/bin/env python3
"""Summarize the eight-neighbour true-vs-isomorphic-wrong-grid sensitivity."""
...
def main() -> None:
    ...
    for subject in ("958", "1084"):
        for path in sorted((ROOT / "training_eight_neighbour" / subject).glob("*/summary.json")):
            data = json.loads(path.read_text())
            graph_id = str(data["graph_id"])
            rows.append({
                "subject": subject, "family": data["family"], "graph_id": graph_id,
    ...
    if len(rows) != 192:
        raise RuntimeError(f"need 192 eight-neighbour units, found {len(rows)}")
    patient = []
    graph_rows = []
    for subject in ("958", "1084"):
        true = sorted([row for row in rows if row["subject"] == subject and row["family"] == "TRUE_GRID"], key=lambda x: x["seed_index"])
        true_values = np.asarray([row["test_contact_nll"] for row in true])
        controls = []
        for graph_index in range(31):
            matched = sorted([
                row for row in rows if row["subject"] == subject
                and row["family"] == "WRONG_GRID" and row["graph_index"] == graph_index
            ], key=lambda x: x["seed_index"])
            values = np.asarray([row["test_contact_nll"] for row in matched])
            controls.append(float(np.median(values)))
            graph_rows.append({
                "subject": subject, "graph_index": graph_index,
    ...
    for path, values in (
        (output / "EIGHT_NEIGHBOUR_GRAPH_EFFECTS.csv", graph_rows),
        (output / "EIGHT_NEIGHBOUR_PATIENT_RESULTS.csv", patient),
    ):
        with path.open("w", newline="") as stream:
            writer = csv.DictWriter(stream, fieldnames=list(values[0]))
            writer.writeheader(); writer.writerows(values)
    payload = {"schema": "topic5_ecog_eight_neighbour_summary_v0.1", "complete": True, "n_units": 192, "patient_results": patient}
    (output / "EIGHT_NEIGHBOUR_SUMMARY.json").write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n")
