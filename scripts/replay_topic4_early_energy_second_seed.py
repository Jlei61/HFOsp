#!/usr/bin/env python3
"""One same-noise15s second-seed measurement replay with full0.1ms spatial spike counts."""
import os
...
OUT=WINDOW/'early_energy_high_resolution_replay_seed9108402'
...
def main():
    ...
    if not plan_path.exists():
        ...
        core.write(plan_path,dict(purpose='Resolve measured temporal aliasing of1ms native field power.',
            maximum_measurement_replays=1,horizon_s=15.,same_existing_condition='e0_t0_s9108402',
            eta_M=.005,tau_M_s=1.,seed=9108402,field_recording_ms=.1,
            new_biological_conditions=0,new_F_samples=0,original_M40_endpoints_unchanged=True,
            reason_for_additional_recording='Second existing noise seed now has verified recurrence, but1ms native energy remains subject to the aliasing demonstrated in seed1. Apply the same observation audit without changing its trajectory.',
            baseline_s=[1.,11.74],target_s=[13.74,14.74],device=1))
    ...
    source_job=core.read(ORIGINAL/'jobs/e0_t0_s9108402.json')
    job={**source_job,'name':'native_readout_0p1ms','horizon_s':15.}
    ...
    field=np.lib.format.open_memmap(field_path,mode='r+' if resumed else 'w+',dtype=np.uint16,shape=(150000,400))
    population=np.lib.format.open_memmap(population_path,mode='r+' if resumed else 'w+',dtype=np.uint16,shape=(150000,2))
    def simulate(p,net,*args,**kwargs):
        ...
        def observe(tm,spk):
            k=round(tm/.1);assert 0<=k<150000
    ...
    result=core.worker(job);assert result['end_s']==15.
    core.OUT=ORIGINAL
    import plot_topic4_m_parameter_modes as plot
    a=plot.load(OUT/'runs/native_readout_0p1ms',end_step=150000)
    # This source closes 10-s blocks; read the committed20s block and compare
    # only the matching15s observation prefix, without altering either source.
    b=plot.load(WINDOW/'early_z_refill_branches/runs/early_z_refill_s9108402',end_step=200000)
    keys=['time_ms','spikes_1ms','regions_1ms','field_1ms','raster','slow_time_ms',
          'Z','M','currents','lfp_time_ms','lfp_raw','inputs']
    for key in keys:
        assert len(b[key])>=len(a[key]),key
        assert np.array_equal(a[key],b[key][:len(a[key])]),key
    assert np.array_equal(field.reshape(15000,10,400).sum(1),a['field_1ms'])
    assert np.array_equal(population.reshape(15000,10,2).sum(1),a['spikes_1ms'])
    ...
    core.write(OUT/'qa.json',dict(status='PASS',observations_bitwise_equal_to_existing_trajectory=keys,
        ...
        true_dt_ms=.1,recording_end_s=15.,source=str(WINDOW/'early_z_refill_branches/runs/early_z_refill_s9108402'),
        reference_closed_block_end_s=20.,compared_observation_prefix_s=[0.,15.],
        measurement_replay_not_new_trial=True,new_F_samples=0,scientific_trial_followup_not_shortened=True))
    print(json.dumps(dict(status='COMPLETE_MEASUREMENT_REPLAY_PASS',end_s=15.,new_samples=0)))
