"""Run one network's paired sham and uniform forced-source route map."""
...
def _event_diagnostics(spikes, *, cmrun, detect_events, detector, trigger_ms,
                       latency_ms):
    ...
    return active, None if triggered is None else {
...
def main():
    ...
    if args.seed not in set(map(int, config["network_seeds"])):
        parser.error("seed is outside the frozen D4 network set")
    for record in config["inputs"].values():
        if _sha256(ROOT / record["path"]) != record["sha256"]:
            raise RuntimeError(f"input hash changed: {record['path']}")
    output_root = ROOT / config["output_root"]
    manifest_path = output_root / "source_grid_manifest.json"
    manifest = json.loads(manifest_path.read_text())
    if (manifest.get("status") != "REV10D4_UNIFORM_SOURCE_GRID_FROZEN"
            or manifest.get("config", {}).get("sha256") != _sha256(config_path)):
        raise RuntimeError("D4 source manifest is stale")

    provenance = _runtime_provenance(args.expected_commit)
    provenance["systemd_unit"] = os.environ.get("REV10D4_SYSTEMD_UNIT")
    ...
    if (provenance["runtime_modules_dirty"] or config_dirty
        ...
        raise RuntimeError("D4 worker modules or config are not frozen")

    stem = f"uniform_source_seed_{args.seed}"
    ...
    if (len(anchor_matches) != 1 or anchor_matches[0]["field_sha256"]
        ...
        raise RuntimeError("D4 Node anchor changed")
    ...
    if detector != float(detector_audit["common_detector"]["central_threshold"]):
        raise RuntimeError("D4 common detector changed")
    ...
    node = _candidate_node(
        anchor, positions, n_total=n_e + n_i, stage=stage,
        config=anchor_config,
    )
    if not np.isclose(node["h"].sum(), float(stage["N_core_manual"]), atol=1e-8):
        raise RuntimeError("D4 Node field budget changed")
    ...
    montage = VirtualMontage(
        contact_xy, contact_names.tolist(), provenance="rev10_d4_readout_only",
    )
    valid = cmrun.valid_mask(montage, positions, engine["L"], params.Rr)
    if not np.all(valid):
        raise RuntimeError("D4 requires all frozen contacts to be readable")
    ...
    if not np.array_equal(target_names, contact_names):
        raise RuntimeError("D4 target and contact order differ")
    ...
    packet_fraction = float(manifest["source_grid"]["packet_fraction_of_E"])
    packet_n = max(1, int(round(packet_fraction * n_e)))
    ...
    for source in manifest["source_grid"]["sources"]:
        ...
        if response_dt != envelope_dt:
            raise RuntimeError("D4 forced/sham envelope dt changed")
        ...
        clean = bool(
            returned and joint and not ood
    ...
    _atomic_npz(
        ...
        source_xy_mm=np.asarray([row["source_xy_mm"] for row in rows], np.float32),
        actual_center_xy_mm=np.asarray([
            row["actual_center_xy_mm"] for row in rows
        ], np.float32),
        ...
        clean=np.asarray([row["clean_route_response"] for row in rows], bool),
        mean_source_h=np.asarray([row["mean_source_h"] for row in rows], np.float32),
        sham_active_fraction=np.asarray(sham_active, np.float32),
    )
    payload = {
        "status": "REV10D4_UNIFORM_SOURCE_WORKER_COMPLETE",
