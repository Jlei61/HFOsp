...
def build(s,r,T,g):
    ...
    _,_,m=o.evaluate(y,r,np.zeros_like(r),True)
    from analytic_gains import gains
    u,v,h=gains(s,m['mu'],m['ve'],m['vi'])
...
def spectrum(path,nev=12):
    ...
    dest=OUT/'analytic_antiperiodic'/path.parent.name;dest.mkdir(parents=True,exist_ok=True)
