"""Exponential change of variables v(t)=exp(alpha*t) q(t).
Delay edge weights must already include exp(-alpha*delay_ms).
All current-state differential equations subtract alpha*q.
"""
...
@njit(cache=True)
def shifted_monodromy4(x,dt,di,dj,delays,val,u,var,tr,rise,decay,D,alpha):
    ...
    for step in range((len(u)-1)//2):
        ...
        a=(-r+var[k]@r+u[k]*mus[0])/tr-alpha*r;b=(r-h)/rise-alpha*h;cc=(h-c)/decay-alpha*c
        r2=r+.5*dt*a;h2=h+.5*dt*b;c2=c+.5*dt*cc
        a2=(-r2+var[k+1]@r2+u[k+1]*mus[1])/tr-alpha*r2;b2=(r2-h2)/rise-alpha*h2;cc2=(h2-c2)/decay-alpha*c2
        r3=r+.5*dt*a2;h3=h+.5*dt*b2;c3=c+.5*dt*cc2
        a3=(-r3+var[k+1]@r3+u[k+1]*mus[1])/tr-alpha*r3;b3=(r3-h3)/rise-alpha*h3;cc3=(h3-c3)/decay-alpha*c3
        r4=r+dt*a3;h4=h+dt*b3;c4=c+dt*cc3
        a4=(-r4+var[k+2]@r4+u[k+2]*mus[2])/tr-alpha*r4;b4=(r4-h4)/rise-alpha*h4;cc4=(h4-c4)/decay-alpha*c4
