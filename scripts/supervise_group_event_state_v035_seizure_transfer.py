#!/usr/bin/env python3
"""Persistent W5 queue; every unit is bound to a frozen W3 trajectory."""
...
def jobs() -> list[dict]:
    ...
    for subject in SUBJECTS:
        for decoder_seed, state_seed in enumerate(LOCKED_SEEDS[:3]):
            ...
            card = OUTPUT_ROOT / "seizure_transfer" / subject / f"decoder_seed{decoder_seed}_state_seed{state_seed}" / "card.json"
...
def main() -> None:
    ap = argparse.ArgumentParser(description=__doc__); ap.add_argument("--workers", type=int, default=4); args = ap.parse_args()
    root = OUTPUT_ROOT / "seizure_supervisor"; logs = root / "logs"; logs.mkdir(parents=True, exist_ok=True)
    scope = OUTPUT_ROOT / "scope_manifest.json"; update_scope_manifest(scope, "W5", "RUNNING", [])
    ...
    while pending or running:
        ...
        for slot in range(args.workers):
            ...
            cmd = [str(PYTHON), str(ROOT / "scripts/run_group_event_state_v035_seizure_transfer.py"),
        ...
        atomic_json(root / "queue_state.json", {
            "format": "group_event_state_v0_3_5_seizure_queue_v1", "updated_epoch": time.time(),
    ...
    update_scope_manifest(scope, "W5", "COMPLETE" if len(complete) == len(jobs()) else "PARTIAL", evidence)
    atomic_json(root / "queue_done.json", {"format": "group_event_state_v0_3_5_seizure_done_v1",
