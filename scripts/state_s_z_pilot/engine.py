# Derived diagnostic engine; original validation engine left unchanged.
# Source SHA256: 8f6ed3d4a703f215e12e25b73ef330d0ca5feccc20a4045f5fd3d2ba5c568e68
# Additions: optional GABA jump control, compact spike callback, order-preserving serial scatter.
"""
...
def simulate_kick(p: Params, net, KICK_BOOST, slow=None, nu_signal_fn=None,
                  ...
                  input_observer=None, current_observer=None,
                  gaba_jump_scale_fn=None, spike_observer=None,
                  record_dense_spikes=True, fast_scatter=False, external_i_state=None):
    ...
    E_spk_bool = np.zeros((nsteps, NE), dtype=bool) if record_dense_spikes else None
    ...
    for t in range(nsteps):
        ...
        if E_spk_bool is not None:
            E_spk_bool[t] = spk[:NE]
        if spike_observer is not None:
            spike_observer(tm, spk)
        ...
        if spk.any():
            ...
            if fast_scatter and not ee_std_on:
                from src.topic4_serial_spike_scatter import scatter
                scatter(ring_sE, spE, a_indptr, a_dst, a_dly, a_w, _abs_step)
            elif spE.size:
            ...
            g_gain = 1.0 if gaba_jump_scale_fn is None else float(gaba_jump_scale_fn(tm))
            if fast_scatter:
                from src.topic4_serial_spike_scatter import scatter
                scatter(ring_sI, spI, g_indptr, g_dst, g_dly, g_w, _abs_step, g_gain)
            elif spI.size:
                st = g_indptr[spI]; cnt = g_indptr[spI + 1] - st; tot = int(cnt.sum())
                if tot:
                    idx = (np.arange(tot) - np.repeat(np.cumsum(cnt) - cnt, cnt)
                           + np.repeat(st, cnt))
                    np.add.at(ring_sI, ((_abs_step + g_dly[idx]) % M, g_dst[idx]), g_w[idx] if gaba_jump_scale_fn is None else g_w[idx]*g_gain)
