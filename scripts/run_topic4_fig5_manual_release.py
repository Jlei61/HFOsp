#!/usr/bin/env python3
"""Bounded Figure 5 experiment: unchanged manual carrier, one refill then native Z."""
...
def worker(job):
    ...
    cfg = MZSlowVarsConfig(use_z=True, use_m=False, tau_z=job['tau_z_ms'], I_th_EI=job['threshold'])
    ...
    prefix_qa = None
    old = np.load(PREVIOUS / 'trajectory.npz') if detail else None
    ...
    def observe(tm, spk):
        ...
        if detail:
            ...
            if seen == 6000:
                assert np.array_equal(raster[:seen], old['sample_spikes'][:seen])
                assert np.array_equal(rates[:seen, 0], old['rate_e_hz'][:seen])
                assert np.array_equal(rates[:seen, 1], old['rate_i_hz'][:seen])
                prefix_qa = '600 ms bitwise identical to previous native SNN'
        recent += ec
        if seen % 100 == 0:
            ...
            if high_ms >= 200 and not high_active:
                ...
                if detail and slow.restore_ms is None:
        ...
        if not detail and triggers and seen * dt >= triggers[0] + 500.:
    ...
    if detail:
        ...
        assert zerror < 1e-12
        if slow.restore_ms is not None:
    ...
    result = dict(status='COMPLETE', job=job, duration_ms=seen*dt, first_trigger_ms=triggers[0] if triggers else None,
                  ...
                  prefix_qa=prefix_qa, M_enabled=False, final_mean_Z=float(slow.z[:ne].mean()))
...
def controller():
    ...
    jobs = [dict(name='continuous_refill_release', seed=9108401, tau_z_ms=5000., threshold=THRESHOLD,
                 duration_ms=26000., detail=True)]
    for tau in (2500., 5000., 10000.):
        for th in (75., THRESHOLD, 120.):
            for seed in (9108401, 9108402, 9108403):
                if tau == 5000. and th == THRESHOLD and seed == 9108401: continue
                jobs.append(dict(name=f'tau{tau:g}_th{th:.4f}_seed{seed}', seed=seed, tau_z_ms=tau,
                                 threshold=th, duration_ms=24000., detail=False))
    protocol = dict(status='DEFINED_BEFORE_NEW_SIMULATIONS', jobs=jobs, max_workers=12,
                    ...
                    M='off', Z_equation='tau_Z dz_i/dt = 1[I_GABA_i < I_th] - z_i; E targets only',
                    intervention='Only representative: one 1-s refill after first high-state trigger +500ms, then release native Z; no reset, no second refill.',
                    transition='Full E 10-ms rate >=200Hz for 200 consecutive ms; operational high activity, not clinical seizure.',
                    sampling='3 noise seeds, one topology; event times are run-level observations.',
                    latency_horizon_ms=24000., tau_semantics='tau_Z changes both depletion and recovery; I_th is a current threshold, not depletion per event.',
                    ...
                    stop='Deliver this bounded batch and figures; no spatial perturbation, next search, official replacement or model freeze.')
