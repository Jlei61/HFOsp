...
OUT=ROOT/'results/topic4_sef_hfo/model_collaborator_report_v2_2026-09-08'
...
PDF=OUT/'model_collaborator_report_v2.pdf'
...
def start(title,wide=False,subtitle=None):
    ...
    para('双核空间 SNN｜合作者讨论稿 · v2',size=8,color=GRAY,space=0)
...
para('合作者讨论稿 · 第 2 版 · 2026-09-08',10,color=GRAY,space=18)
...
para('在独立机制实验中，降低 SNN 的 GABA 输入幅度确实增强放电与招募；修正后的空间 rate 模型则存在 Hopf 型失稳、自限／持续爆发转换和状态共存。尚未证明患者间期工作点、SNN 与 rate 模型共享同一临界点，也尚未接通活动积累—Z/M 演化—自主跨界这条链。',11)
table([['阅读顺序','本节要回答什么'],['模型定义与优化（第 2–4 页）','双核在哪里进入方程？哪些参数可调？训练的样本究竟是什么？'],['患者对照与节律（第 5–10 页）','分布哪些部分相似、哪些不相似？为什么进一步审计波形和核内活动？'],['SNN 干预（第 11–12 页）','相同网络减弱抑制后，真实 spike 与招募如何改变？'],['空间 rate 与分岔（第 13–17 页）','近似保留什么？什么数学对象失稳？状态如何切换？'],['讨论与来源（第 18–19 页）','哪些结论可用，关键缺口在哪里，图由哪些已有实验产生？']],widths=[160,W-244],size=9.3)
...
table([['科学问题','已有证据','尚不能写成'],['同一网络可否产生不同事件？','固定网络连续噪声产生多事件；C1 有 TA/TB 分类事件','两个标签已证明两个起源 core 或两条完整传播路径'],['数据驱动是否有效？','部分参与、rank、空间分布相近；确认平均 loss 降低','患者所有方差恢复、独立泛化成功或全局最优'],['为什么审计节律？','core 更快更规则；读出事件 CV 可掩盖底层规则性','患者具有同样核内同步或同一生理振子'],['抑制能否改变放电状态？','配对 SNN 减抑制后爆发与招募增强；rate 有持续振荡','该 SNN 已证明与 rate 同一临界点或已复现临床发作'],['分岔是否存在？','corrected rate：Hopf 型失稳、fold、状态共存','已确定 Hopf 超／亚临界，或发现单独全局化分岔'],['积累能否解释跨越？','当前 q 为外部控制，可描述参数敏感性','已经完成放电积累驱动 Z/M 自主跨界']],widths=[110,190,W-384],size=9.4)
...
para('患者匹配线应先在固定观察器下同时保留逐事件空间组织、局部波形与事件间隔诊断，确认候选改善不由检测筛选产生，再决定是否把序列统计纳入新目标。机制线应验证相同条件的 SNN／rate 响应与持续态对应，并延续周期分支或定位共存吸引域。只有这些环节成立，才开启活动依赖 Z/M，检查真实轨道是穿越失稳边界还是受扰动换吸引域。',10.5)
...
para('交付包含 PDF、逐图原始路径与快照、正文源、公式矢量资产、渲染预览及版面检查记录。旧版独立保留。Agent 完成版面与来源自查；作者／合作者目视审阅仍待进行。',9.6)
