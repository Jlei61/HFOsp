#!/usr/bin/env python3
"""Wait coarsely for Stage-AB and aggregate the omitted Pareto field once."""
...
def main() -> None:
    ...
    while True:
        if controller.exists():
            ...
            if payload.get("status") != "REV12ND_NODE_WORKER_QUEUE_COMPLETE":
                raise RuntimeError("Stage-AB controller did not complete")
            if payload.get("expected_git_commit") != expected:
                raise RuntimeError("Stage-AB controller commit changed")
            break
        time.sleep(interval)
    subprocess.run([
    ...
    complete = {
        "status": "REV12ND_OMITTED_PARETO_RECOVERY_COMPLETE",
        "git_commit": expected,
    ...
    output = output_root / "status" / "stage_ab_complete.json"
