"""Launch SA6 candidate-seed workers with bounded concurrency and coarse waits."""
...
FREEZER = ROOT / "scripts/freeze_topic4_rev10_sa_dual_shaft_candidates.py"
WORKER = ROOT / "scripts/run_topic4_rev10_sa_dual_shaft_worker.py"
AGGREGATOR = ROOT / "scripts/aggregate_topic4_rev10_sa_dual_shaft_capacity.py"
...
def main():
    ...
    parser.add_argument("--unit-prefix", default="topic4-rev10-sa-capacity")
    ...
    maximum = int(args.max_concurrent or min(
        len(candidates) * len(seeds), config["execution"]["max_workers_sa6"],
    ))
    ...
    while pending or active:
        ...
        while pending and len(active) < maximum:
            ...
            command = [
                ...
                str(job["log"]),
                f"SA6 {job['candidate']} seed={job['seed']}", commit[:8],
    ...
    refresh()
    if failed:
        raise RuntimeError(f"{len(failed)} SA6 worker(s) failed")
    subprocess.run([
    ...
    subprocess.run([
        ...
        f"SA6 dual-shaft capacity completed: {len(completed)}/{len(jobs)}",
    ], check=False)
    print(json.dumps({
        "status": "SA6_DUAL_SHAFT_CAPACITY_LAUNCH_COMPLETE",
