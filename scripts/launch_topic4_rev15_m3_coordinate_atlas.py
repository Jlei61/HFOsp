#!/usr/bin/env python3
"""Launch the rev15 M3 coordinate-atlas controller with systemd and nohup."""
...
MONITOR = ROOT / "scripts/monitor_topic4_rev15_m3_coordinate_atlas.py"
NUMERIC_ENV = (
...
def _command(
    *, config_path: Path, expected_commit: str, artifact_root: Path,
    ...
    prefix = monitor.base._validate_unit_prefix(unit_prefix)
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not 1 <= args.worker_cap <= int(config["resources"]["maximum_workers"]):
        raise ValueError("worker cap exceeds the frozen rev15 limit")
    ...
    unit, command = _command(
        ...
        artifact_root=args.artifact_root.resolve(), unit_prefix=args.unit_prefix,
        worker_cap=args.worker_cap, log_path=log_path,
    )
    payload = {
        "status": "REV15_M3_ATLAS_CONTROLLER_LAUNCHED" if args.execute
        else "REV15_M3_ATLAS_CONTROLLER_DRY_RUN",
        "unit": unit,
