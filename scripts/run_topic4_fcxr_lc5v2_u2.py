...
from src.topic4_fcxr_lc5_finite_episode import classify_u2_excursion  # noqa: E402
...
ARM = OUT / "u2a_tau8_gamma025"
WORK = OUT / ".u2a_tau8_gamma025.work"
MECHANISM_FILES = (
...
def _load_contract():
    ...
    with np.load(prelock["calibration_arrays"], allow_pickle=False) as z:
        p0 = np.asarray(z[prelock["p0_key"]], float)
        u_onset = np.asarray(z[prelock["u_onset_key"]], float)
    imax = float(prelock["Imax_by_gamma"][str(GAMMA)])
...
def _stage_lock():
    ...
    f = (OUT / ".u2a_gamma025.lock").open("w")
    try:
        fcntl.flock(f.fileno(), fcntl.LOCK_EX | fcntl.LOCK_NB)
    except BlockingIOError as exc:
        f.close()
        raise SystemExit("LC5v2 gamma025 is already running") from exc
    return f
...
def _build_start(p0, u_onset, imax):
    ...
    cfg_dict.update(
        ...
        pump_a_load=float(json.loads((CAL / "candidate_prelock.json").read_text())["a_load"]),
        pump_tau_ms=TAU_MS,
...
def stage_gamma025():
    ...
    lock_file = _stage_lock()
    try:
        prelock, p0, u_onset, imax = _load_contract()
        if WORK.exists():
        ...
        started = time.time()
        _write_json(OUT / "RUNNING_U2A_GAMMA025.json", {
            "status": "RUNNING", "pid": os.getpid(), "started": GEO._now(),
            "tau_ms": TAU_MS, "gamma": GAMMA, "Imax": imax,
            "work_dir": str(WORK),
        })
        (OUT / "u2a_gamma025.pid").write_text(f"{os.getpid()}\n")
        _resource_row("U2A_GAMMA025_PREFLIGHT", baseline_swap)

        S, state, cfg_dict = _build_start(p0, u_onset, imax)
        ...
        for chunk in range(n_chunks):
            ...
            row = _resource_row(
                "U2A_GAMMA025_CHUNK", baseline_swap, chunk=chunk + 1,
        ...
        summary = {
            "status": "COMPLETE", "arm": "u2a_tau8_gamma025", "label": label,
        ...
        _write_json(OUT / "DONE_U2A_GAMMA025.json", {
            "status": "DONE", "bundle": str(ARM), "label": label, "finished": GEO._now(),
        })
        (OUT / "RUNNING_U2A_GAMMA025.json").unlink(missing_ok=True)
        shutil.rmtree(WORK)
        return summary
    except BaseException as exc:
        _write_json(OUT / "FAILED_U2A_GAMMA025.json", {
...
def main():
    ...
    parser.add_argument("--stage", choices=("control", "gamma025"), required=True)
    parser.add_argument("--confirm-run", action="store_true")
    args = parser.parse_args()
    if args.stage == "gamma025" and not args.confirm_run:
        raise SystemExit("the 40k gamma025 arm requires --confirm-run")
    result = stage_control() if args.stage == "control" else stage_gamma025()
