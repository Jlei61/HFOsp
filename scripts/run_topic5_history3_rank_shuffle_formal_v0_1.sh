...
ordered_root="${repo_root}/results/topic5_interictal_scaffold_reliability_history_necessity/history_runs_v0_1"
run_root="${repo_root}/results/topic5_interictal_scaffold_reliability_history_necessity/history3_rank_shuffle_runs_v0_1"
mkdir -p "${run_root}"
...
if (( exit_code == 0 )); then
  python "${repo_root}/scripts/summarize_topic5_history3_rank_shuffle_v0_1.py" \
    --ordered-root "${ordered_root}" \
    --shuffle-root "${run_root}" \
    --seeds "${seeds[@]}" \
