...
CONFIG=ROOT/'config/topic4_joint_xy_kernel_v3.json'
KERNEL=OLD_V1/'kernel_qualification/kernel_contract.json'
SOURCES=['scripts/run_topic4_joint_xy_replicated_search.py','src/topic4_xy_replicated_anchors.py','tests/test_topic4_xy_replicated_anchors.py','src/topic4_joint_xy_kernel_objective.py',
         'src/topic4_joint_xy_kernel.py','config/topic4_joint_xy_kernel_v3.json',
...
def main():
    ...
    plan=read(CONFIG)
    if read(OLD_V2/'optimizer_revision_batch_guard.json')['status']!='BATCH_DRAIN_COMPLETE_V2_RETIRED_FOR_OPTIMIZER_REVIEW':
        raise RuntimeError('previous live batch has not drained')
    lock={**read(OLD_V2/'objective_contract.json')['source_hashes'],**read(OLD_V2/'analysis_input_lock.json')['hashes']}
    lock.update({str(ROOT/p):sha(ROOT/p) for p in SOURCES});lock[str(KERNEL)]=sha(KERNEL)
    prior_stage=OLD_V2/'rounds/000'
    completed_ids=read(prior_stage/'race_nomination.json')['candidate_ids']
    prior_files=[prior_stage/f'combined_{cid}.json' for cid in completed_ids]
    for p in [OLD_V2/'baseline_scores.json',prior_stage/'analysis.json',prior_stage/'race_nomination.json']+prior_files:
        lock[str(p)]=sha(p)
    ...
    if not (OUT/'patient_calibration.json').exists():
        shutil.copyfile(OLD_V2/'patient_calibration.json',OUT/'patient_calibration.json')
    if sha(OUT/'patient_calibration.json')!=sha(OLD_V2/'patient_calibration.json'):raise RuntimeError('objective tolerance changed across optimizer-only revision')
    ...
    by_id={r['candidate_id']:r for r in read(OLD_V2/'baseline_scores.json')['candidates']}
    for p in prior_files:
        r=read(p)['candidates'][0];by_id[r['candidate_id']]=r
    old_scores=list(by_id.values())
    ...
    write(OUT/'replicated_bootstrap.json',{'source_round':str(prior_stage),'completed_candidates':completed_ids,
        'eligible_local_anchors':[r['candidate_id'] for r in pool if anchor_eligible(r,plan)],
    ...
    for number in range(1,plan['search']['automatic_rounds_before_capacity_review']+1):
        ...
        if path.exists():rows=read(path)['candidates']
            ...
            rows=v1.new_proposals(proposal_pool(pool,plan),pos,contract['master_seed'],number,plan,diagnosis)
            for r in rows:r['candidate_id']=r['candidate_id'].replace('joint_r','replicated_r')
            write(path,{'candidates':rows,'preceding_diagnosis':diagnosis,'seed_sequence':[contract['master_seed'],number,0],
                'local_anchor_candidates':[r['candidate_id'] for r in pool if anchor_eligible(r,plan)],
