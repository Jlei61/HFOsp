#!/usr/bin/env python3
"""Wait coarsely for Stage-AH, aggregate it once, then run its paired audit."""
...
def main() -> None:
    ...
    while True:
        if controller.exists():
            ...
            if payload.get("status") != "REV12ND_NODE_WORKER_QUEUE_COMPLETE":
                raise RuntimeError("Stage-AH controller ended without completion")
            if payload.get("expected_git_commit") != expected:
                raise RuntimeError("Stage-AH controller commit changed")
            break
        time.sleep(interval)
    subprocess.run([
    ...
    subprocess.run([
        str(PYTHON), str(ROOT / "scripts/analyze_topic4_rev12_signed_depth_mapping_audit.py"),
        "--config", str(config_path), "--artifact-root", str(artifact_root),
    ], cwd=ROOT, check=True)
    result_path = output_root / "analysis" / "signed_depth_mapping_result.json"
    result = json.loads(result_path.read_text())
    complete_path = output_root / "status" / "stage_ah_complete.json"
    complete_path.write_text(json.dumps({
        "status": "REV12ND_SIGNED_DEPTH_MAPPING_AUDIT_COMPLETE",
        "git_commit": expected,
