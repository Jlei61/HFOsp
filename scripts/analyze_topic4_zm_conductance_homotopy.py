...
def _validate_short_prefix(short_root: Path, long_root: Path):
    """Require the smoke trajectory to be an exact prefix of production.

    Dynamic runs deliberately keep their nominal equilibration interval in the
    saved trace.  Therefore no stitching or clock shift is allowed here.
    """
    ...
    np.testing.assert_array_equal(
        short["fine_core_rate_hz"], long["fine_core_rate_hz"][:1250]
    )
    np.testing.assert_array_equal(
        short["coarse_kymo_axial"], long["coarse_kymo_axial"][:, :100]
    )
    np.testing.assert_array_equal(
        short["lfp_raw_synaptic_proxy"],
        long["lfp_raw_synaptic_proxy"][:25000],
    )
...
def _load_long(short_root: Path):
    ...
    _validate_short_prefix(short_root, root)
