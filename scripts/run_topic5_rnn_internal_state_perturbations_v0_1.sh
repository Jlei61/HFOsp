...
log_root="${out_root}/logs/perturbation_cells"
mkdir -p "${log_root}"
...
PY
  xargs -P 16 -n 2 bash -c '
    ...
    OMP_NUM_THREADS=2 MKL_NUM_THREADS=2 OPENBLAS_NUM_THREADS=2 \
      "'"${python_bin}"'" \
      "'"${repo_root}"'/scripts/analyze_topic5_rnn_internal_state_perturbation_cell_v0_1.py" \
      --subject "${subject}" \
      --seed-dir "${seed_dir}" \
      --device cpu \
      --max-order-events 512 \
...
path = Path(
    ...
    "PERTURBATION_ANALYSIS_DONE.json"
)
