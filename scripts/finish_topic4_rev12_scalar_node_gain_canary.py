#!/usr/bin/env python3
"""Wait coarsely for Stage-AJ, then aggregate and analyze once."""
...
def main() -> None:
    ...
    while True:
        if controller.exists():
            ...
            if payload.get("status") != "REV12ND_NODE_WORKER_QUEUE_COMPLETE":
                raise RuntimeError("Stage-AJ controller ended without completion")
            if payload.get("expected_git_commit") != expected:
                raise RuntimeError("Stage-AJ controller commit changed")
            break
        time.sleep(interval)
    subprocess.run([
    ...
    subprocess.run([
        str(PYTHON), str(ROOT / "scripts/analyze_topic4_rev12_scalar_node_gain_canary.py"),
        "--config", str(config_path), "--artifact-root", str(artifact_root),
    ], cwd=ROOT, check=True)
    result_path = output_root / "analysis" / "scalar_node_gain_result.json"
    result = json.loads(result_path.read_text())
    complete_path = output_root / "status" / "stage_aj_complete.json"
    complete_path.write_text(json.dumps({
        "status": "REV12ND_SCALAR_NODE_GAIN_CANARY_COMPLETE",
        "git_commit": expected,
