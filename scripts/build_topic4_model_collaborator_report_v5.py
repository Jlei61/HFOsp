...
OUT=ROOT/'results/topic4_sef_hfo/model_collaborator_report_v5_2026-09-08'
...
PDF=OUT/'model_collaborator_report_v5.pdf'
...
def start(title,wide=False,subtitle=None):
    ...
    para('双核空间 SNN｜合作者讨论稿 · v5',size=8,color=GRAY,space=0)
...
para('合作者讨论稿 · 第 5 版 · 2026-09-08',10,color=GRAY,space=18)
...
para('在独立机制实验中，降低脉冲网络（SNN）的 GABA 输入幅度确实增强放电与招募；仅减弱兴奋性神经元接收的抑制电流时，还出现持续高率态并可在恢复抑制后退出；修正后的空间放电率模型则存在 Hopf 型失稳、自限／持续爆发转换和状态共存。尚未证明患者间期工作点、SNN 与放电率模型共享同一临界点，也尚未接通活动积累—Z/M 演化—自主跨界这条链。',11)
table([['阅读顺序','本节要回答什么'],['模型方法、差异与优化（第 2–9 页）','双核在哪里进入方程？哪些参数可调？训练的样本究竟是什么？'],['患者对照与节律（第 10–15 页）','分布哪些部分相似、哪些不相似？为什么进一步审计波形和核内活动？'],['SNN 干预与 Z（第 16–20 页）','四条件对照，尤其 C 的高态进入／退出，及自主 Z 方案。'],['空间放电率模型与分岔（第 21–25 页）','近似保留什么？什么数学对象失稳？状态如何切换？'],['讨论与来源（第 26–27 页）','哪些结论可用，关键缺口在哪里，图由哪些已有实验产生？']],widths=[160,W-244],size=9.3)
...
para('Z 与 M 怎样对应方程｜蓝色 z 表示有效抑制资源／效能，紫色 m 表示由兴奋性神经元动作电位积累的适应变量。在现有实现中，Z 调节兴奋性神经元接收的 GABA 电流，M 提供 E 细胞的适应负反馈。数据驱动拟合实验未开启自主 Z/M；四种抑制干预中的 C 使用人工 z(t)，自主 Z/M 仍是下一步实验。',10.2)
...
para('与本报告结果的差异｜图 8C 只把人工给定的 z(t) 乘到 E 神经元的 GABA 电流上，没有求解上面的 z 方程，也没有开启 m。人工恢复抑制后退出高态已经观察到；按活动自行恢复尚未验证。如果高态中的抑制电流一直超过 Ith，现有 z 方程会继续耗竭，需要检验适应等负反馈能否先降低活动，再允许 z 回升。',10.2,color=BLUE)
...
start('图 7｜逐神经元放电记录中，自限事件是否存在？',True)
figure('raster_base',315)
explanation('在讨论持续化之前，直接看完整网络是否具有可自行结束、可重复出现的放电事件。',
'固定 C1 网络、连接随机种子 6101、输入随机种子 9108401，模拟 6.5 s；OU 关闭，独立 Poisson 保留，q＝1。放电图使用 0.1 ms 分辨率的动作电位记录，固定抽样 60 核A E、60 核B E、120 核外 E、60 I；群体率用全体细胞、5 ms 时间段。',
'1–6.5 s 有 15 个描述性事件，E 峰 42.1–69.1 Hz，全部 E 细胞平均 6.23 Hz；74.5% 的时间段低于 1 Hz。均值包含爆发，并不是静息基线一直 6 Hz。核内细胞在展示样本中的比例较高也不能用来估计全场参与率。',
'同一连接网络和随机数种子下，已执行连续状态的 GABA 干预（图 8）。本图证明该协议下有自限事件，尚不是患者间期分布确认，也不能用它的 关闭 OU 事件间隔替代图 3 的 开启 OU 比较。',size=9)

start('图 8｜降低抑制幅度会把网络推向什么状态？',True)
figure('raster_jump',312)
explanation('在保留实际动作电位、突触电流和延迟历史的网络中，检验抑制反馈减弱能否提高招募，而不是只在放电率近似中看到强活动。',
'与图 7 同一网络和随机数种子。全局 GABA 突触输入增量的倍率 q：0–1 s 为 1，1–2 s 降至 0.5，2–3.5 s 保持，3.5–4.5 s 回升至 1。q 同时作用 I→E 和 I→I；不是 Z 的自主耗竭，也不是仅调 I→E。',
'低 q 保持期全部 E 细胞的平均放电率 64.97 Hz、峰 281.29 Hz；恢复后的 5.5–6.5 s 均值 3.48 Hz、峰 48.89 Hz。低 q 仍有 37.7% 的时间段低于 1 Hz，故应称更强反复爆发与招募，不能只凭峰值命名无间断 runaway。',
'已用同一参考网络的修正放电率模型分别求固定点、完整延迟稳定性和非线性轨道（图 9–11）。人工恢复 q 说明外部可逆干预，不证明模型能自主结束发作；下面的放电率模型临界值亦未被直接转移到 SNN。',size=9)

start('图 8（续）｜四种抑制干预：同一状态连续进入与退出')
figure('inhibition_cycle',590)
para('同一双核、同一初态，6.5 s 内指定增益 1→0.5→1，全程保留膜电位、突触状态和延迟历史。A：全部 GABA 突触输入增量；B：E/I 两群 GABA 电流；C：仅 E 的 GABA 电流；D：A 加回原 OU 背景。前三行保留相同 Poisson 输入。',9.3)
...
figure('raster_c',305)
explanation('检验与原 Z 一样仅调节兴奋性神经元接收的抑制，是否足以让完整空间 SNN 进入高活动态并在恢复抑制后退出。',
'E 的净突触输入为兴奋电流减 z(t)×抑制电流；I 所受抑制不缩放。低增益窗 2–3.5 s，恢复后窗 5.5–6.5 s；群体率用全体细胞。除既有 5 ms 图，本次从保存的 0.1 ms 放电率复算 1 ms 与区域率，没有新增网络仿真。',
'C 的低增益 E/I 均值为 454.87/575.61 Hz；恢复后为 4.56/11.96 Hz，80.5% 的 5 ms 时间段 中 E<1 Hz。高态 E 在 1 ms 时间段内的标准差 仅 1.24 Hz，核 A/B 均值约 462/461 Hz。因此支持高率态进入—退出，未显示 A/B 那样的大幅群体振荡；微小快波纹不能等同持续群体爆发。',
'已核验干预前 1 s 的三行的动作电位记录完全一致。B→C 只保留 I 自身的 GABA 抑制，改变了 I→I 反馈；这是作用范围敏感性的直接对照。后续以 C 的电流调节方式接入活动依赖 Z，不直接沿用全局 q 的放电率模型 Hopf 临界值。',size=9)
...
eq('eq_zm_scope',[
 r'$I_{{\rm net},i}^E=I_i^{E,E}-z_i I_i^{E,I}-\eta_m m_i,\qquad I_{{\rm net},i}^I=I_i^{I,E}-I_i^{I,I}$',
 r'$\tau_z\dot z_i={\bf 1}[I_i^{E,I}<I_{\rm th}^{E,I}]-z_i$',
 r'$\dot m_i=-m_i/\tau_{\rm adp}+\sum_k\delta(t-t_i^k)$'],height=102)
para('Z 乘在已滤波的 GABA 电流上，仅 E 的 Z 演化；M 是动作电位驱动的适应负反馈，当前 C 两者都未自主开启。已有 Z 规则在原始 GABA 电流持续高于阈值时满足 dz/dt＝−z/τz，因此只会进一步耗竭。C 低增益窗的兴奋性神经元接收的原始 GABA 电流群体均值约 1,630，基线约 45.8（模型驱动单位）；这提示高态时的恢复是关键缺口，但均值不能代替每个细胞的电流超过阈值的时间比例。',10)
table([['下一里程碑的顺序','具体检验与判断标准'],['1. 固定 C 的作用范围与参考网络','保留当前几何、连接、阈值和 Poisson；z 只调节兴奋性神经元接收的 GABA 电流。人工轨迹作为阳性参照，不直接用于自主模拟。'],['2. 先测 只开启 Z 能否自然进入高态','记录逐细胞电流超过阈值的时间比例及 dz/dt；让连续活动驱动 Z，不预设点火或回升时刻。接受依据是活动先改变 Z，随后状态跨越，而非时间脚本触发。'],['3. 明确自然退出缺什么','若 只开启 Z 锁在高态，检查是否缺乏负反馈。优先测试代码已有 M：能否先减弱活动和抑制电流、再让 Z 回升；同时与 只开启 Z、只开启 M 比较，防止 M 只是阻止进入。'],['4. 同时检查高态类型及基底保留','分别报告高率平台、大幅持续振荡及小幅快波纹；检查核内／核外和群体率。恢复后是否仍有自限事件，进入前是否仍覆盖患者 TA/TB，均需独立检验。']],widths=[145,W-229],size=9)
para('不能把本报告的全局 q 分岔线套到 C：前者同时改变 I→E 和 I→I，C 仅改变 E 接受的抑制。应在仅调节兴奋性神经元所受抑制的条件下重新定位平衡点、周期活动及其对应初态范围，再检查 Z/M 实际走过哪条边界。图中的下降与回升轨迹有限速率，单凭进出阈值不同也不能直接命名静态迟滞。',10)
para('本次完成的是既有轨道复算与报告补充；上述自主 Z/M 检验尚未执行。先保住用户原始的“间期活动累积导致进入高态”目标；自然退出作为新增机制目标单独确认，不能为实现退出而破坏原有间期传播。',9.5,color=GRAY)
...
table([['科学问题','已有证据','尚不能写成'],['同一网络可否产生不同事件？','固定网络连续噪声产生多事件；C1 有 TA/TB 分类事件','两个标签已证明两个起源核区 或两条完整传播路径'],['数据驱动是否有效？','部分参与、rank、空间分布相近；独立重复的平均损失降低','患者所有方差恢复、独立泛化成功或全局最优'],['为什么审计节律？','核内活动更快、更规则；模拟电极检测事件变异系数可掩盖底层规则性','患者具有同样核内同步或同一生理振子'],['抑制能否改变放电状态？','SNN 全局减抑制增强爆发，仅减弱 E 所受抑制可进出高率态；放电率模型有持续振荡','该 SNN 已证明与放电率模型同一临界点或已复现临床发作'],['分岔是否存在？','修正后的放电率模型：Hopf 型失稳、鞍结分岔、状态共存','已确定 Hopf 超／亚临界，或发现单独全局化分岔'],['积累能否解释跨越？','C 已支持外部恢复下可退出；Z 的自主回升及负反馈仍待检验','已经完成放电积累驱动 Z/M 自主跨界']],widths=[110,190,W-384],size=9.4)
