#!/usr/bin/env python3
"""Persistent W4 queue, dependency-bound to the exact W3 unit."""
...
from src.topic5_group_event_state.v035.contracts import (  # noqa: E402
    LOCKED_SEEDS, OUTPUT_ROOT, atomic_json, update_scope_manifest,
)
...
def jobs() -> list[dict]:
    ...
    for subject in SUBJECTS:
        for decoder_seed, state_seed in enumerate(LOCKED_SEEDS[:3]):
            unit = OUTPUT_ROOT / "full_mark_state" / subject / f"decoder_seed{decoder_seed}_state_seed{state_seed}"
            card = OUTPUT_ROOT / "functional_readouts" / subject / f"decoder_seed{decoder_seed}_state_seed{state_seed}" / "card.json"
            out.append({"subject": subject, "decoder_seed": decoder_seed, "state_seed": state_seed,
                        "dependency": str(unit / "card.json"), "out": str(card)})
    return out
...
def main() -> None:
    ...
    root = OUTPUT_ROOT / "functional_supervisor"
    ...
    while pending or running:
        for gpu, row in list(running.items()):
            ...
            del running[gpu]
        w3_done = (OUTPUT_ROOT / "full_mark_supervisor" / "queue_done.json").exists()
        for gpu in gpus:
            ...
            for i, job in enumerate(pending):
                ...
                if w3_done:
                    failed.append({**job, "returncode": None, "log": None,
                                   "reason": "exact W3 dependency unavailable"})
                    chosen = (i, None); break
            ...
            cmd = [str(PYTHON), str(ROOT / "scripts/run_group_event_state_v035_functional_readouts.py"),
        ...
        atomic_json(root / "queue_state.json", {
            "format": "group_event_state_v0_3_5_functional_queue_v1", "updated_epoch": time.time(),
    ...
    atomic_json(root / "queue_done.json", {"format": "group_event_state_v0_3_5_functional_done_v1",
