#!/usr/bin/env python3
"""Wait coarsely for Stage-AA and aggregate its fresh fit networks once."""
...
def main() -> None:
    ...
    while True:
        if controller.exists():
            ...
            if payload.get("status") != "REV12ND_NODE_WORKER_QUEUE_COMPLETE":
                raise RuntimeError("Stage-AA controller did not complete")
            if payload.get("expected_git_commit") != expected:
                raise RuntimeError("Stage-AA controller commit changed")
            break
        time.sleep(interval)
    subprocess.run([
    ...
    complete = {
        "status": "REV12ND_GLOBAL_SOFT_FIELD_EXPANSION_COMPLETE",
        "git_commit": expected,
        "fit_status": result["status"],
        "n_candidates": len(result["rows"]),
        "n_networks": len(result["requested_seeds"]),
        "provisional_candidate_ids": result[
            "nomination_decision"
        ]["candidate_ids"],
        "aggregate": str(aggregate),
    }
    output = output_root / "status" / "stage_aa_complete.json"
