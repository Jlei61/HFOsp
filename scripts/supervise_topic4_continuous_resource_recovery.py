#!/usr/bin/env python3
"""Bounded reviewed resource-equation tests, after QA and previous dispatches."""
import os
...
def analyze(names):
    ...
    with (OUT/'analysis.log').open('a') as log:
        for script in ['analyze_topic4_autonomous_recovery.py','analyze_topic4_autonomous_events.py','analyze_topic4_recruitment_origin.py','analyze_topic4_event_extent.py']:
...
def main():
    ...
    auth=read(OUT/'dispatch_authorization.json');jobs=auth['approved_names']
    assert set(jobs)<={j['name'] for j in p['initial_jobs']}
    children={};failed=[];seen=set();last=0
    while True:
        ...
        r3=read(run.PARENT/'activity_global_pool_round3/status.json')
        r4=read(run.PARENT/'stronger_M_redistribution_round4/status.json')
        earlier_pending=bool(r3.get('pending') or r4.get('pending'))
        if pending and not failed and not earlier_pending and time.time()<p['deadline_epoch']-5400:
            total,gpu=resources()
            if total<auth['maximum_combined_workers'] and gpu<24 and psutil.virtual_memory().available/2**30>=120:
                name=pending.pop(0);folder=OUT/'runs'/name;folder.mkdir(parents=True,exist_ok=True)
                with (folder/'worker.log').open('a') as log:
                    children[name]=subprocess.Popen([sys.executable,'-u',str(ROOT/'scripts/run_topic4_continuous_resource_recovery.py'),'worker','--name',name,'--producer-script',str(ROOT/'scripts/run_topic4_autonomous_recovery.py')],stdout=log,stderr=subprocess.STDOUT,cwd=ROOT,start_new_session=True)
        total,gpu=resources()
        write(OUT/'status.json',dict(status='RUNNING' if not failed else 'DRAINING_FAILURE',pid=os.getpid(),updated_at=time.time(),completed=len(complete),total=len(jobs),running={n:c.pid for n,c in children.items()},pending=pending,failed=failed,combined_workers=total,gpu_workers=gpu,waiting_for_earlier_dispatch=earlier_pending))
    ...
    write(OUT/'status.json',dict(status='REVIEW_MILESTONE' if not failed else 'FAILED_REVIEW',pid=os.getpid(),updated_at=time.time(),completed=len(complete),total=len(jobs),running={},pending=pending,failed=failed))
