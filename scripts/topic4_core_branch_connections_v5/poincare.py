...
def operator(path,dtmax,method='heun'):
    ...
    vals=[resample(a,n*(2 if method=='rk4' else 1),axis=0) for a in (mu,ve,vi)];gains=[]
    for j in range(3):
...
def compute(path,dtmax=.1,section='orthogonal',method='heun',nev=1):
    ...
    row.update(source=str(path),section=section,requested_multipliers=nev,multipliers=[[v.real,v.imag] for v in vals],residuals=residual,
    ...
    dest=OUT/'poincare'/path.parent.name/path.stem;dest.mkdir(parents=True,exist_ok=True)
