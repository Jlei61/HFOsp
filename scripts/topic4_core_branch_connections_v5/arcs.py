...
def continue_arc(old_path,current_path,name,steps=40,ds=.08,N=2048,gscale=.01,dsmax=.3):
    ...
    (dest/'initialization.json').write_text(json.dumps(dict(old=str(old_path),current=str(current_path),steps=steps,ds_initial=ds,dsmax=dsmax,N=N,gscale=gscale),indent=2)+'\n')
    for idx in range(steps):
        ...
        if (dest/'STOP').exists() or not .45<g<1.65:
            (dest/'completion.json').write_text(json.dumps(dict(reason='requested stop' if (dest/'STOP').exists() else 'parameter display boundary',last_accepted=str(path)),indent=2)+'\n');break
        ds=min(dsmax,ds*(1.25 if len(history)<5 else .9))
    return rows

if __name__=='__main__':
    ap=argparse.ArgumentParser();ap.add_argument('--old',required=True);ap.add_argument('--current',required=True);ap.add_argument('--name',required=True);ap.add_argument('--steps',type=int,default=60);ap.add_argument('--ds',type=float,default=.08);ap.add_argument('--dsmax',type=float,default=.3);ap.add_argument('--N',type=int,default=2048);ap.add_argument('--gscale',type=float,default=.01);a=ap.parse_args()
    continue_arc(a.old,a.current,a.name,a.steps,a.ds,a.N,a.gscale,a.dsmax)
