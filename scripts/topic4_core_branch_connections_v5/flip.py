...
def refine(left,right,name,N=2048,pair=False):
    ...
    def at(x):
        if x not in cache:
            ...
            vals,vec=eigs(K,k=24,which='LM',ncv=54,tol=2e-10,maxiter=800);j=np.argmin(abs(vals-1))
    ...
    vals,vec=eigs(K.T,k=24,which='LM',ncv=54,tol=2e-10,maxiter=800);j=np.argmin(abs(vals-1));leftvec=vec[:,j].real;leftvec/=np.linalg.norm(leftvec)
    w/=np.sqrt(np.sum(w*w)/N)
    eps=.003;slope=(at(x+eps)[4]-at(x-eps)[4])/(2*eps)
