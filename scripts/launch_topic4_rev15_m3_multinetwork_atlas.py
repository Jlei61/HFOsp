#!/usr/bin/env python3
"""Launch the frozen rev15 multinetwork M3 atlas controller."""
...
from scripts import monitor_topic4_rev15_m3_multinetwork_atlas as monitor
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not 1 <= args.worker_cap <= int(config["resources"]["maximum_workers"]):
        raise ValueError("worker cap exceeds rev15 multinetwork limit")
    ...
    payload = {
        "status": "REV15_M3_MULTINETWORK_CONTROLLER_LAUNCHED" if args.execute
        else "REV15_M3_MULTINETWORK_CONTROLLER_DRY_RUN",
        ...
        "monitor_interval_seconds": config["resources"][
