#!/usr/bin/env python3
"""Native M timing assay: entry, high-state exit and recovery with no interventions."""
...
def worker(job):
    ...
    def observe(tm, spk):
        ...
        if seen % 5000 == 0:
            ...
            write(progress, dict(status='RUNNING', seconds=time.time()-started,job=job,
...
def controller():
    ...
    arms=[('m0_native',0.,2.,30.),('weak_fast',.02,2.,90.),
          ('weak_20s',.02,20.,90.),('matched_gain_80s',.005,80.,90.),
          ('slow_80s',.02,80.,90.),('slow_200s',.02,200.,90.)]
    jobs=[dict(name=name,seed=9108401,tau_z_ms=5000.,threshold=THRESHOLD,duration_ms=dur*1000,
               detail=True,tau_adp_ms=tau*1000,eta_m=eta,refill=False)
          for name,eta,tau,dur in arms]
    protocol=dict(status='DEFINED_BEFORE_NEW_SIMULATIONS',jobs=jobs,max_workers=6,
        question='Can weaker initial adaptation allow native high-state entry, then terminate it autonomously?',
        carrier=str(PREVIOUS/'substrate.json'),
        M='E-only dm/dt=-m/tau_M+sum(delta_spike); adaptation current eta_M*m; I M=0',
        coordinates='eta_M is the adaptation current increment per spike; K_M=eta_M*tau_M_seconds is steady current gain per Hz.',
        comparisons='weak_20s vs matched_gain_80s fixes K_M=.4 while slowing onset; weak_fast/weak_20s/slow_80s/slow_200s fix eta_M=.02 while changing persistence and late strength.',
        intervention='NONE. Z, M, voltage, synapses, delays and OU evolve continuously. No manual refill, clamp, stimulation or state resets.',
        Z='tau_Z=5s, threshold95.19851312666987; tau_Z*dz/dt=1[raw GABA<Ith]-z',
        high_entry='Full E 10-ms bins >=200Hz for >=200ms; retain actual rates, local states and raster.',
        recovery='After a high-entry episode, at least2 consecutive seconds with 1s E means<50Hz and quiet(<5Hz 10ms) fraction>=.2; also quantify return episodes and Z recovery separately.',
        caution='Below200Hz alone is not recovery. A stable lower high-rate plateau fails. Quiet arrest is distinguished from return of finite events.',
        baseline='M current off control has30s horizon and must reproduce old reference before11.18s refill. M-on candidates have90s horizons; no event implies right censoring, not impossibility.',
        sample='One fixed historical manual network and paired seed9108401; exploratory parameters, not patient fit.',
        resource='Six workers, >60GiB free-memory guard before dispatch; all inputs and figures kept in separate directory.',
        stop='Complete this bounded six-arm assay, quantitative review and figures; no automatic further parameter wave or formal model acceptance.')
    ...
    subprocess.run([sys.executable,str(ROOT/'scripts/plot_topic4_fig5_m_return.py')],check=True)
    write(OUT/'delivery_status.json',dict(status='COMPUTED_AND_PLOTTED_PENDING_REVIEW',completed=len(rows)))
