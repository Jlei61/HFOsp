#!/usr/bin/env python3
"""Wait coarsely for Stage-W, then aggregate and analyze it once."""
...
def main() -> None:
    ...
    if status.get("status") != "REV12ND_NODE_WORKER_QUEUE_COMPLETE":
        raise RuntimeError("Stage-W controller ended without queue completion")
    ...
    if complete != expected_workers:
        raise RuntimeError(
            f"Stage-W worker count differs: {complete}/{expected_workers}"
        )
    subprocess.run([
    ...
    subprocess.run([
        str(PYTHON), str(ROOT / "scripts/analyze_topic4_rev12_local_curvature_results.py"),
    ...
    final.write_text(json.dumps({
        "status": "REV12ND_STAGE_W_POSTPROCESS_COMPLETE",
        "controller": str(controller),
        "aggregate": str(output_root / "aggregate" / "fit_cascade_summary.json"),
        "analysis": str(analysis_path),
        "decision": analysis["decision"],
        "fit_advancement_candidates": analysis["fit_advancement_candidates"],
    }, indent=2) + "\n")
