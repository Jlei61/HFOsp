#!/usr/bin/env python3
"""Resume-safe launcher for v0.4 shared-axis patient-specific training units."""
...
RUNNER = ROOT / "scripts/run_topic5_shared_axis_rnn_unit_v0_4.py"
...
def main() -> None:
    ...
    # "none" trains on the whole fit60; first/second train on one
    # chronological half for the axis-stability leg.
    parser.add_argument("--fit-halves", nargs="*", default=["none"])
    ...
    halves = [None if item == "none" else str(item) for item in args.fit_halves]
    tasks = [
        (index, subject, model, int(seed), half)
        for index, (subject, model, seed, half) in enumerate(
            (
                (subject, model, seed, half)
    ...
    def execute(task: tuple[int, str, str, int, str | None]) -> dict[str, Any]:
        task_index, subject, model, seed, half = task
        unit_name = model if half is None else f"{model}__{half}_half"
        run_dir = unit_root / "per_subject" / subject / unit_name / f"seed_{seed}"
        ...
        log_path = log_root / (f"{subject}__{unit_name}__seed{seed}.log")
        command = [
