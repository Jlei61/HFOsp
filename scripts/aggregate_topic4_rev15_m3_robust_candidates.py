#!/usr/bin/env python3
"""Aggregate fresh-network rev15 robust-field simulations."""
...
from scripts import freeze_topic4_rev15_m3_robust_candidates as freezer  # noqa: E402
...
DEFAULT_CONFIG = ROOT / "config/topic4_rev15_m3_robust_candidates.json"
OUTPUT_SCHEMA = "topic4_rev15_m3_robust_candidates_aggregate_v1"
    ...
    *, config: Mapping[str, Any], manifest: Mapping[str, Any],
    ...
    return rows, {
        ...
        "complete_cartesian_product": (
            len(rows) == expected and not missing and not invalid
        ),
    }
...
def _flat(scored: Mapping[str, Any], candidate: Mapping[str, Any]) -> dict[str, Any]:
    ...
    metadata = candidate.get("robust_response_construction") or {}
    row.update({
...
def _summaries(
    ...
    required_a = int(manifest["selection"][
        "fresh_A_improvement_required_networks"
    ])
    required_b = int(manifest["selection"][
        "fresh_B_protection_required_networks"
    ])
    b_ratio = float(manifest["selection"]["B_protection_ratio"])
    support_minimum = float(manifest["selection"][
        "equal_network_effective_support_minimum_per_mode"
    ])
    summaries = []
    for candidate in manifest["candidates"]:
        candidate_id = candidate["candidate_id"]
        ...
        a_count = sum(bool(row["A_improves"]) for row in paired)
        b_count = sum(bool(row["B_within_ratio"]) for row in paired)
        support_a = float(np.mean([
            row["mode_0_effective_events"] for row in paired
        ]))
        support_b = float(np.mean([
    ...
    summaries.sort(key=lambda row: (
        ...
        row["worst_delta_A"], row["worst_B_ratio"],
        row["mean_delta_A"], row["mean_delta_J14"], row["candidate_id"],
    ))
...
def aggregate(
    *, config_path: Path = DEFAULT_CONFIG,
    ...
    status, error, rows, summaries = "INCOMPLETE", None, [], []
    if not provenance["formal_ready"]:
        ...
        try:
            ...
            j14_path = freezer._load_hashed(config, "j14_config", artifact_root)[0]
    ...
    json_path = output_root / "m3_robust_candidates_aggregate.json"
    csv_path = output_root / "m3_robust_candidates_summary.csv"
    ...
    payload = {
        ...
        "claim_boundary": (
            "Fresh-network training-only replication. A usable anchor may advance "
            "to post-selection natural KMeans and Fig.4 inspection, but this stage "
            "does not freeze Node or establish held-out patient recovery."
        ),
