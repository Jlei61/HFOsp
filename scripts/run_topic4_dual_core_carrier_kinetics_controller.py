#!/usr/bin/env python3
"""Resource-bounded controller for the frozen dual-core kinetics canary."""
...
COMPLETE = "DUAL_CORE_CARRIER_KINETICS_COMPLETE"
RUNNING = "DUAL_CORE_CARRIER_KINETICS_RUNNING"
FAILED = "DUAL_CORE_CARRIER_KINETICS_FAILED"
...
def _unit(candidate_id: str, seed: int, commit: str) -> str:
    return f"codex-t4-carrier-{candidate_id}-s{seed}-{commit[:8]}"
...
def _launch(job: dict, config: dict, commit: str) -> None:
    worker_config = ROOT / config["inputs"]["worker_config"]["path"]
    manifest = ROOT / config["inputs"]["confirmation_manifest"]["path"]
    ...
    command = [
        ...
        f"dual-core carrier {job['candidate_id']} seed={job['seed']}", commit,
        "/usr/bin/time", "-v", sys.executable,
        str(ROOT / "scripts/run_topic4_rev10_r_edge_flow_worker.py"),
        "--config", str(worker_config), "--manifest", str(manifest),
        "--candidate-id", "dualcore_s39", "--seed", str(job["seed"]),
        "--expected-commit", commit, "--out-json", str(job["json"]),
        "--out-npz", str(job["npz"]), "--carrier-readout",
        "--duration-ms", str(config["simulation"]["duration_ms"]),
        "--tau-d-ampa-ms", str(job["tau_d_AMPA_ms"]),
        "--tau-d-gaba-ms", str(job["tau_d_GABA_ms"]),
    ]
...
def run(config_path: Path, commit: str) -> dict:
    ...
    while True:
        ...
        payload = {
            ...
            "n_failed": len(failed), "available_memory_gib": available,
            "free_disk_gib": free_disk, "updated_unix_seconds": time.time(),
        }
        if failed:
            payload["failed_jobs"] = [
        ...
        for job, state in zip(jobs, states):
            if state == "pending" and slots > 0:
                _launch(job, config, commit)
                slots -= 1
        _atomic_json(status_path, payload)
...
def main() -> None:
    ...
    payload = run(args.config.resolve(), commit)
