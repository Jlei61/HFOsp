...
CONTROL_MIN_DISTANCE_SEC = 4 * 3600.0
...
def _logistic_auc(x_train, y_train, x_test, y_test, alpha=1.0, iters=300) -> float:
    mu, sd = x_train.mean(0), x_train.std(0)
    sd = np.where(sd > 1e-9, sd, 1.0)
    xt = np.hstack([(x_train - mu) / sd, np.ones((x_train.shape[0], 1))])
    xe = np.hstack([(x_test - mu) / sd, np.ones((x_test.shape[0], 1))])
    ...
    score = xe @ w
    pos, neg = score[y_test == 1], score[y_test == 0]
    if pos.size == 0 or neg.size == 0:
        return float("nan")
    comparisons = (pos[:, None] > neg[None, :]).mean() + 0.5 * (pos[:, None] == neg[None, :]).mean()
...
def analyse_patient(run_dir: Path, dataset_root: Path, device: torch.device, seed: int) -> dict | None:
    ...
    far = (dist_next > CONTROL_MIN_DISTANCE_SEC) & (dist_prev > CONTROL_MIN_DISTANCE_SEC)
    ...
    times = t[idx]
    cut = float(np.median(np.array(case_time)))
    is_train = times < cut
    if label[is_train].sum() < 1 or label[~is_train].sum() < 1:
        return {"subject": subject, "arm": result["arm"], "seed": result["seed"],
    ...
    for name, x in feature_sets.items():
        ...
        aucs[name] = _logistic_auc(x[is_train], label[is_train], x[~is_train], label[~is_train])
        perm = []
        for r in range(50):
            g = np.random.default_rng(seed * 1000 + r)
            y_perm = g.permutation(label[is_train])
            perm.append(_logistic_auc(x[is_train], y_perm, x[~is_train], label[~is_train]))
        nulls[name] = float(np.nanmedian(perm))
...
def main() -> None:
    ...
    if usable:
        ...
        state = np.array([float(np.median(v["persistent_group_event_state"])) for v in per_patient.values()])
        code = np.array([float(np.median(v["memoryless_event_code"])) for v in per_patient.values()])
        ok = np.isfinite(state) & np.isfinite(code)
        summary["state_minus_memoryless"] = {
