        ...
        if runtime:
            try:
                ...
                live=p.status()!=psutil.STATUS_ZOMBIE and any(
                    Path(x).name=='replay_topic4_early_energy_second_seed.py' for x in p.cmdline())
...
def main():
    ...
    source=WINDOW/'early_z_refill_branches/runs/early_z_refill_s9108402'
    r=f.read(source/'result.json');assert r['end_s']>=r['tracker']['entries'][1]['confirmation_s']+10
    a=f.load(source,end_step=round(r['end_s']*10000));metrics=f.analyze(a,r)
    old=f.early(a,metrics)
    raw=np.load(qa['field_path'],mmap_mode='r')
    assert raw.shape==(150000,400)
    assert np.array_equal(raw.reshape(15000,10,400).sum(1),a['field_1ms'][:15000])
    a['early_field_0p1ms']=raw
    a['early_field_source']=dict(path=qa['field_path'],sha256=qa['field_sha256'],
        recording_window_s=[0,15],QA=str(OUT/'qa.json'),same_full_trajectory=True)
    ...
    report=dict(status='COMPLETE_SAME_TRAJECTORY_OBSERVATION_COMPARISON',
        ...
        new_F_samples=0,full_trajectory_end_s=r['end_s'],high_resolution_recording_end_s=15.,
...
if __name__=='__main__':
    ...
    if args.wait:
        ...
        while not (source/'result.json').exists():
            f.write(OUT/'analysis_status.json',dict(status='WAITING_FOR_COMPLETE_SOURCE_TRAJECTORY',pid=os.getpid()))
            progress=f.read(source/'progress.json')
            try:
                p=psutil.Process(progress['pid'])
                live=p.status()!=psutil.STATUS_ZOMBIE and 'early_z_refill_s9108402' in p.cmdline()
            except (psutil.Error,KeyError):live=False
            if not live and not (source/'result.json').exists():
                f.write(OUT/'analysis_status.json',dict(status='SOURCE_TERMINATED_WITHOUT_COMPLETE_RESULT',pid=os.getpid()))
                raise RuntimeError('Source trajectory ended without its full-followup result')
            time.sleep(20)
    main()
