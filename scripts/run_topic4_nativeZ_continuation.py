#!/usr/bin/env python3
"""Exact native-Z state continuation30–60s; observation stop changes only."""
import os
...
OUT=PARENT/'nativeZ_continuation_to60'
SOURCE=PARENT/'paired_recurrence_confirmation_round8'
SOURCE_NAME='resource_rho0_k200_tau10_s9108401'
NAME='nativeZ_k200_tau10_continuation60_s9108401'
...
def prepare():
    ...
    assert saved['engine']['step']==300000 and saved['job']['recovery_ratio']==0.
    assert not saved['job'].get('qa')
    before=engine_hash(saved['engine']);source_job=copy.deepcopy(saved['job'])
    job=copy.deepcopy(source_job);job.update(name=NAME,round=10)
    assert job['horizon_s']==60. and job['device']==1
    ...
    p.update(round=10,created_at=time.time(),initial_jobs=[job],
        ...
        extension='Same nativeZ candidate: full engine/RNG/Z/M/Rg/delays carried from30s. No physical parameter change; observe through originally planned60s rather than stopping after second entry.',
        question='Do finite high/quiet episodes persist to60s, or does nativeZ depletion lead to a persistent high platform?',
