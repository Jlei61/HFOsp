#!/usr/bin/env python3
"""Run persistent H3 readouts after their synthetic instrument passes."""
...
RUN = ROOT / "scripts/run_group_event_state_v037_h3_persistent.py"
...
def main() -> None:
    ...
    if not (args.instrument_root / "HUMAN_RELEASED").exists():
        raise SystemExit("persistent H3 instrument has not released human execution")
    for subject in SUBJECTS:
        for seed in SEEDS:
            if not (args.source_root / subject / f"seed{seed}" / "card.json").exists():
                raise SystemExit(f"missing primary H3 source: {subject} seed{seed}")
    ...
    while pending or running:
        ...
        for key in slots:
            ...
            process = subprocess.Popen(
                [str(PYTHON), str(RUN), "--subject", subject, "--seed", str(seed),
                 "--device", args.device, "--source-root", str(args.source_root),
        ...
        _write(supervisor / "queue_status.json", {
            "format": "group_event_state_v0_3_7_h3_persistent_queue_v1", "status": "RUNNING",
            "total": len(jobs), "complete": len(jobs) - len(pending) - len(running) - len(failures),
            "pending": len(pending),
            "running": [{"subject": subject, "seed": seed, "pid": process.pid, "gpu": gpu, "slot": slot}
    ...
    _write(supervisor / "queue_status.json", {
        "format": "group_event_state_v0_3_7_h3_persistent_queue_v1",
        "status": "FAILED" if failures else "COMPLETE", "total": len(jobs),
        "complete": len(jobs) - len(failures), "pending": 0, "running": [],
        "failures": failures, "observer_checkpoint_used_as_jump": False,
        "development_targets_read": False, "seizure_targets_read": False,
        "sealed_partition_opened": False,
    })
