#!/usr/bin/env python3
"""Resource-bounded controller for rev15 robust-candidate replication."""
...
DEFAULT_UNIT_PREFIX = "codex-t4-r15-m3robust"
CONTROLLER_SCHEMA = "topic4_rev15_m3_robust_candidates_controller_v1"
...
def _validate_unit_prefix(prefix: str) -> str:
    ...
    if not token.startswith(DEFAULT_UNIT_PREFIX):
        raise ValueError(
            f"rev15 robust-candidate unit prefix must start with {DEFAULT_UNIT_PREFIX!r}"
        )
    if any(marker in token for marker in base.base.PROTECTED_UNIT_MARKERS):
        raise ValueError("rev15 robust-candidate prefix overlaps a protected service")
    return token


def configure_base() -> None:
    base.freezer = freezer
    base.WORKER = ROOT / "scripts/run_topic4_rev15_m3_robust_candidate_worker.py"
    ...
    base.QUEUE_EMERGENCY_STATUS = "REV15_M3_ROBUST_EMERGENCY_STOPPED"
    base.QUEUE_COMPLETE_STATUS = "REV15_M3_ROBUST_QUEUE_COMPLETE"
    base.QUEUE_DRAINING_STATUS = "REV15_M3_ROBUST_DRAINING_AFTER_FAILURE"
    base.QUEUE_FAILED_STATUS = "REV15_M3_ROBUST_QUEUE_FAILED"
    base.QUEUE_WAIT_STATUS = "REV15_M3_ROBUST_RESOURCE_WAIT"
    base.QUEUE_RUNNING_STATUS = "REV15_M3_ROBUST_QUEUE_RUNNING"
...
def _load_contract(
    ...
    for key, value in expected_resources.items():
        if resources.get(key) != value:
            raise RuntimeError(f"rev15 robust resource contract changed: {key}")
    ...
    if (
        ...
        raise RuntimeError("rev15 robust-candidate provenance is not formal")
    candidates = manifest.get("candidates", [])
    if len(candidates) != int(config["m3_design"]["candidate_count"]):
        raise RuntimeError("rev15 robust-candidate inventory changed")
    if sum(bool(row["selection_eligible"]) for row in candidates) != int(
        config["m3_design"]["selectable_candidate_count"]
    ):
        raise RuntimeError("rev15 robust selectable inventory changed")
    if [int(seed) for seed in manifest["search"]["active_network_seeds"]] != [
        2341, 2342, 2343,
    ]:
        raise RuntimeError("rev15 robust fresh-network pool changed")
...
def main(argv: list[str] | None = None) -> None:
    ...
    payload = base.run_controller(
        ...
        unit_prefix=args.unit_prefix, execute=bool(args.execute),
