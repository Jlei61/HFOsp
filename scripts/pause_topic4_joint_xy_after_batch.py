...
SERVICE = 'codex-t4-joint-rank-space-xy-20260906.service'
...
def record(**kwargs):
    p = OUT/'loss_revision_batch_guard.json'; tmp = p.with_suffix('.tmp')
...
def main():
    ...
    identity = proc(pid)
    if not identity or 'scripts/run_topic4_joint_xy_adaptive.py' not in identity['command']:
        raise RuntimeError('owned adaptive controller is not live')
    marker = OUT/'rounds/001/analysis.json'
    record(status='WAITING_FIRST_BATCH_ANALYSIS', controller=identity,
           reason='Paired training controls show weak rank/time sensitivity in v1. Finish the first batch, then revise loss under a new version.')
    ...
    while True:
        ...
        record(status='DRAINING_EXISTING_CHILDREN_FOR_LOSS_REVIEW', controller=now, children=live)
        if not live: break
        time.sleep(5)
    ...
    record(status='BATCH_DRAIN_COMPLETE_V1_RETIRED_FOR_LOSS_REVIEW', controller_pid=pid,
