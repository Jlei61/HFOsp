#!/usr/bin/env python3
"""Native neuron-wise Z evolution followed by an explicitly external refill."""
from validate_topic4_fixed_rate_base import ROOT, SOURCE, setup, read, write, make_external_drive, spatial_cell_index
...
REFERENCE = ROOT / 'results/topic4_sef_hfo/snn_raster_inhibition_transition_v1'
...
def run():
    ...
    protocol = {'status': 'DEFINED_BEFORE_TRAJECTORY', 'seed': seed,
                'carrier': str(ROOT / 'config/topic4_rate_model_dynamics_validation_v1.json'),
                ...
                'manual_intervention': 'Suspend only Z ODE; each E z rises linearly from its current value to 1 over 1000 ms; hold z=1 for 2000 ms',
    ...
    def observe_spikes(tm, spk):
        ...
        if slow.restore_ms is not None and seen * dt >= slow.restore_ms + 3000.:
    ...
    from plot_topic4_autonomous_z_manual_restore import render
    render()
