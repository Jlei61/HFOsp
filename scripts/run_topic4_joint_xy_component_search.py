...
OLD_V1=v1.OUT
OLD_V2=ROOT/'results/topic4_sef_hfo/joint_rank_space_dual_core_search_v2'
OLD_V3=ROOT/'results/topic4_sef_hfo/joint_rank_space_dual_core_search_v3'
OUT=ROOT/'results/topic4_sef_hfo/joint_rank_space_dual_core_search_v4'
CONFIG=ROOT/'config/topic4_joint_xy_kernel_v4.json'
KERNEL=OLD_V1/'kernel_qualification/kernel_contract.json'
SOURCES=['scripts/run_topic4_joint_xy_component_search.py', 'src/topic4_xy_component_search.py',
         'tests/test_topic4_xy_component_search.py', 'config/topic4_joint_xy_kernel_v4.json',
         'docs/archive/topic4/sef_hfo/joint_xy_component_coverage_revision_2026-09-06.md']
...
def race(pool,number,obj,calibration,plan,lock):
    ...
    if path.exists():
        ...
        write(path,{'candidate_ids':[r['candidate_id'] for r in chosen],
            ...
            'rule':'Participation, rank and timing component experts, then joint-score spatial diversity; all unexpanded candidates eligible.'})
...
def raw_review_handoff(result, phase, plan, lock):
    # Numerical confirmation alone is not the raw-propagation acceptance contract.
    folder=OUT/'fig5_followup';folder.mkdir(exist_ok=True)
    write(folder/'raw_review_queue.json', {'status':'WAITING_RAW_PROPAGATION_ACCEPTANCE',
        'candidate_id':result['candidate_id'],'confirmation_phase':phase,
        'confirmed_result_sha256':sha(OUT/'qualified_substrate.json'),
        'numerical_confirmation_passed':True,'raw_propagation_accepted':False,
        'fig5_hold_released':False,'goal_remains_active':True})
    v1.status('NUMERICALLY_CONFIRMED_PENDING_RAW_REVIEW', candidate_id=result['candidate_id'],
              fig5_hold_released=False,goal_remains_active=True)
...
def main():
    ...
    previous_guard=open(OLD_V3/'controller.lock','r+');fcntl.flock(previous_guard,fcntl.LOCK_EX|fcntl.LOCK_NB)
    v1.OUT=OUT;v1.fig5_handoff=raw_review_handoff
    v1.base.ENV['LD_LIBRARY_PATH']='/home/honglab/leijiaxin/anaconda3/envs/cuda_env/lib'
    plan=read(CONFIG); old_plan=read(ROOT/'config/topic4_joint_xy_kernel_v3.json')
    for key in ('observation','calibration','confirmation','objective_version','direction_loss_weight','core_axis_prior_weight'):
        if plan[key]!=old_plan[key]:raise RuntimeError('optimizer revision changed frozen science: '+key)
    closure=read(OLD_V3/'cycle_closure_review.json')
    if closure['status']!='EXHAUSTED_UNQUALIFIED_CYCLE_REPLAY_VERIFIED':raise RuntimeError('previous cycle unclosed')
    raw=read(OLD_V3/'rounds/008/raw_propagation_review.json')
    if raw['status']!='ROUND_RAW_DIAGNOSTICS_COMPLETE_MODEL_NOT_QUALIFIED':raise RuntimeError('previous raw review incomplete')
    lock={**read(OLD_V3/'objective_contract.json')['source_hashes'],
          **read(OLD_V3/'analysis_input_lock.json')['hashes'],**closure['source_hashes'],**raw['source_hashes']}
    lock[raw['report_path']]=raw['report_sha256']
    coverage=read(OLD_V3/'component_coverage_review.json');lock.update(coverage['source_hashes'])
    lock.update({str(ROOT/p):sha(ROOT/p) for p in SOURCES});lock[str(KERNEL)]=sha(KERNEL)
    prior_files=[OLD_V3/'baseline_scores.json']
    for number in range(1,closure['rounds']+1):
        stage=OLD_V3/'rounds'/f'{number:03d}';prior_files.append(stage/'scores.json')
        prior_files.extend(stage/f'combined_{cid}.json' for cid in read(stage/'race_nomination.json')['candidate_ids'])
    for p in prior_files+[OLD_V3/'cycle_closure_review.json',OLD_V3/'component_coverage_review.json',OLD_V3/'rounds/008/raw_propagation_review.json']:
        lock[str(p)]=sha(p)
    contract_path=OUT/'objective_contract.json'
    if contract_path.exists():
        contract=read(contract_path)
        if contract['source_hashes']!=lock:raise RuntimeError('prior source/input changed; create new version')
    else:
        contract={'source_hashes':lock,'version':plan['version'],'objective_version':plan['objective_version'],
            'optimizer_only_revision':True,'master_seed':secrets.randbits(32),
    ...
    if not (OUT/'patient_calibration.json').exists():
        shutil.copyfile(OLD_V3/'patient_calibration.json',OUT/'patient_calibration.json')
    if sha(OUT/'patient_calibration.json')!=sha(OLD_V3/'patient_calibration.json'):raise RuntimeError('objective tolerance changed across optimizer-only revision')
    ...
    for p in prior_files:
        for r in read(p)['candidates']:by_id[r['candidate_id']]=r
    pool=score_saved(list(by_id.values()),obj,plan,OUT/'baseline_scores.json')
    if args.score_only:v1.status('KERNEL_BASELINE_RESCORED',n_candidates=len(pool));return
    write(OUT/'replicated_bootstrap.json',{'source_cycle':str(OLD_V3),'n_reused_geometries':len(pool),
        'actual_local_anchors':[r['candidate_id'] for r in local_anchors(pool,plan)],
        'objective_and_tolerances_unchanged':True,'no_repeated_simulations':True})
    ...
    for number in range(1,plan['search']['automatic_rounds_before_capacity_review']+1):
        ...
        if best is not None:history.append(best['exploration_score'])
        ...
        if path.exists():rows=read(path)['candidates']
            ...
            rows=new_proposals(pool,pos,contract['master_seed'],number,plan,diagnosis,v1)
            write(path,{'candidates':rows,'preceding_diagnosis':diagnosis,'seed_sequence':[contract['master_seed'],number,0],
                'local_anchor_candidates':[r['candidate_id'] for r in local_anchors(pool,plan)],
                'short_runs_eligible_as_local_anchors':False})
    ...
    v1.status('NEEDS_MODEL_CAPACITY_REVIEW',rounds=plan['search']['automatic_rounds_before_capacity_review'],
              replicated_incumbent_observations=len(history),goal_remains_active=True,
              reason='Component coverage experiment reached four-round review. Diagnose tradeoffs before more geometry or model changes; no qualification assumed.')
