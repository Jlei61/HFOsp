...
def focused_test_skip_count(text: str) -> int:
    """Return the final pytest skip count, or zero when nothing was skipped."""
    matches = re.findall(r"(?m)\b(\d+)\s+skipped\b", text.lower())
    return int(matches[-1]) if matches else 0
...
def focused_test_log_ok(text: str, expected: int = EXPECTED_FOCUSED_TESTS) -> bool:
    ...
    Count executed plus skipped tests rather than passes alone: some cases in
    the frozen suite skip when an optional upstream cache is absent, and an
    exact pass count would then reject a run in which nothing actually failed.
    """
    lowered = text.lower()
    return bool(
        focused_test_pass_count(lowered) + focused_test_skip_count(lowered) == int(expected)
        ...
        "requirements": {
            ...
            "focused_tests": {
                ...
                "observed": {
                    ...
                    "skipped": focused_test_skip_count(test_text),
                    "expected_collected": EXPECTED_FOCUSED_TESTS,
