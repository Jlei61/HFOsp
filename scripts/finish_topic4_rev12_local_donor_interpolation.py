#!/usr/bin/env python3
"""Wait coarsely for Stage-AC and aggregate the local donor grid once."""
...
def main() -> None:
    ...
    while True:
        if controller.exists():
            ...
            if payload.get("status") != "REV12ND_NODE_WORKER_QUEUE_COMPLETE":
                raise RuntimeError("Stage-AC controller did not complete")
            if payload.get("expected_git_commit") != expected:
                raise RuntimeError("Stage-AC controller commit changed")
            break
        time.sleep(interval)
    subprocess.run([
    ...
    complete = {
        "status": "REV12ND_LOCAL_DONOR_INTERPOLATION_COMPLETE",
        "git_commit": expected,
    ...
    output = output_root / "status" / "stage_ac_complete.json"
