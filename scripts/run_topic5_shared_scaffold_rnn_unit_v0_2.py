#!/usr/bin/env python3
"""Train one patient-specific shared-scaffold Figure 6 unit.
...
MODELS = ("structured", "ordinary_gru", "structured_rank_shuffle")
    ...
    *,
    ...
    if model_name in {"structured", "structured_rank_shuffle"}:
        ...
        return SharedScaffoldPropagationRNN(
            ...
            low_rank=int(config["models"]["structured_low_rank"]),
        )
...
def save_operator(
    ...
    *,
    ...
    if isinstance(model, SharedScaffoldPropagationRNN):
        ...
        payload.update(
            ...
            skew_gain=np.asarray(float(model.skew_gain.detach().cpu())),
            direction_gain=np.asarray(float(model.direction_gain.detach().cpu())),
        )
    else:
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    run_dir = output_root / "per_subject" / args.subject / args.model / f"seed_{args.seed}"
    ...
    for cycle_index in range(completed_cycle, cycles):
        ...
        for update_index, macro_indices in enumerate(update_chunks):
            ...
            aggregate = {name: 0.0 for name in ("joint", "conditional_contacts", "cardinality", "stop")}
    ...
    checkpoint = {
        ...
        "model_hyperparameters": (
            {"low_rank": int(config["models"]["structured_low_rank"])}
            if args.model in {"structured", "structured_rank_shuffle"}
