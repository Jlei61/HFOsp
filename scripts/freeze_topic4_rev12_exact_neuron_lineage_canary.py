#!/usr/bin/env python3
"""Freeze one exact-neuron lineage canary field."""
...
def main() -> None:
    ...
    if subprocess.check_output(
        ...
        raise RuntimeError("exact-neuron canary freezer is not at expected commit")
    tracked = [
    ...
    if subprocess.check_output(
        ...
        raise RuntimeError("exact-neuron canary runtime paths are dirty")
    inputs = {}
    for key, record in config["inputs"].items():
        path = _resolve(root, record["path"])
        digest = _sha256(path)
        if digest != record["sha256"]:
            raise RuntimeError(f"exact-neuron canary input changed: {record['path']}")
    ...
    if invalidation["status"] != "INVALIDATED_APPROXIMATE_BINNED_CONTACT_READOUT":
        raise RuntimeError("approximate fit was not formally invalidated")
    ...
    if any(candidate_id not in rows for candidate_id in requested):
        raise RuntimeError("exact-neuron canary field is absent")
    payload = {
        "schema_id": "topic4_rev12_exact_neuron_lineage_canary_manifest_v1",
        "status": "REV12ND_EXACT_NEURON_LINEAGE_CANARY_FROZEN",
