"""Managed D4.1 launcher with an RSS sentinel and sparse status checks."""
...
AGGREGATOR = ROOT / "scripts/aggregate_topic4_rev10_d4_1_packet_dose.py"
...
def _peak_rss_kib(path):
    matches = re.findall(
        r"Maximum resident set size \(kbytes\):\s*(\d+)", Path(path).read_text(),
    )
    if not matches:
        raise RuntimeError("D4.1 sentinel RSS is absent")
...
def _launch(job, *, config_path, commit, prefix):
    ...
    command = [
        ...
        *[f"--setenv={key}={value}" for key, value in NUMERIC_ENV.items()],
        ...
        f"rev10-D4.1 packet dose seed={job['seed']}", commit[:8],
...
def _wait(job, *, config_sha, manifest_sha, commit, wait_seconds):
    while True:
        ...
        if _state(job["status"]) == "FAILED":
            raise RuntimeError(f"D4.1 worker failed: seed {job['seed']}")
...
def main():
    ...
    parser.add_argument("--unit-prefix", default="topic4-rev10d4-1")
    ...
    if commit != head:
        raise RuntimeError("D4.1 launcher commit must be HEAD")
    root = ROOT / config["output_root"]
    manifest_path = root / "packet_dose_manifest.json"
    if not manifest_path.exists():
        raise RuntimeError("D4.1 manifest is not frozen")
    config_sha, manifest_sha = _sha256(config_path), _sha256(manifest_path)
    if json.loads(manifest_path.read_text())["config"]["sha256"] != config_sha:
    ...
    for seed in map(int, config["network_seeds"]):
        stem = worker_dir / f"packet_dose_seed_{seed}"
        jobs.append({
            "seed": seed,
            "json": stem.with_suffix(".json"), "npz": stem.with_suffix(".npz"),
            "status": log_dir / f"packet_dose_seed_{seed}.status",
            "log": log_dir / f"packet_dose_seed_{seed}.log",
        })
    pending = [
        job for job in jobs
    ...
    if pending:
        ...
        print(json.dumps({"progress": "sentinel_launched", "seed": sentinel["seed"]}), flush=True)
        _wait(sentinel, config_sha=config_sha, manifest_sha=manifest_sha,
        ...
        audit_path.write_text(json.dumps({
            "status": "REV10D4_1_MEMORY_SENTINEL_COMPLETE",
    ...
    verdict = json.loads((root / "packet_dose_verdict.json").read_text())
    subprocess.run([
        "notify-send", "Topic 4 rev10-D4.1",
        f"{verdict['status']}; packet={verdict['selected_packet_n_E']}; workers={maximum}",
    ], check=False)
    print(json.dumps({
        "status": verdict["status"], "completed": len(jobs), "total": len(jobs),
        "selected_packet_n_E": verdict["selected_packet_n_E"],
        "max_workers": maximum,
    }, indent=2))
