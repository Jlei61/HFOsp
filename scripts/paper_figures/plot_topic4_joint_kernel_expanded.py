...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('--round',type=int,required=True);args=parser.parse_args()
    out=run.OUT;plan=run.read(run.CONFIG);stage=out/'rounds'/f'{args.round:03d}'
    ids=run.read(stage/'race_nomination.json')['candidate_ids']
    source_paths=[stage/f'combined_{cid}.json' for cid in ids]
    selected=[run.read(p)['candidates'][0] for p in source_paths]
    if not selected or any(len(r['units'])!=8 for r in selected):raise RuntimeError('complete eight-network candidate summaries required')
    ...
    fig.suptitle('Dual-core candidates after common-seed event expansion',fontsize=17,y=.987,weight='bold')
    ...
    for i,row in enumerate(selected):
        ...
        c.axhline(1,color='black',ls='--',lw=1);c.set(yscale='log',xticks=range(4),xticklabels=['Participation','Rank','Timing','Joint'],ylabel='Distance / patient q95',title='Training diagnostics; confirmation required')
    ...
    fig.text(.065,.013,'Each geometry has eight 8 s training networks. At least 64 actual events are required before independent confirmation.\n'
    ...
    folder=stage/'figures';folder.mkdir(exist_ok=True)
    for ext in ('png','pdf','svg'):fig.savefig(folder/f'candidate_after_event_expansion.{ext}',dpi=160)
    plt.close(fig)
    (folder/'README.md').write_text('''### candidate_after_event_expansion.png / .pdf / .svg
展示按预设规则提名补算的六个候选：左列是双核位置与固定电极，虚线只表示整个网络 E→E 连接的共同方向，并不是中央局部连接带；中列比较参与通道的平均 rank，右列比较完整事件核各分量与患者训练波动阈值的比例。当前每个候选已有八个 8 秒训练网络；仍需通过事件数和全部分布验收，再进行独立网络确认。损失使用完整事件分布，中列平均曲线只是诊断摘要，不能证明两种模式已经出现。
**关注点**：位置是否不同、rank／时滞差异是否仍明显，以及额外共同 seed 后结论是否保持；比例超过 1 表示尚未落入相应开发阈值。
''')
    paths=source_paths+[stage/'analysis.json',stage/'race_nomination.json',out/'patient_calibration.json',run.CONFIG,Path(__file__)]
    write=lambda p,x:p.write_text(json.dumps(x,indent=2)+'\n')
    sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
    write(stage/'candidate_review_metadata.json',{'input_hashes':{str(p):sha(p) for p in paths},'candidate_ids':[r['candidate_id'] for r in selected],
        'output_hashes':{p.name:sha(p) for p in folder.glob('candidate_after_event_expansion.*')},'author_acceptance':False})
