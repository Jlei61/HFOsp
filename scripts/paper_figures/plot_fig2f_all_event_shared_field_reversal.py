...
def _draw_distribution(ax: plt.Axes, rows: list[dict]) -> dict:
...
def _draw_null(ax: plt.Axes, null_delta: np.ndarray, observed: float) -> None:
...
def build(input_root: Path, output_root: Path, *, seed: int, draws: int) -> dict:
    ...
    summary, null_delta = build_cohort_shift_null(
    ...
    with plt.rc_context({
        ...
        distribution = _draw_distribution(distribution_ax, rows)
        _draw_null(null_ax, null_delta, float(summary["observed_median_delta"]))
        fig.text(
    ...
    payload = {
        "schema_version": "figure2f_all_event_timing_plus_space_v1",
...
下图为全触点打乱后重建 TB 场的层级 cohort-median null，观测 Δmedian=
{summary['observed_median_delta']:+.3f}，单侧置换 P={summary['p_negative']:.5g}。
