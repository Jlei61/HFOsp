...
即使这三张图全部为正，能写的也只有一句：**照着病人自己数据调出来的仿真，在没看过的那半段数据上，把"哪个触点排在第几位"这件事复现得比"同一根杆内部随机换标签"更好**。不能写成"复现了 34 位患者的间期活动"，不能当作找到了解剖上的致痫核，不能当作临床波形等价，不能当作模型对没见过的患者也成立，也和发作机制无关。触点摆位是只看电极名字排出来的，不是解剖。

冻结合同原文：{config['claim_boundary']}
"""
