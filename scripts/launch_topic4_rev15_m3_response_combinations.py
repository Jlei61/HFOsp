#!/usr/bin/env python3
"""Launch the frozen rev15 M3 response-combination controller."""
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not 1 <= args.worker_cap <= int(config["resources"]["maximum_workers"]):
        raise ValueError("worker cap exceeds the frozen rev15 combination limit")
    ...
    payload = {
        "status": "REV15_M3_COMBINATION_CONTROLLER_LAUNCHED" if args.execute
        else "REV15_M3_COMBINATION_CONTROLLER_DRY_RUN",
        "unit": unit,
