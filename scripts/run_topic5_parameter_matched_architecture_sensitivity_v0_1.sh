...
tag="${1:-parameter_matched_formal_20260729}"
run_root="${repo_root}/results/topic5_ordered_history_architecture_audit/parameter_matched/${tag}"
...
# GRU(h=32) has 11,246 parameters. These choices put both nongated controls
# within 10% while leaving encoder, offset, decoder, data and optimizer fixed.
specs=(
  "linear_state|64|linear_state_parammatched_h64"
  "vanilla_rnn|48|vanilla_rnn_parammatched_h48"
)
workers=3
jobs=()
for spec in "${specs[@]}"; do
  IFS='|' read -r architecture hidden_size control_name <<< "${spec}"
  for seed in "${seeds[@]}"; do
    for subject in "${subjects[@]}"; do
      jobs+=("${architecture}|${hidden_size}|${control_name}|${seed}|${subject}")
    done
  done
done
expected="${#jobs[@]}"
...
python "${repo_root}/scripts/monitor_topic5_rank_distribution_resources.py" \
  ...
  --interval-seconds 20 &
resource_pid=$!
python "${repo_root}/scripts/monitor_topic5_architecture_controls_v0_1.py" \
  --root "${run_root}" \
  --launcher-pid "$$" \
  --expected "${expected}" \
  --interval-seconds 20 &
...
run_worker() {
  ...
  for job in "${jobs[@]}"; do
    ...
    IFS='|' read -r architecture hidden_size control_name seed subject <<< "${job}"
    local cell="${run_root}/${control_name}/seed_${seed}/${subject}"
    local log="${run_root}/logs/${control_name}_seed${seed}_${subject}.log"
    ...
    conda run --no-capture-output -n cuda_env \
      python "${repo_root}/scripts/train_topic5_architecture_control_v0_1.py" \
        ...
        --hidden-size-override "${hidden_size}" \
        --seed "${seed}" \
        --batch-size 1024 \
        --shared-cycles 1 \
