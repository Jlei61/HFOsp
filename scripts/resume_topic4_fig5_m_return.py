...
OUT = Path(os.environ.get('TOPIC4_M_RETURN_OUT',str(BASE / 'm_runaway_return_v1')))
...
def worker(job):
    ...
    assert not job['refill'], 'This recovery runner only permits native trajectories'
    started = time.time()
    ...
    def checkpoint_sink(k,state):
        ...
        if not job.get('qa_pause') and not high_active and k % 20000:
            return
