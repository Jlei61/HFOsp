#!/usr/bin/env python3
"""Plot the instantaneous-variance sensitivity, NOT the frozen filtered v1."""
import os
...
def main():
    ...
    h1=read('first_hopf_check.json');h2=read('low_hopf_check.json');fold=read('low_fold_check.json')
    status={}
    for name in ['equilibrium_stability_certificates.json','equilibrium_spectrum_followup_N8.json']:
        for r in read(name,[]):
            key=(r['branch'],r['index'])
            if r['status'].startswith('UNSTABLE'):status[key]='unstable'
    # Exact characteristic / converged Chebyshev checks at these individual states.
    for i in [0,3,6,10,20,60]:status['extended_low_equilibria',i]='unstable'
    orbits={};qa=[]
    for f in sorted((OUT/'periodic').glob('s*_N*.npz')):
        ...
        row=dict(s=s,mean_z=1-s,N=N,period_ms=float(a['T']),minimum_hz=float(g.min()),maximum_hz=float(g.max()),mean_hz=float(g.mean()),
                 residual_hz=error,unit_minimum_hz=float(full.min()*1000),unit_maximum_hz=float(full.max()*1000),file=str(f))
        qa.append(row)
        if N>=256 and row['minimum_hz']>=0 and (s not in orbits or N>orbits[s]['N']):orbits[s]=row
    chosen=sorted(orbits.values(),key=lambda x:x['s'])
    ...
    for view in ['full','physical']:
        fig,ax=plt.subplots(figsize=(10.1,8.3));fig.subplots_adjust(left=.13,right=.97,bottom=.19,top=.79)
        fig.text(.13,.96,'Instantaneous-variance sensitivity',fontweight='bold',fontsize=22,va='top')
        fig.text(.13,.905,'Modified dynamics · NOT the frozen filtered-v1 model · native correspondence unvalidated',fontsize=10.5,color='#5e6266')
        if view=='full':
            ax.axvspan(-.32,0,color='#ededed',zorder=-10)
            ax.text(-.285,410,'Mathematical extension: '+r'$Z>1$',fontsize=10,color='#656565',va='top')
        # The stable base and the first oscillatory crossing are outside resource bounds.
        a=np.load(OUT/'extended_resting_equilibria.npz');x=a['s'];y=np.average(a['r_hz'][:,:400],weights=m.count_e,axis=1)
        ...
        for name in ['extended_low_equilibria','low_to_recruited_equilibria','recruited_equilibria']:
            a=np.load(OUT/f'{name}.npz');x=a['s'];y=np.average(a['r_hz'][:,:400],weights=m.count_e,axis=1)
        ...
        if chosen:
            sx=np.array([r['s'] for r in chosen]);lo=np.array([r['minimum_hz'] for r in chosen]);hi=np.array([r['maximum_hz'] for r in chosen]);mean=np.array([r['mean_hz'] for r in chosen])
            ax.fill_between(sx,lo,hi,color=green,alpha=.065,lw=0,zorder=0)
            ax.plot(sx,hi,'o-',color=green,lw=2.3,ms=4.5,zorder=4);ax.plot(sx,lo,'o-',color=green,lw=2.3,ms=4.5,zorder=4)
            ax.plot(sx,mean,'o-',color=orange,lw=2.6,ms=4.5,zorder=5)
        for info,label,dy in [(h1,'H1',34),(h2,'H2',65),(fold,'Fold',37)]:
            if view=='physical':continue
            ax.plot(info['s'],info['mean_e_hz'],'o',mfc='white',mec='#151515',ms=6,zorder=9)
            ax.annotate(label,(info['s'],info['mean_e_hz']),xytext=(3,dy),textcoords='offset points',fontsize=11,arrowprops=dict(arrowstyle='-',color='#555',lw=.8))
        s3=float(eq.ss[list(eq.times).index(9.42)])
        ax.axvline(s3,color='#656565',ls=(0,(2,3)),lw=1.2,zorder=1)
        ax.text(s3-.004,190,'Fig.5 ③\n'+r'$\langle Z_E\rangle=0.7712$',ha='right',va='center',fontsize=11,color='#4e4e4e')
        ...
        ax.set_ylim(-.004,520);ax.set_xlim((-.31,.325) if view=='full' else (-.005,.325))
        ...
        fig.text(.13,.09,'Periodic curves: solved deterministic orbits; Floquet stability is not classified.',fontsize=10,color='#62666a')
        fig.text(.13,.061,'H1 / H2: oscillatory linear crossings. No bifurcation at Fig.5 ③ has been established.',fontsize=10,color='#62666a')
        name='fig5_z_branch_continuation'+('_physical_range' if view=='physical' else '')
