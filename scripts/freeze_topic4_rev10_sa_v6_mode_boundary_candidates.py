...
def build_candidates(config, source_manifest, audit):
    ...
    known_id = design["known_boundary_candidate_id"]
    left = np.asarray(sources[left_id]["coefficients"], float)
    right = np.asarray(sources[right_id]["coefficients"], float)
    known_source = sources[known_id]
    known_t = float(known_source["interpolation_fraction"])
    final_t = float(design["fractions"][-1])
    if not np.isclose(known_t, final_t, atol=0.0, rtol=0.0):
        raise RuntimeError("V6 endpoint does not match the frozen V5 boundary fraction")
    ...
    for fraction in design["fractions"]:
        ...
        if t == 0.0:
            coefficients = left
        elif np.isclose(t, known_t, atol=0.0, rtol=0.0):
            # The endpoint is already frozen in V5. Re-solving the density
            # projection can drift at the last bits across numerical stacks,
            # so reuse the hash-locked coefficients rather than manufacturing
            # a near-identical replacement for the same scientific endpoint.
            coefficients = np.asarray(known_source["coefficients"], float)
        else:
    ...
    known = known_source["field_sha256"]
