#!/usr/bin/env python3
"""Wait coarsely for Stage-Z and aggregate its fit screen exactly once."""
...
def main() -> None:
    ...
    while True:
        if controller.exists():
            ...
            if payload.get("status") != "REV12ND_NODE_WORKER_QUEUE_COMPLETE":
                raise RuntimeError("Stage-Z controller ended without completion")
            if payload.get("expected_git_commit") != expected_commit:
                raise RuntimeError("Stage-Z controller commit changed")
            break
        time.sleep(interval)
    ...
    complete = {
        "status": "REV12ND_GLOBAL_SOFT_FIELD_SCREEN_COMPLETE",
    ...
    output = output_root / "status" / "stage_z_complete.json"
