# Frozen observer fork of run_topic4_rev12_node_worker.py; source and exact diff recorded in the pilot design.
#!/usr/bin/env python3
...
from src.topic4_initial_state_substrate import (  # noqa: E402
...
ALLOWED_SCIENTIFIC_ROLES = {
    "development_only_multidimensional_interictal_pilot",
...
def main() -> None:
    ...
    if config['scientific_role'] == 'development_only_multidimensional_interictal_pilot' and not args.runtime_manifest:
    ...
    if args.runtime_manifest:
        ...
        if config['scientific_role'] != 'development_only_multidimensional_interictal_pilot':
            raise RuntimeError('source-hash execution requires the multidimensional pilot role')
    ...
    if args.runtime_manifest:
        ...
        if np.any(substrate.edge_coefficients != 0):
            raise RuntimeError('pilot excludes learned spatial-flow coefficients')
    ...
    # Engine spike arrays are time-step x excitatory-neuron.  The neuron axis
    # must never be used as a duration proxy.
    actual_duration_ms = float(spikes.shape[0] * float(substrate.engine["dt"]))
    ...
    payload = {
        ...
        "static_array_identity": {
            "positions_E_sha256": array_sha256(np.asarray(
                substrate.positions_e, np.float32,
            )),
            "h_sha256": array_sha256(np.asarray(substrate.h_e, np.float32)),
            "delta_vtheta_sha256": array_sha256(np.asarray(
                substrate.delta_vtheta, np.float32,
            )),
            "vtheta_sha256": array_sha256(np.asarray(
                substrate.vtheta, np.float32,
            )),
            "ampa_topology_sha256": parameter_audit["sparse_pathways"][
                "ampa_by_delay"
            ]["topology_sha256"],
            "ampa_values_sha256": parameter_audit["sparse_pathways"][
                "ampa_by_delay"
            ]["after_sha256"],
            "gaba_topology_sha256": parameter_audit["sparse_pathways"][
                "gaba_by_delay"
            ]["topology_sha256"],
            "gaba_values_sha256": parameter_audit["sparse_pathways"][
                "gaba_by_delay"
            ]["after_sha256"],
            "identity_scope": (
                "positions, E/I partition, delay-binned edges/weights, core mask, "
                "signed threshold offsets and thresholds"
            ),
        },
