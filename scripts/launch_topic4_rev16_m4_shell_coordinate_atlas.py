#!/usr/bin/env python3
"""Launch the frozen rev16 M4-shell atlas controller with systemd and nohup."""
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not 1 <= args.worker_cap <= int(config["resources"]["maximum_workers"]):
        raise ValueError("worker cap exceeds rev16 M4-shell limit")
    ...
    command = [
        ...
        *[f"--setenv={name}=1" for name in NUMERIC_ENV],
        ...
        "--unit-prefix", prefix, "--worker-cap", str(args.worker_cap),
        "--execute",
    ]
    payload = {
        "status": "REV16_M4_SHELL_CONTROLLER_LAUNCHED" if args.execute
        else "REV16_M4_SHELL_CONTROLLER_DRY_RUN",
