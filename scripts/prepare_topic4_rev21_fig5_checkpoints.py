...
from src.topic4_legacy_dual_core_transition import (  # noqa: E402
    build_substrate,
    load_round_config,
    make_external_drive,
    make_slow,
...
# Recorded keys remain historical; hash the module actually used by this entry.
RUNTIME_PATHS = {
    "scripts/run_topic4_rev12_node_worker.py": "scripts/run_topic4_legacy_rev21_worker.py",
    "src/topic4_zm_ictal_transition.py": "src/topic4_legacy_dual_core_transition.py",
}


def runtime_path(relative):
    return ROOT / RUNTIME_PATHS.get(relative, relative)
...
def load_selected_contract(config_path: Path, manifest_path: Path,
    ...
    for relative in RUNTIME_MODULES:
        live = sha256(runtime_path(relative))
        recorded_key = RUNTIME_PATHS.get(relative, relative)
        if recorded_key not in recorded:
            recorded_key = relative
        if recorded.get(recorded_key) != live:
            mismatch[relative] = {"recorded": recorded.get(recorded_key), "live": live}
    if mismatch:
...
def main() -> None:
    ...
    payload = {
        ...
        "runtime_module_sha256": {
            relative: sha256(runtime_path(relative)) for relative in RUNTIME_MODULES},
